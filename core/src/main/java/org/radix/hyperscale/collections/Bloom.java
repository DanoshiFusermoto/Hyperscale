 ...
 *
 * <p>For updates and bugfixes, see http://github.com/magnuss/java-bloomfilter
 *
 * <p>Inspired by the SimpleBloomFilter-class written by Ian Clarke. This implementation provides a
...
public class Bloom extends Serializable {
  static final Charset CHARSET =
      StandardCharsets.UTF_8; // encoding used for storing hash values as strings
  static final String HASHNAME =
      "MD5"; // MD5 gives good enough accuracy in most circumstances. Change to SHA1 if it's needed
  private static final ThreadLocal<MessageDigest> digester =
      ThreadLocal.withInitial(() -> getDigester(HASHNAME, null));
  ...
  /**
   * Constructs a new Bloom filter containing the provided collection with a given false positive
   * probability. The number of bits per element and the number of hash functions is estimated to
  ...
  public Bloom(final double falsePositiveProbability, final Collection<byte[]> elements) {
    ...
    for (byte[] element : elements) add(element);
  }
   ...
   *     expected to contain.
   * @param numberOfAddedElements specifies how many elements have been inserted into the <code>
   *     filterData</code> BitSet.
  ...
  public Bloom(
      ...
      final int numberOfAddedElements,
      final BitSet filterData) {
  ...
  public Bloom(final Bloom bloom) {
    this(
        Objects.requireNonNull(bloom, "Bloom to copy is null").bitSetSize,
        bloom.expectedNumberOfFilterElements,
        bloom.numberOfAddedElements,
        bloom.bitset);
  }
  ...
  public byte[] toByteArray() {
    ...
    ByteBuffer buffer =
        ByteBuffer.allocate(
            Integer.BYTES + Integer.BYTES + Integer.BYTES + Integer.BYTES + bitSetBytes.length);
