...
public final class Events implements Service
{
	...
	private class Subscriber
	{
		...
		private Subscriber(final EventListener listener, final List<MethodInfo> methods)
		{
			...
			for (MethodInfo method : methods)
				this.methods.put(method.getParameterType(), method);
		}
		...
		public boolean equals(Object obj)
		{
			...
			{
				if (this.listener.equals(other.listener) == false)
					return false;
				
				if (this.methods.equals(other.methods) == false)
					return false;
	...
	private class Subscribers
	{
		...
		public void register(final Subscriber subscriber)
		{
			if (isRegistered(subscriber.listener))
				throw new IllegalStateException("Subscriber is already registered: " + subscriber.listener.getClass().getName());
	...
	private final PollingProcessor asyncEventProcessor = new PollingProcessor()
	{
		...
		public void process() throws InterruptedException
		{
			...
			{
				Events.this.context.getMetaData().put("system.events.queue", MathUtils.EWMA(Events.this.context.getMetaData().get("system.events.queue", 0l), Events.this.asyncEventQueue.size(), 0.9));

				if (Events.this.asyncEventQueue.size() > Constants.WARN_ON_QUEUE_SIZE)
					eventLog.warn(Events.this.context.getName()+": System async event queue is "+Events.this.asyncEventQueue.size());
	        ...
	        if (subscribers.hasAsync)
	        	processEvent(event, subscribers, true);
		}
		
		@Override
		public void onError(Throwable thrown) 
		{
			eventLog.fatal(Events.this.context.getName()+": Error processing async event queue", thrown);
		}
    ...
    public synchronized void start() throws StartupException 
    {
        if (this.isRunning == false) 
        {
            ...
            throw new StartupException("Events service is already running");
    }
    ...
    public synchronized void stop() throws TerminationException 
    {
        if (this.isRunning == true) 
        {
            ...
            eventLog.warn(context.getName() + ": Attempted to stop Events service, but it was not running");   
    }
    ...
    public void post(final Event event) 
    {
        ...
        if (this.isRunning == false)
            throw new IllegalStateException("Events service is not running");

        final Subscribers subscribers = this.subscribers.get(event.getClass());
        if (subscribers == null)
        	return;
        
        if (subscribers.hasSync)
        	processEvent(event, subscribers, false);
    ...
    private void processEvent(final Event event, final Subscribers subscribers, final boolean async) 
    {
        ...
        try
        {
        	...
        	if (async == false)
    			subsByType = subscribers.sync;
        	else
    			subsByType = subscribers.async;
    ...
    public synchronized void register(final EventListener listener) 
    {
        ...
        if (this.listenerToSubscriber.containsKey(listener))
       		throw new IllegalStateException("Subscriber is already registered: " + listener.getClass().getName());
        ...
        for (final MethodInfo methodInfo : methodsInfo) 
        {
        	...
        	final Subscribers subscribers = this.subscribers.computeIfAbsent(eventType, t -> new Subscribers(t));
        ...
        eventLog.info(context.getName() + ": Registered listener " + listener.getClass().getSimpleName());
    }
    ...
    public synchronized boolean unregister(final EventListener listener) 
    {
        ...
        if (removed == true)
            eventLog.info(context.getName() + ": Unregistered listener " + listener.getClass().getSimpleName());
        else
            eventLog.warn(context.getName() + ": Attempted to unregister non-existent listener " + listener.getClass().getSimpleName());
	    ...
	    for (final Class<?> supertype : supertypes) 
	    {
	    	for (final Method method : supertype.getDeclaredMethods()) 
	    	{
	    		if (method.isAnnotationPresent(Subscribe.class) && !method.isSynthetic()) 
	    		{
	    			...
	    			Preconditions.checkArgument(parameterTypes[0].isPrimitive() == false,
	    			...
	    			Preconditions.checkArgument(Modifier.isAbstract(clazz.getModifiers()) == false,
