...
public class Configuration extends PersistedConfiguration {
  ...
  static Configuration createAsDefault(String commandLineConfig, String[] commandLineArguments)
      throws IOException {
  ...
  Configuration(String commandLineConfig, String[] commandLineArguments) throws IOException {
    try {
      ...
      InputStream commandLineConfigStream =
          Configuration.class.getResourceAsStream(commandLineConfig);
      if (commandLineConfigStream != null)
        commandLineConfigJSON =
            new JSONObject(
                IOUtils.toString(commandLineConfigStream, StandardCharsets.UTF_8.name()));
      ...
      for (String clKey : commandLineConfigJSON.keySet()) {
        ...
        if (clOption.has("short"))
          ...
          gnuOptions.addOption(
              clKey, clOption.getBoolean("has_arg"), clOption.optString("desc", ""));
      }

      // Using the Apache CommandLine class itself is extremely inefficient if it needs to be
      // queried a lot as it is a List lookup.
  ...
  public <T> T get(String key, T _default) {
    ...
    if (value == null) return super.get(key, _default);
    else if (_default instanceof Byte) return (T) Byte.valueOf(value);
    else if (_default instanceof Short) return (T) Short.valueOf(value);
    else if (_default instanceof Integer) return (T) Integer.valueOf(value);
    else if (_default instanceof Long) return (T) Long.valueOf(value);
    else if (_default instanceof Float) return (T) Float.valueOf(value);
    else if (_default instanceof Double) return (T) Double.valueOf(value);
    else if (_default instanceof Boolean) return (T) Boolean.valueOf(value);
    else if (_default instanceof String) return (T) value;
  ...
  public <T> T getCommandLine(String key, T _default) {
    ...
    if (commandLineOption == null) return _default;

    if (commandLineOption.hasArg() == false) return (T) Boolean.TRUE;

    String value = commandLineOption.getValue();
    if (_default != null) {
      if (_default instanceof Byte) return (T) Byte.valueOf(value);
      else if (_default instanceof Short) return (T) Short.valueOf(value);
      else if (_default instanceof Integer) return (T) Integer.valueOf(value);
      else if (_default instanceof Long) return (T) Long.valueOf(value);
      else if (_default instanceof Float) return (T) Float.valueOf(value);
