    ...
    public void lock() {
      ...
      try {
        ...
        if (lockTimeMS > MonitoredReadWriteLock.this.warningThresholdMS)
          locksLog.warn(
              "Read lock acquisition took "
                  + lockTimeMS
                  + "ms ( "
                  + getReadLocksCount()
                  + " / "
                  + getAverageReadLockWaitTime()
                  + "ms / "
                  + getTotalReadLockWaitTime()
                  + "ms )",
              new Exception("Read lock acquisition stack trace"));
      }
    }
  }
  ...
  private class MonitoredWriteLock extends WriteLock {
    ...
    public void lock() {
      ...
      try {
        ...
        if (lockTimeMS > MonitoredReadWriteLock.this.warningThresholdMS)
          locksLog.warn(
              "Write lock acquisition took "
                  + lockTimeMS
                  + "ms ( "
                  + getWriteLocksCount()
                  + " / "
                  + getAverageWriteLockWaitTime()
                  + "ms / "
                  + getTotalWriteLockWaitTime()
                  + "ms )",
              new Exception("Write lock acquisition stack trace"));
      }
    }
  }
