...
// TODO upgrade allowing multiple WebService instances for each context.  Needs multiple WebService
...
public class WebService implements Service {
  ...
  public static synchronized WebService create() {
    if (instance != null) throw new RuntimeException("WebService instance already created");
  ...
  public static synchronized WebService getInstance() {
    if (instance == null) throw new RuntimeException("WebService instance not created");
  ...
  public void start() throws StartupException {
    try {
      ...
      for (String knownType : knownTypes) apiLog.info(knownType);
  ...
  private Context getContext(Request req) {
    ...
    if (value != null) {
      ...
      if (context == null) throw new IllegalArgumentException("Context " + value + " not found");
    ...
    if (value != null) {
      ...
      for (Context context : Context.getAll()) {
        ShardGroupID contextShardGroupID =
            ShardMapper.toShardGroup(
                context.getNode().getIdentity(), context.getLedger().numShardGroups());
        if (requiredShardGroup.equals(contextShardGroupID)) return context;
      }

      throw new IllegalArgumentException(
          "Context with shard group " + requiredShardGroup + " not found");
    }
  ...
  private void contextNode() {
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/node",
        (req, res) -> {
          ...
          try {
            ...
            ShardGroupID shardGroupID =
                ShardMapper.toShardGroup(node.getIdentity(), numShardGroups);
            ...
            uptimeJSON.put(
                "duration",
                DurationFormatUtils.formatDuration(context.getUptime(), "H:mm:ss", true));
  ...
  private void contextUniverse() {
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/universe",
        (req, res) -> {
          JSONObject responseJSON = new JSONObject();
          ...
          try {
            responseJSON.put(
                "universe",
                Serialization.getInstance().toJsonObject(Universe.getDefault(), Output.ALL));
            status(responseJSON, 200);
          } catch (Throwable t) {
            status(responseJSON, 500, t.toString());
            apiLog.error("Failed to fetch Universe object - " + t.getMessage());
  ...
  private void ledgerHead() {
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/ledger/head",
        (req, res) -> {
          JSONObject responseJSON = new JSONObject();
  ...
  private void ledgerLatest() {
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/ledger/latest",
        (req, res) -> {
          JSONObject responseJSON = new JSONObject();
          ...
          try {
            ...
            for (Hash atomCertificateHash : head.getInventory(InventoryType.COMMITTED)) {
              AtomCertificate atomCertificate =
                  context.getLedger().get(atomCertificateHash, AtomCertificate.class);
              Future<SubstateSearchResponse> atomSubstate =
                  context
                      .getLedger()
                      .get(
                          new SubstateSearchQuery(
                              StateAddress.from(Atom.class, atomCertificate.getAtom()), true));
              SubstateSearchResponse substateSearchResponse = atomSubstate.get(1, TimeUnit.SECONDS);
              if (substateSearchResponse.getResult() == null) continue;

              substateCommits.add(substateSearchResponse.getResult());
              if (substateCommits.size() == 10) break;
            }
            ...
            if (substateCommits.isEmpty() == false) {
              for (SubstateCommit substateCommit : substateCommits)
                latestJSON.put(
                    Serialization.getInstance().toJsonObject(substateCommit, Output.API));
            }
  ...
  private void ledgerStatistics() {
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/ledger/statistics",
        (req, res) -> {
          JSONObject responseJSON = new JSONObject();
          ...
          try {
            ...
            ageJSON.put(
                ...
                DurationFormatUtils.formatDuration(
                    head.getHeight() * Constants.BLOCK_INTERVAL_TARGET_MILLISECONDS,
                    "H:mm:ss",
                    true));
            ...
            final JSONObject processed = new JSONObject();
            {
              ...
              transactions.put(
                  "failed",
                  context.getMetaData().get("ledger.processed.atoms.timedout.accept", 0l)
              ...
              transactions.put(
                  "local", context.getMetaData().get("ledger.processed.atoms.local", 0l));
              transactions.put(
                  "total", context.getMetaData().get("ledger.processed.atoms.total", 0l));
              processed.put("transactions", transactions);

              final JSONObject executions = new JSONObject();
              executions.put(
                  "local", context.getMetaData().get("ledger.processed.executions.local", 0l));
              executions.put(
                  "total", context.getMetaData().get("ledger.processed.executions.total", 0l));
            ...
            final JSONObject throughput = new JSONObject();
            {
              ...
              transactions.put(
                  ...
                  context
                      ...
                      .average(
                          "local",
                          timestamp - TimeUnit.SECONDS.toMillis(30),
                          timestamp,
              ...
              transactions.put(
                  "shards_average",
                  Math.max(
                      context.getMetaData().get("ledger.throughput.shards.touched", 1.0), 1.0));
              ...
              for (final String field : executionTimeSeriesFields) {
                ...
                if (field.startsWith("local:"))
                  executionsLocal.put(
                      field.substring("local:".length()), fieldAverage.doubleValue());
                else if (field.startsWith("total:"))
                  executionsTotal.put(
                      field.substring("total:".length()), fieldAverage.doubleValue());
                else
                  throw new IllegalArgumentException(
                      "Field " + field + " is unsupported in execution timeseries");
              }
              ...
              finality.put(
                  "consensus", context.getMetaData().get("ledger.throughput.latency.accepted", 0l));
              finality.put(
                  "client", context.getMetaData().get("ledger.throughput.latency.witnessed", 0l));
              ...
              data.put(
                  "total",
                  context.getLedger().numShardGroups(epoch)
                      * context.getMetaData().get("ledger.blocks.bytes", 0l));
            ...
            final JSONObject proposals = new JSONObject();
            {
              proposals.put(
                  "size_average",
                  context.getMetaData().get("ledger.blocks.bytes", 0l) / (head.getHeight() + 1));
  ...
  private void ledgerSearch() {
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH)
            + "/ledger/primitive/:type/:hash",
        (req, res) -> processPrimitiveSearch(req, res).toString(4));

    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH)
            + "/ledger/state/:context/:scope",
        (req, res) -> processSubstateSearch(req, res).toString(4));

    // TODO should be deprecated in favour of the POST method
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/ledger/association",
        (req, res) -> {
          JSONObject responseJSON = new JSONObject();
          ...
          try {
            ...
            Order order =
                Order.valueOf(
                    req.queryParamOrDefault("order", Order.ASCENDING.toString()).toUpperCase());
            ...
            Future<AssociationSearchResponse> associationSearchResponseFuture =
                context
                    .getLedger()
                    .get(
                        new AssociationSearchQuery(
                            associations, matchon, filterContext, order, offset, limit));
            AssociationSearchResponse associationSearchResponse =
                associationSearchResponseFuture.get(5, TimeUnit.SECONDS);

            if (associationSearchResponse.isEmpty())
              status(
                  responseJSON,
                  404,
                  "No instances found associated with " + req.queryParamOrDefault("keys", ""));
            else {
              ...
              status(
                  responseJSON,
                  200,
                  associationSearchResponse.getQuery().getOffset(),
                  associationSearchResponse.getNextOffset(),
                  limit,
                  associationSearchResponse.isEOR());
            }
          } catch (Throwable t) {
            status(responseJSON, 500, t.toString());
    ...
    spark.Spark.post(
        ...
        (req, res) -> {
          ...
          try {
            ...
            Order order =
                Order.valueOf(
                    requestJSON.optString("order", Order.ASCENDING.toString()).toUpperCase());
            ...
            if (associationSearchResponse.isEmpty())
              status(
                  responseJSON,
                  404,
                  "No instances found associated with " + requestJSON.getJSONArray("keys"));
            else {
              JSONArray substatesArray = new JSONArray();
              for (SubstateCommit result : associationSearchResponse.getResults())
                substatesArray.put(Serialization.getInstance().toJsonObject(result, Output.API));
            ...
            status(responseJSON, 500, t.toString());
            apiLog.error(
                "Failed to fetch associations " + requestJSON.getJSONArray("keys").toString(), t);
          }
  ...
  private void ledgerSubmit() {
    spark.Spark.post(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/ledger/atom/submit",
        (req, res) -> {
          JSONObject responseJSON = new JSONObject();
          Atom atom = null;
          try {
  ...
  private JSONObject processPrimitiveSearch(Request req, Response res) {
    ...
    try {
      ...
      final Class<? extends Primitive> primitiveType =
          (Class<? extends Primitive>)
              Serialization.getInstance().getClassForId(req.params("type"));

      Future<PrimitiveSearchResponse> primitiveFuture =
          WebService.this
              .getContext(req)
              .getLedger()
              .get(new PrimitiveSearchQuery(primitiveHash, primitiveType));
      PrimitiveSearchResponse primitiveResult = primitiveFuture.get(5, TimeUnit.SECONDS);
      if (primitiveResult.getResult() == null)
        status(
            responseJSON,
            404,
            "No primitive found " + primitiveHash + " of type " + req.params("type"));
      else {
        ...
        if (Atom.class.isInstance(primitive)) {
          ...
          JSONObject certificateJSON =
              Serialization.getInstance().toJsonObject(certificate, Output.API);
          ...
          status(responseJSON, 200);
        } else status(500, "Primitive type " + primitive.getClass() + " not supported");
      }
    } catch (Throwable t) {
      status(responseJSON, 500, t.toString());
      apiLog.error(
          "Failed to fetch primitive "
              + req.params("hash")
              + " of type "
              + req.params("type")
              + " - "
  ...
  private JSONObject processSubstateSearch(Request req, Response res) {
    ...
    try {
      StateAddress stateAddress =
          StateAddress.from(
              req.params("context"), Hash.valueOf(decodeQueryValue(req.params("scope"))));
      Future<SubstateSearchResponse> substateSearchFuture =
          WebService.this
              .getContext(req)
              .getLedger()
              .get(
                  new SubstateSearchQuery(
                      stateAddress,
                      Isolation.valueOf(
                          req.queryParamOrDefault("isolation", "COMMITTED").toUpperCase())));
      ...
      if (substateSearchResponse.getResult() == null)
        ...
        JSONObject substateJSON =
            Serialization.getInstance()
                .toJsonObject(substateSearchResponse.getResult(), Output.API);
  ...
  private void networkBootstrap() {
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/bootstrap",
        (req, res) -> {
          ...
          try {
            ...
            if (reqInetAddress.isAnyLocalAddress()
                || reqInetAddress.isLoopbackAddress()
                || reqInetAddress.isLinkLocalAddress()
                || reqInetAddress.isSiteLocalAddress()) {
              final StandardDiscoveryFilter standardDiscoveryFilter =
                  new StandardDiscoveryFilter(context);
              peers =
                  context.getNetwork().getPeerHandler().get(standardDiscoveryFilter, offset, limit);
            } else {
              final NotLocalPeersFilter notLocalPeersFilter =
                  new NotLocalPeersFilter(context.getNode());
              final StandardDiscoveryFilter standardDiscoveryFilter =
                  new StandardDiscoveryFilter(context);
              peers =
                  context
                      ...
                      .getPeerHandler()
                      .get(
                          new MultiPeerFilter(notLocalPeersFilter, standardDiscoveryFilter),
                          offset,
                          limit);
            }
            ...
            if (peers.isEmpty()) {
              ...
              for (final Peer peer : peers) peersArray.put(peer.getURI());
    ...
    spark.Spark.post(
        ...
        (req, res) -> {
          ...
          try {
            ...
            // Check gateway status of requesting node and update peer store as a gateway if
            // successful
  ...
  private void networkStatistics() {
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/network/statistics",
        (req, res) -> {
          JSONObject responseJSON = new JSONObject();
          ...
          try {
            ...
            JSONObject bandwidth = new JSONObject();
            {
              ...
              transferred.put(
                  "inbound", context.getMetaData().get("network.transferred.inbound", 0l));
              transferred.put(
                  "outbound", context.getMetaData().get("network.transferred.outbound", 0l));
  ...
  private void networkConnections() {
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/network/connections",
        (req, res) -> {
          JSONObject responseJSON = new JSONObject();
          ...
          try {
            ...
            else targetShardGroupID = ShardGroupID.from(req.queryParams("shardgroup"));
            ...
            if (targetShardGroupID == null) {
              ...
              List<AbstractConnection> syncConnections =
                  context
                      .getNetwork()
                      .get(
                          StandardConnectionFilter.build(context)
              ...
              for (int sg = 0; sg < numShardGroups; sg++) {
                ...
                List<AbstractConnection> shardConnections =
                    context
                        .getNetwork()
                        .get(
                            StandardConnectionFilter.build(context)
                                .setShardGroupID(ShardGroupID.from(sg))
                                .setStates(ConnectionState.CONNECTED),
                            false);
                JSONArray shardgroup = new JSONArray();
                for (AbstractConnection shardConnection : shardConnections)
                  shardgroup.put(
                      Serialization.getInstance().toJsonObject(shardConnection, Output.API));
              ...
              JSONObject shardgroups = new JSONObject();
              {
                ...
                List<AbstractConnection> shardConnections =
                    context
                        ...
                        .get(
                            StandardConnectionFilter.build(context)
                                .setShardGroupID(targetShardGroupID)
                                .setStates(ConnectionState.CONNECTED));
                for (AbstractConnection shardConnection : shardConnections)
                  shardgroup.put(
                      Serialization.getInstance().toJsonObject(shardConnection, Output.API));
  ...
  private void networkKnown() {
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/network/known",
        (req, res) -> {
          JSONObject responseJSON = new JSONObject();
          ...
          try {
            ...
            final Collection<Peer> knownPeers =
                context
                    .getNetwork()
                    .getPeerHandler()
                    .get(
                        new NotLocalPeersFilter(context.getNode()).setBanned(true).setInvalid(true),
                        0,
                        Short.MAX_VALUE);
            final int numShardGroups = context.getLedger().numShardGroups();
            for (final Peer knownPeer : knownPeers) {
              final ShardGroupID shardGroupID =
                  ShardMapper.toShardGroup(knownPeer.getIdentity(), numShardGroups);
              if (targetShardGroupID != null && shardGroupID.equals(targetShardGroupID) == false)
                continue;

              JSONArray knownByShardGroupJSON =
                  knownJSON.optJSONArray(Integer.toString(shardGroupID.intValue()));
              ...
              knownByShardGroupJSON.put(
                  Serialization.getInstance().toJsonObject(knownPeer, Output.API));
            }
  ...
  private void networkSearch() {
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/network/search",
        (req, res) -> {
          JSONObject responseJSON = new JSONObject();
          ...
          try {
            ...
            List<Peer> discoveredPeers =
                context.getNetwork().getPeerHandler().get(discoveryFilter, offset, limit);
  ...
  private void statisticsAll() {
    spark.Spark.get(
        Configuration.getDefault().get("api.url", DEFAULT_API_PATH) + "/statistics",
        (req, res) -> {
          JSONObject responseJSON = new JSONObject();
          ...
          try {
            ...
            metaData.forEach(
                (s, o) -> {
                  if (o instanceof String value) statistics.put(s, value);
                  else if (o instanceof Number value) statistics.put(s, value);
          ...
          try {
            ...
            if (timeSeries == null)
              throw new UnsupportedOperationException(
                  "No timeseries statistics for " + req.params(":name"));

            final List<String> timeSeriesFields = timeSeries.getFields();
            final HashMap<String, String> trimmedFieldCache =
                new HashMap<>(timeSeriesFields.size());
                ...
                if (timeSeriesName.equals("throughput")) {
                  ...
                  for (int i = 0; i < timeSeriesFields.size(); i++) {
                    ...
                    record.put(
                        ...
                        timeSeries.average(
                            field,
                            timestamp - TimeUnit.SECONDS.toSeconds(30),
                            timestamp,
                            TimeUnit.SECONDS));
                  }
                  ...
                  for (int i = 0; i < timeSeriesFields.size(); i++) {
                    ...
                    if (field.startsWith("local:"))
                      local.put(
                          trimmedFieldCache.computeIfAbsent(
                              field, k -> k.substring("local:".length())),
                          timeSeries.average(
                              field,
                              timestamp - TimeUnit.SECONDS.toSeconds(30),
                              timestamp,
                      ...
                      total.put(
                          trimmedFieldCache.computeIfAbsent(
                              field, k -> k.substring("total:".length())),
                          timeSeries.average(
                              field,
                              timestamp - TimeUnit.SECONDS.toSeconds(30),
                              timestamp,
                  ...
                  double touchedAvgValue =
                      timeSeries.average(
                          ...
                          TimeUnit.SECONDS);
                  record =
                      new JSONObject().put("timestamp", timestamp).put("touched", touchedAvgValue);
                  ...
                  double consensusValue =
                      timeSeries.mean("consensus", 30, timestamp, TimeUnit.SECONDS);
        ...
        (req, res) -> {
          ...
          if (resourceStream != null) {
            try {
              ...
              int totalLength = 0;
              int length;
              while ((length = resourceStream.read(buf)) > 0) totalLength += length;
          ...
          if (dir != null && url != null && endPoint.startsWith(url.toLowerCase())) {
            ...
            if (staticFile.exists() == false || staticFile.isDirectory()) res.status(404);
            else {
          ...
          if (endPoint.startsWith(JacksonCodecConstants.HASH_STR_VALUE))
            stateAddress =
                StateAddress.from(
                    Blob.class,
                    Serialization.getInstance().<Hash>fromTypedString(endPoint.toLowerCase()));
          else stateAddress = StateAddress.from(Blob.class, endPoint.toLowerCase());

          final Future<SubstateSearchResponse> substateSearchResponseFuture =
              Context.get()
                  .getLedger()
                  .get(
                      new SubstateSearchQuery(
          ...
          try {
            final SubstateSearchResponse substateSearchResponse =
                substateSearchResponseFuture.get(5, TimeUnit.SECONDS);
            if (substateSearchResponse.getResult() == null) res.status(404);
            else {
              final Atom atom = substateSearchResponse.getResult().getSubstate().get("primitive");
              if (atom == null) res.status(404);
              else {
                boolean found = false;
  ...
  private JSONObject status(int code, String message) {
    JSONObject status = new JSONObject();
    status.put("code", code);
    if (message != null && message.isEmpty() == false) status.put("message", message);
    return status;
  }

  private JSONObject status(
      int code, String message, long offset, long nextOffset, int limit, boolean eor) {
    JSONObject status = new JSONObject();
    status.put("code", code);
    status.put("offset", offset);
    status.put("next_offset", nextOffset);
  ...
  private void status(
      JSONObject response, int code, long offset, long nextOffset, int limit, boolean eor) {
  ...
  private Class<?> decodeQueryType(String value) {
    if (value.startsWith(JacksonCodecConstants.BYTE_STR_VALUE)) return byte[].class;

    if (value.startsWith(JacksonCodecConstants.HASH_STR_VALUE)) return Hash.class;

    if (value.startsWith(JacksonCodecConstants.IDENTITY_STR_VALUE)) return Identity.class;

    if (value.startsWith(JacksonCodecConstants.U256_STR_VALUE)) return UInt256.class;
