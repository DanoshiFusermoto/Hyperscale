...
public class RadixCBORGenerator extends GeneratorBase
{
    ...
    protected int[] _elementCounts = new int[4];
    ...
    public RadixCBORGenerator(IOContext ctxt, int stdFeatures, int formatFeatures, ObjectCodec codec, OutputStream out) 
    {
    ...
    protected void maybeCopyTag(JsonParser p) throws IOException {
        if (p instanceof CBORParser cborParser) 
        {
            if (p.hasCurrentToken()) {
                final int currentTag = cborParser.getCurrentTag();
    ...
    private final void _pushRemainingElements() {
        if (_elementCounts.length == _elementCountsPtr) // initially, as well as if full
            _elementCounts = Arrays.copyOf(_elementCounts, _elementCounts.length+GROW_STEP);
    ...
    /**
     * Custom method for directly inserting specified bytes with a type prefix in output 
     * at current position.
     * <p>
     * NOTE: only use this method if you really know what you are doing.
     */
    public void writeBinaryWithPrefix(byte prefix, byte[] data, int offset, int len) throws IOException 
    {
        if (data == null) {
            writeNull();
            return;
        ...
        _verifyValueWrite("write Binary value");
        _writeLengthMarker(PREFIX_TYPE_BYTES, len+1);
        _writeByte(prefix);
        _writeBytes(data, offset, len);
    }
    ...
    {
        if (i < 24) {
            _ensureRoomForOutput(1);
            _outputBuffer[_outputTail++] = (byte) (majorType + i);
            return;
        }
        
        if (i <= 0xFF) {
            _ensureRoomForOutput(2);
	...
	*/
    ...
    private final void closeComplexElement() throws IOException {
        ...
        _currentRemainingElements = (_elementCountsPtr == 0) ? INDEFINITE_LENGTH : _elementCounts[--_elementCountsPtr];
    }
