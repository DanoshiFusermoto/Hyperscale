...
public class RadixCBORGenerator extends GeneratorBase {
  ...
  /** Bit flag composed of bits that indicate which {@link CBORGenerator.Feature}s are enabled. */
  ...
  /**
   * Custom method for directly inserting specified bytes with a type prefix in output at current
   * position.
   *
   * <p>NOTE: only use this method if you really know what you are doing.
   */
  public void writeBinaryWithPrefix(byte prefix, byte[] data, int offset, int len)
      throws IOException {
    if (data == null) {
      writeNull();
      return;
  ...
  private final int _encode(int outputPtr, String str, int len) {
    ...
    for (int i = 0; i < len; ++i) {
      int c = str.charAt(i);
      if (c > 0x7F) return _encode2(i, outputPtr, str, len, outputStart);
