...
public class DsonFieldFilter extends SimpleBeanPropertyFilter {
  private static final class DsonFieldFilterEntry {
    ...
    public boolean equals(Object obj) {
      ...
      if (this.clazz.equals(((DsonFieldFilterEntry) obj).clazz) == false) return false;

      if (this.field.equals(((DsonFieldFilterEntry) obj).field) == false) return false;
   ...
   */
  public static FilterProvider filterProviderFor(final Map<Class<?>, Set<String>> includedFields)
      throws SerializationException {
    // Create an optimised version of the includedFields map
    MutableLongObjectMap<DsonFieldFilterEntry> optimizedIncludedFields =
        LongObjectMaps.mutable.ofInitialCapacity(includedFields.size() * 2);
      ...
      for (String field : fieldSet) {
        ...
        if (optimizedIncludedFields.put(dsonFieldFilterEntry.getID(), dsonFieldFilterEntry) != null)
          throw new SerializationException(
              "Duplicate field ID found in FilterProvider for " + clazz + ":" + field);
      }
    }

    return new SimpleFilterProvider()
        .addFilter(MapperConstants.DSON_FILTER_NAME, new DsonFieldFilter(optimizedIncludedFields));
  }
  ...
  private boolean shouldInclude(final Class<?> cls, final String property) {
    ...
    if (fieldEntry == null) {
      ...
      if (Map.class.isAssignableFrom(cls) || Collection.class.isAssignableFrom(cls)) return true;
    } else if (fieldEntry.clazz == cls && fieldEntry.field.equals(property)) return true;
