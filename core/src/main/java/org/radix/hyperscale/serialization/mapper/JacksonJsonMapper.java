...
public class JacksonJsonMapper extends ObjectMapper {
	...
	public static JacksonJsonMapper create(SerializerIds idLookup, FilterProvider filterProvider, boolean sortProperties) {
		...
		jsonModule.addSerializer(ShardID.class, new JacksonJsonObjectStringSerializer<>(
			ShardID.class,
			JacksonCodecConstants.SHARD_ID_STR_VALUE,
			ShardID::toString
		));
		jsonModule.addSerializer(UInt256.class, new JacksonJsonUInt256Serializer());
		jsonModule.addSerializer(MerkleProof.class, new JacksonJsonObjectStringSerializer<>(
				MerkleProof.class,
				JacksonCodecConstants.MERKLE_PROOF_STR_VALUE,
				MerkleProof::toString
		));
		jsonModule.addSerializer(BLSSignature.class, new JacksonJsonObjectStringSerializer<>(
				BLSSignature.class,
				JacksonCodecConstants.BLS_SIGNATURE_STR_VALUE,
				s -> Bytes.toHexString(s.toByteArray())
			));
		jsonModule.addSerializer(EDSignature.class, new JacksonJsonObjectStringSerializer<>(
				EDSignature.class,
		...
		jsonModule.addDeserializer(MerkleProof.class, new JacksonJsonObjectStringDeserializer<>(
				MerkleProof.class,
				JacksonCodecConstants.MERKLE_PROOF_STR_VALUE,
				MerkleProof::from
		));
		jsonModule.addDeserializer(BLSSignature.class, new JacksonJsonObjectStringDeserializer<>(
				BLSSignature.class,
				JacksonCodecConstants.BLS_SIGNATURE_STR_VALUE,
				s -> BLSSignature.from(Bytes.fromHexString(s))
		));
		jsonModule.addDeserializer(EDSignature.class, new JacksonJsonObjectStringDeserializer<>(
				EDSignature.class,
				JacksonCodecConstants.EC_SIGNATURE_STR_VALUE,
...
class StandardKeySerializer extends StdSerializer<Object>
{
	...
	public void serialize(Object value, JsonGenerator gen, SerializerProvider provider) throws IOException 
	{
		if (value instanceof String)
			...
			throw new IOException("Unsupported key type "+value.getClass());
	}
}
