...
final class Lock<T> {
  ...
  boolean hasLock(final Transaction transaction) {
    synchronized (this.monitor) {
      if (this.owner == transaction) return true;

      return false;
    }
  }

  boolean tryLock(final Transaction transaction, final long timeout, final TimeUnit unit)
      ...
      try {
        ...
        if (this.signals.decrementAndGet() < 0)
          throw new IllegalStateException(
              "Signals negative for lock on transaction " + transaction);
        ...
        if (this.owner == transaction) {
          ...
          if (lockTime > Lock.DEFAULT_LOCK_WARN_THRESHOLD_MS)
            locksLog.warn(
                "Lock acquisition took " + lockTime + "ms at:",
  ...
  void unlock(final Transaction transaction) {
    synchronized (this.monitor) {
      if (transaction != this.owner)
        throw new IllegalStateException(
            "Unlock " + this.key + " is not assigned to transaction " + transaction);

      this.owner = null;

      int remainingSignals = this.signals.get();
      if (remainingSignals == 0) this.stale = true;
      else if (remainingSignals < 0)
        throw new IllegalStateException(
            "Signals negative for unlocked lock on transaction " + transaction);
