...
public class Cursor {
  ...
  public Cursor(final Database database, final DatabaseEntry key) {
    ...
    if (database.getConfig().getAllowDuplicates() == false)
      throw new IllegalStateException(
          "Database " + database.getName() + " does not support cursors");
  ...
  public OperationStatus getFirst(DatabaseEntry value) throws IOException {
    IndexItem indexItem =
        this.database.getEnvironment().getIndex().readIndexItem(this.database, this.internalKey);
    if (indexItem == null) return OperationStatus.NOTFOUND;
    ...
    ExtensionNode extensionNode =
        this.database.getEnvironment().getLog().readExtensionNode(indexItem);
    ...
    ExtensionItem extensionItem =
        this.database
            .getEnvironment()
            .getLog()
            .readExtensionItem(indexItem.getKey(), this.currentPosition);
    this.currentExtensionItem = extensionItem;

    value.setData(
        this.database.getEnvironment().getLog().readLogData(extensionItem.getLogPosition()));

    return OperationStatus.SUCCESS;
  }

  public OperationStatus getPrev(DatabaseEntry value) throws IOException {
    if (this.currentExtensionItem.getPrevious() == -1) return OperationStatus.NOTFOUND;
    ...
    ExtensionItem extensionItem =
        this.database
            ...
            .getLog()
            .readExtensionItem(
                this.currentExtensionItem.getKey(), this.currentExtensionItem.getPrevious());
    this.currentExtensionItem = extensionItem;
  ...
  public OperationStatus getNext(DatabaseEntry value) throws IOException {
    if (this.currentExtensionItem.getNext() == -1) return OperationStatus.NOTFOUND;
    ...
    ExtensionItem extensionItem =
        this.database
            ...
            .getLog()
            .readExtensionItem(
                this.currentExtensionItem.getKey(), this.currentExtensionItem.getNext());
    this.currentExtensionItem = extensionItem;
