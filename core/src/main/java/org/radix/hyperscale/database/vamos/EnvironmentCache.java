...
class EnvironmentCache {
  ...
  EnvironmentCache(final Environment environment) {
    ...
    if (this.environment.getConfig().hasIndexNodeCache())
      this.indexNodeCache =
          new LRUCacheMap<IndexNodeID, IndexNode>(
              this.environment.getConfig().getIndexNodeCacheMaxEntries());
    else this.indexNodeCache = null;

    if (this.environment.getConfig().hasIndexItemCache())
      this.indexItemCache =
          new LRUCacheMap<InternalKey, IndexItem>(
              this.environment.getConfig().getIndexItemCacheMaxEntries());
    else this.indexItemCache = null;

    if (this.environment.getConfig().hasLogCache())
      this.logOperationCache =
          new LRUCacheMap<Long, LogOperation>(this.environment.getConfig().getLogCacheMaxEntries());
    else this.logOperationCache = null;
  ...
  void clearIndexNodes() {
    if (this.indexNodeCache == null) return;

    this.indexNodeCache.clear();
  }

  IndexNode getIndexNode(IndexNodeID ID) {
    if (this.indexNodeCache == null) return null;
  ...
  void clearIndexItems() {
    if (this.indexItemCache == null) return;

    this.indexItemCache.clear();
  }

  IndexItem getIndexItem(InternalKey key) {
    if (this.indexItemCache == null) return null;

    final IndexItem indexItem = this.indexItemCache.get(key);
    if (indexItem == null) this.indexItemCacheMisses.incrementAndGet();
    else this.indexItemCacheHits.incrementAndGet();
  ...
  void clearLogOperations() {
    if (this.logOperationCache == null) return;

    this.logOperationCache.clear();
  }

  LogOperation getLogOperation(long position) {
    if (this.logOperationCache == null) return null;

    final LogOperation logOperation = this.logOperationCache.get(position);
    if (logOperation == null) this.logCacheMisses.incrementAndGet();
    else this.logCacheHits.incrementAndGet();
  ...
  void putLogOperation(LogOperation logOperation) {
    ...
    if (logOperation.isExtension()) return;
    ...
    if (TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis() - this.opened) % 10 == 0) {
      if (logging.compareAndSet(false, true) == true) {
        ...
        vamosLog.info(
            String.format(
                "     Index Items %d %d",
                this.indexItemCacheHits.get(), this.indexItemCacheMisses.get()));
        vamosLog.info(
            String.format(
                "     Index Nodes %d %d",
                this.indexNodeCacheHits.get(), this.indexNodeCacheMisses.get()));
        vamosLog.info(
            String.format(
                "  Log Operations %d %d", this.logCacheHits.get(), this.logCacheMisses.get()));
        vamosLog.info(
            String.format(
                "  Log Cache Size %d %d",
                this.logOperationCache.size(), logCacheBytes.longValue()));
