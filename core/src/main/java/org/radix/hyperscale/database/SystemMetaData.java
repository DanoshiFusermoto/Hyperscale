...
public final class SystemMetaData extends DatabaseStore implements Service {
  ...
  public void start() throws StartupException {
    try {
      if (this.context.getConfiguration().getCommandLine("clean", false) == true) clean();
    ...
    this.flush =
        Executor.getInstance()
            .scheduleWithFixedDelay(
                new ScheduledExecutable(30, 30, TimeUnit.SECONDS) {
                  @Override
                  public void execute() {
                    try {
                      flush();
  ...
  public void close() throws IOException {
    if (this.flush != null) this.flush.cancel(false);
  ...
  public synchronized void flush() throws DatabaseException {
    ...
    try {
      ...
      for (Map.Entry<String, Object> entry : this.metaDataCache.entrySet()) {
        ...
        } else throw new IllegalArgumentException("Unknown value type: " + valueClass.getName());
  ...
  public boolean has(final String name) {
    ...
    if (name.isEmpty()) throw new IllegalArgumentException("Name length is zero");
  ...
  public String get(final String name, final String option) {
    ...
    if (option.isEmpty()) throw new IllegalArgumentException("Option length is zero");
  ...
  public byte[] get(final String name, final byte[] option) {
    ...
    Objects.requireNonNull(option, "Option is null");
    if (option.length == 0) throw new IllegalArgumentException("Option length is zero");
    if (option.length > MAX_LENGTH)
      throw new IllegalArgumentException("Option length is greater than " + MAX_LENGTH);
  ...
  public long increment(final String name, final long increment) {
    ...
    final MutableDouble value =
        (MutableDouble) this.metaDataCache.computeIfAbsent(name, k -> new MutableDouble(0));
    synchronized (value) {
      value.add(increment);
    }
  ...
  public void put(final String name, final String value) {
    ...
    if (value.isEmpty()) throw new IllegalArgumentException("Value length is zero");
  ...
  public void put(final String name, final byte[] value) {
    ...
    Objects.requireNonNull(value, "Value is null");
    if (value.length == 0) throw new IllegalArgumentException("Value length is zero");
    if (value.length > MAX_LENGTH)
      throw new IllegalArgumentException("Value length is greater than " + MAX_LENGTH);
  ...
  private static long asLong(final Object value) {
    Objects.requireNonNull(value, "Value is null");
    if (value instanceof MutableDouble mutable) return mutable.longValue();
  ...
  private static double asDouble(final Object value) {
    Objects.requireNonNull(value, "Value is null");
    if (value instanceof MutableDouble mutable) return mutable.doubleValue();
