...
class IndexItem {
  ...
  IndexItem(final ByteBuffer buffer) throws IOException {
    ...
    int type = buffer.get();
    if (type == 0) this.type = Type.DATA;
    else if (type == 1) this.type = Type.EXTENSION;
  ...
  void write(final ByteBuffer output) {
    if (this.type == Type.DATA) output.put((byte) 0x00);
    else if (this.type == Type.EXTENSION) output.put((byte) 0x01);
    else throw new IllegalArgumentException("Type " + this.type + " is unsupported");
  ...
  public boolean equals(final Object obj) {
    ...
    if (obj instanceof IndexItem other) {
      ...
      else if (this.key.equals(other.key) == false) return false;
