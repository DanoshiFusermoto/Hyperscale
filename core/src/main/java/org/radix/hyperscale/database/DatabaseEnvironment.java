...
public final class DatabaseEnvironment {
  ...
  private static final boolean ENV_MINIMAL_CACHE = true;
  private static Set<DatabaseEnvironment> environments =
      Collections.synchronizedSet(new HashSet<DatabaseEnvironment>());
  ...
  static {
    DatabaseEnvironment.environmentCheckpointRunnable =
        new Runnable() {
          @Override
          public void run() {
            while (1 == 1) {
              ...
              List<DatabaseEnvironment> environments =
                  new ArrayList<DatabaseEnvironment>(DatabaseEnvironment.environments);
              for (DatabaseEnvironment environment : environments) {
                try {
                  if (environment.context.getConfiguration().get("ledger.pruning", Boolean.TRUE)
                      == Boolean.TRUE) {
                    if (environment.getEnvironment().isClosed() == false
                        && DatabaseEnvironment.environments.contains(environment) == true)
                  ...
                  log.fatal(
                      environment.context.getName() + ": Database environment checkpoint failed",
                      ex);
                }
              }
              ...
              try {
                long sleepFor =
                    TimeUnit.MINUTES.toMillis(15) - (System.currentTimeMillis() - start);
    ...
    DatabaseEnvironment.environmentCheckpointThread =
        new Thread(DatabaseEnvironment.environmentCheckpointRunnable, "Database checkpoint");
  ...
  static {
    DatabaseEnvironment.environmentMaintenanceRunnable =
        new Runnable() {
          @Override
          public void run() {
            while (1 == 1) {
              ...
              for (DatabaseEnvironment environment : environments) {
                try {
                  ...
                  log.fatal(
                      environment.context.getName() + ": Database environment maintenance failed",
                      ex);
                }
              }
    ...
    DatabaseEnvironment.environmentMaintenanceThread =
        new Thread(DatabaseEnvironment.environmentMaintenanceRunnable, "Database maintenance");
  ...
  public DatabaseEnvironment(final Context context, final File home) {
    ...
    environmentConfig.setConfigParam(
        ...
        "1"); // TODO check one is enough, otherwise -> String.valueOf(Math.max(1,
    // Runtime.getRuntime().availableProcessors() / 4)));
    environmentConfig.setConfigParam(
        EnvironmentConfig.EVICTOR_CRITICAL_PERCENTAGE,
        "10"); // TODO make sure this is accounted for in cache sizing!
    ...
    if (this.context.getConfiguration().get("ledger.pruning", Boolean.TRUE) == Boolean.TRUE) {
      ...
      environmentConfig.setConfigParam(
          EnvironmentConfig.CLEANER_BYTES_INTERVAL, String.valueOf(1l << 32l));
      environmentConfig.setConfigParam(
          EnvironmentConfig.CLEANER_MIN_AGE, String.valueOf((int) (logFileMinClean / logFileSize)));
    ...
    environmentConfig.setConfigParam(
        EnvironmentConfig.CHECKPOINTER_BYTES_INTERVAL, String.valueOf(1l << 28l));
    ...
    if (ENV_MINIMAL_CACHE) {
      ...
      long cacheSize =
          (long) (Runtime.getRuntime().maxMemory() * 0.125)
              / this.context.getConfiguration().getCommandLine("contexts", 1);
      environmentConfig.setCacheSize(cacheSize);
    }
  ...
  public void close() {
    ...
    if (DatabaseEnvironment.environments.remove(this) == false)
      log.fatal(
          this.context.getName() + ": Database environment not in global database environments");
  ...
  public OperationStatus put(
      final Transaction transaction, final String resource, final String key, final byte[] value) {
    return this.put(
        transaction, resource, new DatabaseEntry(key.getBytes()), new DatabaseEntry(value));
  }
  ...
  public OperationStatus put(
    ...
    if (resource.isEmpty()) throw new IllegalArgumentException("Resource length is 0");

    Objects.requireNonNull(key, "Key is null");
    Objects.requireNonNull(key.getData(), "Key data is null");
    if (key.getData().length == 0) throw new IllegalArgumentException("Key data length is 0");

    Objects.requireNonNull(value, "Value is null");
    Objects.requireNonNull(value.getData(), "Value data is null");
    if (value.getData().length == 0) throw new IllegalArgumentException("Value data length is 0");
  ...
  public OperationStatus get(
      final String resource, final DatabaseEntry key, final DatabaseEntry value) {
    Objects.requireNonNull(resource, "Resource is null");
    if (resource.isEmpty()) throw new IllegalArgumentException("Resource length is 0");
  ...
  public Database openDatabase(
      final Transaction transaction, final String name, final DatabaseConfig config) {
  ...
  public long truncateDatabase(
      final Transaction transaction, final String name, final boolean count) {
