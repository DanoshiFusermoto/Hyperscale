...
class ExtensionItem extends ExtensionObject {
  public static final int BYTES =
      ExtensionObject.BYTES + Long.BYTES + Long.BYTES + Long.BYTES + Long.BYTES + Byte.BYTES;

  /* Thread local ByteBuffer used when serializing I/O */
  private static final ThreadLocal<ByteBuffer> byteBuffer =
      ThreadLocal.withInitial(() -> ByteBuffer.allocate(BYTES));
  ...
  ExtensionItem(final ByteBuffer buffer) throws IOException {
    super(
        InternalKey.from(
            Objects.requireNonNull(buffer, "Byte buffer is null").getInt(), buffer.getLong()),
        buffer.getLong());
    ...
    if (this.next != -1) Numbers.lessThan(this.next, this.previous, "Next is less than previous");
  }
  ...
  ExtensionItem(
    ...
    if (next != -1) Numbers.lessThan(next, previous, "Next is less than previous");
  ...
  public boolean equals(Object obj) {
    ...
    if (obj instanceof ExtensionItem other) {
      ...
      if (this.logPosition != other.logPosition) return false;
      if (this.next != other.next) return false;
      if (this.previous != other.previous) return false;
