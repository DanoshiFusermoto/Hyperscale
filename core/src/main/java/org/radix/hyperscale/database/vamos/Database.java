...
public class Database {
  ...
  static final Charset charset =
      Charset.forName("UTF-8"); // encoding used for storing hash values as strings
  ...
  public OperationStatus delete(Transaction transaction, DatabaseEntry key, LockMode lockMode)
      throws IOException {
    if (transaction == null) {
      ...
      return status;
    } else return deleteInternal(transaction, key, lockMode);
  }

  private OperationStatus deleteInternal(
      Transaction transaction, DatabaseEntry key, LockMode lockMode) throws IOException {
  ...
  public OperationStatus put(
      Transaction transaction, DatabaseEntry key, DatabaseEntry value, LockMode lockMode)
      throws IOException {
  ...
  public OperationStatus put(
    ...
    if (transaction == null) {
      ...
      return status;
    } else return putInternal(transaction, key, value, operation, lockMode);
  }

  private OperationStatus putInternal(
      Transaction transaction,
      DatabaseEntry key,
      DatabaseEntry value,
      Operation operation,
      LockMode lockMode)
      throws IOException {
    if (operation.equals(Operation.PUT) == false
        && operation.equals(Operation.PUT_NO_OVERWRITE) == false)
    ...
    try {
      OperationStatus status =
          transaction.put(this, key, value, internalKey, indexNodeID, operation, lockMode);
      return status;
    } catch (IOException ioex) {
      vamosLog.error(ioex);
      throw ioex;
  ...
  public OperationStatus get(
    ...
    if (transaction == null) {
      ...
      return status;
    } else return getInternal(transaction, key, value, lockMode);
  }

  private OperationStatus getInternal(
      Transaction transaction, DatabaseEntry key, DatabaseEntry value, LockMode lockMode)
      throws IOException {
    InternalKey internalKey = InternalKey.from(this, key);
    IndexNodeID indexNodeID = this.environment.toIndexNodeID(internalKey);
    ...
    try {
      OperationStatus status =
          transaction.get(this, key, value, internalKey, indexNodeID, lockMode);
      return status;
    } catch (IOException ioex) {
      vamosLog.error(ioex);
      throw ioex;
    ...
    if (this.totalOperations > 0 && this.totalOperations % 100000 == 0) {
      String info =
          this.name
              ...
              + "/"
              + (this.totalOperations
                  / Math.max(1, ((System.currentTimeMillis() - opened) / 1000)));
      //			info += " Idx Node R/W/CR/CW:
      // "+this.indexNodeReads+"/"+this.indexNodeWrites+"/"+this.contiguousIndexNodeReads+"/"+this.contiguousIndexNodeWrites;
      //			info += " Idx R/W/CR/CW:
      // "+this.indexPositionReads+"/"+this.indexPositionWrites+"/"+this.contiguousIndexPositionReads+"/"+this.contiguousIndexPositionWrites;
      //			info += " Log R/W/CR/CW:
      // "+this.logReads+"/"+this.logWrites+"/"+this.contiguousLogReads+"/"+this.contiguousLogWrites;
      //			if (this.config.getAllowDuplicates() == true)
      //				info += " Ext Node R/W/CR/CW:
      // "+this.extensionNodeReads+"/"+this.extensionNodeWrites+"/"+this.contiguousExtensionReads+"/"+this.contiguousExtensionWrites+" Ext Item R/W: "+this.extensionItemReads+"/"+this.extensionItemWrites;
