...
class Index {
  ...
  private static final ThreadLocal<ByteBuffer> indexNodeByteBuffer =
      ThreadLocal.withInitial(() -> ByteBuffer.allocate(65535));

  private static final Logger vamosLog = Logging.getLogger("vamos");

  static final int INDEX_HEADER_LENGTH = 0;
  static final long POSITION_UNKNOWN = -1;
  static final long POSITION_NOT_EXISTS = Long.MIN_VALUE;
  ...
  private class IndexWorker implements Runnable {
    ...
    public void run() {
      ...
      while (this.terminated == false) {
        ...
        try {
          ...
          try {
            if (this.indexItems.isEmpty()) continue;
            ...
            if (this.indexItemsToQueue.isEmpty() == false) {
              for (int i = 0; i < this.indexItemsToQueue.size(); i++) {
                ...
                final IndexNodeID indexNodeID =
                    Index.this.environment.toIndexNodeID(indexItem.getKey());
                ...
                if (Index.this.environment.getConfig().isIndexNodeDeferredWriteAvailable() == false
                    || indexItems.size()
                        >= Index.this.environment.getConfig().getIndexNodeDeferredWriteHint())
          ...
          try {
            for (int i = 0; i < indexItemsToWrite.size(); i++) {
                ...
                try {
                  Index.this
                      ...
                      .lock(
                          indexNodeID,
                          transaction,
                          Index.this.environment.getConfig().getLockTimeout(),
                          TimeUnit.SECONDS);
                } catch (InterruptedException ex) {
                  Thread.currentThread().interrupt();
                  throw new DatabaseException(
                      "Lock interruption on index node ID "
                          + indexNodeID
                          + " when writing "
                          + indexItemsToWrite.size()
                          + " index items");
                }
              ...
              else lockedIndexNode.put(indexItem);
            }
          ...
          try {
            for (int i = 0; i < indexItemsToWrite.size(); i++) {
              ...
              this.indexItemsToNodes.remove(
                  Index.this.environment.toIndexNodeID(indexItem.getKey()), indexItem);
            }
          } finally {
    ...
    private void queueIndexItems(final Collection<IndexItem> indexItems) {
      ...
      final EnvironmentCache cache =
          Index.this.environment.getConfig().hasIndexItemCache()
              ? Index.this.environment.getCache()
              : null;
      this.lock.writeLock().lock();
      try {
        for (IndexItem indexItem : indexItems) {
          ...
          if (cache != null) cache.putIndexItem(indexItem.getKey(), indexItem);
  ...
  Index(final Environment environment) throws IOException {
    ...
    this.indexFiles =
        new IndexFile
            [environment.getConfig().getIndexNodeCount()
                / environment.getConfig().getIndexNodesPerFile()];
    ...
    do {
      ...
      if (indexFileExists == true) performIndexInit = false;
    ...
    if (performIndexInit) {
      ...
      for (long n = 0; n < this.environment.getConfig().getIndexNodeCount(); n++) {
        indexNodePositionHeaderBaos.write(
            Longs.toByteArray(
                indexNodeOffsetPosition
                    + (n
                        * this.environment
                            .getConfig()
                            .getIndexNodeSize()))); // Longs.toByteArray(-1));
      ...
      for (int n = 0; n < this.environment.getConfig().getIndexNodeCount(); n++) {
        ...
        IndexNode indexNode =
            new IndexNode(indexNodeID, this.environment.getConfig().getIndexNodeSize());
  ...
  IndexItem getIndexItem(final Transaction transaction, final InternalKey internalKey)
      throws IOException {
    ...
    try {
      this.environment
          ...
          .lock(
              indexNodeID,
              transaction,
              this.environment.getConfig().getLockTimeout(),
              TimeUnit.SECONDS);
  ...
  IndexItem readIndexItem(final Database database, final InternalKey internalKey)
      throws IOException {
    IndexItem indexItem = this.indexWorker.getIndexItem(internalKey);
    if (indexItem != null && indexItem.equals(IndexItem.VACANT) == false) return indexItem;
  ...
  private Collection<IndexNode> readIndexNodes(final Collection<IndexNodeID> indexNodeIDs)
      throws IOException {
    final MutableObjectLongMap<IndexNodeID> indexNodePositions =
        ObjectLongMaps.mutable.ofInitialCapacity(indexNodeIDs.size());
    synchronized (this.indexMutex) {
      for (final IndexNodeID indexNodeID : indexNodeIDs) {
        ...
        if (indexNode != null) indexNodePositions.getIfAbsentPut(indexNodeID, Long.MAX_VALUE);
        else
    ...
    for (final IndexNodeID indexNodeID : indexNodePositions.keySet()) {
      ...
      else indexNodes.add(readIndexNode(indexNodeID, indexNodePositions.getOrThrow(indexNodeID)));
    }
        ...
        if (vamosLog.hasLevel(Logging.DEBUG))
          vamosLog.debug(
              "Created index node "
                  ...
                  + indexNode.size()
                  + " items");
      } else indexNode = readIndexNode(indexNodeID, indexNodePosition);
  ...
  private IndexNode readIndexNode(final IndexNodeID indexNodeID, final long position)
      throws IOException {
    ...
    synchronized (indexFile) {
      if (indexFile.getLastPosition() != remappedPosition) indexFile.get().seek(remappedPosition);
      else this.consecutiveIndexNodeReads.incrementAndGet();
    ...
    if (vamosLog.hasLevel(Logging.DEBUG)) vamosLog.debug("Read index node " + indexNode);
  ...
  private long getIndexNodePosition(final IndexNodeID indexNodeID, final boolean allocate)
      throws IOException {
    long indexNodeOffsetPosition = this.environment.getConfig().getIndexNodeCount() * Long.BYTES;
    long indexNodePosition =
        indexNodeOffsetPosition
            + ((long) indexNodeID.value() * this.environment.getConfig().getIndexNodeSize());
  ...
  void writeIndexNode(final IndexNode indexNode) throws IOException {
    if (this.environment.getConfig().isIndexNodeDeferredWriteAvailable() == false
        ...
        || System.currentTimeMillis() - indexNode.modifiedAt()
            >= this.environment.getConfig().getCheckpointInterval(TimeUnit.MILLISECONDS) / 2)
      writeIndexNodeInternal(indexNode);
    else this.indexNodeModifications.addAndGet(indexNode.modifications());
  }
  ...
  private void writeIndexNodes(final Collection<IndexNode> indexNodes) throws IOException {
    if (this.environment.getConfig().isIndexNodeDeferredWriteAvailable() == false)
      ...
      for (IndexNode indexNode : indexNodes) writeIndexNode(indexNode);
    }
  }

  private void writeIndexNodesInternal(final Collection<IndexNode> indexNodes) throws IOException {
    final List<Entry<Long, IndexNode>> indexNodePositions =
        new ArrayList<Entry<Long, IndexNode>>(indexNodes.size());
    synchronized (this.indexMutex) {
      for (final IndexNode indexNode : indexNodes)
        indexNodePositions.add(
            new AbstractMap.SimpleEntry<Long, IndexNode>(
                getIndexNodePosition(indexNode.getID(), true), indexNode));
    }

    indexNodePositions.sort(
        (o1, o2) -> {
  ...
  private void writeIndexNodeInternal(
      final IndexNode indexNode, final long position, final ByteBuffer serialized)
      throws IOException {
    final IndexFile indexFile = mapPositionToIndexFile(position);
    final long remappedPosition = remapPositionByIndexFile(position);
    ...
    synchronized (indexFile) {
      if (indexFile.getLastPosition() != remappedPosition) indexFile.get().seek(remappedPosition);
      else this.consecutiveIndexNodeWrites.incrementAndGet();
    ...
    if (vamosLog.hasLevel(Logging.DEBUG)) vamosLog.debug("Wrote index node " + indexNode);
  ...
  private void logInfo() {
    ...
    if (age == 0 || age <= this.lastLog) return;
    ...
    if (TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis() - this.opened) % 10 == 0) {
      ...
      vamosLog.info(
          String.format(
              "  Item Reads %d %d/s",
              this.indexItemReads.get(), (this.indexItemReads.get() / age)));
      vamosLog.info(
          String.format(
              "  Item Writes %d %d/s",
              this.indexItemWrites.get(), (this.indexItemWrites.get() / age)));
      vamosLog.info(
          String.format(
              "  Node Reads %d %d/s %d",
              this.indexNodeReads.get(),
              (this.indexNodeReads.get() / age),
              this.consecutiveIndexNodeReads.get()));
      vamosLog.info(
          String.format(
              "  Node Writes %d %d/s %d %d",
              this.indexNodeWrites.get(),
              (this.indexNodeWrites.get() / age),
              this.consecutiveIndexNodeWrites.get(),
              this.indexNodeModifications.get()));
  ...
  private IndexFile mapPositionToIndexFile(final long position) {
    final long indexNodeOffsetPosition =
        this.environment.getConfig().getIndexNodeCount() * Long.BYTES;
    final long adjustedPosition = position - indexNodeOffsetPosition;
    if (adjustedPosition < 0)
      throw new IllegalArgumentException(
          "Position  is negative after adjusting for index node offset");

    int i = (int) (adjustedPosition / this.environment.getConfig().getIndexNodeSize());
    i /= this.environment.getConfig().getIndexNodesPerFile();
  ...
  private long remapPositionByIndexFile(final long position) {
    ...
    if (i == 0) {
      ...
      final long remappedPosition =
          adjustedPosition
              - (this.environment.getConfig().getIndexNodesPerFile()
                  * i
                  * this.environment.getConfig().getIndexNodeSize());
      return remappedPosition;
    }
  }
}
