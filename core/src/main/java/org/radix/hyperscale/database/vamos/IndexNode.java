...
class IndexNode 
{
	...
	IndexItem put(final IndexItem item) throws IOException
	{
		synchronized(this)
		{
			if (this.items.size() == this.capacity)
				throw new IndexNodeCapacityException(this);
			else if (this.items.size() == ((this.capacity / 4)*3))
				vamosLog.warn("IndexNodeID "+this.id.value()+" is at 75% of "+this.capacity+" capacity");
			else if (this.items.size() == ((this.capacity / 10)*9))
				vamosLog.warn("IndexNodeID "+this.id.value()+" is at 90% of "+this.capacity+" capacity");
	...
	private void write(final ByteBuffer output, final boolean padding) throws IOException
	{
		...
		synchronized(this)
		{
			...
			for(IndexItem indexNodeItem : this.items.values())
				indexNodeItem.write(output);
		}

		if (padding)
			output.position(startPos + length());
	}
	...
	{
		return "[id="+ this.id.value() + " size=" + this.length+ " capacity=" + this.capacity+" items="+this.items.size()+"]";
	}
}
