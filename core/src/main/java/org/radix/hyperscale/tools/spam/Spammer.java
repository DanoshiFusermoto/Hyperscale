...
public abstract class Spammer extends Executable {
  ...
  protected Spammer(
      final Spamathon spamathon,
      final int iterations,
      final int rate,
      final Range<Integer> saturation,
      final ShardGroupID targetShardGroup,
  ...
  public boolean nextIsIsolatedShard() {
    return ThreadLocalRandom.current().nextInt(this.iterations)
        < (this.iterations * this.shardFactor);
  }
  ...
  public void submitOverInterval(
      final Context submitTo,
    ...
    while (scheduledDelay < intervalMillis) {
      ...
      while (scheduledAtomSubmissions < intervalSubmitCount && atomIterator.hasNext()) {
        ...
        Executor.getInstance()
            .schedule(() -> submitTo.getLedger().submit(atom), scheduledDelay, unit);
        this.atoms.add(atom);
      }
  ...
  void submitAtomAndWaitAtPendingThreshold(
      final Context submitTo, final Atom atom, final int maxAtomsWaitThreshold) throws Exception {
  ...
  void submitAtomAndWaitAtPendingThreshold(
      final Context submitTo,
      final Atom atom,
      final int maxAtomsWaitThreshold,
  ...
  void waitUntilAtomsAreCompleted()
      throws InterruptedException, ExecutionException, TimeoutException {
    if (this.atoms.isEmpty()) return;

    waitUntilAtomsAreCompleted(this.atoms);
    this.atoms.clear();
  }

  void waitUntilAtomsAreCompleted(final Runnable completionCallback)
      throws InterruptedException, ExecutionException, TimeoutException {
    if (this.atoms.isEmpty()) return;
  ...
  void waitUntilAtomsAreCompleted(final Collection<Atom> atoms)
      throws InterruptedException, ExecutionException, TimeoutException {
    waitUntilAtomsAreCompleted(atoms, null);
  }

  void waitUntilAtomsAreCompleted(final Collection<Atom> atoms, final Runnable completionCallback)
      throws InterruptedException, ExecutionException, TimeoutException {
    ...
    while (this.atoms.isEmpty() == false) {
      ...
      for (final Atom atom : this.atoms) {
        Future<SubstateSearchResponse> response =
            Context.get()
                .getLedger()
                .get(new SubstateSearchQuery(StateAddress.from(Atom.class, atom.getHash())));
      ...
      for (final Future<SubstateSearchResponse> searchFuture : searchFutures) {
        ...
        if (result != null && result.getSubstate().get(NativeField.CERTIFICATE) != null)
          statuses.put(
              searchFuture.get().getQuery().getAddress().scope(), AtomCompletionStatus.COMPLETED);
        else if (System.currentTimeMillis()
            > start + TimeUnit.SECONDS.toMillis(Constants.ATOM_ACCEPT_TIMEOUT_SECONDS))
          statuses.put(
              searchFuture.get().getQuery().getAddress().scope(), AtomCompletionStatus.TIMEDOUT);
      }
      ...
      while (atomIterator.hasNext()) {
        ...
        if (completionStatus == AtomCompletionStatus.COMPLETED) atomIterator.remove();
      ...
      if (this.atoms.isEmpty() == false) {
        ...
        if (iterationInterval > 0) Thread.sleep(iterationInterval);
      }
    }
  ...
  void waitUntilAtomIsCompleted(final Atom atom, final long timeout, final TimeUnit unit)
      throws InterruptedException, ExecutionException, TimeoutException {
    final long start = System.currentTimeMillis();
    ...
    while (isCompleted == false && isTimedout == false) {
      ...
      if (isAtomCompleted(atom)) isCompleted = true;
    ...
    if (isTimedout) throw new TimeoutException("Completion timeout for atom " + atom.getHash());
  }
  ...
  private boolean isAtomCompleted(final Atom atom) throws InterruptedException, ExecutionException {
    ...
    if (result != null && result.getSubstate().get(NativeField.CERTIFICATE) != null) return true;
  ...
  int pluckSaturationInRange() {
    if (this.saturation.getMinimum().equals(this.saturation.getMaximum()))
      ...
      return (int)
          Math.floor(
              this.saturation.getMinimum()
                  + (this.saturation.getMaximum() - this.saturation.getMinimum())
                      * (1 - Math.exp(-x)));
    }
  }
}
