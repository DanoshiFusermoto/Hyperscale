...
final class UniqueValueSpammer extends Spammer {
  ...
  protected UniqueValueSpammer(
      Spamathon spamathon,
      int iterations,
      int rate,
      Range<Integer> saturation,
      ShardGroupID targetShardGroup,
      double shardFactor) {
  ...
  public void execute() {
    ...
    try {
      ...
      final String uniqueValueComponentContext =
          UniqueValueComponent.class.getAnnotation(StateContext.class).value();

      final List<Context> contexts =
          Collections.unmodifiableList(new ArrayList<Context>(Context.getAll()));
      while (getProcessed().get() < getIterations() && isCancelled() == false) {
        ...
        try {
          ...
          if (signer != null) pubkey = signer.getPublicKey();
          else {
          ...
          if (getTargetShardGroup() == null) {
            ...
            targetShardGroupID =
                ShardMapper.toShardGroup(
                    StateAddress.from(
                        uniqueValueComponentContext, Hash.valueOf(UInt256.from(value))),
                    numShardGroups);
          } else {
            long value;
            ...
            if (isIsolatedShard == false && saturation > 1) {
              for (int u = 1; u < saturation; u++) {
                ...
                atomShardGroups.add(
                    ShardMapper.toShardGroup(
                        StateAddress.from(
                            uniqueValueComponentContext, Hash.valueOf(UInt256.from(extraValue))),
                        numShardGroups));
              }
            }

            atom = atomBuilder.build();
          } while (isIsolatedShard
              && ShardMapper.toShardGroup(
                          StateAddress.from(atomContext, atom.getHash()), numShardGroups)
                      .equals(targetShardGroupID)
                  == false);

          if (signer != null) atom.sign(signer);
          else atom.sign(pubkey, EDSignature.NULL);
        } catch (Throwable t) {
          spammerLog.error(t);
        } finally {
        ...
        try {
          ...
          for (Context context : contexts) {
            final ShardGroupID contextShardGroupID =
          ...
          if (submitted == false) {
            for (ShardGroupID shardGroupID : atomShardGroups) {
              final Context submissionContext =
                  contexts.get(ThreadLocalRandom.current().nextInt(contexts.size()));
              final SubmitAtomsMessage submitAtomsMessage = new SubmitAtomsMessage(atom);
              final StandardConnectionFilter submissionConnectionFilter =
                  StandardConnectionFilter.build(submissionContext)
                      .setStates(ConnectionState.CONNECTED)
                      .setShardGroupID(shardGroupID)
                      .setSynced(true)
                      .setStale(false);
              final List<AbstractConnection> connections =
                  submissionContext.getNetwork().get(submissionConnectionFilter);
              if (connections.isEmpty() == false) {
                submissionContext
                    .getNetwork()
                    .getMessaging()
                    .send(submitAtomsMessage, connections.getFirst());
                break;
              }
            }
          }
        ...
        if (getProcessed().get() == nextBurstInterval) {
          ...
          if (adjustedWaitDurationNano > 0) LockSupport.parkNanos(adjustedWaitDurationNano);

          nextBurstInterval += 32;
          burstStartNano = System.nanoTime();

          if (System.currentTimeMillis() - lastProgressReport > 1000
              && getProcessed().get() >= getRate()) {
            spammerLog.info(
                "Submitted "
                    + getProcessed()
                    + "/"
                    + getIterations()
                    + " of spam atoms to ledger "
                    + (getProcessed().get()
                            / Math.max(
                                1,
                                TimeUnit.MILLISECONDS.toSeconds(
                                    System.currentTimeMillis() - spamStart))
                        + " / "
                        + getRate()));
