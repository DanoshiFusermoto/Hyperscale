...
public class Hyperscale
{
	...
	public static void main(String[] args)
	{
		...
		{
			System.out.println("Processor cores of "+Runtime.getRuntime().availableProcessors()+" is too low for efficient operation!");
		...
		if (XMX < minXMX) // 4GB
		{
			System.out.println("Allocated heap of "+Runtime.getRuntime().maxMemory()+" is too low for efficient operation!");
			System.out.println("Please increase allocated heap to AT LEAST 2GB by including the runtime option -Xmx.");
		...
		if(Runtime.version().feature() < Constants.JAVA_VERSION_MIN || Runtime.version().feature() > Constants.JAVA_VERSION_MAX)
		{
			System.out.println("Java runtime version "+Runtime.version()+" not supported!");
			System.out.println("Supported Java runtimes are versions "+Constants.JAVA_VERSION_MIN+" to "+Constants.JAVA_VERSION_MAX);
			System.exit(911);
		}
		...
		try
		{
			...
			// Logging //
			System.setProperty("info", Boolean.toString(Configuration.getDefault().getCommandLine("info", false)));
			...
			if (Boolean.getBoolean("console") == true)
			{
				...
				if (Boolean.getBoolean("godix") == true)
				{
					new Console(System.in, System.out, new org.radix.hyperscale.console.Ledger(), new org.radix.hyperscale.console.Network(), 
						   	   						   new org.radix.hyperscale.console.Atoms(), new org.radix.hyperscale.console.Serializer(), 
													   new org.radix.hyperscale.console.System(), new org.radix.hyperscale.console.Validators(),
												   	   new org.radix.hyperscale.console.Wallet(), new org.radix.hyperscale.console.Tokens(),
	...
	private static class JarDetector 
	{
	    public static boolean isInstanceRunning() throws Exception 
	    {
	        ...
	        final String jarPath = JarDetector.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath();
            ...
            final Set<ObjectName> runtimeBeans = mbs.queryNames(new ObjectName("java.lang:type=Runtime,*"), null);
	...
	private Hyperscale() throws Exception
	{
		...
		if (Configuration.getDefault().has("contexts.definitions"))
		{
			...
			contexts.addAll(Context.createAndStart(Configuration.getDefault().getCommandLine("contexts", 1), "node", Configuration.getDefault()));
		
		// KILL LISTENER // 
		for (final Context context : contexts)
		{
			context.getNetwork().getMessaging().register(KillMessage.class, this.getClass(), new MessageProcessor<KillMessage>()
			{
				@Override
				public void process(final KillMessage killMessage, final AbstractConnection connection)
				{
					try
					{
						...
						for (final AbstractConnection relayConnection : allConnections)
						{
							...
							{
								log.error(context.getName()+": Could not relay "+killMessage.getClass().getAnnotation(SerializerId2.class)+" to " + relayConnection, ioex);
							}
						}
					...
					{
						log.error(context.getName()+": "+killMessage.getClass().getAnnotation(SerializerId2.class)+" " + connection, ex);
					}
				}
			});
		}
		...
		if (Configuration.getDefault().get("system.monitor.deadlocks", Boolean.TRUE))
		{
			...
			final long deadlockDetectionInterval = TimeUnit.SECONDS.toMillis(Long.parseLong(Configuration.getDefault().get("system.monitor.deadlocks.interval", "5")));
