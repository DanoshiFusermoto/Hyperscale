...
public final class EDPublicKey extends PublicKey<EDSignature> {
  @JsonValue private final byte[] publicKey;
  ...
  public synchronized Identity getIdentity() {
    if (this.identity == null) this.identity = new Identity(Identity.EC, this);
  ...
  public boolean verify(final byte[] hash, final EDSignature signature) throws CryptoException {
    ...
    if (hash.length == 0) throw new IllegalArgumentException("Hash length is zero");
    ...
    if (EDKeyPair.SKIP_VERIFICATION) return true;
