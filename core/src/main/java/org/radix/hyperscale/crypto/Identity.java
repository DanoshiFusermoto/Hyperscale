...
public final class Identity extends Key {
  ...
  public static Identity from(final String identity, final int offset) {
    byte[] bytes =
        Base58.fromBase58(Objects.requireNonNull(identity, "Identity string is null"), offset);
    return from(bytes, 0);
  }
  ...
  private Identity(byte[] bytes, int offset) {
    ...
    if (this.prefix == 0x00 && Bytes.allZero(bytes, offset, bytes.length - offset))
      this.key = ComputeKey.NULL;
    else if (this.prefix == EC) this.key = EDPublicKey.from(bytes, offset + 1);
    else if (this.prefix == BLS) this.key = BLSPublicKey.from(bytes, offset + 1);
    else if (this.prefix == COMPUTE) this.key = ComputeKey.from(bytes, offset + 1);
    else
      throw new IllegalArgumentException(
          "Identity of type " + this.prefix + " is not supported for " + Base58.toBase58(bytes));
  }
  ...
  boolean computeEqual(final Object object) {
    ...
    if (object instanceof Identity identity) {
      if (this.prefix != identity.prefix) return false;

      if (Arrays.equals(this.key.toByteArray(), identity.key.toByteArray()) == false) return false;
