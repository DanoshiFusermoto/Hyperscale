...
public final class BLS12381 {
  ...
  static {
    if (SKIP_SIGNING) System.err.println("Warning: BLS signing is being skipped");
    if (SKIP_VERIFICATION) System.err.println("Warning: BLS verification is being skipped");
  }
  ...
  public static BLSSignature sign(final BLSPrivateKey privateKey, final byte[] message) {
    ...
    if (SKIP_SIGNING) return BLSSignature.NULL;
  ...
  public static boolean verify(
      final BLSPublicKey publicKey, final BLSSignature signature, final byte[] message) {
    ...
    GTPoint e =
        AtePairing.pair2(hashInGroup1, publicKey.g2Point(), signature.g1Point(), g2GeneratorNeg);
    return e.isunity();
  }
  ...
  public static BLSPublicKey aggregatePublicKey(final List<BLSPublicKey> publicKeys) {
    ...
    if (SKIP_VERIFICATION) return BLSPublicKey.NULL;

    if (publicKeys.size() == 1) return publicKeys.get(0);
