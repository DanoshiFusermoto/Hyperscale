...
public abstract class Key implements Comparable<Key>, Hashable {
  ...
  public final synchronized int hashCode() {
    if (this.hashCode == Long.MAX_VALUE) this.hashCode = computeHashCode();
  ...
  boolean computeEqual(Object object) {
    if (object instanceof Key key) return Arrays.equals(key.toByteArray(), toByteArray());
  ...
  public final synchronized String toString(int truncateTo) {
    if (truncateTo > -1) Numbers.lessThan(truncateTo, 4, "Key truncation must be at least 4");

    if (this.cachedBase58 == null) this.cachedBase58 = Base58.toBase58(toByteArray());
    ...
    if (truncateTo >= 4) {
      if (this.cachedBase58.length() > truncateTo) {
        ...
        truncatedBuilder.append(
            this.cachedBase58,
            this.cachedBase58.length() - halfTruncate,
            this.cachedBase58.length());
