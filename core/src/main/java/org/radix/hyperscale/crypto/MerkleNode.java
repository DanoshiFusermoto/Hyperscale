...
public class MerkleNode implements Hashable {
  ...
  public void setLeftNode(MerkleNode node) {
    if (node.hash == null) throw new InvalidParameterException("Node hash must be initialized!");
  ...
  public void setRightNode(MerkleNode node) {
    ...
    if (this.leftNode != null) computeHash();
  }
  ...
  public synchronized void computeHash() {
    if (this.hash == null) {
      ...
      else this.hash = Hash.hash(this.leftNode.hash, this.rightNode.hash);
    }
