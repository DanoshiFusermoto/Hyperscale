...
/**
 * The BLAKE2s digest algorithm. This BLAKE2 flavor is optimized for 8 to 32-bit platforms and
 * produces digests of any size between 1 and 32 bytes. This class implements the BLAKE2s algorithm
 ...
 * features are considered optional and not covered by the RFC. BLAKE2s can be directly keyed,
...
public final class Blake2s implements Blake2 {
   ...
   * @throws IllegalArgumentException if {@code digestLength} is not in the {@code [1, 32]} range.
   */
  ...
  /**
   * Creates a new keyed {@code Blake2s} instance. If the key is empty, then the created instance is
   ...
   * @throws IllegalArgumentException if {@code key}'s length is greater than {@code 32} or if
   *     {@code digestLength} is not in the {@code [1, 32]} range.
   */
