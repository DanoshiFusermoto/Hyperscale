	...
	private void F(byte[] input, boolean lastBlock)
	{
        ...
        if (c <= BLOCK_LENGTH) 
        {
            ...
            for (int i = 0; i < fullLongs; i++)
        		m[i] = (long) LONG_HANDLE.get(input, i * 8);
            ...
            if (c % 8 != 0) 
            {
                ...
                for (int i = 0; i < c % 8; i++)
                    partialLong |= (long)(input[start + i] & 0xFF) << (8 * i);

                m[fullLongs] = partialLong;
            }
            
            // Zero-fill the rest of m
            for (int i = (c + 7) / 8; i < 16; i++)
                m[i] = 0;
        } 
        else 
        {
        	m[0]  = (long) LONG_HANDLE.get(input,   0); m[1]  = (long) LONG_HANDLE.get(input,   8);
        	m[2]  = (long) LONG_HANDLE.get(input,  16); m[3]  = (long) LONG_HANDLE.get(input,  24);
        	m[4]  = (long) LONG_HANDLE.get(input,  32); m[5]  = (long) LONG_HANDLE.get(input,  40);
        	m[6]  = (long) LONG_HANDLE.get(input,  48); m[7]  = (long) LONG_HANDLE.get(input,  56);
        	m[8]  = (long) LONG_HANDLE.get(input,  64); m[9]  = (long) LONG_HANDLE.get(input,  72);
        	m[10] = (long) LONG_HANDLE.get(input,  80); m[11] = (long) LONG_HANDLE.get(input,  88);
        	m[12] = (long) LONG_HANDLE.get(input,  96); m[13] = (long) LONG_HANDLE.get(input, 104);
        	m[14] = (long) LONG_HANDLE.get(input, 112); m[15] = (long) LONG_HANDLE.get(input, 120);
        }
