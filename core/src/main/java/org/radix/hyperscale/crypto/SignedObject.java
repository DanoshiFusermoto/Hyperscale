...
public final class SignedObject<K extends PublicKey, T> extends Serializable
{
	...
	public final synchronized void sign(final KeyPair<?, ?, ?> key) throws CryptoException, SerializationException
	{
		...
		Hash objectHash;
		if (this.object instanceof Hashable hashable)
			objectHash = hashable.getHash();
		else
			objectHash = Hash.hash(Serialization.getInstance().toDson(this.object, Output.HASH));
	...
	public final synchronized boolean verify(final K key) throws CryptoException, SerializationException
	{
		...
		if (getOwner() == null)
			throw new CryptoException("Owner is not present");

		if (key.equals(getOwner()) == false)
			throw new CryptoException("Owner does not match key");
