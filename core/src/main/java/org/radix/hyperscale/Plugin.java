...
{
    /**
     * Loads all components of a type hierarchy, including interfaces, abstract classes,
     * and concrete implementations to ensure all static blocks are executed.
     * 
     ...
     */
    public static <T> void loadTypeHierarchy(final String packageName, final Class<T> baseType, final Consumer<Class<?>> callback) 
    {
        ...
        if (baseType.equals(Object.class) == false)
            typesToLoad.add(baseType);
        
        final Set<Class<?>> interfaces = allSubTypes.stream().filter(Class::isInterface).collect(Collectors.toSet());
        typesToLoad.addAll(interfaces);
        
        final Set<Class<?>> abstractClasses = allSubTypes.stream()
                								   .filter(clazz -> !clazz.isInterface() && Modifier.isAbstract(clazz.getModifiers()))
                								   .collect(Collectors.toSet());
        typesToLoad.addAll(abstractClasses);
        
        final Set<Class<?>> concreteClasses = allSubTypes.stream()
                									.filter(clazz -> !clazz.isInterface() && Modifier.isAbstract(clazz.getModifiers()) == false)
                									.collect(Collectors.toSet());
        ...
        if (log.hasLevel(Logging.DEBUG))
        {
	        ...
	        log.debug("- Base type: "+(baseType.isInterface() ? "interface" : "class")+" "+baseType.getName());
            ...
            if (loadedTypes.contains(type) == false) 
            {
                ...
                try 
                {
                    ...
                    if (type.isInterface()) 
                    {
                        ...
                        if (log.hasLevel(Logging.DEBUG))
                        	log.debug("Processed interface: "+type.getName());
                    } 
                    else 
                    {
                        ...
                        if (log.hasLevel(Logging.DEBUG))
                        	log.debug("Loaded "+(Modifier.isAbstract(type.getModifiers()) ? "abstract class: " : "concrete class: ")+type.getName());
                    }
     ...
     */
    private static void loadParentTypesRecursively(final Class<?> type, final Set<Class<?>> loadedTypes) 
    {
    	final Logger log = Logging.getLogger();

    	// Skip if already loaded or Object
        if (loadedTypes.contains(type) || type.equals(Object.class))
            return;
        ...
        if (superclass != null && superclass.equals(Object.class) == false) 
        {
            ...
            if (superclass.isInterface() == false && loadedTypes.contains(superclass) == false) 
            {
                try 
                {
                    ...
                    if (log.hasLevel(Logging.DEBUG))
                    	log.debug("Loaded parent " + (Modifier.isAbstract(superclass.getModifiers()) ? "abstract class: " : "class: ") + superclass.getName());
                } 
                catch (ClassNotFoundException | ExceptionInInitializerError ex) 
                {
