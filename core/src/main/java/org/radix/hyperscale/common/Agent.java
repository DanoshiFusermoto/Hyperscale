...
public final class Agent
{
	...
	public static final URI getURI(String host)
	{
		...
		if (host.toUpperCase().startsWith(Agent.URI_PREFIX) == false)
	...
	public static final URI getURI(final String host, final int port)
	{
		...
		if (port > 0)
			return URI.create(Agent.URI_PREFIX+host+":"+port);
		else
			return URI.create(Agent.URI_PREFIX+host);
	}
	...
	public static final URI getURI(final String host, final Node node) 
	{
		...
		if (node.getServices().isEmpty() == false)
		{
			...
			for (Services service : node.getServices())
				servicesJoiner.add(service.name());
