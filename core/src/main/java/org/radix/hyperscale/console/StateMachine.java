...
public class StateMachine extends Function {
  private static final Options options =
      new Options()
          .addOption(Option.builder("construct").desc("Starts an atom construction").build())
          ...
          .addOption(
              Option.builder("instantiate").desc("Instantiate a blueprint").numberOfArgs(3).build())
          .addOption(
              Option.builder("constructor")
                  .desc("Calls a components constructor")
                  .numberOfArgs(1)
                  .build())
          .addOption(
              Option.builder("method").desc("Calls a component method").numberOfArgs(2).build())
  ...
  public void execute(Context context, String[] arguments, PrintStream printStream)
    ...
    if (commandLine.hasOption("construct")) {
      ...
      if (commandLine.getArgs().length != 0) {
        ...
        if (wallet == null) throw new IllegalStateException("No wallet is open to sign");
      ...
      try {
        ...
        printStream.println(
            "Submitting atom "
                + atom.getHash()
                + " -> "
                + Serialization.getInstance().toJsonObject(atom, Output.API).toString(4));
      ...
      final String blueprintCode =
          FileUtils.readFileToString(blueprintFile, StandardCharsets.UTF_8);
      ...
      try {
        identity =
            Serialization.getInstance()
                .fromTypedString(deployOptionValues[deployOptionValues.length - 1]);
      } catch (Exception ex) {
        if (wallet == null) printStream.println("ERROR: No identity supplied");
      ...
      if (Pattern.matches("^(?:[*]*[a-zA-Z0-9.-]){4}[*a-zA-Z0-9.-]*$", componentContext) == false)
        throw new IllegalArgumentException(
            "Component context " + componentContext + " is not valid");
      ...
      if (atomBuilder == null) {
        printStream.println(
            "Instantiating blueprint " + blueprintContext + " as context " + componentContext);
        atomBuilder = new Atom.Builder();
      }
      ...
      Atom.Builder atomBuilder = this.atomBuilder;
      if (atomBuilder == null) atomBuilder = new Atom.Builder();
      ...
      final StringBuilder builder = new StringBuilder();
      builder
          .append(commandLine.getOptionValues("method")[0])
          .append('.')
          .append(commandLine.getOptionValues("method")[1])
          .append('(');
      builder.append(String.join(", ", commandLine.getArgList()));
      builder.append(")");
      atomBuilder.push(builder.toString());
      ...
      final StateAddress stateAddress =
          StateAddress.from(substateContext, Hash.valueOf(decodedSubstateScope));
      final Future<SubstateSearchResponse> searchFuture =
          context.getLedger().get(new SubstateSearchQuery(stateAddress, Isolation.COMMITTED));
      ...
      final JSONObject substateJSON =
          Serialization.getInstance().toJsonObject(searchResult.getResult(), Output.API);
