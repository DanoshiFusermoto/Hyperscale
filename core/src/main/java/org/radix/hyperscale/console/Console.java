...
public class Console {
  ...
  private void run() {
    ...
    while (command.equalsIgnoreCase("exit") == false && this.scanner.hasNext()) {
      ...
      if (contextIndex == -1) context = Context.get();
      else {
        ...
        } else context = Context.get();
      }

      final int spaceIndex = input.indexOf(' ');
      if (spaceIndex == -1) command = input;
      else {
    ...
    synchronized (this.functions) {
      if (this.functions.containsKey(function.getName().toLowerCase()))
        throw new IllegalStateException(
            "Function " + function.getName() + " is already registered");
  ...
  public boolean deregister(String function) {
    ...
    synchronized (this.functions) {
      if (this.functions.remove(function) == null) return false;
