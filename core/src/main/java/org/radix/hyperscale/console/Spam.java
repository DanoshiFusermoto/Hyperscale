...
public class Spam extends Function {
  private static final Options options =
      new Options()
          .addOption("i", "iterations", true, "Quantity of spam iterations")
          .addOption("r", "rate", true, "Rate at which to produce events")
          .addOption("spammer", true, "Which spammer to run")
          .addOption(
              "saturation", true, "Saturation range per atom (SwapPoolSpammer, UniqueValueSpammer)")
          .addOption(
              "shardtarget", true, "The shard group to target for all atoms (UniqueValueSpammer)")
          .addOption(
              "shardfactor",
              true,
              "Factor of forced single shard atoms (UniqueValueSpammer, TokenTransferSpammer)")
          .addOption(
              "fundingsource",
              true,
              "File path to wallet key to use as funding source (TokenTransferSpammer)")
          .addOption(
              "destinations",
              true,
              "List of comma delimited idenitites to include as destinations (TokenTransferSpammer)")
  ...
  public void execute(Context context, String[] arguments, PrintStream printStream)
    ...
    final Spammer spammer =
        Spamathon.getInstance()
            .spam(
                name,
                Integer.parseInt(commandLine.getOptionValue("iterations")),
                rate,
                saturation,
                commandLine.hasOption("shardTarget")
                    ? ShardGroupID.from(Integer.parseInt(commandLine.getOptionValue("shardtarget")))
                    : null,
    ...
    printStream.println(
        "Starting spam "
            + name
            + " of "
            + commandLine.getOptionValue("iterations")
            + " iterations at rate of "
            + commandLine.getOptionValue("rate")
            + " ... ");
    ...
    if (commandLine.hasOption("profile")) {
      ...
      while (spammer.getProcessed().get() != spammer.getIterations()) {
        ...
        if (context.getLedger().getHead().getHeight() > lastCommittedBlockHeight) {
          ...
          long committedAtoms =
              context.getLedger().getHead().getInventory(InventoryType.ACCEPTED).size();
          totalAtomsCommittedCount += committedAtoms;
          printStream.println(
              "Atoms: "
                  + totalAtomsCommittedCount
                  + " TPS: "
                  + (totalAtomsCommittedCount / TimeUnit.MILLISECONDS.toSeconds(duration)));
