...
public class Godix extends Function {
  private static final Options options =
      new Options()
          .addOption(
              Option.builder("killnetwork")
                  .desc("Forcibly attempts to kill all nodes in the network")
                  .optionalArg(true)
                  .numberOfArgs(2)
                  .build());
  ...
  public void execute(
    ...
    if (commandLine.hasOption("killnetwork")) {
      ...
      if (optionValues.length > 2) throw new IllegalArgumentException("Too many arguments");
      ...
      for (final String arg : optionValues) {
        ...
        else godixKeypath = new File(arg);
      }
