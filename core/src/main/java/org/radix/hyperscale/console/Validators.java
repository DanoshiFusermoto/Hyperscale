...
{
	private static final Options options = new Options().addOption("remote", false, "Return remote ledger information").addOption("known", false, "All known validator identities")
																													   .addOption(Option.builder("powers").desc("Validators and vote power").optionalArg(true).numberOfArgs(1).build());
	public Validators()
	{
	...
	public void execute(Context context, String[] arguments, PrintStream printStream) throws Exception
	{
		...
		else if (commandLine.hasOption("powers"))
		{
			...
			if (commandLine.getOptionValue("powers") == null)
			{
				...
				epoch = Epoch.from(Integer.parseInt(commandLine.getOptionValue("powers")));
			...
			powers.getAll().forEach((identity, power) -> printStream.println(identity.toString(Constants.TRUNCATED_IDENTITY_LENGTH)+" = "+power));
			printStream.println(powers.size()+" validators with power");
			
			if (remoteShardGroupIDs.isEmpty())
				printStream.println("Total vote power: "+context.getLedger().getValidatorHandler().getTotalVotePower(epoch, localShardGroupID));
			else
				printStream.println("Total vote power: "+context.getLedger().getValidatorHandler().getTotalVotePower(epoch, localShardGroupID)+" / "+context.getLedger().getValidatorHandler().getTotalVotePower(epoch, remoteShardGroupIDs));
		}
	}
}