...
{
	private static final Options options = new Options().addOption("exit", false, "Terminates the Cassandra process").
	...
	public void execute(Context context, String[] arguments, PrintStream printStream) throws Exception
	{
		...
		else if (commandLine.hasOption("locks"))
		{
	    	...
	    	printStream.println(context.getName()+": Vamos Key Locks "+vamosLockManager.getKeyLockCount()+" / "+vamosLockManager.getAverageKeyLockWaitTime()+"ms | "+vamosLockManager.getTotalKeyLockWaitTime()+"ms");
	    	printStream.println(context.getName()+": Vamos Node Locks "+vamosLockManager.getNodeLockCount()+" / "+vamosLockManager.getAverageNodeLockWaitTime()+"ms | "+vamosLockManager.getTotalNodeLockWaitTime()+"ms");
	    	printStream.println();
		...
		{
			printStream.println("Up time: "+DurationFormatUtils.formatDurationWords(context.getUptime(), true, false));
