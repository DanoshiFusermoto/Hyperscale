...
public class BootstrapService extends Executable {
  ...
  public BootstrapService(final Context context) {
    ...
    for (String unparsedURL :
        this.context.getConfiguration().get("network.discovery.urls", "").split(",")) {
      unparsedURL = unparsedURL.trim();
      if (unparsedURL.isEmpty()) continue;
    ...
    for (String unparsedHost : unparsedHosts) {
      ...
      if (unparsedHost.isEmpty()) continue;
      try {
  ...
  public void execute() {
    ...
    final int connectionTimeout =
        this.context.getConfiguration().get("network.discovery.connection.timeout", 10000);
    final int readTimeout =
        this.context.getConfiguration().get("network.discovery.read.timeout", 10000);
    while (isCompleted == false && isTerminated() == false) {
      ...
      for (URI bootstrapHost : shuffledHosts) {
        ...
        try {
          ...
          if (bootstrapHost.getPort() == -1)
            bootstrapHost =
                new URI(
                    bootstrapHost.getScheme(),
                    bootstrapHost.getUserInfo(),
                    bootstrapHost.getHost(),
                    WebService.DEFAULT_PORT,
                    bootstrapHost.getPath(),
                    bootstrapHost.getQuery(),
                    bootstrapHost.getFragment());
          ...
          if (bootstrapHost.getPath() == null || bootstrapHost.getPath().isEmpty())
            bootstrapHost =
                new URI(
                    ...
                    bootstrapHost.getUserInfo(),
                    bootstrapHost.getHost(),
                    bootstrapHost.getPort(),
                    WebService.DEFAULT_API_PATH + "/bootstrap",
                    bootstrapHost.getQuery(),
                    bootstrapHost.getFragment());
          ...
          os.write(
              Serialization.getInstance()
                  .toJson(this.context.getNode(), Output.WIRE)
          ...
          if (response.has("peers")) {
            ...
            if (peersArray != null) {
              for (int p = 0; p < peersArray.length(); p++) {
                final Peer peer =
                    new Peer(URI.create(peersArray.getString(p)), Protocol.TCP, Protocol.UDP);
            ...
            networkLog.info(
                this.context.getName()
                    + ": host "
                    + bootstrapHost
                    + " responsed with null peers array");
      ...
      if (isCompleted == false) {
        try {
          ...
          if (completionDelay < 180000) completionDelay += completionDelay;
        } catch (InterruptedException ex) {
          Thread.currentThread().interrupt();
          networkLog.warn(
              this.context.getName() + ": Bootstrap service polling delay interrupted", ex);
        }
