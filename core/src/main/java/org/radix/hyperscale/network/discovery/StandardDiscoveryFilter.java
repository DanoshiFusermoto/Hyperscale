...
public class StandardDiscoveryFilter implements DiscoveryFilter
{
	...
	public boolean filter(final Peer peer)
	{
		...
		try
		{
			...
			if (peer.getIdentity().equals(this.context.getNode().getIdentity()))
				return false;
	    	...
	    	if (this.shardGroupID != null && ShardMapper.toShardGroup(peer.getIdentity(), this.context.getLedger().numShardGroups()).equals(this.shardGroupID) == false)
				return false;

			if (this.protocol != null && peer.hasProtocol(this.protocol) == false)
				return false;
			
			if (this.identity != null && peer.getIdentity().equals(this.identity) == false)
				return false;

			if (this.uri != null && peer.getURI().equals(this.uri) == false)
				return false;

			if (this.with != null && this.with.apply(peer) == false)
				return false;
