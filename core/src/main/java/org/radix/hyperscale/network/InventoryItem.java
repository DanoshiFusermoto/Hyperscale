...
public final class InventoryItem {
  ...
  public InventoryItem(final Primitive primitive) {
    this(
        Serialization.getInstance()
            .getIdForClass(Objects.requireNonNull(primitive, "Primitive is null").getClass()),
        primitive.getHash(),
        primitive);
  }
  ...
  private InventoryItem(final String type, final Hash item, final Primitive primitive) {
    ...
    else this.typeClass = this.primitive.getClass();
  ...
  public boolean equals(final Object obj) {
    ...
    if (obj instanceof InventoryItem other) {
      if (this.hashCode == other.hashCode) {
        if (other.hash.equals(this.hash) && other.type.equals(this.type)) return true;
      }
    }
  ...
  boolean isUrgent() {
    final TransportParameters transportParameters =
        this.typeClass.getAnnotation(TransportParameters.class);
  ...
  boolean isStale() {
    return System.currentTimeMillis() - this.timestamp
            > TimeUnit.SECONDS.toMillis(Constants.PRIMITIVE_STALE)
        ? true
        : false;
  }
  ...
  public String toString() {
    return "[type = "
        + Serialization.getInstance().getClassForId(this.type).getName()
        + ", hash = "
        + this.hash
        + "]";
  }
