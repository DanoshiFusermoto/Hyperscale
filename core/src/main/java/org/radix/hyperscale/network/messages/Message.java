...
public abstract class Message extends Serializable implements Hashable, Comparable<Message>
{
	...
	private static final ThreadLocal<ByteBuffer> headerBuffer = ThreadLocal.withInitial(() -> ByteBuffer.allocate(HEADER_SIZE));
	private static final ThreadLocal<ByteBuffer> payloadBuffer = ThreadLocal.withInitial(() -> ByteBuffer.allocate(PAYLOAD_BUFFER_SIZE));
	...
	private static final ThreadLocal<ByteBufferOutputStream> payloadOutputStream = ThreadLocal.withInitial(() -> new ByteBufferOutputStream(Message.payloadBuffer.get().clear(), true));
	public static final boolean ALWAYS_COMPRESS = false; // FIXME weird issue with hashes / signatures when compressed
	 ...
	 * payload			= byte[n]
	 */
	public static Message inbound(DataInputStream dataInputStream, AbstractConnection connection) throws IOException, BanException, CryptoException
	{
		...
		{
			final ByteBuffer compressionBuffer = payloadLength > COMPRESSION_BUFFER_SIZE ? ByteBuffer.allocate(payloadLength) : Message.compressionBuffer.get().clear();
			dataInputStream.readFully(compressionBuffer.array(), 0, payloadLength);
			final int uncompressedLength = Snappy.uncompressedLength(compressionBuffer.array(), 0, payloadLength);
			payloadBuffer = uncompressedLength > PAYLOAD_BUFFER_SIZE ? ByteBuffer.allocate(uncompressedLength) : Message.payloadBuffer.get().clear();
			Snappy.uncompress(compressionBuffer.array(), 0, payloadLength, payloadBuffer.array(), 0);
			payloadBuffer.position(uncompressedLength);
		}
		else
		{
			payloadBuffer = payloadLength > PAYLOAD_BUFFER_SIZE ? ByteBuffer.allocate(payloadLength) : Message.payloadBuffer.get();
		...
		if (connection.isHandshaked())
		{
			if (connection.requiresSignatures() || message.requiresSignature())
			{
				...
				final EDSignature signature = Serialization.getInstance().fromDson(signatureBytes, EDSignature.class);
			
				if (connection.getEphemeralRemotePublicKey().verify(payloadHash, signature) == false)
					throw new CryptoException(payloadHash+": Did not validate against connection key "+connection.getEphemeralRemotePublicKey());
			}
		}
	...
	public static void outbound(final Message message, final DataOutputStream dataOutputStream, final AbstractConnection connection) throws IOException, CryptoException
	{
		final ByteBuffer headerBuffer = Message.headerBuffer.get();
		headerBuffer.clear();
		...
		synchronized(message)
		{
			...
			if (connection.requiresSignatures() || message.requiresSignature())
			{
				...
				if (signatureBytes.length > Message.MAX_SIGNATURE_SIZE)
					throw new IOException("Message signature size "+signatureBytes.length+" is too large");
			}
			
			message.setSize(Message.HEADER_SIZE+(signatureBytes == null ? 0 : signatureBytes.length) + payloadLength);
		}
		...
		if (signatureBytes != null)
			dataOutputStream.write(signatureBytes);
		dataOutputStream.write(payloadBytes, 0, payloadLength);
	}
	
	public final String MSN() { return this.getClass().getAnnotation(SerializerId2.class).value(); }
	...
	{
		if (this.size == 0)
			throw new IllegalStateException("Size is unknown for message "+this);
	...
	{
		final TransportParameters transportParameters = getClass().getAnnotation(TransportParameters.class);
