...
public class StandardConnectionFilter implements ConnectionFilter<AbstractConnection> {
  ...
  public boolean filter(final AbstractConnection connection) {
    ...
    if (this.synced != null
        && (connection.getNode() == null || this.synced != connection.getNode().isSynced()))
      return false;
    ...
    if (this.identity != null
        && (connection.getNode() == null
            || connection.getNode().getIdentity().equals(this.identity) == false)) return false;
    ...
    if (this.uri != null) {
      if (this.uri.getScheme() != null
          && connection.getURI().getScheme().equalsIgnoreCase(this.uri.getScheme()) == false)
        return false;

      if (this.uri.getHost() != null
          && connection.getURI().getHost().equalsIgnoreCase(this.uri.getHost()) == false)
        return false;
    ...
    if (this.stale != null && connection.isStale() != this.stale) return false;

    if (this.with != null && this.with.apply(connection) == false) return false;
