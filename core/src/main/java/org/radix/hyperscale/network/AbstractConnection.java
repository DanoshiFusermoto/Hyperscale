...
public abstract class AbstractConnection extends Serializable
  ...
  private class TCPInboundProcessor implements Runnable {
    ...
    public void run() {
      ...
      try {
        bufferedInputStream = new BufferedInputStream(AbstractConnection.this.inputStream, 1 << 18);
        dataInputStream = new DataInputStream(bufferedInputStream);

        if (AbstractConnection.this.direction.equals(Direction.INBOUND)) doHandshake();
        ...
        while (AbstractConnection.this.socket.isConnected()
          ...
          try {
            ...
            AbstractConnection.this
                .context
                .getMetaData()
                .increment("network.transferred.inbound", message.getSize());

            AbstractConnection.this.timeseries.increment(
                "inbound", message.getSize(), System.currentTimeMillis(), TimeUnit.MILLISECONDS);
            AbstractConnection.this
                .context
          ...
          try {
            AbstractConnection.this
                ...
                .getNetwork()
                .getMessaging()
                .received(message, AbstractConnection.this);
          } catch (Exception ex) {
            messagingLog.error(
                AbstractConnection.this.context.getName()
                    + ": Message processing error for "
                    + message
                    + " on "
                    + AbstractConnection.this,
                ex);
        ...
        if (networkLog.hasLevel(Logging.DEBUG))
          networkLog.debug(
              "TCPProcessor thread "
                  + AbstractConnection.this.inboundThread.getName()
                  + " is quitting");
      } catch (Throwable t) {
        networkLog.error(
            "TCPProcessor thread "
    ...
    private final BlockingQueue<Message> outboundQueue =
        new PriorityBlockingQueue<Message>(
            AbstractConnection.this
                .context
                .getConfiguration()
                .get("messaging.outbound.queue_max", 1 << 12),
            new Comparator<Message>() {
              ...
              public int compare(final Message m1, final Message m2) {
                ...
                if (m1.getTimestamp() / Constants.BROADCAST_POLL_TIMEOUT
                    < m2.getTimestamp() / Constants.BROADCAST_POLL_TIMEOUT) return -1;
                if (m1.getTimestamp() / Constants.BROADCAST_POLL_TIMEOUT
                    > m2.getTimestamp() / Constants.BROADCAST_POLL_TIMEOUT) return 1;
    ...
    public void run() {
      ...
      try {
        bufferedOutputStream =
            new BufferedOutputStream(AbstractConnection.this.outputStream, 1 << 18);
        ...
        while (AbstractConnection.this.socket.isConnected()
          ...
          if (AbstractConnection.this.strikes > 0
              && System.currentTimeMillis() > AbstractConnection.this.strikeResetAt) {
            AbstractConnection.this.strikes--;
            if (AbstractConnection.this.strikes > 0)
              AbstractConnection.this.strikeResetAt =
                  System.currentTimeMillis() + TimeUnit.MINUTES.toMillis(1);
            else AbstractConnection.this.strikeResetAt = Long.MAX_VALUE;
          }

          try {
            message = this.outboundQueue.poll(1, TimeUnit.SECONDS);
          } catch (InterruptedException ex) {
            Thread.currentThread().interrupt();
            messagingLog.warn(
                AbstractConnection.this.context.getName()
                    + ": Message outbound processing was interrupted for "
                    + AbstractConnection.this,
                ex);
            continue;
          }
          ...
          try {
            ...
            // Send now
            if (this.outboundQueue.peek() == null) dataOutputStream.flush();

            AbstractConnection.this.totalEgress.addAndGet(message.getSize());
            AbstractConnection.this
                .context
                .getMetaData()
                .increment("network.transferred.outbound", message.getSize());

            AbstractConnection.this.timeseries.increment(
                "outbound", message.getSize(), System.currentTimeMillis(), TimeUnit.MILLISECONDS);
            AbstractConnection.this
                .context
                .getTimeSeries("bandwidth")
                .increment(
        ...
        if (networkLog.hasLevel(Logging.DEBUG))
          networkLog.debug(
              "TCPProcessor thread "
                  + AbstractConnection.this.outboundThread.getName()
                  + " is quitting");
      } catch (Throwable t) {
        networkLog.error(
            "TCPProcessor thread "
  ...
  AbstractConnection(final Context context, final URI uri) throws IOException {
    ...
    try {
      ...
      this.socket.setSendBufferSize(
          this.context.getConfiguration().get("network.tcp.buffer", Constants.DEFAULT_TCP_BUFFER));
      this.socket.connect(
          new InetSocketAddress(uri.getHost(), uri.getPort()),
          (int)
              TimeUnit.SECONDS.toMillis(
                  this.context.getConfiguration().get("network.peer.connect.timeout", 10)));

      networkLog.info(
          this.context.getName() + ": OUTBOUND connection opened to " + this.id + " " + uri);
      networkLog.info(
  ...
  AbstractConnection(final Context context, final Socket socket) throws IOException {
    ...
    networkLog.info(
        this.context.getName() + ": INBOUND connection from " + this.id + " " + getURI().getHost());
    ...
    try {
      ...
      throw new IOException(
          "INBOUND connection from " + this.id + " " + getURI().getHost() + " failed", ex);
    }
  }

  private final void checkBuffersAndWarn() throws SocketException {
    if (AbstractConnection.bufferWarning == false) {
      if (this.socket.getReceiveBufferSize()
              < this.context
                  .getConfiguration()
                  .get("network.tcp.buffer", Constants.DEFAULT_TCP_BUFFER)
          || this.socket.getSendBufferSize()
              < this.context
                  .getConfiguration()
                  .get("network.tcp.buffer", Constants.DEFAULT_TCP_BUFFER)) {
        System.err.println(
            "WARNING: Connection send / receive buffers not set according to 'network.tcp.buffer' of "
                + this.context
                    .getConfiguration()
                    .get("network.tcp.buffer", Constants.DEFAULT_TCP_BUFFER)
                + " bytes");
        System.err.println(
            "           SND_BUF = "
                ...
                + this.socket.getReceiveBufferSize());
        System.err.println(
            "         Performance of connectivity may be sub-par and may need OS level re-configuration to allow larger buffers");
  ...
  private Identity getIdentity() {
    return this.node.getIdentity();
  }

  @JsonProperty("head")
  @DsonOutput(Output.API)
  private Hash getHead() {
    return this.node.getHead().getHash();
  }
  ...
  public final boolean equals(Object object) {
    ...
    if (object instanceof AbstractConnection abstractConnection) {
      if (getID() != abstractConnection.getID()) return false;
  ...
  final void doHandshake() throws IOException {
    ...
    send(
        new HandshakeMessage(
            ...
            this.ephemeralLocalKeyPair.getPublicKey(),
            this.ephemeralKeyBinding));
  ...
  public final void handshake(final EDPublicKey ephemeralRemotePublicKey) throws IOException {
    ...
    networkLog.info(
        this.context.getName()
            + ": Ephemeral keys set as "
            + this.ephemeralLocalKeyPair.getPublicKey()
            + " <> "
            + this.ephemeralRemotePublicKey
            + " for "
            + this);

    if (this.direction.equals(Direction.OUTBOUND)) doHandshake();
  ...
  public final void ban(final String reason, final long duration, final TimeUnit unit)
      throws IOException {
    networkLog.info(
        this.context.getName()
            + ": "
  ...
  public final void strikeOrDisconnect(final String reason) throws IOException {
    this.strikes++;
    if (this.strikes == Constants.MAX_STRIKES_FOR_DISCONNECT) disconnect(reason);
    else {
      networkLog.error(
          this.context.getName()
              + ": "
              ...
              + this.strikes
              + "/"
              + Constants.MAX_STRIKES_FOR_DISCONNECT
              + " - "
              + reason);
  ...
  public final void disconnect(final String reason, final Throwable throwable) throws IOException {
    if (getState().equals(ConnectionState.DISCONNECTING)
        || getState().equals(ConnectionState.DISCONNECTED)) return;
    ...
    try {
      ...
      if (reason != null) {
        ...
        if (throwable != null
            && EOFException.class.isAssignableFrom(throwable.getClass()) == false) {
          if (networkLog.hasLevel(Logging.DEBUG))
            networkLog.error(
                this.context.getName() + ": " + toString() + " - Disconnected - " + reason,
                throwable);
        ...
        if (throwable != null
          ...
          if (networkLog.hasLevel(Logging.DEBUG))
            networkLog.error(
                this.context.getName() + ": " + toString() + " - Disconnected - ", throwable);
            ...
            networkLog.error(
                this.context.getName()
                    ...
                    + " - Disconnected - "
                    + throwable.getClass());
        } else networkLog.info(this.context.getName() + ": " + toString() + " - Disconnected");
      }
    } catch (Exception e) {
      networkLog.error(
          "Exception in disconnect of " + this.context.getName() + ": " + toString(), e);
    } finally {
      if (this.socket != null && this.socket.isClosed() == false) this.socket.close();
  ...
  public final void updateLatency(final long latency) {
    ...
    this.latency.updateAndGet(v -> (int) MathUtils.EWMA(v, latency, 0.9));
  }
  ...
  public long getNextTimeout(int requestWeight, TimeUnit timeUnit) {
    ...
    final long timeoutWeightAllowance =
        (long)
            (Math.sqrt(baselineTimeout)
                * pendingWeight()); // Additional allowance for outstanding requests weight
    final long timeoutRequestAllowance =
        (long) (getLatency() * pendingRequests()); // Additional allowance for outstanding requests
    final long gossipRequestTimeout =
        (long)
            (baselineTimeout
                + timeoutRequestAllowance
                + timeoutWeightAllowance
                + (Math.sqrt(baselineTimeout)
                    * requestWeight)); // Sum with additional allowance for request size
  ...
  public final void updateRequests(int increment) {
    ...
    this.timeseries.increment(
        "requests", increment, System.currentTimeMillis(), TimeUnit.MILLISECONDS);
  }

  public final void updateRequested(int increment) {
    this.totalRequested.addAndGet(increment);
    this.timeseries.increment(
        "requested", increment, System.currentTimeMillis(), TimeUnit.MILLISECONDS);
  }
  ...
  void setConnectedAt(final long timestamp) {
    ...
    final long shuffleRandom =
        TimeUnit.MILLISECONDS.convert(
            ThreadLocalRandom.current()
                .nextInt(Constants.DEFAULT_CONNECTION_STICKY_DURATION_SECONDS),
            TimeUnit.SECONDS);
    this.shuffleAt =
        this.connectedAt
            + TimeUnit.MILLISECONDS.convert(
                Constants.DEFAULT_CONNECTION_STICKY_DURATION_SECONDS, TimeUnit.SECONDS)
            + shuffleRandom;
  }
  ...
  public String toString() {
    return this.id
        ...
        + getProtocol()
        + " "
        + (getNode() != null && getNode().isSynced() ? "synced" : "unsynced")
        + " "
        + getURI().getHost()
        + ":"
        + getURI().getPort()
