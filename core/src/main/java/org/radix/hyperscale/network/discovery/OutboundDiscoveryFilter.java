...
public final class OutboundDiscoveryFilter extends StandardDiscoveryFilter {
  ...
  public boolean filter(final Peer peer) {
    ...
    if (peer.getServices().contains(Services.GATEWAY) == false
        && peer.getServices().contains(Services.BOOTSTRAP) == false) return false;

    if (ShardMapper.toShardGroup(peer.getIdentity(), getContext().getLedger().numShardGroups())
            .equals(this.shardGroupID)
        == false) return false;

    if (peer.getAttemptedAt() > 0
        && peer.getDisconnectedAt() > 0
        && Time.getSystemTime() - peer.getDisconnectedAt()
            < TimeUnit.SECONDS.toMillis(
                getContext().getConfiguration().get("network.peer.inactivity", 30))) return false;

    if (peer.getAttemptAt() > 0
        && Time.getSystemTime() < peer.getAttemptAt() + TimeUnit.SECONDS.toMillis(10)) return false;
