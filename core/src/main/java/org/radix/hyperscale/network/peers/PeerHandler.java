...
public class PeerHandler implements Service {
  ...
  public static final class ConnectionDistanceComparator
      extends IdentityDistanceComparator<AbstractConnection> {
  ...
  private final Map<AbstractConnection, Long> pings =
      Collections.synchronizedMap(new HashMap<AbstractConnection, Long>());
    ...
    try {
      this.context
          ...
          .register(
              PeersMessage.class,
              this.getClass(),
              new MessageProcessor<PeersMessage>() {
                @Override
                public void process(
                    final PeersMessage peersMessage, final AbstractConnection connection) {
                  for (final URI uri : peersMessage.getPeers()) {
                    if (uri.getUserInfo() == null) {
                      networklog.warn(
                          PeerHandler.this.context.getName()
                              + ": Received peer does not have an identity");
                      // Disconnect and ban
                      continue;
                    }
                    ...
                    try {
                      ...
                      if (PeerHandler.this.get(identity) != null) continue;

                      final Peer peer = new Peer(uri);
                      PeerHandler.this.update(peer);
                    } catch (IOException ioex) {
                      networklog.error(
                          PeerHandler.this.context.getName() + ": Failed to update peer " + uri,
                          ioex);
                    }
                  }
                }
      ...
      this.context
          ...
          .register(
              ...
              this.getClass(),
              new MessageProcessor<GetPeersMessage>() {
                @Override
                public void process(
                    final GetPeersMessage getPeersMessage, final AbstractConnection connection) {
                  try {
                    ...
                    final NotLocalPeersFilter notLocalPeersFilter =
                        new NotLocalPeersFilter(PeerHandler.this.context.getNode());
                    final StandardDiscoveryFilter standardDiscoveryFilter =
                        new StandardDiscoveryFilter(PeerHandler.this.context);
                    final List<Peer> peersList =
                        PeerHandler.this
                            .getPeerStore()
                            .get(new MultiPeerFilter(notLocalPeersFilter, standardDiscoveryFilter));
                    ...
                    for (final Peer peer : peersList) {
                      ...
                      if (peersSublist.size() == 64) {
                        PeerHandler.this
                            ...
                            .getNetwork()
                            .getMessaging()
                            .send(new PeersMessage(peersSublist), connection);
                    ...
                    networklog.error(
                        PeerHandler.this.context.getName() + ": peers.get " + connection, ex);
                  }
                }
              });
      ...
      this.context
          ...
          .register(
              ...
              this.getClass(),
              new MessageProcessor<PeerPingMessage>() {
                @Override
                public void process(
                    final PeerPingMessage message, final AbstractConnection connection) {
                  try {
                    networklog.debug(
                        "peer.ping from " + connection + " with nonce " + message.getNonce());
                    PeerHandler.this
                        .context
                        .getNetwork()
                        .getMessaging()
                        .send(
                            new PeerPongMessage(
                                PeerHandler.this.context.getNode(), message.getNonce()),
                            connection);
                  } catch (Exception ex) {
                    networklog.error(
                        PeerHandler.this.context.getName() + ": peer.ping " + connection, ex);
                  }
                }
              });
      ...
      this.context
          ...
          .register(
              ...
              this.getClass(),
              new MessageProcessor<PeerPongMessage>() {
                @Override
                public void process(
                    final PeerPongMessage message, final AbstractConnection connection) {
                  try {
                    final Long nonce = PeerHandler.this.pings.remove(connection);
                    if (nonce != message.getNonce())
                      connection.disconnect(
                          "Got peer.pong with unexpected nonce " + message.getNonce());
                    else if (networklog.hasLevel(Logging.DEBUG))
                      networklog.debug(
                          "Got peer.pong from " + connection + " with nonce " + message.getNonce());
                  } catch (Exception ex) {
                    networklog.error(
                        PeerHandler.this.context.getName() + ": peer.pong " + connection, ex);
                  }
                }
              });
      ...
      long houseKeepingInterval =
          this.context.getConfiguration().get("network.peers.broadcast.interval", 60);
      this.houseKeepingTaskFuture =
                  ...
                  new ScheduledExecutable(
                    ...
                    public void execute() {
                      try {
                        ...
                        for (final Peer peer :
                            PeerHandler.this.getPeerStore().get(new AllPeersFilter())) {
                          if (peer.getIdentity() != null
                              && PeerHandler.this
                                  ...
                                  .has(
                                      ...
                                      ConnectionState.CONNECTED)) continue;

                          // Is aged?
                          if (TimeUnit.MILLISECONDS.toSeconds(
                                  Time.getSystemTime() - peer.getKnownAt())
                              >= PeerHandler.this
                                  .context
                                  .getConfiguration()
                                  .get("network.peers.aged", 3600)) {
                            if (peer.getAttempts() == 10
                                || TimeUnit.MILLISECONDS.toSeconds(
                                        Time.getSystemTime() - peer.getConnectedAt())
                                    >= PeerHandler.this
                                        .context
                                        .getConfiguration()
                                        .get("network.peers.aged", 3600)
                                || TimeUnit.MILLISECONDS.toSeconds(
                                        Time.getSystemTime() - peer.getDisconnectedAt())
                                    >= PeerHandler.this
                                        .context
                                        .getConfiguration()
                                        .get("network.peers.aged", 3600)) {
                              ...
                              else PeerHandler.this.getPeerStore().delete(peer.getURI());
                            }
                          }
                        }
                        ...
                        for (final AbstractConnection connection :
                            PeerHandler.this
                                ...
                                .getNetwork()
                                .get(
                                    StandardConnectionFilter.build(PeerHandler.this.context)
                          ...
                          synchronized (PeerHandler.this.pings) {
                            ...
                            PeerPingMessage ping =
                                new PeerPingMessage(
                                    PeerHandler.this.context.getNode(),
                                    ThreadLocalRandom.current().nextLong());
                            PeerHandler.this.pings.put(connection, ping.getNonce());

                            if (networklog.hasLevel(Logging.DEBUG))
                              networklog.debug(
                                  "Pinging " + connection + " with nonce " + ping.getNonce());
                        ...
                        for (final AbstractConnection connection :
                          ...
                          PeerHandler.this
                              ...
                              .getNetwork()
                              .getMessaging()
                              .send(new GetPeersMessage(), connection);
                      } catch (Throwable t) {
      ...
      if (this.bootstrapper != null) {
        Thread bootstrapperThread =
            new Thread(this.bootstrapper, this.context.getName() + " - Bootstrapper");
  ...
  public void stop() throws TerminationException {
    if (this.houseKeepingTaskFuture != null) this.houseKeepingTaskFuture.cancel(false);
  ...
  public List<Peer> get(final PeerFilter filter, final int limit, final Comparator<Peer> sorter)
      throws IOException {
    return get(filter, 0, limit, sorter);
  }

  public List<Peer> get(final PeerFilter filter, final int offset, final int limit)
      throws IOException {
    return get(filter, 0, Integer.MAX_VALUE, null);
  }

  public List<Peer> get(
      final PeerFilter filter, final int offset, final int limit, final Comparator<Peer> sorter)
      throws IOException {
    Objects.requireNonNull(filter, "Peer filter is null");

    final List<Peer> peers = getPeerStore().get(filter, offset, limit);
    if (sorter != null) peers.sort(sorter);
  ...
  public List<Peer> get(final Collection<Identity> identities, final PeerFilter filter)
      throws IOException {
    return get(identities, filter, null);
  }

  public List<Peer> get(
      final Collection<Identity> identities, final PeerFilter filter, final Comparator<Peer> sorter)
      throws IOException {
    ...
    for (final Identity identity : identities) {
      ...
      if (filter != null && filter.filter(peer) == false) continue;
  ...
  /**
   * Checks if the node is accessible by probing the bootstrap endpoint. If it is, then it is likely
   * other ports are correctly routed and can be set as a GATEWAY.
   *
  ...
  public void checkAccessible(final URI nodeURI, final Node node) {
    try {
      ...
      URL url =
          new URL(
              WebService.DEFAULT_SCHEME
                  + nodeURI.getHost()
                  + ":"
                  + node.getAPIPort()
                  + Configuration.getDefault().get("api.url", WebService.DEFAULT_API_PATH)
                  + "/ledger/head");
      ...
      if (responseCode >= 200 && responseCode < 300) {
        ...
        String servicesQuery =
            services.stream().map(s -> s.name()).collect(Collectors.joining("&"));
      ...
      if (networklog.hasLevel(Logging.DEBUG))
        networklog.error(
            "Gateway check failure, returned response " + responseCode + " code " + nodeURI);
  ...
  public void update(final Peer peer) throws IOException {
    ...
    if (knownPeerByIdentity != null) {
      // Check for host switch for identities.  If within an hour of the last known IP for the
      // identity, disconnect and ban.
      if (knownPeerByIdentity.getIdentity().equals(peer.getIdentity())
          && knownPeerByIdentity.getHost().equals(peer.getHost()) == false) {
        long lastKnownOrAttemptTimestamp =
            Math.max(knownPeerByIdentity.getKnownAt(), knownPeerByIdentity.getAttemptedAt());
        if (TimeUnit.MILLISECONDS.toHours(lastKnownOrAttemptTimestamp - Time.getSystemTime()) < 1)
          isInvalid = true;
        else getPeerStore().delete(knownPeerByIdentity.getIdentity());
      }
    }

    final Peer knownPeerByHost = getPeerStore().get(peer.getURI());
    if (knownPeerByHost != null) {
      // Check for host duplicates but different identities.  If within an hour of the last known IP
      // for the identity, disconnect and ban.
      if (knownPeerByHost.getIdentity().equals(peer.getIdentity()) == false) {
        long lastKnownOrAttemptTimestamp =
            Math.max(knownPeerByHost.getKnownAt(), knownPeerByHost.getAttemptedAt());
        if (TimeUnit.MILLISECONDS.toHours(lastKnownOrAttemptTimestamp - Time.getSystemTime()) < 1)
          isInvalid = true;
        else getPeerStore().delete(knownPeerByHost.getIdentity());
      }
    }
    ...
    if (isInvalid) {
      if (knownPeerByIdentity != null && knownPeerByIdentity.isInvalid() == false) {
        ...
        networklog.warn(
            PeerHandler.this.context.getName()
                + ": Updated "
                + knownPeerByIdentity.getHost()
                + " for "
                + knownPeerByIdentity.getIdentity()
                + " as invalid");
      }
  ...
  private EventListener peerListener =
      new EventListener() {
          ...
          try {
            ...
            if (existingPeer == false) {
              ...
              Executor.getInstance()
                  .schedule(
                      () ->
                          PeerHandler.this.checkAccessible(
                              peerURI, event.getConnection().getNode()),
                      0,
                      TimeUnit.SECONDS);
            } else PeerHandler.this.update(peer);
            ...
            if (event
                ...
                .equals(
                    Protocol
                        .TCP)) // TODO what if the UDP connection is the ONLY connection we have to
              // the node?
              PeerHandler.this
                  .context
                  .getNetwork()
                  .getMessaging()
                  .send(new GetPeersMessage(), event.getConnection());
          } catch (IOException ioex) {
            networklog.debug(
                "Failed to request known peer information from " + event.getConnection(), ioex);
          }
        }
        ...
        public void on(final DisconnectedEvent event) {
          ...
          try {
            Peer peer =
                event.getConnection().getNode() != null
                    ? PeerHandler.this.get(event.getConnection().getNode().getIdentity())
            ...
            // Existing peer to update
              ...
              if (event.getConnection().getBannedUntil() > 0)
                peer.setBanned(
                    event.getConnection().getBanReason(), event.getConnection().getBannedUntil());

              // Update attempts if didn't connect, was a short lived connection
              if (event.getConnection().getConnectedAt() == 0
                  || event.getConnection().getDisconnectedAt()
                          - event.getConnection().getConnectedAt()
                      < TimeUnit.SECONDS.toMillis(Constants.DEFAULT_MINIMUM_CONNECTION_DURATION)) {
                    ...
                    Time.getSystemTime()
                        + (TimeUnit.SECONDS.toMillis(
                                PeerHandler.this
                                    .context
                                    .getConfiguration()
                                    .get("network.peer.reattempt", 60))
                            * (peer.getAttempts() * peer.getAttempts())));
                peer.setAcceptAt(
                    Time.getSystemTime()
                        + TimeUnit.SECONDS.toMillis(
                            PeerHandler.this
