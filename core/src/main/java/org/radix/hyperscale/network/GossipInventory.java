...
public abstract class GossipInventory
{
	...
	public GossipInventory(final Context context)
	{
	...
	public final Collection<Hash> required(final Class<? extends Primitive> type, final Collection<Hash> items, final AbstractConnection connection) throws Throwable
	{
		...
		finally
		{
			...
			if (System.currentTimeMillis() - this.lastReportAt >= TimeUnit.MILLISECONDS.convert(this.reportInterval, this.reportTimeUnit))
			{
				...
				gossipLog.info(this.context.getName()+": Inventory processor for "+type.getSimpleName()+" "+this.iterations+" iterations duration="+TimeUnit.NANOSECONDS.toMillis(this.duration)+"ms average="+averageDurationMS+"ms");
	...
	public abstract Collection<Hash> process(final Class<? extends Primitive> type, final Collection<Hash> items, final AbstractConnection connection) throws Throwable;
}
