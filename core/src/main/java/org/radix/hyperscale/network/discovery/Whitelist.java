...
public class Whitelist {
  ...
  private int[] convert(final String host) {
    ...
    for (int s = 0; s < segments.length; s++) {
      if (segments[s].equalsIgnoreCase("*")) break;
  ...
  private boolean isInRange(final String parameter, final String address) {
    ...
    if (hosts.length != 2) throw new IllegalStateException("Range is invalid");

    int[] target = convert(address);
    int[] low = convert(hosts[0]);
    int[] high = convert(hosts[1]);

    if (low.length != high.length || target.length != low.length) return false;
    ...
    for (int s = 0; s < low.length; s++) {
      ...
      if (target[s] < low[s] || target[s] > high[s]) return false;
    }
  ...
  private boolean isMask(final String parameter) {
    ...
    if (parameter.contains("*") || parameter.contains("::")) return true;

    return false;
  }

  private boolean isMasked(final String parameter, final String address) {
    ...
    if (target.length != mask.length) return false;

    for (int s = 0; s < mask.length; s++) {
      if (mask[s] == Integer.MAX_VALUE) return true;
      else if (target[s] != mask[s]) return false;
    }
  ...
  public boolean accept(final URI host) {
    ...
    try {
      for (String parameter : this.parameters) {
        if (parameter.equalsIgnoreCase(host.getHost())) return true;
        else if (isRange(parameter)
            && isInRange(parameter, InetAddress.getByName(host.getHost()).getHostAddress()))
          return true;
        else if (isMask(parameter)
            && isMasked(parameter, InetAddress.getByName(host.getHost()).getHostAddress()))
          return true;
      }
