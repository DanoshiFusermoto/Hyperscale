...
public class Node extends BasicObject
{
	...
	public Node(final Identity identity, final BlockHeader head, final String agent, final int agentVersion, final int protocolVersion, 
				final int networkPort, final int apiPort, final int websocketPort, final Set<Services> services, final boolean synced)
	{
		this();
		...
		this.services = Collections.unmodifiableSet(new LinkedHashSet<Services>(Objects.requireNonNull(services, "Services is null")));
	...
	{
		return this.getIdentity().toString(Constants.TRUNCATED_IDENTITY_LENGTH)+" @ "+this.head.toString();
	}
}
