...
{
	public static final LocalNode load(final String name, final Configuration configuration, final boolean create) throws CryptoException 
	{
		try
		{
			final BLSKeyPair BLSKey = BLSKeyPair.fromFile(new File(configuration.get("node.key.path", name+".key")), create);
	...
	public LocalNode(final BLSKeyPair BLSKey, final int networkPort, final int apiPort, final int websocketPort, final Set<Services> services, final BlockHeader head) throws CryptoException
	{
		this(BLSKey, networkPort, apiPort, websocketPort, services, head, Agent.AGENT, Agent.AGENT_VERSION, Agent.PROTOCOL_VERSION);
	}

	public LocalNode(final BLSKeyPair BLSKey, final int networkPort, final int apiPort, final int websocketPort, final Set<Services> services, final BlockHeader head, 
			         final String agent, final int agentVersion, final int protocolVersion) throws CryptoException
	{
		super(Objects.requireNonNull(BLSKey, "Key is null").getIdentity(), head, agent, agentVersion, protocolVersion, networkPort, apiPort, websocketPort, services, false);
		
		this.BLSKey = BLSKey;
		this.ephemeralKeyPair = new EDKeyPair();
		this.ephemeralBinding = this.BLSKey.getPrivateKey().sign(ephemeralKeyPair.getPublicKey().toByteArray());
	}
	...
	public void fromPersisted(final Node persisted)
	{
		...
		if (persisted.getIdentity().equals(this.BLSKey.getIdentity()) == false)
			throw new IllegalArgumentException("Persisted node BLS key does not match "+this.BLSKey.getIdentity());
