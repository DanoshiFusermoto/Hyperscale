...
/**
 * Simple Websocket service that clients can connect to and receive a stream of JSON formatted
 * events regarding atoms.
 *
 * <p>Needs lots of additional functionality to be a real Websocket service. Currently used to
 * assist with testing, hence simplicity.
 *
 * @author Dan
 */
public final class WebSocketService extends WebSocketServer {
  ...
  // TODO hack to keep a copy of the connections list rather than calling getConnections()
  // repeatedly on the super class
  //		which returns a new ArrayList every time.  Furthermore if there are no collections, it creates
  // a new ArrayList
  ...
  public WebSocketService(final Context context) {
    super(
        new InetSocketAddress(
            context.getConfiguration().get("websocket.address", "0.0.0.0"),
            context.getConfiguration().get("websocket.port", WebSocketImpl.DEFAULT_PORT)),
        context
            .getConfiguration()
            .get(
                "websocket.decoders",
                    ...
                    + Math.max(
                        ...
                        MathUtils.log2(
                            Runtime.getRuntime().availableProcessors()
                                / context.getConfiguration().getCommandLine("contexts", 1)))));
  ...
  public void onMessage(WebSocket conn, String message) {
    ...
    if (type.equalsIgnoreCase("submit")) {
      ...
      try {
        ...
        for (Object instruction : atom.getManifest()) manifest.put(instruction);
  ...
  private EventListener asyncAtomListener =
      new EventListener() {
        ...
        public void on(AtomCommitEvent event) throws JSONException {
          if (WebSocketService.this.connections.get() == 0) return;

          JSONObject eventJSON = new JSONObject();

          if (event.getDecision().equals(CommitDecision.ACCEPT)) eventJSON.put("type", "committed");
            ...
            else eventJSON.put("reason", "unknown");
          } else
            throw new UnsupportedOperationException(
                "State decision of type " + event.getDecision() + " is not supported");

          eventJSON.put(
              "atom", Serialization.getInstance().toJsonObject(event.getAtom(), Output.API));
          eventJSON.put(
              "certificate",
              Serialization.getInstance()
                  .toJsonObject(event.getPendingAtom().getCertificate(), Output.API));
        ...
        private void timedout(PendingAtom pendingAtom, String timeout) {
          ...
          eventJSON.put(
              "atom", Serialization.getInstance().toJsonObject(pendingAtom.getAtom(), Output.API));
