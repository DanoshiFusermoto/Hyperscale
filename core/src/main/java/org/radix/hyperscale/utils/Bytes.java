...
public class Bytes {
  ...
  public static byte[] fromHexString(final String s, final int offset) {
    ...
    if ((inputLength & 1) != 0) bytes[b++] = fromHexNybble(s.charAt(i++));
  ...
  public static byte[] trimLeadingZeros(byte[] bytes) {
    if (bytes == null || bytes.length <= 1 || bytes[0] != 0) return bytes;

    int trimLeadingZeros = 1;
    int maxTrim = bytes.length - 1;
    while (trimLeadingZeros < maxTrim && bytes[trimLeadingZeros] == 0) trimLeadingZeros += 1;
  ...
  public static boolean allZero(byte[] bytes, int offset, int length) {
    for (int i = offset; i < offset + length; i++) if (bytes[i] != 0) return false;
