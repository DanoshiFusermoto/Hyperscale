...
public final class UInt256 implements Comparable<UInt256> {
  ...
  public static UInt256 from(long value) {
    if (value >= 0) {
      if (value <= 10) return numbers[(int) value];
  ...
  public static UInt256 from(byte[] bytes) {
    ...
    if (bytes.length == 0) throw new IllegalArgumentException("bytes is 0 bytes long");
  ...
  public static UInt256 from(byte[] bytes, int offset) {
    ...
    long v3 =
        ((long) bytes[offset + 31] & 0xFF)
            | ((long) (bytes[offset + 30] & 0xFF) << 8)
            | ((long) (bytes[offset + 29] & 0xFF) << 16)
            | ((long) (bytes[offset + 28] & 0xFF) << 24)
            | ((long) (bytes[offset + 27] & 0xFF) << 32)
            | ((long) (bytes[offset + 26] & 0xFF) << 40)
            | ((long) (bytes[offset + 25] & 0xFF) << 48)
            | ((long) (bytes[offset + 24] & 0xFF) << 56);

    long v2 =
        ((long) bytes[offset + 23] & 0xFF)
            | ((long) (bytes[offset + 22] & 0xFF) << 8)
            | ((long) (bytes[offset + 21] & 0xFF) << 16)
            | ((long) (bytes[offset + 20] & 0xFF) << 24)
            | ((long) (bytes[offset + 19] & 0xFF) << 32)
            | ((long) (bytes[offset + 18] & 0xFF) << 40)
            | ((long) (bytes[offset + 17] & 0xFF) << 48)
            | ((long) (bytes[offset + 16] & 0xFF) << 56);

    long v1 =
        ((long) bytes[offset + 15] & 0xFF)
            | ((long) (bytes[offset + 14] & 0xFF) << 8)
            | ((long) (bytes[offset + 13] & 0xFF) << 16)
            | ((long) (bytes[offset + 12] & 0xFF) << 24)
            | ((long) (bytes[offset + 11] & 0xFF) << 32)
            | ((long) (bytes[offset + 10] & 0xFF) << 40)
            | ((long) (bytes[offset + 9] & 0xFF) << 48)
            | ((long) (bytes[offset + 8] & 0xFF) << 56);
  ...
  public static UInt256 from(String s, int offset) {
    ...
    int len = s.length();
    if (len <= offset) throw new NumberFormatException(s);
    ...
    if (len - i == 1) {
      ...
      if (digit >= 0 && digit <= 9) return numbers[digit];
    ...
    if (s.charAt(i) == '1') {
      ...
      int zeros = 0;
      int j = i + 1;
      for (; j < s.length() && s.charAt(j) == '0'; j++) zeros++;
  ...
  private static byte[] extend(byte[] bytes) {
    if (bytes.length >= BYTES) return bytes;
  ...
  public synchronized byte[] toByteArray() {
    if (this.internal == true) throw new UnsupportedOperationException("Internal UInt256");
  ...
  public UInt256 subtract(UInt256 other) {
    ...
    borrow =
        (Long.compareUnsigned(this.v2, other.v2) < 0 || (this.v2 == other.v2 && borrow == 1))
            ? 1
            : 0;

    long diff1 = this.v1 - other.v1 - borrow;
    borrow =
        (Long.compareUnsigned(this.v1, other.v1) < 0 || (this.v1 == other.v1 && borrow == 1))
            ? 1
            : 0;
  ...
  private void subtractInternal(UInt256 other) {
    if (this.internal == false) throw new UnsupportedOperationException("Not an internal UInt256");
  ...
  public UInt256 subtract(UInt128 other) {
    ...
    borrow =
        (Long.compareUnsigned(this.v2, other.getHigh()) < 0
                || (this.v2 == other.getHigh() && borrow == 1))
            ? 1
            : 0;
  ...
  public UInt256 multiply(UInt256 other) {
    ...
    return new UInt256((r7 << 32) | r6, (r5 << 32) | r4, (r3 << 32) | r2, (r1 << 32) | r0);
  }
  ...
  public UInt256 divide(UInt256 divisor) {
    ...
    if (this.equals(divisor)) return ONE;
    ...
    for (int i = SIZE - (leading + 1); i >= 0; i--) {
      ...
      if (getBit(i)) remainder.orInternal(ONE);
  ...
  public UInt256 remainder(UInt256 divisor) {
    ...
    for (int i = SIZE - (leading + 1); i >= 0; i--) {
      ...
      if (divisor.compareTo(remainder) <= 0) remainder.subtractInternal(divisor);
    }
  ...
  public UInt256 pow(int exponent) {
    ...
    if (exponent == 0) return UInt256.ONE;

    if (exponent == 1 || this.equals(UInt256.ONE) || this.isZero()) return this;

    UInt256 base = this;
    UInt256 result = UInt256.ONE;

    while (exponent > 0) {
      if ((exponent & 1) == 1) result = result.multiply(base);
  ...
  public UInt256 shiftLeft(int n) {
    ...
    if (n < 0) return shiftRight(Math.abs(n));
  ...
  public UInt256 or(UInt256 other) {
    return new UInt256(
        this.v0 | other.v0, this.v1 | other.v1, this.v2 | other.v2, this.v3 | other.v3);
  }
  ...
  public int numberOfLeadingZeros() {
    if (this.v0 != 0) return Long.numberOfLeadingZeros(this.v0);

    if (this.v1 != 0) return 64 + Long.numberOfLeadingZeros(this.v1);

    if (this.v2 != 0) return 128 + Long.numberOfLeadingZeros(this.v2);
  ...
  public String toString(int radix) {
    ...
    // Allocate StringBuilder enough to accommodate half the maximum characters based on the bits
    // used and the radix
  ...
  private UInt256 setBit(int n) {
    ...
    if (n < 64) return new UInt256(this.v0, this.v1, this.v2, this.v3 | (1L << n));
    else if (n < 128) return new UInt256(this.v0, this.v1, this.v2 | (1L << (n - 64)), this.v3);
    else if (n < 192) return new UInt256(this.v0, this.v1 | (1L << (n - 128)), this.v2, this.v3);
    else return new UInt256(this.v0 | (1L << (n - 192)), this.v1, this.v2, this.v3);
  }
  ...
  private void setBitInternal(int n) {
    ...
    if (n < 64) this.v3 = this.v3 | (1L << n);
    else if (n < 128) this.v2 = this.v2 | (1L << (n - 64));
    else if (n < 192) this.v1 = this.v1 | (1L << (n - 128));
    else this.v0 = this.v0 | (1L << (n - 192));
  }
