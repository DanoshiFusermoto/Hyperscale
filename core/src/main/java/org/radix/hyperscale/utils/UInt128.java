...
public final class UInt128 implements Comparable<UInt128> {
  ...
  public UInt128 divide(UInt128 divisor) {
    if (divisor.isZero()) throw new IllegalArgumentException("Can't divide by zero");
    ...
    for (int i = 0; i < SIZE; ++i) {
      r = r.shiftLeft();
      q = q.shiftLeft();
      if (n.high < 0) r = r.or(UInt128.ONE);
  ...
  public int compareTo(UInt128 n) {
    ...
    if (cmp == 0) cmp = Long.compareUnsigned(this.getLow(), n.getLow());
  ...
  public boolean equals(Object obj) {
    ...
    if (obj instanceof UInt128 uint128) {
      return this.high == uint128.high
          && this.midHigh == uint128.midHigh
          && this.midLow == uint128.midLow
