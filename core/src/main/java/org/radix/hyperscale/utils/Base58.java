...
public final class Base58 {
  ...
  public static String toBase58(byte[] input) {
    ...
    for (int inputStart = zeros; inputStart < input.length; ) {
      ...
      if (input[inputStart] == 0) ++inputStart; // optimization - skip leading zeros
    }
    ...
    return new String(
        encoded, outputStart, encoded.length - outputStart, StandardCharsets.US_ASCII);
  }
  ...
  public static byte[] fromBase58(String input, int offset) {
    ...
    for (int inputStart = zeros; inputStart < input58.length; ) {
      ...
      if (input58[inputStart] == 0) ++inputStart; // optimization - skip leading zeros
    }
