...
public class TimeSeriesStatistics 
{
	...
	private volatile transient long lastPurge = 0;
	...
	private void purgeAged()
	{
		...
		{
			if (this.purging.tryAcquire() == false)
				return;
...
//				this.range.add(record);
	...
	public void put(final String field, final double value, final long timestamp, final TimeUnit unit)
	{
		final long timestampSeconds = unit.toSeconds(timestamp);
		synchronized(this.records)
		{
...
//				this.range.add(record);
	...
	public long get(final String field, final long _default, final long timestamp, final TimeUnit unit)
	{
		final long timestampSeconds = unit.toSeconds(timestamp);
		synchronized(this.records)
		{
	...
	public double get(final String field, final double _default, final long timestamp, final TimeUnit unit)
	{
		final long timestampSeconds = unit.toSeconds(timestamp);
		synchronized(this.records)
		{
	...
	public long increment(final String field, final long value, final long timestamp, final TimeUnit unit)
	{
		final long timestampSeconds = unit.toSeconds(timestamp);
		synchronized(this.records)
		{
...
//				this.range.add(record);
	...
	public double increment(final String field, final double value, final long timestamp, final TimeUnit unit)
	{
		final long timestampSeconds = unit.toSeconds(timestamp);
		synchronized(this.records)
		{
	...
	{
		return average(field, start, unit.convert(System.currentTimeMillis(), TimeUnit.MILLISECONDS), unit);
	}
	...
	public double mean(final String field, final int maxSamples, final long timestamp, final TimeUnit unit)
	{
		double total = 0;
		long samples = 0;
