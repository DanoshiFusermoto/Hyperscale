...
public final class Context implements Service {
  ...
  public static final Context create(final Configuration config) throws StartupException {
    return Context.create(
        "node-" + contexts.size(), Objects.requireNonNull(config, "Configuration is null"));
  }
  ...
  public static final Context createAndStart(final String name, final Configuration config)
  ...
  public static final Collection<Context> createAndStart(
      final int count, final String name, final Configuration configuration)
      throws StartupException {
      ...
      for (int cc = 0; cc < count; cc++) {
        ...
        if (count > 1) {
          ...
          try {
            ...
            if (keyFile.exists() == false) BLSKey = null;
            else BLSKey = BLSKeyPair.fromFile(keyFile, false);

            if (BLSKey == null
                || (BLSKey != null
                    && ShardMapper.toShardGroup(
                                BLSKey.getIdentity(), Universe.getDefault().shardGroupCount())
                            .equals(shardGroupID)
                        == false)) {
              if (BLSKey == null
                  || Universe.getDefault().getValidators().stream()
                          .anyMatch(bp -> bp.equals(BLSKey.getIdentity()))
                      == false) {
                BLSKeyPair validatorKey;
                do {
                  validatorKey = new BLSKeyPair();
                  if (shardGroupID.equals(
                          ShardMapper.toShardGroup(
                              validatorKey.getIdentity(), Universe.getDefault().shardGroupCount()))
                      == false) validatorKey = null;
            ...
            throw new StartupException(
                "Existing identity key pair for "
                    + name.toLowerCase()
                    + "-"
                    + incrementer.get()
                    + " may be corrupt",
        ...
        Context context =
            Context.create(name.toLowerCase() + "-" + incrementer.get(), configuration);
        context.start();
  ...
  public static final Context create(final String name, final Configuration configuration)
      throws StartupException {
    ...
    synchronized (contexts) {
      ...
      contextConfig.set(
          "network.port",
          configuration.get("network.port", Universe.getDefault().getPort()) + incrementer.get());
      contextConfig.set(
          "network.udp",
          configuration.get("network.udp", Universe.getDefault().getPort()) + incrementer.get());
      contextConfig.set(
          "websocket.port",
          configuration.get("websocket.port", WebSocketImpl.DEFAULT_PORT) + incrementer.get());
      ...
      if (incrementer.get() > 0) {
        ...
        if (configuration.has("network.seeds")) {
          ...
          for (String seed : seeds) seedSet.add(seed);
        }

        if (seedSet.isEmpty()) seedSet.add(contextConfig.get("network.address", "127.0.0.1"));
      ...
      for (Context context : contexts.values())
        if (context.node.getIdentity().equals(node.getIdentity()))
          throw new IllegalStateException(
              "Context " + name.toLowerCase() + ":" + node + " already created");
      ...
      if (Context._default == null) Context._default = context;
  ...
  public static final void stop(final Context context) throws TerminationException {
    ...
    if (Context._default == context) Context._default = null;
  }

  public static final void stopAll() throws TerminationException {
    synchronized (contexts) {
      for (Context context : contexts.values()) Objects.requireNonNull(context).stop();
  ...
  public Context(final String name, final LocalNode node, final Configuration configuration)
      throws IOException {
    ...
    try {
      ...
      if (this.metaData.has("node.local")) {
        try {
          ...
          if (persisted.getIdentity().equals(Context.this.node.getIdentity())
              == false) // TODO what happens if has changed?  Dump everything?
          log.warn(
                "Node key has changed from "
                    + persisted.getIdentity()
                    + " to "
                    + Context.this.node.getIdentity());
      ...
      this.metaDataTaskFuture =
          Executor.getInstance()
              .scheduleAtFixedRate(
                  new ScheduledExecutable(1, 1, TimeUnit.SECONDS) {
                    @Override
                    public void execute() {
                      try {
                        byte[] nodeBytes =
                            Serialization.getInstance()
                                .toDson(Context.this.getNode(), Output.PERSIST);
  ...
  public long getUptime() {
    if (this.stoppedAt < this.startedAt) return System.currentTimeMillis() - this.startedAt;
    else return this.stoppedAt - this.startedAt;
  }
