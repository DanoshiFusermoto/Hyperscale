...
public final class StateMachine {
  ...
  public StateMachine(
      ...
      final Map<StateAddress, PendingState> pendingStates)
  ...
  final boolean exists(final StateAddress stateAddress) {
    ...
    if (substateLog.hasWrites() == false && substateLog.isVoid()) return false;
  ...
  final void assertNotExists(final StateAddress stateAddress) {
    final SubstateLog substateLog = this.pendingStates.get(stateAddress).getSubstateLog();
    if (substateLog.hasWrites() == false && substateLog.isVoid()) return;
  ...
  void lock(final StateAddress stateAddress, final StateLockMode lockMode) {
    synchronized (this) {
      final PendingState pendingState =
          this.pendingStates.computeIfAbsent(
              stateAddress, sa -> new PendingState(this.context, sa, this.pendingAtom));
      pendingState.lock(lockMode, this);

      if (lockMode.equals(StateLockMode.READ)) this.readLockCount++;
      else if (lockMode.equals(StateLockMode.WRITE)) this.writeLockCount++;
    }
  }
  ...
  Vault vault(final StateAddress stateAddress, final Identity authority) {
    if (stateAddress
            .context()
            .equalsIgnoreCase(Vault.class.getAnnotation(StateContext.class).value())
        == false)
    ...
    synchronized (this) {
      ...
      if (this.block.getHeight() > 0) {
        ...
        if (pendingState == null)
          throw new IllegalStateException(
              "Attempt to construct a vault with an unloaded substate "
                  + stateAddress
                  + " in "
                  + this.pendingAtom.getHash());
        ...
        if (pendingState.getSubstateLog() == null)
          throw new IllegalStateException(
              "Attempt to modify unloaded substate "
                  + stateAddress
                  + " in "
                  + this.pendingAtom.getHash());
      }
      ...
      Vault vault =
          (Vault)
              this.primitives.computeIfAbsent(
                  stateAddress,
                  (sa) -> {
                    SubstateLog vaultSubstateLog = pendingState.getSubstateLog();
                    // Set the authority if the substate is void
                    if (vaultSubstateLog.isVoid()) vaultSubstateLog.setAuthority(authority);

                    boolean debitable = true;
                    if (vaultSubstateLog.getAuthority() != null
                        && vaultSubstateLog.getAuthority().equals(authority) == false
                        && this.pendingAtom.getAtom().hasAuthority(vaultSubstateLog.getAuthority())
                            == false) debitable = false;
  ...
  <T> T get(final StateAddress stateAddress, final String field, final T def) {
    synchronized (this) {
      ...
      if (this.block.getHeight() > 0) {
        ...
        if (pendingState == null)
          throw new IllegalStateException(
              "Attempt to read a substate that doesn't exist "
                  + stateAddress
                  + " in "
                  + this.pendingAtom.getHash());
        ...
        if (pendingState.getSubstateLog() == null)
          throw new IllegalStateException(
              "Attempt to read unloaded substate "
                  + stateAddress
                  + " in "
                  + this.pendingAtom.getHash());
      }
  ...
  <T> void set(
      final StateAddress stateAddress, final String field, final T value, final Identity caller) {
  ...
  private <T> void set_internal(
      final StateAddress stateAddress, final String field, final T value, final Identity caller) {
    synchronized (this) {
      final PendingState pendingState = this.pendingStates.get(stateAddress);
      if (this.block.getHeight()
          > 0) // Universe.getDefault().getGenesis().contains(this.pendingAtom.getHash()) == false)
      {
        if (this.status.before(State.EXECUTING))
          throw new IllegalStateException(
              "Attempt to read substate "
        ...
        if (pendingState == null)
          throw new IllegalStateException(
              "Attempt to modify a substate that doesn't exist "
                  + stateAddress
                  + " in "
                  + this.pendingAtom.getHash());
  ...
  boolean associate(final StateAddress address, final Hash association) {
    synchronized (this) {
      if (this.associations.containsEntry(address, association)) return false;
  ...
  public boolean isProvisioned() {
    synchronized (this) {
      if (this.status.before(State.PREPARED)) return false;

      if (this.status.after(State.PREPARED)) return true;

      synchronized (this.pendingStates) {
        if (this.pendingStates.isEmpty()) return false;
        ...
        if (this.pendingStates instanceof MutableMap<StateAddress, PendingState> mutableMap) {
          if (mutableMap.allSatisfy(ps -> ps.isProvisioned()) == false) return false;
        } else {
          for (final PendingState pendingState : this.pendingStates.values()) {
            if (pendingState.isProvisioned() == false) return false;
          }
        }
      }
      ...
      for (int i = 0; i < this.instructions.size(); i++) {
        ...
        if (instruction instanceof Instantiate instantiate) {
          ...
          if (this.packages.containsKey(packageAddress) == false) return false;
        } else if (instruction instanceof Method method) {
          if (method.isNative() == false) {
            final PendingState componentState =
                this.pendingStates.get(((Method) instruction).getComponentAddress());
            final StateAddress packageAddress =
                StateAddress.from(
                    Package.class,
                    componentState
                        .getStateInput()
                        .getSubstate()
                        .<String>get(NativeField.BLUEPRINT));
  ...
  public boolean isProvisioned(final StateAddress stateAddress) {
    synchronized (this) {
      if (Package.class.getAnnotation(StateContext.class).value().equals(stateAddress.context())) {
        if (this.packages.isEmpty()) return false;

        if (this.packages.containsKey(stateAddress) == false) return false;
      } else {
        synchronized (this.pendingStates) {
          if (this.pendingStates.isEmpty()) return false;

          if (this.pendingStates.get(stateAddress).isProvisioned() == false) return false;
        }
      }
  ...
  void provisioned() {
    ...
    if (stateMachineLog.hasLevel(Logging.INFO))
      stateMachineLog.info(
          this.context.getName()
              + ": State machine for pending atom "
              + this.getPendingAtom().getHash()
              + " is provisioned");
  }

  public int numInstructions() {
    if (this.pendingAtom.getAtom() == null) throw new IllegalStateException("Atom is null");
  ...
  public <T> List<T> getInstructions(final Class<T> type) {
    ...
    for (int i = 0; i < this.instructions.size(); i++) {
      final Object instruction = this.instructions.get(i);
      if (type.isInstance(instruction)) instructions.add(type.cast(instruction));
    }
    ...
    for (int i = 0; i < this.instructions.size(); i++) {
      ...
      if (instruction instanceof Instruction) {
        if (instruction instanceof Deploy deploy) {
          if (deploy.getBlueprintAddress().scope().equals(packageAddress.scope())) return true;
        }
  ...
  public List<Entry<StateAddress, Hash>> getAssociations() {
    synchronized (this) {
      if (this.associations.isEmpty()) return Collections.emptyList();

      return Collections.unmodifiableList(
          new ArrayList<Entry<StateAddress, Hash>>(this.associations.entries()));
    }
  }
  ...
  private boolean isRequired(final StateAddress packageAddress) {
    if (this.status.before(State.PREPARED))
      throw new IllegalStateException(
          "State machine for atom " + this.pendingAtom.getHash() + " is not PREPARED");

    for (int i = 0; i < this.instructions.size(); i++) {
      final Object instruction = this.instructions.get(i);
      if (instruction instanceof Instantiate instantiate) {
        if (packageAddress.equals(instantiate.getPackageAddress())) return true;
      } else if (instruction instanceof Method method) {
        if (method.isNative() == false) {
          final PendingState componentState =
              this.pendingStates.get(((Method) instruction).getComponentAddress());
          if (componentState.isProvisioned()) {
            if (packageAddress.equals(
                StateAddress.from(
                    Package.class,
                    componentState
                        .getStateInput()
                        .getSubstate()
                        .<String>get(NativeField.BLUEPRINT)))) return true;
          }
        }
      }
    }
    ...
    synchronized (this) {
      if (isProvisioned())
        throw new IllegalStateException(
            "State machine for atom "
                + this.pendingAtom.getHash()
                + " is already PROVISIONED when loading package "
                + packageAddress);
      ...
      if (this.packages.containsKey(packageAddress))
        throw new IllegalStateException(
            "Package "
                + packageAddress
                + " is already loaded for atom "
                + this.pendingAtom.getHash());

      // Bit of a hack, but really don't want to expose the PackageHander.load function publicly for
      // the case of a single call from here
      try {
        java.lang.reflect.Method loadMethod;
        loadMethod =
            org.radix.hyperscale.ledger.PackageHandler.class.getDeclaredMethod(
                "load", StateAddress.class, PendingAtom.class);
        loadMethod.setAccessible(true);
        loadMethod.invoke(
            this.context.getLedger().getPackageHandler(), packageAddress, this.pendingAtom);
  ...
  public Collection<PolyglotPackage> load(
      final Collection<PolyglotPackage> pakages, boolean throwIfNotRequired) {
    synchronized (this) {
      if (isProvisioned())
        throw new IllegalStateException(
            "State machine for atom "
                + this.pendingAtom.getHash()
                + " is already PROVISIONED when loading package "
                + pakages.stream().map(p -> p.getAddress()).collect(Collectors.toList()));

      final Collection<PolyglotPackage> loadedPackages =
          new ArrayList<PolyglotPackage>(packages.size());
  ...
  public void prepare() throws StateMachinePreparationException {
    synchronized (this) {
      if (this.status.after(State.NONE))
        throw new IllegalStateException(
            "State machine for atom " + this.pendingAtom.getHash() + " is already PREPARED");
      ...
      for (int i = 0; i < this.instructions.size(); i++) {
        final Object instruction = this.instructions.get(i);
        if (instruction instanceof Instruction) {
          if (instruction instanceof Deploy deploy) prepare(deploy);
          else if (instruction instanceof Epoch epoch) prepare(epoch);
          else if (instruction instanceof Shards shards) prepare(shards);
          else if (instruction instanceof Instantiate instantiate) prepare(instantiate);
          else if (instruction instanceof Method method) prepare(method);
          else if (instruction instanceof Persist persist) prepare(persist);
          else
            throw new UnsupportedOperationException(
                "Instruction type " + instruction.getClass() + " is not supported");
          ...
          if (instruction instanceof ArgumentedInstruction argumentedInstruction) {
            ...
            for (int a = 0; a < arguments.length; a++) {
              ...
              if (argument instanceof SubstateArgument<?> substateArgument) {
                List<Entry<StateAddress, StateLockMode>> addresses =
                    substateArgument.getAddresses();
          ...
          if (instruction instanceof GlobalInstruction) this.global = true;

          this.currentInstruction++;
        } else
          throw new UnsupportedOperationException(
              "Manifest type " + instruction.getClass() + " is not supported");
      }
      this.currentInstruction = -1;
      ...
      if (this.writeLockCount <= 1)
        throw new IllegalStateException(
            "Atom " + getPendingAtom().getHash() + " does not modify any state");
  ...
  private void prepare(final Deploy deploy) throws StateMachinePreparationException {
    try {
      if (stateMachineLog.hasLevel(Logging.DEBUG))
        stateMachineLog.debug(
            this.context.getName() + ": Preparing deployment of blueprint " + deploy);
  ...
  private void prepare(final Instantiate instantiate) throws StateMachinePreparationException {
    try {
      if (stateMachineLog.hasLevel(Logging.DEBUG))
        stateMachineLog.debug(
            this.context.getName()
                + ": Preparing instantiation of blueprint "
                + instantiate.getBlueprintName()
                + " to component "
                + instantiate.getComponentName());
  ...
  private void prepare(final Method method) throws StateMachinePreparationException {
    try {
      final Class<? extends NativeComponent> nativeClass =
          Component.getNative(method.getComponentAddress());
      ...
      if (component == null) {
        if (nativeClass != null) {
          ...
          if (component == null)
            throw new StateMachineExecutionException(
                "Native component " + method.getComponentAddress() + " not found");
        }
        ...
        if (nativeClass == null) {
          ...
          lock(
              StateAddress.from(
                  Vault.class,
                  ComputeKey.from(method.getComponentAddress().getHash()).getIdentity()),
              StateLockMode.WRITE);
        }
      ...
      if (nativeClass != null) component.prepare(method.getMethod(), method.getArguments());
    }
    // FIX ME this catches everything, some exceptions we want to report as implementation
    //		  exceptions NOT execution excetions (e.g IllegalStateException due to state snapshot!)
    catch (Exception ex) {
  ...
  public void accepted(final BlockHeader header) throws IOException {
    ...
    final long localShardGroupVoteThreshold =
        this.context
            .getLedger()
            .getValidatorHandler()
    ...
    synchronized (this) {
      if (this.block != null)
        throw new IllegalStateException(
            "Statemachine for " + this.pendingAtom.getHash() + " is already accepted");
      ...
      synchronized (this.pendingStates) {
        for (final PendingState pendingState : this.pendingStates.values()) {
          ...
          else pendingState.accepted(this.block);
        }
      }
    ...
    if (stateInput.getAtom().equals(this.pendingAtom.getHash()) == false)
      throw new IllegalArgumentException(
          "State input "
              ...
              + " not relevant for Statemachine "
              + this.pendingAtom.getHash());

    if (isProvisioned())
      throw new IllegalStateException(
          "Statemachine for atom " + this.pendingAtom.getHash() + " is already provisioned");

    synchronized (this) {
      final PendingState pendingState =
          this.pendingStates.get(stateInput.getSubstate().getAddress());
      if (pendingState.getStateInput() == null) pendingState.setStateInput(stateInput);
      ...
      if (stateInput
          ...
          .getAddress()
          .context()
          .equals(Deploy.class.getAnnotation(StateContext.class).value())) {
        packageAddress =
            StateAddress.from(Package.class, stateInput.getSubstate().getAddress().scope());
        if (isPackageEmbedded(packageAddress)) packageAddress = null;
      } else if (stateInput
          .getSubstate()
          .getAddress()
          .context()
          .equals(Component.class.getAnnotation(StateContext.class).value())) {
        String blueprint = stateInput.getSubstate().<String>get(NativeField.BLUEPRINT);
        if (blueprint != null) packageAddress = StateAddress.from(Package.class, blueprint);
      }

      if (packageAddress != null) {
        if (isLoaded(packageAddress) == false) load(packageAddress);
      } else if (isProvisioned()) provisioned();
    }
  }
    ...
    synchronized (this) {
      try {
        if (isProvisioned() == false)
          throw new StateMachineExecutionException(
              "Statemachine for atom " + this.pendingAtom.getHash() + " is not provisioned");
        ...
        try {
          ...
          for (int i = 0; i < this.instructions.size(); i++) {
            final Object instruction = this.instructions.get(i);
            if (instruction instanceof Instruction) {
              if (instruction instanceof Deploy deploy) execute(deploy);
              else if (instruction instanceof Epoch epoch) execute(epoch);
              else if (instruction instanceof Shards shards) execute(shards);
              else if (instruction instanceof Method method) call(method);
              else if (instruction instanceof Instantiate instantiate) execute(instantiate);
              else if (instruction instanceof Persist persist) execute(persist);
              else
                throw new UnsupportedOperationException(
                    "Instruction type " + instruction.getClass() + " is not supported");
            } else
  ...
  private void execute(final Deploy deploy) throws StateMachineExecutionException {
    try {
      if (stateMachineLog.hasLevel(Logging.DEBUG))
        stateMachineLog.debug(
            this.context.getName() + ": Executing deployment of blueprint " + deploy);
  ...
  private void execute(final Instantiate instantiate) throws StateMachineExecutionException {
    try {
      if (stateMachineLog.hasLevel(Logging.DEBUG))
        stateMachineLog.debug(
            this.context.getName() + ": Executing instantiation of blueprint " + instantiate);
  ...
  private void call(final Method method)
      throws StateMachineExecutionException, ReflectiveOperationException {
    synchronized (this) {
      Component component = this.components.get(method.getComponentAddress());
      if (component == null) {
        final Class<? extends NativeComponent> nativeRef =
            Component.getNative(method.getComponentAddress());
        if (nativeRef == null) {
          if (stateMachineLog.hasLevel(Logging.DEBUG))
            stateMachineLog.debug(
                this.context.getName() + ": Creating component " + method.getComponentAddress());

          PendingState componentSubstate = this.pendingStates.get(method.getComponentAddress());
          if (componentSubstate == null)
            throw new StateMachineExecutionException(
                "Substate for component " + method.getComponentAddress() + " not found");

          if (componentSubstate.getSubstateLog().isVoid())
            throw new StateMachineExecutionException(
                "Substate is void for component " + method.getComponentAddress());

          StateAddress packageAddress =
              StateAddress.from(
                  Package.class,
                  componentSubstate.getSubstateLog().<String>get(NativeField.BLUEPRINT));
          Package pakage = this.packages.get(packageAddress);
          if (pakage == null)
            throw new StateMachineExecutionException("Package " + packageAddress + " not found");

          component =
              Component.load(this, (PolyglotPackage) pakage, componentSubstate.getSubstateLog());
          if (component == null)
            throw new StateMachineExecutionException(
                "Component " + method.getComponentAddress() + " could not be loaded");
  ...
  void call(final StateAddress componentAddress, final String method, final Object[] arguments)
      throws StateMachineExecutionException, ReflectiveOperationException {
    synchronized (this) {
      try {
        ...
        if (component == null) {
          ...
          if (nativeRef == null) {
            ...
            if (componentSubstate == null)
              throw new StateMachineExecutionException(
                  "Substate for component " + componentAddress + " not found");
            ...
            if (component == null)
              throw new StateMachineExecutionException(
                  "Component " + componentAddress + " could not be loaded");
          } else {
            component = Component.load(this, nativeRef);
            if (component == null)
              throw new StateMachineExecutionException(
                  "Native component " + componentAddress + " not found");
          }
        ...
        if (stateMachineLog.hasLevel(Logging.INFO)) {
          stateMachineLog.error(
              StateMachine.this.context.getName()
  ...
  private void buildExecutionOutput() {
    if (thrown() == null) {
      ...
      for (final PendingState pendingState : orderedPendingStates) {
        ...
        if (pendingState
            .getAddress()
            .context()
            .equals(Atom.class.getAnnotation(StateContext.class).value())) continue;
        ...
        if (substateLog.hasWrites()) {
          ...
          if (stateMachineLog.hasLevel(Logging.INFO)) {
            ...
            stateMachineLog.info(
                this.context.getName()
                    + ": 							    "
                    + transitions.toSubstate(SubstateOutput.TRANSITIONS).values());
          }
        } else {
    ...
    } else this.executionDigest = Hash.ZERO;
  ...
  private void buildCommitOperation() {
    if (thrown() == null) {
      ...
      final org.radix.hyperscale.ledger.Epoch epoch =
          org.radix.hyperscale.ledger.Epoch.from(this.block);
      final int numShardGroups = this.context.getLedger().numShardGroups(epoch);
      final ShardGroupID localShardGroupID =
          ShardMapper.toShardGroup(this.context.getNode().getIdentity(), numShardGroups);

      // Filter substates that are relevant to the local shard group.
      // NOTE Blob substates are persisted in all shard groups the atom touches as they provide a
      // mapping to the parent atom & proposal.
      ...
      synchronized (this.pendingStates) {
        ...
        for (final PendingState pendingState : this.pendingStates.values()) {
          final ShardGroupID shardGroupID =
              ShardMapper.toShardGroup(pendingState.getAddress().getShardID(), numShardGroups);
          if (pendingState
                      .getAddress()
                      .context()
                      .equalsIgnoreCase(Blob.class.getAnnotation(SerializerId2.class).value())
                  == false
              && shardGroupID.equals(localShardGroupID) == false) continue;

          final SubstateLog substateLog = pendingState.getSubstateLog();
          if (substateLog.hasWrites() == false) continue;
      ...
      for (final Entry<StateAddress, Hash> association : this.associations.entries()) {
        final ShardGroupID shardGroupID =
            ShardMapper.toShardGroup(association.getKey().getShardID(), numShardGroups);
        if (shardGroupID.equals(localShardGroupID) == false) continue;

        if (filteredAssociations.put(association.getKey(), association.getValue()) == false)
          stateMachineLog.warn(
              this.context.getName()
                  + ": Duplicate association for state address "
                  + association.getKey()
                  + " in atom "
                  + this.pendingAtom.getHash());
      }
      ...
      this.commitOperation =
          new CommitOperation(
              ...
              this.block,
              this.pendingAtom.getAtom(),
              filteredSubstateTransitions,
              filteredAssociations.entries(),
              new SubstateOpLog(this.pendingAtom.getHash(), snapshotSubstateTransitions));
    } else
      this.commitOperation =
          new CommitOperation(
              ...
              Collections.emptyList(),
              new SubstateOpLog(this.pendingAtom.getHash()));
  }
  ...
  private void thrown(final Exception t) {
    ...
    stateMachineLog.error(
        this.context.getName() + ": Failed to execute atom " + this.pendingAtom.getHash(), t);
