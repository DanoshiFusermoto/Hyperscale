...
public final class Block extends ExtendedObject implements CompoundPrimitive {
  ...
  Block(
      ...
      final Collection<Atom> accepted,
      final Collection<Atom> unaccepted,
      final Collection<ExecutionTimeout> unexecuted,
      final Collection<AtomCertificate> certificates,
      final Collection<CommitTimeout> uncommitted,
    ...
    //		allowed currently to allow testing of duplicate atom injections which should fail during
    // consensus
    this.accepted =
        accepted.isEmpty() ? Collections.emptyList() : Lists.immutable.ofAll(accepted).castToList();
    this.unaccepted =
        unaccepted.isEmpty()
            ? Collections.emptyList()
            : Lists.immutable.ofAll(unaccepted).castToList();
    this.unexecuted =
        unexecuted.isEmpty()
            ? Collections.emptyList()
            : Lists.immutable.ofAll(unexecuted).castToList();
    this.certificates =
        certificates.isEmpty()
            ? Collections.emptyList()
            : Lists.immutable.ofAll(certificates).castToList();
    this.uncommitted =
        uncommitted.isEmpty()
            ? Collections.emptyList()
            : Lists.immutable.ofAll(uncommitted).castToList();
    this.packages =
        packages.isEmpty() ? Collections.emptyList() : Lists.immutable.ofAll(packages).castToList();
  }
  ...
  public Block(
      ...
      final Collection<AtomCertificate> certificates,
      final Collection<CommitTimeout> uncommitted,
      final Collection<Hash> executables,
      final Collection<Hash> latent,
      final Collection<PolyglotPackage> packages) {
    this(
        height,
        previous,
  ...
  public Block(
    ...
    inventory.put(
        ...
        executables.isEmpty() ? Collections.emptyList() : new ArrayList<Hash>(executables));
    inventory.put(
        InventoryType.LATENT,
        latent.isEmpty() ? Collections.emptyList() : new ArrayList<Hash>(latent));

    this.header =
        new BlockHeader(
            height, previous, difficulty, work, nonce, index, inventory, timestamp, proposer);
  }
  ...
  private <T extends Hashable> List<Hash> createHashList(Collection<T> collection) {
    ...
    for (T item : collection) hashes.add(item.getHash());
  ...
  public boolean contains(final Hash hash) {
    ...
    for (int a = 0; a < this.accepted.size(); a++) {
      ...
      if (atom.getHash().equals(hash)) return true;
    }
  ...
  public synchronized int getSize() throws SerializationException {
    if (this.size == -1) {
      ...
      for (Atom atom : getAccepted()) {
        ...
        if (cached == null) cached = Serialization.getInstance().toDson(atom, Output.WIRE);
      ...
      for (AtomCertificate certificate : getCertificates()) {
        ...
        if (cached == null) cached = Serialization.getInstance().toDson(certificate, Output.WIRE);
      ...
      for (ExecutionTimeout timeout : getUnexecuted()) {
        ...
        if (cached == null) cached = Serialization.getInstance().toDson(timeout, Output.WIRE);
      ...
      for (PolyglotPackage pakage : getPackages()) {
        ...
        if (cached == null) cached = Serialization.getInstance().toDson(pakage, Output.WIRE);
