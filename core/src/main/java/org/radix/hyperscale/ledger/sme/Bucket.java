...
public class Bucket 
{
	...
	public void put(String symbol, UInt256 quantity)
	{
		synchronized(this.tokens)
		{
			...
			if (current == null)
				this.tokens.put(symbol, quantity);
			else
				this.tokens.put(symbol, current.add(quantity));
		}
	}
	...
	public void put(Hash token, UInt256 quantity)
	{
		synchronized(this.tokens)
		{
			...
			if (current == null)
				this.tokens.put(tokenHex, quantity);
			else
				this.tokens.put(tokenHex, current.add(quantity));
		}
	}
	...
	public void take(String symbol, UInt256 quantity) throws ValidationException
	{
		synchronized(this.tokens)
		{
			...
			if (balance.compareTo(quantity) < 0)
				throw new ValidationException("Bucket "+this.label+" does not hold a balance of "+quantity+" "+symbol+" but "+balance+" "+symbol);
			
			balance = balance.subtract(quantity);
			if (balance.compareTo(UInt256.ZERO) == 0)
				this.tokens.remove(symbol);
			else
				this.tokens.put(symbol, balance);
		}
	}
	...
	public void take(Hash token, UInt256 quantity) throws ValidationException
	{
		synchronized(this.tokens)
		{
			...
			if (balance.compareTo(quantity) < 0)
				throw new ValidationException("Bucket "+this.label+" does not hold a balance of "+quantity+" "+token+" but "+balance+" "+token);
			
			balance = balance.subtract(quantity);
			if (balance.compareTo(UInt256.ZERO) == 0)
				this.tokens.remove(token.toString());
			else
				this.tokens.put(token.toString(), balance);
		}
	}
}
