...
public class PendingBranch {
  ...
  private final LinkedList<PendingBlock>
      blocks; // TODO ensure supers are updated whenever this is modified!
  ...
  PendingBranch(
      ...
      final StateAccumulator accumulator) {
  ...
  PendingBranch(
      ...
      final BlockHeader root,
      final StateAccumulator accumulator,
      final Collection<PendingBlock> pendingBlocks)
  ...
  PendingBranch(
      ...
      throws ValidationException, IOException, LockException {
  ...
  PendingBranch(
      ...
      final StateAccumulator accumulator,
      final Collection<PendingBlock> pendingBlocks)
      throws ValidationException, LockException, IOException {
    this(context, type, root, accumulator);
  ...
  public boolean equals(Object other) {
    ...
    if (other instanceof PendingBranch pendingBranch) {
      if (pendingBranch.id == this.id) return true;
    }
      ...
      try {
        if (this.meta.containsKey(block.getHash()))
          throw new ValidationException(
              block, "Block " + block.getHash() + " is already added to branch " + toString());

        if (block.getHeight() <= this.root.getHeight())
          throw new ValidationException(
              block,
              "Block is " + block.getHash() + " is before branch root " + this.root.getHash());

        if (this.blocks.isEmpty() == false
            && this.blocks.getLast().getHash().equals(block.getHeader().getPrevious()) == false)
          throw new IllegalStateException(
              "Block " + block.getHash() + " does not attach to end of branch " + toString());

        if (this.blocks.isEmpty() == true
            && this.root.getHash().equals(block.getHeader().getPrevious()) == false)
          throw new IllegalStateException(
              "Block " + block.getHash() + " does not attach to branch root " + toString());

        this.blocks.add(block);
        this.meta.put(block.getHash(), Boolean.FALSE);

        if (block.isUnbranched()) block.setInBranch(this);
  ...
  private void apply(final PendingBlock pendingBlock)
      throws LockException, ValidationException, SerializationException {
    Objects.requireNonNull(pendingBlock, "Pending block is null");

    if (pendingBlock.getBlock() == null)
      throw new IllegalStateException(
          "Can not apply an un-constructed block " + pendingBlock.getHeader());
    ...
    if (Block.MAX_BLOCK_SIZE != Integer.MAX_VALUE) {
      ...
      if (blockSize > Message.MAX_MESSAGE_SIZE)
        throw new ValidationException(
            this, "Block " + pendingBlock.getBlock() + " is too large " + blockSize + " bytes");
    }
    ...
    synchronized (this) {
      if (pendingBlock.getHeader().getPrevious().equals(this.root.getHash()) == false) {
        if (contains(pendingBlock.getHeader().getPrevious()) == false)
          throw new IllegalStateException(
              "Can not apply block " + pendingBlock.getHeader() + " when previous absent");

        if (isApplied(pendingBlock.getHeader().getPrevious()) == false)
          throw new IllegalStateException(
              "Can not apply block " + pendingBlock.getHeader() + " when previous is not applied");
      }
      ...
      if (pendingBlock.isApplied() == false) {
        for (PendingAtom pendingAtom : pendingBlock.<PendingAtom>get(InventoryType.ACCEPTED)) {
          ...
          InventoryType referenceType =
              references(pendingAtom.getHash(), pendingBlock, BlockHeader.ACCEPT_VERIFY_FILTER);
          if (referenceType != null)
            throw new ValidationException(
                pendingBlock,
                "Accepted atom " + pendingAtom.getAtom() + " is " + referenceType + " in branch");
        }
        ...
        for (PendingAtom pendingAtom : pendingBlock.<PendingAtom>get(InventoryType.UNACCEPTED)) {
          ...
          InventoryType referenceType =
              references(pendingAtom.getHash(), pendingBlock, BlockHeader.UNACCEPT_VERIFY_FILTER);
          if (referenceType != null)
            throw new ValidationException(
                pendingBlock,
                "Unaccepted atom " + pendingAtom.getAtom() + " is " + referenceType + " in branch");
        }
        ...
        for (PendingAtom pendingAtom : pendingBlock.<PendingAtom>get(InventoryType.EXECUTABLE)) {
          ...
          if (pendingAtom.isExecuteSignalled())
            throw new ValidationException(
                pendingAtom,
                "Executable atom " + pendingAtom.getAtom() + " is already execute signalled");
          ...
          if (pendingAtom.getStatus().after(State.PREPARED) == false
            ...
            throw new ValidationException(
                pendingAtom, "Executable atom " + pendingAtom.getAtom() + " is not ACCEPTED");

          InventoryType referenceType =
              references(pendingAtom.getHash(), pendingBlock, BlockHeader.EXECUTE_VERIFY_FILTER);
          if (referenceType != null)
            throw new ValidationException(
                pendingBlock,
                "Executable atom " + pendingAtom.getAtom() + " is " + referenceType + " in branch");
        }
        ...
        for (PendingAtom pendingAtom : pendingBlock.<PendingAtom>get(InventoryType.LATENT)) {
          ...
          if (pendingAtom.isExecuteLatentSignalled())
            throw new ValidationException(
                pendingAtom,
                "Latent atom " + pendingAtom.getAtom() + " is already latent execute signalled");

          if (pendingAtom.getStatus().after(State.PREPARED) == false
              && references(pendingAtom.getHash(), pendingBlock, InventoryType.ACCEPTED) == null)
            throw new ValidationException(
                pendingAtom, "Latent atom " + pendingAtom.getAtom() + " is not ACCEPTED");
          ...
          if (referenceType != null) {
            ...
            throw new ValidationException(
                pendingBlock,
                "Latent atom " + pendingAtom.getAtom() + " is " + referenceType + " in branch");
          }
        }
        ...
        for (PendingAtom pendingAtom : pendingBlock.<PendingAtom>get(InventoryType.UNEXECUTED)) {
          ...
          if (pendingAtom.getStatus().after(State.PREPARED) == false
              && references(pendingAtom.getHash(), pendingBlock, InventoryType.ACCEPTED) == null)
            throw new ValidationException(
                pendingAtom, "Unexecuted atom " + pendingAtom.getAtom() + " is not ACCEPTED");

          InventoryType referenceType =
              references(pendingAtom.getHash(), pendingBlock, BlockHeader.UNEXECUTED_VERIFY_FILTER);
          if (referenceType != null)
            throw new ValidationException(
                pendingBlock,
                "Unexecuted atom " + pendingAtom.getAtom() + " is " + referenceType + " in branch");
        }
        ...
        for (PendingAtom pendingAtom : pendingBlock.<PendingAtom>get(InventoryType.COMMITTED)) {
          ...
          if (pendingAtom.getStatus().after(State.PREPARED) == false
              && references(pendingAtom.getHash(), pendingBlock, InventoryType.ACCEPTED) == null)
            throw new ValidationException(
                pendingAtom, "Commit atom " + pendingAtom.getAtom() + " is not ACCEPTED");

          InventoryType referenceType =
              references(pendingAtom.getHash(), pendingBlock, BlockHeader.COMMIT_VERIFY_FILTER);
          if (referenceType != null)
            throw new ValidationException(
                pendingBlock,
                "Commit atom " + pendingAtom.getAtom() + " is " + referenceType + " in branch");
        }
        ...
        for (PendingAtom pendingAtom : pendingBlock.<PendingAtom>get(InventoryType.UNCOMMITTED)) {
          ...
          if (pendingAtom.getStatus().after(State.PREPARED) == false
              && references(pendingAtom.getHash(), pendingBlock, InventoryType.ACCEPTED) == null)
            throw new ValidationException(
                pendingAtom, "Uncommitted atom " + pendingAtom.getAtom() + " is not ACCEPTED");

          InventoryType referenceType =
              references(
                  pendingAtom.getHash(), pendingBlock, BlockHeader.UNCOMMITTED_VERIFY_FILTER);
          if (referenceType != null)
            throw new ValidationException(
                pendingBlock,
                "Uncommitted atom "
        ...
        for (Hash item : pendingBlock.getHeader().getInventory(InventoryType.PACKAGES)) {
          if (references(item, pendingBlock, InventoryType.PACKAGES) != null)
            throw new ValidationException(
                pendingBlock, "Package " + item + " is already in branch");
        }
      ...
      if (blocksLog.hasLevel(Logging.DEBUG))
        blocksLog.debug(
            context.getName()
                ...
                + this.root
                + " -> "
                + this.accumulator.locked().size()
                + " locked in accumulator "
                + this.accumulator.locked().stream()
                    .map(StateAddress::getHash)
                    .reduce((a, b) -> Hash.hash(a, b)));
    }
  }
  ...
  void update() throws LockException, ValidationException, SerializationException {
    synchronized (this) {
      ...
      while (blocksIterator.hasNext()) {
        ...
        if (vertex.getBlock() == null) continue;

        if (isApplied(vertex.getHash()) == false) {
          try {
            if (isApplied(vertex.getHeader().getPrevious())) apply(vertex);
            else break;
          } catch (Throwable t) {
            // Record the exception thrown in this pending block
            vertex.thrown(t);

            // Rest of the branch can not be applied if a StateLockException is thrown here so
            // truncate it
  ...
  InventoryType references(
      final Hash item, final PendingBlock excludeTo, final InventoryType... types) {
    ...
    synchronized (this) {
      for (int b = 0; b < this.blocks.size(); b++) {
        ...
        if (excludeTo != null && pendingBlock.getHeight() >= excludeTo.getHeight()) continue;
  ...
  boolean isCanonical() {
    synchronized (this) {
      ...
      while (blockIterator.hasNext()) {
        ...
        if (current != null) {
          if (previous.getHash().equals(current.getHeader().getPrevious()) == false) return false;

          current = previous;
        }

        current = previous;
        if (current.getHeader().getPrevious().equals(this.root.getHash())) return true;
      }
  ...
  boolean isMergable(final Collection<PendingBlock> blocks) {
    ...
    for (PendingBlock block : blocks) {
      if (last == null || last.getHeight() < block.getHeight()) last = block;
    }

    synchronized (this) {
      if (this.blocks.contains(last)) return false;

      final Hash branchTip;
      if (this.blocks.isEmpty()) branchTip = this.root.getHash();
      else branchTip = this.blocks.getLast().getHash();
      ...
      for (PendingBlock block : blocks) {
        ...
        if (branchTip.equals(block.getHeader().getPrevious()) == false) continue;
  ...
  void merge(final Collection<PendingBlock> blocks)
      throws IOException, ValidationException, LockException {
    Objects.requireNonNull(blocks, "Pending blocks is null");
    Numbers.isZero(blocks.size(), "Pending blocks is empty");
    ...
    synchronized (this) {
      ...
      Hash previous;
      if (this.blocks.isEmpty()) previous = this.root.getHash();
      else previous = this.blocks.getLast().getHash();

      for (int i = 0; i < sortedBlocks.size(); i++) {
        final PendingBlock sortedBlock = sortedBlocks.get(i);
        if (sortedBlock.getHeader() == null)
          throw new IllegalStateException(
              "Block " + sortedBlock.getHash() + " does not have a header");

        if (sortedBlock.getHeight() <= Block.toHeight(previous)) continue;
      ...
      if (mergeBlocks.isEmpty() == false) {
        if (blocksLog.hasLevel(Logging.DEBUG))
          blocksLog.debug(
              this.context.getName() + ": Merging branch " + this.blocks + " with " + blocks);
      ...
      for (PendingBlock sortedBlock : sortedBlocks) {
        ...
        for (PendingBlock vertex : this.blocks) {
          if (vertex.getHash().equals(sortedBlock.getHeader().getPrevious())) {
            if (vertex.equals(this.blocks.getLast()) == false) return true;
            else return false;
          }
        }
      }
  ...
  PendingBranch fork(final Collection<PendingBlock> blocks)
      throws LockException, IOException, ValidationException {
    Objects.requireNonNull(blocks, "Pending blocks is null");
    Numbers.isZero(blocks.size(), "Pending blocks is empty");
    ...
    synchronized (this) {
      ...
      if (forkBlock != null) {
        ...
        for (final PendingBlock vertex : this.blocks) {
          ...
          if (vertex.equals(forkBlock)) break;
        }

        for (final PendingBlock sortedBlock : sortedBlocks) {
          if (sortedBlock.getHeight() <= forkBlock.getHeight()) continue;

          forkBlocks.add(sortedBlock);
        }

        forkBranch =
            new PendingBranch(this.context, Type.FORK, this.root, this.accumulator, forkBlocks);
      }
  ...
  boolean intersects(final PendingBlock block) {
    ...
    synchronized (this) {
      if (this.meta.containsKey(block.getHeader().getPrevious())) return true;
  ...
  boolean intersects(final PendingBranch branch) {
    ...
    synchronized (this) {
      for (PendingBlock sortedBlock : sortedBlocks)
        if (this.meta.containsKey(sortedBlock.getHeader().getPrevious())) return true;
          ...
          if (this.meta.remove(vertex.getHash()).equals(Boolean.FALSE))
            blocksLog.warn(
                this.context.getName()
                    + ": Removing unapplied block "
                    + vertex
                    + " from "
                    + this.root
                    + " "
                    + this.blocks.getLast().getHeader());
    ...
    synchronized (this) {
      ...
      while (vertexIterator.hasNext()) {
        ...
        if (vertex.getBlock() == null)
          throw new IllegalStateException(
              "Unconstructed proposal "
                  + vertex.getHeader()
                  + " found when commiting to "
                  + block.getHeader());
        ...
        if (vertex.getHash().equals(block.getHash())) break;
      }
      ...
      if (committed.isEmpty() == false) {
        if (committed.getLast().equals(block) == false)
          blocksLog.warn(
              this.context.getName()
                  + ": Committed partial branch "
                  + this.root
                  + " -> "
                  + committed.getLast().getHeader());
  ...
  PendingBlock commitable(int superCount) {
    synchronized (this) {
      ...
      // there will always be at least one super block in a pending branch.
      // The required quantity of super blocks in a branch is defined by minSupers and may be larger
      // than 2 depending on certain conditions.
      ...
      if (supers.size() < Math.max(superCount, Constants.MIN_COMMIT_SUPERS)) return null;

      if (blocksLog.hasLevel(Logging.DEBUG))
        blocksLog.debug(
            this.context.getName() + ": Found commit branch " + this + " with supers " + supers);
      ...
      while (superBlockIterator.hasNext()) {
        ...
        if (supers.getLast().equals(superBlock) == false) break;
  ...
  LinkedList<PendingBlock> supers() {
    ...
    synchronized (this) {
      ...
      while (vertexIterator.hasNext()) {
        PendingBlock vertex = vertexIterator.next();
        if (vertex.getHeader().getCertificate() != null) supers.add(vertex);
      }
    }
    return supers;
  }
  ...
  private boolean isApplied(final Hash vertex) {
    synchronized (this) {
      if (vertex.equals(this.root.getHash())) return true;
  ...
  private void lockable(final Collection<PendingAtom> pendingAtoms) throws LockException {
    ...
    synchronized (this) {
      for (PendingAtom pendingAtom : pendingAtoms) lockable(pendingAtom);
    }
  }
  ...
  void lockable(final PendingAtom pendingAtom, final Collection<StateAddress> stateAddresses)
    ...
    synchronized (this) {
      for (int i = this.blocks.size() - 1; i >= 0; i--) {
        ...
        if (isApplied(pendingBlock.getHeader().getHash()) == false) continue;
        ...
        while (stateAddressesIterator.hasNext()) {
          ...
          if (pendingBlock.isLocked(stateAddress))
            throw new StateLockException(
                stateAddress,
                pendingAtom.getHash(),
                pendingBlock.getLockedBy(stateAddress).getHash());
        }

        if (stateAddressesCopy.isEmpty()) break;
      }
  ...
  void lockable(final PendingAtom pendingAtom, final StateAddress stateAddress)
      throws LockException {
    Objects.requireNonNull(pendingAtom, "Pending atom is null");
      ...
      if (checkAccumulator) {
        if (this.accumulator.isLocked(stateAddress))
          throw new StateLockException(
              stateAddress,
              pendingAtom.getHash(),
              this.accumulator.getLockedBy(stateAddress).getHash());
      }
    }
  ...
  void unlockable(final Collection<PendingAtom> pendingAtoms) throws LockException {
    ...
    synchronized (this) {
      for (PendingAtom pendingAtom : pendingAtoms) unlockable(pendingAtom);
    }
  }
  ...
  void unlockable(final PendingAtom pendingAtom, final Collection<StateAddress> stateAddresses)
      throws LockException {
    Objects.requireNonNull(pendingAtom, "Pending atom is null");
  ...
  LinkedList<PendingBlock> getBlocksTo(Hash block) {
    synchronized (this) {
      ...
      for (PendingBlock pendingBlock : this.blocks) {
        ...
        if (pendingBlock.getHash().equals(block)) return blocks;
      }
  ...
  public PendingBlock get(final Hash block) {
    ...
    synchronized (this) {
      for (PendingBlock vertex : this.blocks) if (vertex.getHash().equals(block)) return vertex;
  ...
  long getVotePower(final long height, final Identity identity, boolean matured)
      throws IOException {
    ...
    synchronized (this) {
      if (this.root.getHeight() == height)
        throw new IllegalArgumentException(
            "Block at height " + height + " is branch root " + this.root);
  ...
  /**
   * Returns a {@link BlockHeader} which represents the highest fully qualified build head in this
   * branch up to the specified progress round. <br>
   * <br>
  ...
  BlockHeader getBuildable(final ProgressRound round) {
    ...
    synchronized (this) {
      if (this.root.getHeight() >= round.clock()) return null;

      BlockHeader buildableHead = this.root;
      for (PendingBlock vertex : this.blocks) {
        if (vertex.getHeight() < round.clock()
            && vertex.getBlock() != null
            && isApplied(vertex.getHash()) == true
            && vertex.getHeight() > buildableHead.getHeight()) buildableHead = vertex.getHeader();
        else break;
      }
  ...
  public boolean isBuildable(BlockHeader header) {
    synchronized (this) {
      if (this.root.getHash().equals(header.getHash())) return true;

      for (PendingBlock vertex : this.blocks) {
        if (vertex.getHash().equals(header.getHash())
            && vertex.getBlock() != null
            && isApplied(vertex.getHash())) return true;
      }
  ...
  public boolean isBuildable() {
    synchronized (this) {
      if (isConstructed() == false || isPrepared() == false || isCanonical() == false) return false;
  ...
  public boolean isConstructed() {
    synchronized (this) {
      for (PendingBlock vertex : this.blocks) {
        if (vertex.getBlock() == null) return false;
      }
      return true;
    }
  }
  ...
  public boolean isPrepared() {
    synchronized (this) {
      for (PendingBlock vertex : this.blocks) {
        if (isApplied(vertex.getHash()) == false) return false;
      }
      return true;
    }
  }
