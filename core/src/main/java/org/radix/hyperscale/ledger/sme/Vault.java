...
public class Vault
{
	private final static List<String> ALL_TOKEN_WHITELIST = Collections.singletonList("*");
	...
	Vault(final StateMachine stateMachine, final SubstateLog substateLog, final boolean debitable)
	{
		...
		if (getClass().getAnnotation(StateContext.class).value().equalsIgnoreCase(substateLog.getAddress().context()) == false)
			throw new IllegalArgumentException("Substate log "+substateLog.getAddress()+" is not a vault");
	...
	public UInt256 put(final String symbol, final UInt256 quantity, final Bucket bucket) throws ValidationException
	{
		Objects.requireNonNull(symbol, "Token symbol is null");
		...
		{
			final List<String> acceptedTokens = this.stateMachine.get(this.accountAddress, "tokens.accept", ALL_TOKEN_WHITELIST);
			if(acceptedTokens.contains("*") == false && acceptedTokens.contains(lowerSymbol) == false)
				throw new ValidationException("Account associated with "+this.stateMachine.get(accountAddress, NativeField.AUTHORITY)+" does not accept "+lowerSymbol.toUpperCase());
		}
	...
	public UInt256 put(final Hash token, final UInt256 quantity, final Bucket bucket) throws ValidationException
	{
		Objects.requireNonNull(token, "Token hash is null");
		Objects.requireNonNull(quantity, "Token quantity is null");
		Objects.requireNonNull(bucket, "Bucket is null");
		...
		{
			final List<String> acceptedTokens = this.stateMachine.get(this.accountAddress, "tokens.accept", ALL_TOKEN_WHITELIST);
			if(acceptedTokens.contains("*") == false && acceptedTokens.contains(token.toString()) == false)
				throw new ValidationException("Account associated with "+this.stateMachine.get(accountAddress, NativeField.AUTHORITY)+" does not accept "+token.toString());
		}
	...
	public UInt256 take(final String symbol, final UInt256 quantity, final Bucket bucket) throws ValidationException
	{
		Objects.requireNonNull(symbol, "Token symbol is null");
		Objects.requireNonNull(quantity, "Token quantity is null");
		Objects.requireNonNull(bucket, "Bucket is null");
		...
		if (this.debitable == false)
			throw new ValidationException("Vault "+this.substateLog.getAddress()+" authority not present for debit of "+quantity+" "+lowerSymbol);

		UInt256 balance = this.substateLog.getOrDefault(lowerSymbol, UInt256.ZERO);
		if (balance.compareTo(quantity) < 0)
			throw new ValidationException("Vault "+this.substateLog.getAddress()+" does not hold a balance of "+quantity+" "+lowerSymbol+" but "+balance+" "+lowerSymbol);
	...
	public UInt256 take(final Hash token, final UInt256 quantity, final Bucket bucket) throws ValidationException
	{
		Objects.requireNonNull(token, "Token hash is null");
		Objects.requireNonNull(quantity, "Token quantity is null");
		Objects.requireNonNull(bucket, "Bucket is null");
		...
		if (this.debitable == false)
			throw new ValidationException("Vault "+this.substateLog.getAddress()+" authority not present for debit of "+quantity+" "+token);

		UInt256 balance = this.substateLog.getOrDefault(token.toString(), UInt256.ZERO);
		if (balance.compareTo(quantity) < 0)
			throw new ValidationException("Vault "+this.substateLog.getAddress()+" does not hold a balance of "+quantity+" "+token+" but "+balance+" "+token);
