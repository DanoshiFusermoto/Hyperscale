...
{
	// A network with more than 4096 shards is HUGE.  Throughput capabilities in excess of 20M per second.  Large enough for now. 
	private static volatile ShardGroupID[] shardGroupIDs = new ShardGroupID[4096];
	
	static 
	{
		for (int sg = 0 ; sg < shardGroupIDs.length ; sg++)
			shardGroupIDs[sg] = new ShardGroupID(sg);
	}
    ...
    {
    	Numbers.isNegative(value, "Shard group ID is negative");
    	Numbers.greaterThan(value, Integer.MAX_VALUE, "Shard group ID value is greater than Integer.MAX");
    ...
    {
    	Numbers.inRange(value, MIN_VALUES, MAX_VALUES, "Shard group ID is not within range "+MIN_VALUES+" -> "+MAX_VALUES);
        this.value = value;
    }
    ...
    public boolean equals(Object obj) 
    {
        ...
        if (obj instanceof ShardGroupID shardGroupID)
            return this.value == shardGroupID.intValue();
