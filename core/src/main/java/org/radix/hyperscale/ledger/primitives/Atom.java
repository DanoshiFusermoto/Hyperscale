...
public final class Atom extends ExtendedObject implements Primitive {
  ...
  public static final class Builder {
    ...
    private Builder(long nonce) {
      this.nonce = nonce;
      this.manifest =
          new ArrayList<String>(6); // Rarely will a manifest be larger than 6 instructions
      this.signers =
          new HashMap<Identity, KeyPair<?, ?, ?>>(
              3); // Rarely will there be more than three signers
    }
    ...
    public Builder push(final Blob blob) {
      ...
      if (this.manifest.size() == Atom.MAX_MANIFEST_ITEMS)
        throw new IllegalStateException(
            "Manifest contains maximum items of " + Atom.MAX_MANIFEST_ITEMS);
    ...
    public Builder push(final Collection<String> manifest) {
      ...
      if (this.manifest.size() + manifest.size() > Atom.MAX_MANIFEST_ITEMS)
        throw new IllegalStateException(
            "Manifest would exceed maximum items of " + Atom.MAX_MANIFEST_ITEMS);
    ...
    public Atom build(final int difficulty, List<KeyPair<?, ?, ?>> signers) throws CryptoException {
      ...
      if (difficulty > 0) discoverPOW(this.atom, difficulty);

      // Signers set via builder
      if (this.signers.isEmpty() == false) {
        for (final KeyPair<?, ?, ?> signer : this.signers.values()) this.atom.sign(signer);
      }

      // Signers passed as parameters
      if (signers != null && signers.isEmpty() == false) {
        for (final KeyPair<?, ?, ?> signer : signers) this.atom.sign(signer);
      }
   ...
   *
   * <p>TODO this probably isn't required anymore as Atoms done carry a state snapshot with them.
  ...
  private Atom(final long nonce, final List<String> manifest) {
    ...
    Numbers.greaterThan(
        manifest.size(),
        Atom.MAX_MANIFEST_ITEMS,
        "Manifest exceeds maximum items of " + Atom.MAX_MANIFEST_ITEMS);

    this.nonce = nonce;
    this.manifest = new ArrayList<String>(manifest.size());
    for (int i = 0; i < manifest.size(); i++) this.manifest.add(manifest.get(i));
  ...
  private void throwIfSealed() {
    if (isSealed()) throw new IllegalStateException("Atom " + getHash() + " is sealed");
  }

  private void throwIfImmutable() {
    if (this.immutable) throw new IllegalStateException("Atom " + getHash() + " is sealed");
  }
  ...
  public Blob get(final Hash hash) {
    ...
    for (String instruction : this.manifest) {
      if (instruction.startsWith("data:") == false) continue;

      Blob blob = new Blob(instruction);
      if (blob.getHash().equals(hash)) return blob;
    }
  ...
  public void validate() throws ValidationException {
    if (this.nonce == 0) throw new ValidationException("Nonce is zero");

    if (this.manifest == null) throw new ValidationException("Manifest is nuil");

    if (this.manifest.isEmpty()) throw new ValidationException("Manifest is empty");

    if (this.signatures == null) throw new ValidationException("Signatures is nuil");

    if (this.signatures.isEmpty()) throw new ValidationException("Signatures is empty");
  }

  public boolean verify() throws CryptoException {
    if (this.signatures.isEmpty()) throw new CryptoException("Signatures is empty");

    for (final Entry<Identity, Signature> entry : this.signatures.entrySet()) {
      try {
        if (entry.getKey().canVerify() == false) return false;
    ...
    try {
      if (getCachedDsonOutput() == null) Serialization.getInstance().toDson(this, Output.PERSIST);
      else serializerlog.warn("DSON cache already present for atom " + this);
