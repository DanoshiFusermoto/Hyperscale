...
class BlockBuilder {
  ...
  PendingBlock build(
    ...
    if ((branch.isEmpty() == false
            && buildable.getHash().equals(branch.getHigh().getHash()) == false)
    ...
    if (branch.getRoot().equals(head) == false && branch.contains(head.getHash()) == false) {
      blocksLog.warn(
          this.context.getName()
              + ": Branch is stale, ledger head has changed from expected "
              + branch.getRoot()
              + " to "
              + head);
      return null;
    ...
    final List<PendingAtom> acceptAtoms =
        this.context
            .getLedger()
            .getAtomHandler()
            .acceptable(
                BlockHeader.MAX_INVENTORY_TYPE_PRIMITIVES,
                o -> {
                  boolean exclude = false;
                  if (exclude == false) exclude = this.inventory.containsValue(o);
                  if (exclude == false)
                    exclude =
                        branch.references(o.getHash(), BlockHeader.ACCEPT_SELECT_FILTER) != null;
                  if (o instanceof PendingAtom a && exclude == false) {
                    ...
                    for (final PendingState pendingState : pendingStates) {
                      ...
                      try {
                        branch.lockable(a, pendingState.getAddress());
                      } catch (Exception ex) {
                        exclude = true;
                      }
                      if (exclude == true) break;
      ...
      try {
        if (acceptAtom.getStatus().after(AtomStatus.State.PREPARED))
          throw new ValidationException(
              acceptAtom,
              "Accepted atom " + acceptAtom.getAtom() + " has status " + acceptAtom.getStatus());

        InventoryType referenceType =
            branch.references(acceptAtom.getHash(), BlockHeader.ACCEPT_VERIFY_FILTER);
        if (referenceType != null)
          throw new ValidationException(
              acceptAtom,
              "Accepted atom " + acceptAtom.getAtom() + " is " + referenceType + " in branch");
        ...
        if (Block.MAX_BLOCK_SIZE != Integer.MAX_VALUE) {
          ...
          if (candidateAcceptedSize > (Message.MAX_MESSAGE_SIZE / 2)) break;
        }
    ...
    do {
      ...
      for (PendingAtom unacceptedAtom :
          this.context
              .getLedger()
              .getAtomHandler()
              .unaccepted(
                  BlockHeader.MAX_INVENTORY_TYPE_PRIMITIVES,
                  o ->
                      branch.references(o.getHash(), BlockHeader.UNACCEPT_SELECT_FILTER) != null
        ...
        try {
          if (unacceptedAtom.getStatus().after(AtomStatus.State.PREPARED))
            throw new ValidationException(
                unacceptedAtom,
                "Unaccepted atom "
                    + unacceptedAtom.getAtom()
                    + " has status "
                    + unacceptedAtom.getStatus());

          InventoryType referenceType =
              branch.references(unacceptedAtom.getHash(), BlockHeader.UNACCEPT_VERIFY_FILTER);
          if (referenceType != null)
            throw new ValidationException(
                unacceptedAtom,
                "Unaccepted atom "
          ...
          if (Block.MAX_BLOCK_SIZE != Integer.MAX_VALUE) {
            byte[] bytes =
                Serialization.getInstance().toDson(unacceptedAtom.getAtom(), Output.WIRE);
            if (blockSizeRemaining - bytes.length < 0) break;
      ...
      for (PendingAtom executableAtom :
          this.context
              .getLedger()
              .getAtomHandler()
              .executable(
                  BlockHeader.MAX_INVENTORY_TYPE_PRIMITIVES,
                  o ->
                      branch.references(o.getHash(), BlockHeader.EXECUTE_SELECT_FILTER) != null
                          || BlockBuilder.this.inventory.containsValue(o))) {
        try {
          if (executableAtom.getStatus().after(AtomStatus.State.PREPARED) == false)
            throw new ValidationException(
                executableAtom, "Executable atom " + executableAtom.getAtom() + " is not ACCEPTED");

          if (executableAtom.getStatus().after(AtomStatus.State.PROVISIONED))
            throw new ValidationException(
                executableAtom,
                "Executable atom "
                    + executableAtom.getAtom()
                    + " has status "
                    + executableAtom.getStatus());

          if (executableAtom.isExecuteSignalled())
            throw new ValidationException(
                executableAtom,
                "Executable atom " + executableAtom.getAtom() + " is already execute signalled");

          InventoryType referenceType =
              branch.references(executableAtom.getHash(), BlockHeader.EXECUTE_VERIFY_FILTER);
          if (referenceType != null)
            throw new ValidationException(
                executableAtom,
                "Executable atom "
      ...
      for (PendingAtom latentAtom :
          this.context
              ...
              .latent(
                  BlockHeader.MAX_INVENTORY_TYPE_PRIMITIVES,
                  o ->
                      branch.references(o.getHash(), BlockHeader.LATENT_SELECT_FILTER) != null
                          || BlockBuilder.this.inventory.containsValue(o))) {
        try {
          if (latentAtom.getStatus().after(AtomStatus.State.PREPARED) == false)
            throw new ValidationException(
                latentAtom, "Latent atom " + latentAtom.getAtom() + " is not ACCEPTED");

          if (latentAtom.getStatus().after(AtomStatus.State.PROVISIONED))
            throw new ValidationException(
                latentAtom,
                "Latent atom " + latentAtom.getAtom() + " has status " + latentAtom.getStatus());

          if (latentAtom.isExecuteLatentSignalled())
            throw new ValidationException(
                latentAtom,
                "Latent atom " + latentAtom.getAtom() + " is already latent execute signalled");

          InventoryType referenceType =
              branch.references(latentAtom.getHash(), BlockHeader.LATENT_VERIFY_FILTER);
          if (referenceType != null)
            throw new ValidationException(
                latentAtom,
                "Latent atom " + latentAtom.getAtom() + " is " + referenceType + " in branch");
      ...
      for (PendingAtom unexecutedAtom :
          this.context
              .getLedger()
              .getAtomHandler()
              .unexecuted(
                  BlockHeader.MAX_INVENTORY_TYPE_PRIMITIVES,
                  o ->
                      branch.references(o.getHash(), BlockHeader.UNEXECUTED_SELECT_FILTER) != null
                          || BlockBuilder.this.inventory.containsValue(o))) {
        try {
          if (unexecutedAtom.getStatus().after(AtomStatus.State.PREPARED) == false)
            throw new ValidationException(
                unexecutedAtom, "Unexecuted atom " + unexecutedAtom.getAtom() + " is not ACCEPTED");

          if (unexecutedAtom.getStatus().after(AtomStatus.State.EXECUTING))
            throw new ValidationException(
                unexecutedAtom,
                "Unexecuted atom "
                    + unexecutedAtom.getAtom()
                    + " has status "
                    + unexecutedAtom.getStatus());

          InventoryType referenceType =
              branch.references(unexecutedAtom.getHash(), BlockHeader.UNEXECUTED_VERIFY_FILTER);
          if (referenceType != null)
            throw new ValidationException(
                unexecutedAtom,
                "Unexecuted atom "
          ...
          if (Block.MAX_BLOCK_SIZE != Integer.MAX_VALUE) {
            byte[] bytes =
                Serialization.getInstance().toDson(unexecutedAtom.getTimeout(), Output.WIRE);
            if (blockSizeRemaining - bytes.length < 0) break;
      ...
      for (PendingAtom commitAtom :
          this.context
              ...
              .completed(
                  BlockHeader.MAX_INVENTORY_TYPE_PRIMITIVES,
                  o ->
                      branch.references(o.getHash(), BlockHeader.COMMIT_SELECT_FILTER) != null
                          || BlockBuilder.this.inventory.containsValue(o))) {
        try {
          if (commitAtom.getStatus().after(AtomStatus.State.PREPARED) == false)
            throw new ValidationException(
                commitAtom, "Commit atom " + commitAtom.getAtom() + " is not ACCEPTED");

          if (commitAtom.getStatus().after(AtomStatus.State.FINALIZED))
            throw new ValidationException(
                commitAtom,
                "Commit atom " + commitAtom.getAtom() + " has status " + commitAtom.getStatus());

          InventoryType referenceType =
              branch.references(commitAtom.getHash(), BlockHeader.COMMIT_VERIFY_FILTER);
          if (referenceType != null)
            throw new ValidationException(
                commitAtom,
                "Commit atom " + commitAtom.getAtom() + " is " + referenceType + " in branch");

          branch.unlockable(commitAtom);

          if (Block.MAX_BLOCK_SIZE != Integer.MAX_VALUE) {
            byte[] bytes =
                Serialization.getInstance().toDson(commitAtom.getCertificate(), Output.WIRE);
            if (blockSizeRemaining - bytes.length < 0) break;
      ...
      for (PendingAtom uncommittedAtom :
          this.context
              .getLedger()
              .getAtomHandler()
              .uncommitted(
                  BlockHeader.MAX_INVENTORY_TYPE_PRIMITIVES,
                  o ->
                      branch.references(o.getHash(), BlockHeader.UNCOMMITTED_SELECT_FILTER) != null
                          || BlockBuilder.this.inventory.containsValue(o))) {
        try {
          if (uncommittedAtom.getStatus().after(AtomStatus.State.PREPARED) == false)
            throw new ValidationException(
                uncommittedAtom,
                "Uncommitted atom " + uncommittedAtom.getAtom() + " is not ACCEPTED");

          if (uncommittedAtom.getStatus().after(AtomStatus.State.FINALIZED))
            throw new ValidationException(
                uncommittedAtom,
                "Uncommitted atom "
                    + uncommittedAtom.getAtom()
                    + " has status "
                    + uncommittedAtom.getStatus());

          InventoryType referenceType =
              branch.references(uncommittedAtom.getHash(), BlockHeader.UNCOMMITTED_VERIFY_FILTER);
          if (referenceType != null)
            throw new ValidationException(
                uncommittedAtom,
                "Uncommitted atom "
          ...
          if (Block.MAX_BLOCK_SIZE != Integer.MAX_VALUE) {
            byte[] bytes =
                Serialization.getInstance().toDson(uncommittedAtom.getTimeout(), Output.WIRE);
            if (blockSizeRemaining - bytes.length < 0) break;
      ...
      for (PolyglotPackage pakage :
          this.context
              .getLedger()
              .getPackageHandler()
              .uncommitted(
                  BlockHeader.MAX_PACKAGES,
                  o ->
                      branch.references(o.getHash(), InventoryType.PACKAGES) != null
                          || BlockBuilder.this.inventory.containsValue(o))) {
        try {
      ...
      miningBlock =
          new Block(
              buildable.getHeight() + 1,
              buildable.getHash(),
              Long.MAX_VALUE,
              buildable.getTotalWork(),
              ThreadLocalRandom.current().nextLong(),
              buildable.getNextIndex(),
              timestamp,
              this.context.getNode().getIdentity(),
              this.inventory.get(InventoryType.ACCEPTED).stream()
                  .map(PendingAtom.class::cast)
                  .map(pa -> pa.getAtom())
                  .collect(Collectors.toList()),
              this.inventory.get(InventoryType.UNACCEPTED).stream()
                  .map(PendingAtom.class::cast)
                  .map(pa -> pa.getAtom())
                  .collect(Collectors.toList()),
              this.inventory.get(InventoryType.UNEXECUTED).stream()
                  .map(PendingAtom.class::cast)
                  .filter(pa -> pa.getTimeout() instanceof ExecutionTimeout)
                  .map(pa -> pa.<ExecutionTimeout>getTimeout())
                  .collect(Collectors.toList()),
              this.inventory.get(InventoryType.COMMITTED).stream()
                  .map(PendingAtom.class::cast)
                  .map(pa -> pa.getCertificate())
                  .collect(Collectors.toList()),
              this.inventory.get(InventoryType.UNCOMMITTED).stream()
                  .map(PendingAtom.class::cast)
                  .filter(pa -> pa.getTimeout() instanceof CommitTimeout)
                  .map(pa -> pa.<CommitTimeout>getTimeout())
                  .collect(Collectors.toList()),
              this.inventory.get(InventoryType.EXECUTABLE).stream()
                  .map(PendingAtom.class::cast)
                  .map(pa -> pa.getHash())
                  .collect(Collectors.toList()),
              this.inventory.get(InventoryType.LATENT).stream()
                  .map(PendingAtom.class::cast)
                  .map(pa -> pa.getHash())
                  .collect(Collectors.toList()),
              this.inventory.get(InventoryType.PACKAGES).stream()
                  .map(PolyglotPackage.class::cast)
                  .collect(Collectors.toList()));

      // Adjust the expected header size to accomadate for over sized blocks.
      // This isn't perfect due to the manner in which certificates and timeout atoms are selected
      // on each pass
      // even with an adjustment it may still oversize.
      // We take 2x the adjustment calculated to prevent this
      // TODO having to 2x the adjustment does seem a little strange though, investigate this isn't
      // a bug
      if (Block.MAX_BLOCK_SIZE != Integer.MAX_VALUE) {
        ...
        if (miningBlockHeaderSize > maxBlockHeaderSize || miningBlockSize > Block.MAX_BLOCK_SIZE) {
          ...
          if (miningBlockSize > Message.MAX_MESSAGE_SIZE) {
            ...
            blocksLog.warn(
                this.context.getName()
                    + ": Block "
                    + miningBlock.getHeader().getHash()
                    + ":"
                    + miningBlock.getHeader().getHeight()
                    + " is larger than "
            ...
            if (miningBlockHeaderSize - maxBlockHeaderSize > miningSizeAdjustment) {
              ...
              blocksLog.warn(
                  this.context.getName()
                      ...
                      + ":"
                      + miningBlock.getHeader().getHeight()
                      + " is larger than expected size "
                      + maxBlockHeaderSize
                      + " bytes at "
                      + miningBlockHeaderSize
                      + " bytes");
              blocksLog.warn(
                  this.context.getName()
                      + ":     Max block header size adjusted to "
                      + maxBlockHeaderSize
                      + " bytes");
            }
          }
          ...
          blockSizeRemaining =
              (Message.MAX_MESSAGE_SIZE - maxBlockHeaderSize) - candidateAcceptedSize;
    ...
    PendingBlock candidateBlock =
        new PendingBlock(this.context, miningBlock.getHeader(), this.inventory);
