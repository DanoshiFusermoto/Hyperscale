...
public abstract class NativeComponent extends Component {
  private static final Map<Class<? extends NativeComponent>, MethodInfo[]> methods =
  ...
  private static final Map<Class<? extends NativeComponent>, Substate> nativeComponentSubstates =
      new HashMap<>();

  private static final SubstateLog createNativeComponentSubstate(
      StateMachine stateMachine, Class<? extends NativeComponent> clazz) {
    ...
    synchronized (nativeComponentSubstates) {
      ...
      if (nativeComponentSubstate == null) {
        ...
        nativeComponentSubstate =
            new Substate(StateAddress.from(Component.class, component))
                .set(NativeField.COMPONENT, component)
                .set(NativeField.AUTHORITY, ComputeKey.NULL.getIdentity());
  ...
  @Override
  protected final void prepare(String method, Object[] arguments)
      throws ReflectiveOperationException {
    final MethodInfo[] methodInfo =
        NativeComponent.methods.computeIfAbsent(
            getClass(), NativeComponent::computeComponentMethodInfo);
    for (int m = 0; m < methodInfo.length; m++) {
      if (methodInfo[m].getMethod().getName().equals("lock") == false) continue;

      Class<?>[] parameterTypes = methodInfo[m].getParameterTypes();
      if (parameterTypes.length != 2) continue;

      if (parameterTypes[0].isAssignableFrom(String.class) == false) continue;

      if (parameterTypes[1].isAssignableFrom(Object[].class) == false) continue;

      Object[] methodArguments = new Object[arguments.length];
      for (int i = 0; i < arguments.length; i++) {
        if (arguments[i] instanceof Argument argument) methodArguments[i] = argument.get();
        else methodArguments[i] = arguments[i];
      }
    ...
    // TODO need to throw?
    //        throw new NoSuchMethodException(getClass().getName() + "." + "lock(method, args) not
    // found");
  }
  ...
  @Override
  protected final <T> T call(String method, Object[] arguments)
      throws ReflectiveOperationException {
    Object[] methodArguments = new Object[arguments.length];
    for (int i = 0; i < arguments.length; i++) {
      if (arguments[i] instanceof Argument argument) methodArguments[i] = argument.get();
    ...
    final MethodInfo[] methodInfo =
        NativeComponent.methods.computeIfAbsent(
            getClass(), NativeComponent::computeComponentMethodInfo);
    for (int m = 0; m < methodInfo.length; m++) {
      if (methodInfo[m].getMethod().getName().equals(method) == false) continue;
      ...
      if (matches) return (T) methodInfo[m].getMethod().invoke(this, methodArguments);
    }

    throw new NoSuchMethodException(
        getClass().getName() + "." + method + "(" + Arrays.toString(methodArguments) + ")");
  }
}
