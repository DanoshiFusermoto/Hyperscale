...
class StateVoteBlockCollector {
  ...
  public synchronized void vote(StateVoteBlock stateVoteBlock, long weight)
      throws ValidationException {
    if (this.voted.contains(stateVoteBlock.getOwner()))
      throw new IllegalStateException(
          stateVoteBlock.getOwner() + " has already voted on SVB " + stateVoteBlock);
    ...
    if (stateVoteBlock.getID().equals(this.ID) == false)
      throw new ValidationException(
          stateVoteBlock,
          "SVB with ID " + stateVoteBlock.getID() + " doesn't match collector ID " + this.ID);

    if (stateVoteBlock.getBlock().equals(this.block) == false)
      throw new ValidationException(
          stateVoteBlock,
          "SVB with ID " + stateVoteBlock.getID() + " doesn't match collector block" + this.block);

    if (statePoolLog.hasLevel(Logging.DEBUG))
      statePoolLog.debug(
          this.context.getName()
              + ": Applying state vote block "
              + stateVoteBlock.getHash()
              + " with vote weight "
              + weight
              + " from "
              + stateVoteBlock.getOwner());
    ...
    if (this.votes.size() > 1) {
      statePoolLog.warn(
          this.context.getName()
              + ": Detected "
              + this.votes.size()
              + " conflicting state vote block outputs for state ID "
              + this.ID);
    ...
    if (statePoolLog.hasLevel(Logging.INFO)) {
      if (stateVoteBlock.isHeader() == false)
        ...
        statePoolLog.info(
            this.context.getName() + ": Applied state vote block header " + stateVoteBlock);
    }
  }
  ...
  synchronized StateVoteBlock getSuper() {
    ...
    for (Hash vote : this.votes.keySet()) {
      if (this.votes.get(vote) < this.voteThreshold) continue;
    ...
    for (StateVoteBlock stateVoteBlock : this.stateVoteBlocks.values()) {
      if (stateVoteBlock.getExecutionMerkle().equals(superHash) == false) continue;

      superStateVoteBlock = stateVoteBlock;

      if (superStateVoteBlock.isHeader() == false) break;
    }
  ...
  synchronized Collection<StateVoteBlock> tryComplete() {
    ...
    if (superStateVoteBlock == null) return null;

    if (superStateVoteBlock.isHeader()) return null;

    List<StateVoteBlock> superStateVoteBlocks =
        new ArrayList<StateVoteBlock>(this.stateVoteBlocks.size());
    for (StateVoteBlock stateVoteBlock : this.stateVoteBlocks.values()) {
      if (stateVoteBlock.getExecutionMerkle().equals(superStateVoteBlock.getExecutionMerkle())
          == false) continue;
  ...
  public boolean isStale() {
    if (Time.getSystemTime() - this.timestamp
        >= TimeUnit.SECONDS.toMillis(Constants.PRIMITIVE_STALE)) return true;
