...
public class BlockHandler implements Service {
  ...
  private LatchedProcessor blockProcessor =
      new LatchedProcessor(
          Constants.BLOCK_INTERVAL_TARGET_MILLISECONDS / ProgressRound.State.values().length,
          TimeUnit.MILLISECONDS) {
        @Override
        public void process() {
          if (BlockHandler.this.context.getNode().isSynced() == false) return;
        ...
        public void onError(Throwable thrown) {
          blocksLog.fatal(
              BlockHandler.this.context.getName() + ": Error processing blocks", thrown);
        }
  ...
  // 1024 queue size should be sufficient to hold enough progress events even at fast proposal times
  // as a backlog before an OOS is triggered
  private BlockingQueue<ProgressPhaseEvent> progressPhaseQueue =
      new ArrayBlockingQueue<ProgressPhaseEvent>(1024);
  private PollingProcessor buildAndCommitProcessor =
      new PollingProcessor() {
        @Override
        public void process() throws InterruptedException {
          final ProgressPhaseEvent progressRoundEvent =
              BlockHandler.this.progressPhaseQueue.poll(1, TimeUnit.SECONDS);
          if (progressRoundEvent == null) return;
          ...
          try {
            ...
            if (progressRoundPhase.equals(ProgressRound.State.NONE))
              ...
              prebuild(progressRound, ProgressRound.State.VOTING);
                ...
                && progressRound.isProposalsLatent()
                && progressRound.getProposers().isEmpty())
        ...
        public void onError(Throwable thrown) {
          blocksLog.fatal(
              BlockHandler.this.context.getName() + ": Error processing progress rounds", thrown);
        }

        @Override
        public void onTerminated() {
          blocksLog.log(
              BlockHandler.this.context.getName() + ": Block builder and committer is terminated");
        }
      };
  ...
  // TODO temporary fix to ensure that validation of new proposals, and the commitment of existing
  // ones don't cause deadlocks between each other
  ...
  BlockHandler(Context context) {
    ...
    this.guardLock =
        new MonitoredReentrantLock(this.context.getName() + " Block Handler Guard Lock");
    this.syncLock =
        new MonitoredReadWriteLock(this.context.getName() + " Block Handler Sync Lock", true);
  }
  ...
  public void start() throws StartupException {
    ...
    this.context
        ...
        .register(
            BlockHeader.class,
            new GossipFilter<BlockHeader>(this.context) {
              @Override
              public Set<ShardGroupID> filter(BlockHeader blockHeader) {
                final Epoch epoch = Epoch.from(blockHeader);
                final int numShardGroups =
                    BlockHandler.this.context.getLedger().numShardGroups(epoch);
                final ShardGroupID blockShardGroupID =
                    ShardMapper.toShardGroup(blockHeader.getProposer(), numShardGroups);
                final ShardGroupID localShardGroupID =
                    ShardMapper.toShardGroup(
                        BlockHandler.this.context.getNode().getIdentity(), numShardGroups);
                if (blockShardGroupID.equals(localShardGroupID) == false) {
                  blocksLog.warn(
                      BlockHandler.this.context.getName()
                          + ": Block header is for shard group ID "
                          + blockShardGroupID
                          + " but expected local shard group ID "
                          + localShardGroupID);
            ...
            new GossipInventory() {
              ...
              public Collection<Hash> required(
                ...
                if (type.equals(BlockHeader.class) == false) {
                  blocksLog.error(
                      BlockHandler.this.context.getName()
                          + ": Block header type expected but got "
                          + type);
                ...
                required.removeAll(
                    BlockHandler.this.context.getLedger().getLedgerStore().has(required, type));
    ...
    this.context
        ...
        .register(
            BlockHeader.class,
            new GossipReceiver<BlockHeader>() {
              @Override
              public void receive(
                  final Collection<BlockHeader> headers, final AbstractConnection connection)
                  throws IOException {
                if (BlockHandler.this.context.getNode().isSynced() == false) return;
                ...
                for (BlockHeader header : headers) {
                  ...
                  final ShardGroupID localShardGroupID =
                      ShardMapper.toShardGroup(
                          BlockHandler.this.context.getNode().getIdentity(), numShardGroups);
                  final ShardGroupID blockVoteShardGroupID =
                      ShardMapper.toShardGroup(header.getProposer(), numShardGroups);
                  if (localShardGroupID.equals(blockVoteShardGroupID) == false) {
                    blocksLog.warn(
                        BlockHandler.this.context.getName()
                            ...
                            + " for "
                            + header.getProposer()
                            + " is for shard group ID "
                            + blockVoteShardGroupID
                            + " but expected local shard group ID "
                            + localShardGroupID);
                    // TODO disconnect and ban;
                    continue;
                  ...
                  if (BlockHandler.this
                    ...
                    if (BlockHandler.this
                            ...
                            .getNetwork()
                            .getGossipHandler()
                            .broadcast(header, localShardGroupID)
                        == false)
                      blocksLog.warn(
                          BlockHandler.this.context.getName()
                              + ": Failed to broadcast block header "
                              + header);
                  } else
                    blocksLog.warn(
                        BlockHandler.this.context.getName()
                            + ": Failed to store block header "
                            + header);
                }
              }
            });
    ...
    this.context
        ...
        .register(
            BlockHeader.class,
            new GossipFetcher<BlockHeader>() {
              @Override
              public Collection<BlockHeader> fetch(
                ...
                while (toFetchIterator.hasNext()) {
                  ...
                  if (blockHeader == null) {
                    ...
                    if (pendingBlock == null || pendingBlock.getHeader() == null) continue;
    ...
    this.context
        ...
        .register(
            BlockVote.class,
            new GossipFilter<BlockVote>(this.context) {
              @Override
              public Set<ShardGroupID> filter(BlockVote blockVote) {
                ...
                final int numShardGroups =
                    BlockHandler.this.context.getLedger().numShardGroups(epoch);
                final ShardGroupID blockShardGroupID =
                    ShardMapper.toShardGroup(blockVote.getOwner().getIdentity(), numShardGroups);
                final ShardGroupID localShardGroupID =
                    ShardMapper.toShardGroup(
                        BlockHandler.this.context.getNode().getIdentity(), numShardGroups);
                if (blockShardGroupID.equals(localShardGroupID) == false) {
                  blocksLog.warn(
                      BlockHandler.this.context.getName()
                          + ": Block vote is for shard group ID "
                          + blockShardGroupID
                          + " but expected local shard group ID "
                          + localShardGroupID);
                  // TODO disconnect and ban;
            ...
            new GossipInventory() {
              ...
              public Collection<Hash> required(
                ...
                if (type.equals(BlockVote.class) == false) {
                  blocksLog.error(
                      BlockHandler.this.context.getName()
                          + ": Block vote type expected but got "
                          + type);
                  return Collections.emptyList();
                }
                ...
                List<Hash> required = new ArrayList<Hash>(items.size());
                for (Hash item : items) {
                  if (BlockHandler.this.votesToVerify.containsKey(item)) continue;

                  if (BlockHandler.this.postponedVotes.containsKey(item)) continue;
    ...
    this.context
        ...
        .register(
            BlockVote.class,
            new GossipReceiver<BlockVote>() {
              @Override
              public void receive(Collection<BlockVote> blockVotes, AbstractConnection connection)
                  throws IOException {
                if (BlockHandler.this.context.getNode().isSynced() == false) return;
                ...
                for (final BlockVote blockVote : blockVotes) {
                  ...
                  if (BlockHandler.this
                    ...
                    if (BlockHandler.this
                            ...
                            .getNetwork()
                            .getGossipHandler()
                            .broadcast(blockVote, localShardGroupID)
                        == false)
                      blocksLog.warn(
                          BlockHandler.this.context.getName()
                              + ": Failed to broadcast block vote "
                              + blockVote);
                  } else
                    blocksLog.warn(
                        BlockHandler.this.context.getName()
                            + ": Failed to store block vote "
                            + blockVote);
                }
    ...
    this.context
        ...
        .register(
            BlockVote.class,
            new GossipFetcher<BlockVote>() {
              public Collection<BlockVote> fetch(
                  final Collection<Hash> items, final AbstractConnection connection)
                  throws IOException {
                ...
                while (toFetchIterator.hasNext()) {
                  ...
                  if (blockVote == null) blockVote = BlockHandler.this.postponedVotes.get(item);
                  if (blockVote == null) continue;
              ...
              public void process(
                ...
                try {
                  ...
                  while (syncInventoryHeight
                      <= BlockHandler.this.context.getLedger().getHead().getHeight()) {
                    Hash syncBlockHash =
                        BlockHandler.this
                            .context
                            .getLedger()
                            .getLedgerStore()
                            .getSyncBlock(syncInventoryHeight);
                    ...
                    BlockHandler.this
                        ...
                        .getLedger()
                        .getLedgerStore()
                        .getSyncInventory(syncInventoryHeight, BlockHeader.class)
                        .forEach(bh -> pendingBlockInventory.add(bh.getHash()));
                    BlockHandler.this
                        .context
                        .getLedger()
                        .getLedgerStore()
                        .getSyncInventory(syncInventoryHeight, BlockVote.class)
                        .forEach(bv -> blockVoteInventory.add(bv.getHash()));
                    syncInventoryHeight++;
                  }
                  ...
                  if (syncLog.hasLevel(Logging.DEBUG))
                    syncLog.debug(
                        BlockHandler.this.context.getName()
                            + ": Broadcasting blocks "
                            + pendingBlockInventory.size()
                            + " / "
                            + pendingBlockInventory
                            + " to "
                            + connection);
                  else
                    syncLog.log(
                  ...
                  while (pendingBlockInventory.isEmpty() == false) {
                    InventoryMessage pendingBlockInventoryMessage =
                    ...
                    BlockHandler.this
                        ...
                        .getNetwork()
                        .getMessaging()
                        .send(pendingBlockInventoryMessage, connection);
                    pendingBlockInventory.removeAll(
                        pendingBlockInventoryMessage.asInventory().stream()
                            .map(ii -> ii.getHash())
                  ...
                  if (syncLog.hasLevel(Logging.DEBUG))
                    syncLog.debug(
                        BlockHandler.this.context.getName()
                            + ": Broadcasting block votes "
                            + blockVoteInventory.size()
                            + " / "
                            + blockVoteInventory
                            + " to "
                            + connection);
                  ...
                  while (blockVoteInventory.isEmpty() == false) {
                    InventoryMessage blockVoteInventoryMessage =
                        new InventoryMessage(
                            blockVoteInventory,
                            0,
                            Math.min(
                    ...
                    BlockHandler.this
                        ...
                        .getNetwork()
                        .getMessaging()
                        .send(blockVoteInventoryMessage, connection);
                    blockVoteInventory.removeAll(
                        blockVoteInventoryMessage.asInventory().stream()
                            .map(ii -> ii.getHash())
                            .collect(Collectors.toList()));
                  }
                } catch (Exception ex) {
                  blocksLog.error(
                      BlockHandler.this.context.getName()
                          + ": ledger.messages.block.get.pool "
                          + connection,
                      ex);
                }
              }
  ...
  private void _updateBlocks(final BlockHeader head) {
    ...
    synchronized (this.pendingBlocks) {
      ...
      Collections.sort(
          pendingBlocks,
          (pb1, pb2) -> {
            if (pb1.getHeight() < pb2.getHeight()) return -1;
            if (pb1.getHeight() > pb2.getHeight()) return 1;
    ...
    for (int pb = 0; pb < pendingBlocks.size(); pb++) {
      ...
      if (pendingBlock.getHeader() == null) continue;
      ...
      try {
        ...
        if (proposalRound.canPropose(pendingBlock.getHeader().getProposer()) == true) {
          final long roundVotePower =
              this.context
                  .getLedger()
                  .getValidatorHandler()
                  .getVotePower(proposalRound.epoch(), pendingBlock.getHeader().getProposer());
          if (proposalRound.propose(
                  pendingBlock.getHash(), pendingBlock.getHeader().getProposer(), roundVotePower)
              == false)
            blocksLog.warn(
                this.context.getName()
                    + ": Progress round "
                    + pendingBlock.getHeight()
                    + " already has a proposal from "
                    + pendingBlock.getHeader().getProposer());
          else if (blocksLog.hasLevel(Logging.INFO))
            blocksLog.info(
                this.context.getName()
                    + ": Seen proposal "
                    + pendingBlock.getHash()
                    + " for progress round "
                    + proposalRound.clock()
                    + " from "
                    + pendingBlock.getHeader().getProposer());
        ...
        if (pendingBlock.getBlock() == null) {
          final PendingBlock previousBlock =
              this.pendingBlocks.get(pendingBlock.getHeader().getPrevious());
          if (pendingBlock.getHeader().getPrevious().equals(head.getHash()) == false
              && previousBlock != null
              && previousBlock.getBlock() == null) continue;

          if (blocksLog.hasLevel(Logging.DEBUG))
            blocksLog.debug(this.context.getName() + ": Updating block " + pendingBlock);

          boolean isLatent = false;
          if (this.reportedLatent == -1
              && System.currentTimeMillis() - pendingBlock.getHeader().getTimestamp()
                  > TimeUnit.SECONDS.toMillis(5)) {
            this.reportedLatent = pendingBlock.getHeight();
            isLatent = true;
          }

          // TODO A real hunk of boilerplate stuff to build the contents.  Needs improving at some
          // point
          ...
          for (int i = 0; i < absentInventory.size(); i++) {
            ...
            final PendingAtom pendingAtom =
                this.context.getLedger().getAtomHandler().get(unacceptedHash);
            if (pendingAtom == null
                || pendingAtom.getTimeout() == null
                || pendingAtom.getTimeout() instanceof AcceptTimeout == false) {
              if (isLatent) {
          ...
          for (int i = 0; i < absentInventory.size(); i++) {
            ...
            final PendingAtom pendingAtom =
                this.context.getLedger().getAtomHandler().get(latentHash);
            if (pendingAtom == null
                || pendingAtom.getTimeout() == null
                || pendingAtom.getTimeout() instanceof ExecutionLatentTimeout == false) {
              if (isLatent) {
          ...
          for (int i = 0; i < absentInventory.size(); i++) {
            ...
            final PendingAtom pendingAtom =
                this.context.getLedger().getAtomHandler().certificate(certificateHash);
          ...
          for (int i = 0; i < absentInventory.size(); i++) {
            ...
            final PendingAtom pendingAtom =
                this.context.getLedger().getAtomHandler().unexecuted(unexecutedHash);
            if (pendingAtom == null
                || pendingAtom.getTimeout() == null
                || pendingAtom.getTimeout() instanceof ExecutionTimeout == false
                || pendingAtom.getTimeout().getHash().equals(unexecutedHash) == false) {
              if (isLatent) {
                this.latentHash = unexecutedHash;
                blocksLog.error(
                    this.context.getName()
                        + ": Atom execution timeout "
                        + unexecutedHash
                        + " for block "
                        + pendingBlock.getHash()
                        + " has critical delay");
          ...
          for (int i = 0; i < absentInventory.size(); i++) {
            ...
            final PendingAtom pendingAtom =
                this.context.getLedger().getAtomHandler().uncommitted(uncommitedHash);
            if (pendingAtom == null
                || pendingAtom.getTimeout() == null
                || pendingAtom.getTimeout() instanceof CommitTimeout == false
                || pendingAtom.getTimeout().getHash().equals(uncommitedHash) == false) {
              if (isLatent) {
          ...
          for (int i = 0; i < absentInventory.size(); i++) {
            ...
            final PolyglotPackage pakage =
                this.context.getLedger().getPackageHandler().uncommitted(packageHash);
            if (pakage == null) {
              if (isLatent) {
          ...
          if (pendingBlock.isConstructable()) {
            ...
            if (blocksLog.hasLevel(Logging.INFO))
              blocksLog.info(
                  this.context.getName() + ": Constructed proposal " + pendingBlock.toString());
          }
        }
      } catch (Exception e) {
        blocksLog.error(
            this.context.getName() + ": Update of proposal " + pendingBlock.toString() + " failed",
            e);
      }
    }
  }
  ...
  private void _updateBranches(final BlockHeader head) {
    ...
    for (int pb = 0; pb < pendingBlocks.size(); pb++) {
      ...
      if (pendingBlock.thrown() != null) continue;
        ...
        try {
          if (pendingBranch.getRoot().getHash().equals(head.getHash()) == false
            ...
            if (blocksLog.hasLevel(Logging.DEBUG))
              blocksLog.debug(
                  this.context.getName()
                      + ": Branch doesn't attach to ledger "
                      + pendingBranch.getRoot());
          ...
          blocksLog.error(
              this.context.getName() + ": Branch update " + pendingBranch.toString() + " failed",
              ex);
        }
      }
    }
  ...
  private void _processHeaders(final ProgressRound progressRound) {
    final List<BlockHeader> headersToVerify =
        new ArrayList<BlockHeader>(this.headersToVerify.size());
    BlockHandler.this.headersToVerify.forEach(
        (h, bh) -> {
          if (bh.getHeight() <= progressRound.clock() + 1) headersToVerify.add(bh);
        });
    ...
    if (headersToVerify.isEmpty() == false) {
      ...
      while (headersToVerifyIterator.hasNext()) {
        ...
        try {
          ...
          if (status.equals(BlockInsertStatus.SUCCESS)) {
            ...
            if (headerToVerify.getHeight() == progressRound.clock()
                && headerToVerify.getCertificate() != null) progressRound.terminate();
          } else if (status.equals(BlockInsertStatus.POSTPONED)) headersToVerifyIterator.remove();
        } catch (IOException ex) {
          blocksLog.error(
              this.context.getName() + ": Insertion of block header " + headerToVerify + " failed",
              ex);
        } catch (ValidationException | CryptoException ex) {
          blocksLog.error(
              this.context.getName() + ": Validation of block header " + headerToVerify + " failed",
              ex);
        }
      }
      ...
      for (final BlockHeader headerToVerify : headersToVerify) {
        if (this.headersToVerify.remove(headerToVerify.getHash(), headerToVerify) == false
            && this.context.getNode().isSynced())
          blocksLog.warn(
              this.context.getName() + ": Block header peek/remove failed for " + headerToVerify);
      }
    }
  }
  ...
  private void _verifyPendingVotes(final ProgressRound progressRound, final BlockHeader head) {
    if (this.votesToVerify.isEmpty() == false) {
      ...
      this.votesToVerify.forEach(
          (h, bv) -> {
            if (bv.getHeight() <= progressRound.clock()) votesToVerify.add(bv);
            // TODO supermajority this check
            else if (this.shardClock.get() < bv.getHeight()) this.shardClock.set(bv.getHeight());
          });
        ...
        for (final BlockVote blockVote : votesToVerify) {
          if (blockVote.getHeight() <= head.getHeight()) {
            if (this.votesToVerify.remove(blockVote.getHash()) == null)
              blocksLog.warn(
                  this.context.getName()
                      + ": Stale block vote remove failed "
                      + blockVote.getHash()
                      + " for "
                      + blockVote.getHeight()
                      + "@"
          ...
          final BlockVoteCollector blockVoteCollector =
              this.blockVoteCollectors.computeIfAbsent(
                  blockVote.getHeight(), h -> new BlockVoteCollector(this.context, voteRound));
            ...
            try {
              final long roundVotePower =
                  this.context
                      .getLedger()
                      .getValidatorHandler()
                      .getVotePower(voteRound.epoch(), blockVote.getOwner().getIdentity());
              if (roundVotePower == 0)
                blocksLog.warn(
                    this.context.getName()
                        + ": Vote power is zero in epoch "
                        + voteRound.epoch()
                        + " for "
                        + blockVote.getOwner().toString(12));
              ...
              if (voteRound.vote(blockVote.getOwner().getIdentity(), roundVotePower) == false)
                blocksLog.warn(
                    this.context.getName()
                        + ": Block vote "
                        + blockVote.getHash()
                        + " already seen in progress round "
                        + voteRound.clock()
                        + " for "
                        + blockVote.getOwner().toString(12));
              ...
              blocksLog.warn(
                  this.context.getName()
                      + ": Failed to pre-process block vote "
                      + blockVote.getHash()
                      + " in progress round "
                      + voteRound.clock()
                      + " for "
  ...
  private void _collateVotesAndApply(final ProgressRound progressRound, final BlockHeader head) {
    if (this.blockVoteCollectors.isEmpty() == false) {
      ...
      synchronized (this.blockVoteCollectors) {
        final Iterator<BlockVoteCollector> blockVoteCollectorIterator =
            this.blockVoteCollectors.values().iterator();
        while (blockVoteCollectorIterator.hasNext()) {
          final BlockVoteCollector blockVoteCollector = blockVoteCollectorIterator.next();
          if (blockVoteCollector.canVerify() == false) continue;

          try {
            final long currentRoundVoteWeight =
                blockVoteCollector.getProgressRound().getVoteWeight();
            final List<BlockVote> validBlockVotes = blockVoteCollector.tryVerify();
            if (validBlockVotes.isEmpty() == false) {
              if (blocksLog.hasLevel(Logging.DEBUG)) {
                blocksLog.debug(
                    this.context.getName()
                        + ": Valid block votes to process for progress round "
                        + blockVoteCollector.getProgressRound().clock());
                for (final BlockVote validBlockVote : validBlockVotes)
                  blocksLog.debug(
                      this.context.getName()
                          + ":       "
                          + validBlockVote.getHeight()
                          + ":"
                          + validBlockVote.getBlock()
                          + " from "
                          + validBlockVote.getOwner().getIdentity().toString(12));
              }
              ...
              for (final BlockVote blockVote : validBlockVotes) {
                try {
                  ...
                  blocksLog.error(
                      this.context.getName() + ": Error processing block vote " + blockVote, ex);
                }
              }

              // Signal vote weight threshold reached?
              if (currentRoundVoteWeight < progressRound.getVoteThreshold()) {
                if (progressRound.isVoteCompleted()) signalVotePhaseCompleted = true;
              }
            }
          } catch (Exception ex) {
            blocksLog.error(
                this.context.getName()
                    + ": Error processing block vote collector for progress round "
                    + blockVoteCollector.getHeight(),
                ex);
          }
        }
      }

      if (signalVotePhaseCompleted) this.blockProcessor.signal();
    }
  }
    ...
    if (this.postponedVotes.isEmpty() == false) {
      ...
      if (blocksLog.hasLevel(Logging.DEBUG))
        blocksLog.info(
            this.context.getName()
                + ": Evaluating "
                + this.postponedVotes.size()
                + " postponed block votes");

      boolean signalVotePhaseCompleted = false;
      synchronized (this.postponedVotes) {
        final long currentRoundVoteWeight = progressRound.getVoteWeight();
        final Iterator<BlockVote> votesToCountDelayedIterator =
            this.postponedVotes.values().iterator();
        while (votesToCountDelayedIterator.hasNext()) {
          ...
          try {
            if (blocksLog.hasLevel(Logging.DEBUG))
              blocksLog.debug(
                  this.context.getName()
                      + ": Evaluating delayed block vote "
                      + delayedBlockVote.getHash()
                      + " for block "
                      + delayedBlockVote.getBlock()
                      + " by "
                      + delayedBlockVote.getOwner());
            ...
            if (status.equals(BlockVoteStatus.POSTPONED) == false) {
              if (blocksLog.hasLevel(Logging.INFO))
                blocksLog.info(
                    this.context.getName()
                        + ": Processed delayed block vote "
                        + delayedBlockVote.getHash()
                        + " for "
                        + delayedBlockVote.getHeight()
                        + "@"
                        + delayedBlockVote.getBlock()
                        + " for "
                        + delayedBlockVote.getOwner());
            ...
            blocksLog.error(
                this.context.getName()
                    + ": Maintenence of delayed block vote failed for "
                    + delayedBlockVote.getHash()
                    + " for block "
                    + delayedBlockVote.getBlock()
                    + " by "
  ...
  private void _livenessTick(final ProgressRound progressRound) {
    ...
    if (progressRound.getState().equals(ProgressRound.State.TRANSITION)) {
      ...
      if (progressRound.isTransitionLatent() || progressRound.isFullyProposed()) {
        for (final Hash proposal : progressRound.getProposals()) {
          ...
          if (pendingBlock.getBlock() == null) continue;
          try {
            contructedVotePower +=
                this.context
                    .getLedger()
                    .getValidatorHandler()
                    .getVotePower(progressRound.epoch(), pendingBlock.getHeader().getProposer());
          } catch (IOException ioex) {
            blocksLog.error(
                this.context.getName()
                    + ": Failed to get vote power for proposer "
                    + pendingBlock.getHeader().getProposer().toString(12)
                    + " of proposal "
                    + pendingBlock.getHeight()
                    + ":"
                    + pendingBlock.getHash(),
    ...
    if (progressRound.getState().equals(ProgressRound.State.VOTING)) {
      ...
      if (progressRound.driftClock() <= 0
        ...
        try {
          if (selectedBranch != null) vote(progressRound, selectedBranch);
        } catch (IOException | CryptoException | ValidationException ex) {
          blocksLog.error(
              this.context.getName()
                  + ": Failed to cast vote on progress round "
                  + progressRound
                  + " to branch "
                  + selectedBranch,
              ex);
    ...
    if (progressRound.getState().equals(ProgressRound.State.COMPLETED)) {
      ...
      final long targetRoundDuration =
          Math.max(
              Constants.MINIMUM_ROUND_DURATION_MILLISECONDS,
              Configuration.getDefault().get("ledger.liveness.delay", 0));
      final long roundDelayDuration =
          (targetRoundDuration - progressRound.getDuration())
              + Math.min(progressRound.driftMilli() / 2, 0);
      ...
      // Too slow
        ...
        if (blocksLog.hasLevel(Logging.INFO)) {
          if (this.shardClock.get() > progressRound.clock())
            blocksLog.info(
                this.context.getName()
                    + ": Skipping round delay for "
                    + progressRound.clock()
                    + " because shard clock is ahead "
                    + this.shardClock.get());
          else
            blocksLog.info(
                this.context.getName()
                    + ": Skipping round delay for "
                    + progressRound.clock()
                    + " because of long round interval "
                    + progressRound.getDuration()
                    + "ms");
        }
      }
      ...
      if (blocksLog.hasLevel(Logging.INFO))
        blocksLog.info(
            this.context.getName() + ": Progress round is now " + nextProgressRound.toString());
    }
  }

  private void _decideCommit(
      final ProgressRound progressRound, final ProgressRound.State progressRoundPhase) {
    // If the round was latent on completion then do not attempt a commit and continue swiftly to
    // the next round.
    ...
    if (progressRoundPhase.equals(ProgressRound.State.COMPLETED)) {
      ...
      if (durationDelay < 0) {
        if (blocksLog.hasLevel(Logging.DEBUG))
          blocksLog.debug(
              BlockHandler.this.context.getName()
                  + ": Skipping commit on COMPLETED for progress round "
                  + progressRound.clock()
                  + " due to round duration latency of "
                  + (-durationDelay)
                  + "ms");
        return;
      }
    ...
    try {
      ...
      if (selectedBranch != null) {
        ...
        if (commitTo != null) commit(commitTo, selectedBranch);
      }
    } catch (Exception ex) {
      // TODO additional clean up here, trim bad branch, etc
      blocksLog.error(
          this.context.getName() + ": Failed to commit proposal on " + progressRound, ex);
    }
  }
  ...
  private ProgressRound get(final long clock) {
    ...
    synchronized (this.progressRounds) {
      progressRound =
          this.progressRounds.getIfAbsentPut(
              clock,
              () -> {
                // FIXME needs to acquire value from ledger / branches but need vote power epochs
                // etc
                final Epoch epoch = this.context.getLedger().getEpoch();
                final ShardGroupID localShardGroupID =
                    ShardMapper.toShardGroup(
                        this.context.getNode().getIdentity(),
                        this.context.getLedger().numShardGroups());
                ...
                try {
                  ...
                  nextProposers =
                      this.context
                          .getLedger()
                          .getValidatorHandler()
                          .getProposers(clock, Hash.ZERO, localShardGroupID);
                  nextProposersVotePower =
                      this.context
                          .getLedger()
                          .getValidatorHandler()
                          .getVotePower(epoch, nextProposers);
                  nextTotalVotePower =
                      this.context
                          .getLedger()
                          .getValidatorHandler()
                  ...
                  blocksLog.fatal(
                      this.context.getName()
                          + ": Failed to get progress sync vote power at "
                          + clock,
                      e);
                ...
                return new ProgressRound(
                    clock,
                    nextProposers,
                    nextProposersVotePower,
                    nextTotalVotePower,
                    this.shardClock.get() - this.progressClock.get());
              });
    }
  ...
  private BlockVoteStatus process(final BlockVote blockVote, final BlockHeader head)
      throws IOException, CryptoException, ValidationException {
    ...
    if (blockVote.getHeight() < head.getHeight()) {
      if (blocksLog.hasLevel(Logging.DEBUG))
        blocksLog.warn(
            BlockHandler.this.context.getName()
                + ": Removed stale delayed block vote "
                + blockVote);
    ...
    // The vote should be applied even if the pending block is not yet know to ensure responsiveness
    // and liveness of progress rounds
    if (voteRound.hasVoted(blockVote.getOwner().getIdentity()) == false) {
      ...
      if (roundVotePower == 0)
        blocksLog.warn(
            BlockHandler.this.context.getName()
                + ": Progress vote power is zero for "
                + blockVote.getOwner());
      ...
      if (blocksLog.hasLevel(Logging.DEBUG))
        blocksLog.warn(
            this.context.getName()
                + ": Block vote "
                + blockVote.getHash()
                + " is postponed for unbranched block "
                + blockVote.getHeight()
                + "@"
                + blockVote.getBlock()
                + " by "
    ...
    if (pendingBlock.voted(blockVote.getOwner()) == false) {
      ...
      try {
        final long branchVotePower =
            pendingBranch.getVotePower(
                blockVote.getHeight(), blockVote.getOwner().getIdentity(), true);
        ...
        if (pendingBlock.vote(blockVote, branchVotePower) == true) {
          ...
          if (preWeight < pendingBlock.getVoteThreshold()
              && pendingBlock.getVoteWeight() >= pendingBlock.getVoteThreshold()) {
            if (blocksLog.hasLevel(Logging.INFO))
              blocksLog.info(
                  this.context.getName()
                      + ": Pending block is now a super "
                      + pendingBlock.toString());

            if (pendingBlock.buildCertificate() == null)
              blocksLog.error(
                  this.context.getName()
                      + ": Expected to construct block certificate for "
                      + pendingBlock.toString());
          }

          return BlockVoteStatus.SUCCESS;
        } else return BlockVoteStatus.SKIPPED;
      } finally {
        this.context.getMetaData().increment("ledger.blockvote.processed");
        this.context
            .getMetaData()
            .increment("ledger.blockvote.latency", blockVote.getAge(TimeUnit.MILLISECONDS));
      }
    } else
      blocksLog.warn(
          this.context.getName()
              + ": Already seen block vote "
              + blockVote.getHash()
              + " for block "
              + blockVote.getHeight()
              + "@"
   ...
   */
  private BlockVote vote(final ProgressRound round, final PendingBranch branch)
      throws IOException, CryptoException, ValidationException {
    if (branch.isEmpty()) throw new IllegalStateException("Branch is empty " + branch);

    if (round.clock() != this.progressClock.get())
      throw new ValidationException(
          "Attempted to vote on progress round "
              + round.clock()
              + " but vote clock is "
              + this.progressClock.get());
    ...
    if (round.hasVoted(this.context.getNode().getIdentity())) {
      blocksLog.warn(
          this.context.getName()
              + ": Progress vote is already cast in progress round "
              + round.clock()
              + " by "
              + this.context.getNode().getIdentity());
      return null;
    ...
    if (pendingBlock == null) {
      blocksLog.warn(
          this.context.getName()
              + ": No proposal available at progress round "
              + round.clock()
              + " to vote on in branch "
              + branch);
      return null;
    }

    if (pendingBlock.isApplied() == false) return null;

    if (pendingBlock.getBlock() == null) return null;

    BlockVote blockVote = null;
    long votePower = branch.getVotePower(round.clock(), this.context.getNode().getIdentity());
    if (votePower > 0) {
      blockVote =
          new BlockVote(pendingBlock.getHash(), this.context.getNode().getIdentity().getKey());
      ...
      if (this.context
        ...
        if (BlockHandler.this
          ...
          blocksLog.warn(
              BlockHandler.this.context.getName()
                  + ": Failed to broadcast own block vote "
                  + blockVote);
        ...
        // TODO handle better?
        blocksLog.error(
            BlockHandler.this.context.getName() + ": Vote on block " + pendingBlock + " failed");
      }
    }
  ...
  private void prebuild(
      final ProgressRound progressRound, final ProgressRound.State progressPhase) {
    // TODO This should actually throw, but complex to handle
    if (this.buildClock.get() > progressRound.clock()) return;

    // If the local instances shard is behind, don't build
    if (progressRound.driftClock() > 0) return;

    // Initial genesis MUST build on phase NONE
    if (progressRound.clock() <= 1 && progressPhase.equals(ProgressRound.State.VOTING)) return;

    final ProgressRound buildRound;
    if (progressPhase.equals(ProgressRound.State.NONE)
        || progressPhase.equals(ProgressRound.State.TRANSITION)) {
      ...
      if (buildRound == null)
        throw new IllegalArgumentException(
            "Expected to find next progress round "
                + (progressRound.clock() + 1)
                + " for build on progress phase "
                + progressPhase);
    } else
      throw new IllegalArgumentException(
          "Called builder with progress phase "
              + progressPhase
              + " for progress round "
              + progressRound);
    ...
    if (this.context.getConfiguration().get("ledger.liveness.recovery", Boolean.FALSE)
            == Boolean.TRUE
        && buildRound.getState().equals(ProgressRound.State.TRANSITION)) build = false;

    if (this.context.getConfiguration().has("ledger.faults.produce_unbuildable_branches_every")) {
      int interval =
          this.context
              .getConfiguration()
              .get("ledger.faults.produce_unbuildable_branches_every", -1);
      if (interval > 0 && buildRound.clock() % interval == 0) build = false;
    }
    ...
    if (build == true && this.buildLock == false) {
        ...
        if (generatedBlocks != null) {
          ...
          for (final PendingBlock generatedBlock : generatedBlocks) {
            if (this.context
                .getLedger()
                .getLedgerStore()
                .store(generatedBlock.getHeader())
                .equals(OperationStatus.SUCCESS)) {
              final BlockInsertStatus status = insert(generatedBlock);
              if (status != BlockInsertStatus.SUCCESS)
                throw new IllegalStateException(
                    "Failed to insert generated proposal "
                        + generatedBlock
                        + " into pennding blocks");

              if (this.context
                      .getNetwork()
                      .getGossipHandler()
                      .broadcast(generatedBlock.getHeader(), localShardGroupID)
                  == false)
                blocksLog.warn(
                    this.context.getName()
                        + ": Failed to broadcast generated block header "
                        + generatedBlock.getHeader());
            } else {
              blocksLog.error(
                  BlockHandler.this.context.getName()
                      + ": Failed to store generated proposal on "
                      + generatedBlock.getHeader());
              break;
            }
          }
      ...
      blocksLog.warn(
          this.context.getName() + ": Build lock is set: Skipping build on proposal " + buildRound);
  }

  private List<PendingBlock> build(final ProgressRound progressRound)
      throws IOException, LockException, ValidationException, CryptoException {
    Objects.requireNonNull(progressRound, "Progress round is null");

    if (progressRound.getState().equals(ProgressRound.State.COMPLETED))
      blocksLog.warn(
          this.context.getName()
              + ": Building on progress round "
              + progressRound.clock()
              + " which is COMPLETED");

    if (this.buildClock.get() > progressRound.clock())
      throw new IllegalStateException(
          "Build clock is " + this.buildClock.get() + " but progress round is " + progressRound);

    final Entry<BlockHeader, StateAccumulator> ledgerState =
        BlockHandler.this.context.getLedger().current();
      ...
      if (ledgerState.getKey().getHeight() == 0 && this.pendingBranches.size() == 1) {
        selectedBranch =
            new PendingBranch(
                this.context,
                ledgerState.getKey(),
                ledgerState.getValue(),
                Collections.emptyList());
        buildableHeader = selectedBranch.getRoot();
      } else {
        blocksLog.warn(
            this.context.getName()
                + ": No branch selected at "
                + this.buildClock.get()
                + " building on ledger head "
                + ledgerState.getKey());
    ...
    } else buildableHeader = selectedBranch.getBuildable(progressRound);

    if (buildableHeader == null) {
      blocksLog.warn(
          this.context.getName()
              + ": No buildable header available at "
              + this.buildClock.get()
              + " on selected branch "
              + selectedBranch);
      return null;
    ...
    if (ledgerState.getKey().equals(selectedBranch.getRoot()) == false)
      throw new IllegalStateException(
          "Build branch is stale when preparing, ledger head has changed from expected "
              + selectedBranch.getRoot()
              + " to "
              + ledgerState.getKey());

    final PendingBranch buildableBranch =
        new PendingBranch(
            this.context,
            ledgerState.getKey(),
            ledgerState.getValue(),
            selectedBranch.getBlocksTo(buildableHeader.getHash()));
    ...
    while (buildClock <= progressRound.clock() - 1) {
      PendingBlock generatedBlock =
          this.blockBuilder.build(buildableHeader, buildableBranch, ledgerState.getKey());
      if (generatedBlock == null)
        throw new IllegalStateException(
            "Failed to build all required blocks in branch " + buildableBranch);

      buildableBranch.add(generatedBlock);
      buildableBranch.update();
      if (buildableBranch.isBuildable(generatedBlock.getHeader()) == false)
        throw new IllegalStateException(
            "Generated block is not applied after insert "
                + generatedBlock
                + " into branch "
                + buildableBranch);
  ...
  private LinkedList<PendingBlock> commit(final PendingBlock block, final PendingBranch branch) {
    ...
    if (proposalsToCommit.isEmpty()) return proposalsToCommit;
    ...
    for (PendingBlock proposalToCommit : proposalsToCommit) {
      if (this.guardLock.tryLock()) {
        ...
        blocksLog.warn(
            this.context.getName()
                + ": Failed to acquire commit guard lock on "
                + proposalToCommit);
        break;
      }
    }

    if (proposalsCommitted.isEmpty()) return proposalsCommitted;

    branch.committed(proposalsCommitted.getLast());

    // Discover blocks that can't be processed anymore due to this commit
    Set<PendingBlock> pendingBlocksToRemove =
        Sets.mutable.withInitialCapacity(this.pendingBlocks.size());
    Set<PendingBlock> invalidPendingBlocks =
        Sets.mutable.withInitialCapacity(this.pendingBlocks.size());
    synchronized (this.pendingBlocks) {
      for (PendingBlock pendingBlock : this.pendingBlocks.values()) {
        if (pendingBlock.getHeight() <= proposalsCommitted.getLast().getHeight()) continue;
          ...
          if (blocksLog.hasLevel(Logging.DEBUG))
            blocksLog.debug(
                context.getName()
                    + ": Pending block "
                    + pendingBlock
                    + " will be discarded as no longer validatable");
        }
      }
    ...
    synchronized (this.pendingBranches) {
      ...
      while (pendingBranchIterator.hasNext()) {
        ...
        if (pendingBranch.isEmpty() == false
            && pendingBranch.getLow().getHeight() <= proposalsCommitted.getLast().getHeight())
      ...
      while (pendingBranchIterator.hasNext()) {
        ...
        for (PendingBlock invalidPendingBlock : invalidPendingBlocks) {
          if (pendingBranch.contains(invalidPendingBlock) == false) continue;

          Collection<PendingBlock> trimmed =
              pendingBranch.trimFrom(proposalsCommitted.getLast().getHeader());
          pendingBlocksToRemove.addAll(trimmed);

          if (pendingBranch.isEmpty()) break;
        }
      }
      ...
      while (pendingBranchIterator.hasNext()) {
        ...
        if (pendingBranch.equals(branch)) continue;

        if (pendingBranch.isEmpty()
            || pendingBranch.getRoot().equals(proposalsCommitted.getLast().getHeader()) == false)
    ...
    if (blocksLog.hasLevel(Logging.DEBUG))
      blocksLog.debug(
          this.context.getName()
              + ": Removing "
              + pendingBlocksToRemove.size()
              + " pending blocks containing "
              + invalidPendingBlocks.size()
              + " invalid blocks post commit of head "
              + proposalsCommitted.getLast().toString());
    ...
    for (final PendingBlock pendingBlock : pendingBlocksToRemove) {
      ...
      if (pendingBlock.getBlock() != null) {
        ...
        this.context
            .getMetaData()
            .increment(
                "ledger.block.latency", pendingBlock.getBlock().getAge(TimeUnit.MILLISECONDS));
      }
    }
    ...
    for (final PendingBlock proposalCommitted : proposalsCommitted) {
      if (this.progressRounds.remove(proposalCommitted.getHeight()) == null)
        blocksLog.warn(
            this.context.getName()
                + ": Progress round "
                + proposalCommitted.getHeight()
                + " not removed when committing proposal "
                + proposalCommitted);

      this.buildClock.updateAndGet(
          v -> v < proposalCommitted.getHeight() ? proposalCommitted.getHeight() : v);
    }
  ...
  public Collection<BlockHeader> getPendingHeaders() {
    synchronized (this.headersToVerify) {
      final List<BlockHeader> pendingHeaders =
          new ArrayList<BlockHeader>(this.headersToVerify.values());
      pendingHeaders.sort(
          (bh1, bh2) -> {
            if (bh1.getHeight() < bh2.getHeight()) return -1;
            else if (bh1.getHeight() > bh2.getHeight()) return 1;
  ...
  public Collection<PendingBranch> getPendingBranches() {
    synchronized (this.pendingBranches) {
      final List<PendingBranch> pendingBranches =
          new ArrayList<PendingBranch>(this.pendingBranches);
      pendingBranches.sort(
          (pb1, pb2) -> pb1.getHigh().getHash().compareTo(pb2.getHigh().getHash()));
  ...
  private PendingBranch getPendingBranch(PendingBlock pendingBlock) {
    synchronized (this.pendingBranches) {
      for (PendingBranch pendingBranch : this.pendingBranches) {
        ...
        if (pendingBranch.contains(pendingBlock.getHeader().getPrevious())) return pendingBranch;
      }
  ...
  public Collection<PendingBlock> getPendingBlocks() {
    synchronized (this.pendingBlocks) {
      final List<PendingBlock> pendingBlocks =
          new ArrayList<PendingBlock>(this.pendingBlocks.values());
      pendingBlocks.sort(
          (pb1, pb2) -> {
            if (pb1.getHeight() < pb2.getHeight()) return -1;
            else if (pb1.getHeight() > pb2.getHeight()) return 1;
  ...
  BlockInsertStatus insert(final BlockHeader header)
      throws IOException, ValidationException, CryptoException {
   ...
   *
   * <p>Should be private but is visible for testing
   *
   * @param pendingBlock
   * @throws IOException
   * @throws CryptoException
   ...
   */
  @VisibleForTesting
  BlockInsertStatus insert(final PendingBlock pendingBlock)
      throws CryptoException, ValidationException, IOException {
    Objects.requireNonNull(pendingBlock, "Pending block is null");
    if (pendingBlock.getHeader() == null)
      throw new IllegalStateException(
          "Pending block " + pendingBlock.getHash() + " does not have a header");

    if (pendingBlock.getBlock() == null)
      throw new IllegalStateException(
          "Pending block " + pendingBlock.getHash() + " does not have a constructed block");
    ...
    if (pendingBlock.getHeader().getProposer().equals(this.context.getNode().getIdentity())
      ...
      throw new IllegalStateException(
          "Pending block "
              + pendingBlock.getHash()
              + " is not generated by local validator "
              + this.context.getNode().getIdentity());
    ...
    if (pendingBlock.getHeight() <= this.context.getLedger().getHead().getHeight()) {
      ...
      blocksLog.warn(
          this.context.getName()
              + ":     Head is now "
              + this.context.getLedger().getHead().toString());
    ...
    if (1 == 0) {
      if (pendingBlock.getHeader().verify(pendingBlock.getHeader().getProposer().getKey()) == false)
        throw new ValidationException(
            pendingBlock.getHeader(), "Signature is invalid for block header");
    ...
    if (this.progressClock.get() == pendingBlock.getHeight()) {
      ...
      final long roundVotePower =
          this.context
              ...
              .getValidatorHandler()
              .getVotePower(progressRound.epoch(), pendingBlock.getHeader().getProposer());
      if (progressRound.propose(
              pendingBlock.getHash(), pendingBlock.getHeader().getProposer(), roundVotePower)
          == false)
        //				blocksLog.warn(this.context.getName()+": Progress round "+pendingBlock.getHeight()+"
        // already has a proposal from "+pendingBlock.getHeader().getProposer());
        throw new ValidationException(
            "Progress round "
                + pendingBlock.getHeight()
                + " already has a proposal from "
    ...
    if (this.pendingBlocks.putIfAbsent(pendingBlock.getHash(), pendingBlock) != null)
      throw new IllegalStateException(
          "Generated block " + pendingBlock.getHash() + " is already inserted");

    if (blocksLog.hasLevel(Logging.DEBUG))
      blocksLog.debug(
          this.context.getName()
              + ": Inserted pending block "
              + pendingBlock.getHeader().toString());
   ...
   *
   * <p>TODO improve and test more extensively. Pending branch validation will catch anything
   * missing here for now but much cheaper to catch as many validation violations here as possible.
   *
   * @throws ValidationException
   * @throws IOException
   */
  private void validate(final BlockHeader header) throws ValidationException, IOException {
    ...
    final Map<Hash, StateAddress> stateAddresses =
        Maps.mutable.ofInitialCapacity(header.getTotalInventorySize());
    header
        .getInventory(InventoryType.ACCEPTED)
        .forEach(h -> stateAddresses.put(h, StateAddress.from(Atom.class, h)));
    header
        .getInventory(InventoryType.UNACCEPTED)
        .forEach(h -> stateAddresses.put(h, StateAddress.from(Atom.class, h)));
    header
        .getInventory(InventoryType.UNEXECUTED)
        .forEach(h -> stateAddresses.put(h, StateAddress.from(ExecutionTimeout.class, h)));
    header
        .getInventory(InventoryType.UNCOMMITTED)
        .forEach(h -> stateAddresses.put(h, StateAddress.from(CommitTimeout.class, h)));
    header
        .getInventory(InventoryType.COMMITTED)
        .forEach(h -> stateAddresses.put(h, StateAddress.from(AtomCertificate.class, h)));
    header
        .getInventory(InventoryType.PACKAGES)
        .forEach(h -> stateAddresses.put(h, StateAddress.from(PolyglotPackage.class, h)));

    // TODO large optimization possibility here as many of the primitives which constitute this
    // block can be validated
    //		without having to touch the store.  For example if an atom in the inventory is valid, then
    // it should be pending
    //		in the atom handler if the local instance knows about it.
    final Map<StateAddress, SubstateCommit> primitiveSubstateCommits =
        this.context.getLedger().getLedgerStore().search(stateAddresses.values());

    // Check certificates
    for (Hash certificate : header.getInventory(InventoryType.COMMITTED)) {
      if (duplicates.contains(certificate))
        throw new ValidationException(
            certificate, "Atom certificate " + certificate + " is duplicated in block " + header);
      ...
      if (substateCommit != null) {
        if (substateCommit.getSubstate().get(NativeField.CERTIFICATE) != null)
          throw new ValidationException(
              certificate, "Atom certificate exists for " + certificate + " and is committed");

        // TODO how to check this with certificates?
        // if (commit.getPath().get(Element.TIMEOUT) != null)
        //     throw new ValidationException(atom, "Atom "+atom+" is already timed out by
        // "+commit.getPath().get(Element.TIMEOUT));
    ...
    for (Hash timeout : header.getInventory(InventoryType.UNEXECUTED)) {
      if (duplicates.contains(timeout))
        throw new ValidationException(
            timeout, "Atom execution timeout " + timeout + " is duplicated in block " + header);

      if (exclusions.contains(timeout))
        throw new ValidationException(
            timeout,
            "Atom execution timeout " + timeout + " can not be included in block " + header);

      SubstateCommit primitiveSubstateCommit =
          primitiveSubstateCommits.get(stateAddresses.get(timeout));
      if (primitiveSubstateCommit != null) {
        ...
        if (primitiveSubstateCommit.getSubstate().get(NativeField.TIMEOUT) != null)
          throw new ValidationException(
              ...
              "Atom "
                  + atom
                  + " is already timed out by "
                  + primitiveSubstateCommit.getSubstate().get(NativeField.TIMEOUT));
    ...
    for (Hash timeout : header.getInventory(InventoryType.UNCOMMITTED)) {
      if (duplicates.contains(timeout))
        throw new ValidationException(
            timeout, "Atom commit timeout " + timeout + " is duplicated in block " + header);

      if (exclusions.contains(timeout))
        throw new ValidationException(
            timeout, "Atom commit timeout " + timeout + " can not be included in block " + header);
      ...
      if (primitiveSubstateCommit != null) {
        ...
        if (exclusions.contains(atom))
          throw new ValidationException(
              atom, "Atom " + atom + " can not be commit timedout in block " + header);
    ...
    for (Hash accepted : header.getInventory(InventoryType.ACCEPTED)) {
      if (duplicates.contains(accepted))
        throw new ValidationException(
            accepted, "Atom " + accepted + " is accept duplicated in block " + header);

      if (exclusions.contains(accepted))
        throw new ValidationException(
            accepted, "Atom " + accepted + " can not be accepted in block " + header);

      SubstateCommit primitiveSubstateCommit =
          primitiveSubstateCommits.get(stateAddresses.get(accepted));
      if (primitiveSubstateCommit != null) {
        if (primitiveSubstateCommit.getSubstate().get(NativeField.CERTIFICATE) != null)
          throw new ValidationException(
              ...
              "Atom "
                  + accepted
                  + " has a certificate "
                  + primitiveSubstateCommit.getSubstate().get(NativeField.CERTIFICATE)
                  + " and is accepted in proposal "
                  + Block.toHeight(
                      primitiveSubstateCommit.getSubstate().<Hash>get(NativeField.BLOCK))
                  + ":"
                  + primitiveSubstateCommit.getSubstate().get(NativeField.BLOCK));
        ...
        if (primitiveSubstateCommit.getSubstate().get(NativeField.BLOCK) != null)
          throw new ValidationException(
              ...
              "Atom "
                  + accepted
                  + " is already accepted in proposal "
                  + Block.toHeight(
                      primitiveSubstateCommit.getSubstate().<Hash>get(NativeField.BLOCK))
                  + ":"
                  + primitiveSubstateCommit.getSubstate().get(NativeField.BLOCK));
    ...
    for (Hash unaccepted : header.getInventory(InventoryType.UNACCEPTED)) {
      if (duplicates.contains(unaccepted))
        throw new ValidationException(
            unaccepted, "Atom " + unaccepted + " is duplicated unaccepted in block " + header);

      if (exclusions.contains(unaccepted))
        throw new ValidationException(
            unaccepted, "Atom " + unaccepted + " can not be unaccepted in block " + header);

      SubstateCommit primitiveSubstateCommit =
          primitiveSubstateCommits.get(stateAddresses.get(unaccepted));
      if (primitiveSubstateCommit != null) {
        if (primitiveSubstateCommit.getSubstate().get(NativeField.CERTIFICATE) != null)
          throw new ValidationException(
              unaccepted, "Atom certificate exists for " + unaccepted + " and is committed");
    ...
    for (Hash executable : header.getInventory(InventoryType.EXECUTABLE)) {
      if (duplicates.contains(executable))
        throw new ValidationException(
            header, "Atom " + executable + " is executable duplicated in block " + header);

      if (exclusions.contains(executable))
        throw new ValidationException(
            header, "Atom " + executable + " can not be executable in block " + header);

      SubstateCommit primitiveSubstateCommit =
          primitiveSubstateCommits.get(stateAddresses.get(executable));
      if (primitiveSubstateCommit != null) {
        if (primitiveSubstateCommit.getSubstate().get(NativeField.CERTIFICATE) != null)
          throw new ValidationException(
              header, "Atom certificate exists for " + executable + " and is committed");
    ...
    for (Hash latent : header.getInventory(InventoryType.LATENT)) {
      if (duplicates.contains(latent))
        throw new ValidationException(
            header, "Atom " + latent + " is latent duplicated in block " + header);

      if (exclusions.contains(latent))
        throw new ValidationException(
            header, "Atom " + latent + " can not be latent in block " + header);

      SubstateCommit primitiveSubstateCommit =
          primitiveSubstateCommits.get(stateAddresses.get(latent));
      if (primitiveSubstateCommit != null) {
        if (primitiveSubstateCommit.getSubstate().get(NativeField.CERTIFICATE) != null)
          throw new ValidationException(
              header, "Atom certificate exists for " + latent + " and is committed");
    ...
    for (Hash pakage : header.getInventory(InventoryType.PACKAGES)) {
      if (duplicates.contains(pakage))
        throw new ValidationException(
            header, "Package " + pakage + " is duplicated in block " + header);

      if (exclusions.contains(pakage))
        throw new ValidationException(
            header, "Package " + pakage + " can not be in block " + header);

      SubstateCommit primitiveSubstateCommit =
          primitiveSubstateCommits.get(stateAddresses.get(pakage));
      if (primitiveSubstateCommit != null) {
        if (primitiveSubstateCommit.getSubstate().get(NativeField.BLOCK) != null)
          throw new ValidationException(
              pakage,
  ...
  BlockCertificateStatus apply(final BlockCertificate blockCertificate) {
    ...
    if (pendingBlock == null) return BlockCertificateStatus.POSTPONED;
    ...
    if (pendingBlock.getHeader().getCertificate() != null) return BlockCertificateStatus.SKIPPED;
  ...
  private void updateBranchesWith(final PendingBlock pendingBlock)
      throws LockException, IOException, ValidationException {
    Objects.requireNonNull(pendingBlock, "Pending block is null");
    if (pendingBlock.getHeader() == null)
      throw new IllegalStateException(
          "Pending block " + pendingBlock.getHash() + " does not have a header");
    ...
    if (pendingBlock.getBlock() == null)
      throw new IllegalStateException(
          "Pending block " + pendingBlock.getHash() + " is not constructed");

    if (pendingBlock.isUnbranched() == false)
      throw new IllegalStateException(
          "Pending block " + pendingBlock.getHash() + " is already in a branch");
    ...
    while (currentPendingBlock != null) {
      ...
      if (currentPendingBlock.thrown() != null) return;

      branch.add(currentPendingBlock);
      if (currentPendingBlock.getHeader().getPrevious().equals(ledger.getHash())) break;

      PendingBlock previous = this.pendingBlocks.get(currentPendingBlock.getHeader().getPrevious());
      if (previous != null && previous.getHeader() != null) currentPendingBlock = previous;
      else currentPendingBlock = null;
    }
    ...
    if (branch
            ...
            .getHeader()
            .getPrevious()
            .equals(this.context.getLedger().getHead().getHash())
        == false) {
      if (blocksLog.hasLevel(Logging.DEBUG))
        blocksLog.error(
            BlockHandler.this.context.getName()
                + ": Branch for pending block "
                + pendingBlock.getHeader()
                + " does not terminate at ledger head "
                + this.context.getLedger().getHead().getHash()
                + " but at "
                + branch.getFirst().getHeader());
    ...
    synchronized (this.pendingBranches) {
      for (PendingBranch pendingBranch : this.pendingBranches) {
        if (pendingBranch.isMergable(branch)) pendingBranch.merge(branch);
      }
    }
  ...
  private PendingBranch findHighestWorkBranch(
      final List<PendingBranch> branches, final long height, boolean strict) {
    PendingBranch selectedBranch = null;
    BlockHeader highestWork = null;

    for (final PendingBranch branch : branches) {
      if (height <= branch.getRoot().getHeight()) continue;

      for (final PendingBlock block : branch.getBlocks()) {
        if (branch.isBuildable(block.getHeader()) == false) break;

        if (strict && block.getHeight() != height) continue;

        final BlockHeader header = block.getHeader();
        if (highestWork == null
            || header.getTotalWork().compareTo(highestWork.getTotalWork()) > 0) {
          highestWork = header;
  ...
  private PendingBranch selectSuperBranch(final long height, boolean strict) {
    ...
    for (final PendingBranch branch : candidateBranches) {
      if (branch.supers().isEmpty() == false) branchesWithSupers.add(branch);
    }

    // If we have branches with supers, ONLY consider those
    List<PendingBranch> filteredCandidates;
    if (branchesWithSupers.isEmpty() == false) filteredCandidates = branchesWithSupers;
    else filteredCandidates = candidateBranches;
    ...
    if (bestBranch != null) {
      ...
      for (PendingBranch candidateBranch : filteredCandidates) {
        if (candidateBranch.intersects(bestBranch)) consensusBranches.add(candidateBranch);
      }
  ...
  private PendingBranch selectBranchToExtend(final ProgressRound round) {
    return selectSuperBranch(
        round.getState().equals(ProgressRound.State.VOTING) ? round.clock() : round.clock() - 1,
        false);
  }
  ...
  private PendingBranch selectBranchWithQuorum(final ProgressRound round) {
    ...
    if (this.progressClock.get() > round.clock()) targetHeight = round.clock();
    else targetHeight = round.clock() - 1;
      ...
      while (pendingBranchIterator.hasNext()) {
        ...
        try {
          ...
          if (pendingBranch.getLow().getHeader().getPrevious().equals(head.getHash()) == false) {
            if (blocksLog.hasLevel(Logging.DEBUG))
              blocksLog.debug(
                  this.context.getName()
                      + ": Branch doesn't attach to ledger "
                      + pendingBranch.getLow());

            continue;
          }

          LinkedList<PendingBlock> supers = pendingBranch.supers();
          if (supers.size() < Constants.MIN_COMMIT_SUPERS) continue;
          ...
          while (superBlockIterator.hasNext()) {
            ...
            if (superBlock.getHeight() <= targetHeight
                && supers.getLast().equals(superBlock) == false) break;

            superBlock = null;
          }

          if (superBlock == null) continue;
          ...
          if (candidateBlock != null && candidateBlock.equals(superBlock) == false) {
            ...
            if (superBlock.getHeight() > candidateBlock.getHeight()) continue;
          }
          ...
          blocksLog.error(
              this.context.getName()
                  + ": Branch selection with quorum "
                  + pendingBranch.toString()
                  + " failed",
              ex);
        }
      }
  ...
  long getTimestampEstimate() {
    ...
    synchronized (this.pendingBranches) {
      if (this.pendingBranches.isEmpty()) return head.getTimestamp();
      ...
      for (final PendingBranch pendingBranch : this.pendingBranches) {
        synchronized (pendingBranch) {
          if (pendingBranch.size() == 0) continue;

          if (pendingBranch.getHigh().getHeight() <= minHeight) continue;

          minHeight =
              Math.min(
                  head.getHeight(),
                  pendingBranch.getHigh().getHeight() - Constants.MIN_COMMIT_SUPERS);
          eligibleBranches++;
        }
      }

      if (eligibleBranches == 0) return head.getTimestamp();
      ...
      else return timestamps.get(middle);
    }
  }

  // SYNC PROGRESS LISTENER //
  private EventListener syncProgressListener =
      new SynchronousEventListener() {
        ...
        public void on(final ProgressPhaseEvent event) {
          if (BlockHandler.this.progressPhaseQueue.offer(event) == false)
            ...
            blocksLog.fatal(
                BlockHandler.this.context.getName()
                    + ": Failed to add progress round to queue "
                    + event.getProgressRound());
  ...
  private SynchronousEventListener syncBlockListener =
      new SynchronousEventListener() {
        ...
        public void on(BlockCommittedEvent blockCommittedEvent) {
          Set<Hash> headerRemovals =
              Sets.mutable.withInitialCapacity(BlockHandler.this.headersToVerify.size());
          BlockHandler.this.headersToVerify.forEach(
              (h, bh) -> {
          ...
          synchronized (BlockHandler.this.blockVoteCollectors) {
            final Iterator<BlockVoteCollector> blockVoteCollectorsIterator =
                BlockHandler.this.blockVoteCollectors.values().iterator();
            while (blockVoteCollectorsIterator.hasNext()) {
              final BlockVoteCollector blockVoteCollector = blockVoteCollectorsIterator.next();
              if (blockVoteCollector.getHeight()
                  > blockCommittedEvent.getPendingBlock().getHeader().getHeight()) continue;
              ...
              if (blockVoteCollector.hasMetThreshold() == false)
                blocksLog.warn(
                    BlockHandler.this.context.getName()
                        + ": Removed incomplete block vote collector for progress round "
                        + blockVoteCollector.getHeight());
              else if (blocksLog.hasLevel(Logging.DEBUG))
                blocksLog.debug(
                    BlockHandler.this.context.getName()
                        + ": Removed completed block vote collector for progress round "
                        + blockVoteCollector.getHeight());
            }
          }
  ...
  private EventListener asyncBlockListener =
      new EventListener() {
        @Subscribe
        public void on(final BlockAppliedEvent blockAppliedEvent) {
          if (blocksLog.hasLevel(Logging.DEBUG))
            blocksLog.debug(
                BlockHandler.this.context.getName()
                    + ": Proposal for round "
                    + blockAppliedEvent.getPendingBlock().getHeight()
                    + " has been applied "
                    + blockAppliedEvent.getPendingBlock());
        }
      };
      ...
      new SynchronousEventListener() {
        ...
        public void on(final SyncAcquiredEvent event) {
          ...
          try {
            blocksLog.log(
                BlockHandler.this.context.getName()
                    + ": Sync status acquired, setting block handler state");
            ...
            // NOTE Collecting known block votes is redundant and carries a risk that they can cause
            // safety issues locally on the
            // 		suncing replica.  In edge cases depending on network state, they being counted
            // towards 2f+1 AFTER the progress round
            // 		has been completed by the network at large.  The network would be safe, but the
            // syncing replica could suffer a local
            ...
            blocksLog.info(
                BlockHandler.this.context.getName()
                    + ": Progress round post sync is "
                    + progressRound.toString());

            // TODO meh ... getting head from event, but accumulator from ledger because the
            // accumulator use is referenced based and the
            // 		event carries a sync accumulator which gets reset and causes all manner of locking
            // issues if used for this initial branch
            BlockHandler.this.pendingBranches.add(
                new PendingBranch(
                    BlockHandler.this.context,
                    Type.NONE,
                    event.getHead(),
                    BlockHandler.this.context.getLedger().getStateAccumulator()));
          } finally {
        ...
        public void on(final SyncLostEvent event) {
          ...
          try {
            blocksLog.log(
                BlockHandler.this.context.getName() + ": Sync status lost, flushing block handler");
            BlockHandler.this.pendingBlocks.clear();
            BlockHandler.this.pendingBranches.clear();
            BlockHandler.this.progressPhaseQueue.clear();
            BlockHandler.this.postponedVotes.clear();
