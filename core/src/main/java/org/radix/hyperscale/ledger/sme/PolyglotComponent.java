...
public final class PolyglotComponent extends Component {
  ...
  // TODO sharing the engine between polyglot contexts should be fine, but need to check all the
  // security
  ...
  private static final LinkedListMultimap<Hash, ContextInstance> contexts =
  ...
  public final class PolyglotInterface {
    ...
    public final ShardGroupID shardGroup() {
      ...
      final int numShardGroups =
          this.polyglot.getStateMachine().getContext().getLedger().numShardGroups(epoch);
      return ShardMapper.toShardGroup(
          this.polyglot.getStateMachine().getContext().getNode().getIdentity(), numShardGroups);
    }
    ...
    public final Vault vault(Hash identifier, Identity authority) {
      return this.polyglot.vault(
          StateAddress.from(Vault.class.getAnnotation(StateContext.class).value(), identifier),
          authority);
    }
    ...
    synchronized (contexts) {
      ...
      if (contextInstancesByKey.isEmpty()) {
        ...
        Numbers.greaterThan(
            code.length(),
            MAX_CODE_SIZE,
            "Code size " + code.length() + " is greater than max " + MAX_CODE_SIZE);

        final Value bindings;
        final Engine engine = getEngine(getStateMachine().getContext().getNode().getIdentity());
        final Context context =
            Context.newBuilder(this.language).engine(engine).allowAllAccess(true).build();
        context.enter();
        try {
