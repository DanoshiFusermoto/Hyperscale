...
public final class StateAccumulator implements LedgerProvider
{
	...
	StateAccumulator(final String name, final StateAccumulator accumulator)
	{
		...
		try
		{
			...
			this.lock = new MonitoredReadWriteLock(this.context.getName()+" State Accumulator '"+name+"' Lock", true);
		}
		finally
		{
	...
	void lock(final PendingAtom pendingAtom) throws LockException
	{
		...
		if (stateAccumulatorLog.hasLevel(Logging.DEBUG))
			stateAccumulatorLog.debug(this.context.getName()+": "+this.name+" -> Locking state in "+pendingAtom);
		...
		try
		{
			...
			for (int i = 0 ; i < stateAddresses.size() ; i++)
			{
				...
				if (stateAccumulatorLog.hasLevel(Logging.DEBUG))
					stateAccumulatorLog.debug(this.context.getName()+": "+this.name+" -> Locked state "+stateAddress+" "+" via "+pendingAtom);
			}
		}
		finally
		{
	...
	void unlock(final PendingAtom pendingAtom) throws LockException
	{
		...
		{
			if (stateAccumulatorLog.hasLevel(Logging.DEBUG))
				stateAccumulatorLog.debug(this.context.getName()+": "+this.name+" -> Unlocking state in "+pendingAtom);
			...
			for (int i = 0 ; i < stateAddresses.size() ; i++)
			{
				...
				if (stateAccumulatorLog.hasLevel(Logging.DEBUG))
					stateAccumulatorLog.debug(this.context.getName()+": "+this.name+" Unlocked state "+stateAddress+" via "+pendingAtom);
			}
		}
		finally
		{
	...
	boolean lockable(final StateAddress address, final PendingAtom pendingAtom)
	{
		...
		try
		{
			if (this.locked.containsKey(pendingAtom.getAddress()))
				return false;
			
			if (this.locked.containsKey(address))
				return false;
	...
	void lockable(final List<StateAddress> stateAddresses, final PendingAtom pendingAtom) throws StateLockException
	{
	...
	void unlockable(final List<StateAddress> stateAddresses, final PendingAtom pendingAtom) throws StateUnlockException
	{
		Objects.requireNonNull(stateAddresses, "State addresses is null");
	...
	boolean anyLocked(final List<StateAddress> stateAddresses)
	{
		...
		try
		{
			...
			{
				final StateAddress stateAddress = stateAddresses.get(i);
				if (this.locked.containsKey(stateAddress))
					return true;
			}
	...
	boolean isLocked(final PendingAtom pendingAtom)
	{
		...
		try
		{
			...
			{
				final StateAddress stateAddress = stateAddresses.get(i);
				if (isLockedBy(stateAddress, pendingAtom)) 
					return true;
			}
	...
	boolean isLockedBy(final StateAddress address, final PendingAtom pendingAtom)
	{
		...
		try
		{
			if (pendingAtom.equals(this.locked.get(address))) 
				return true;
	...
	Set<PendingAtom> getPendingAtoms() 
	{
		...
		try
		{
			final MutableSet<PendingAtom> pendingAtoms = Sets.mutable.ofInitialCapacity(this.locked.size());
			this.locked.forEach((sa, pa) -> {
				if (pa.getAddress().equals(sa))
					pendingAtoms.add(pa);
			});
