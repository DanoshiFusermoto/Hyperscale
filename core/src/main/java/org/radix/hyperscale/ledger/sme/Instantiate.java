...
public final class Instantiate extends ArgumentedInstruction {
  ...
  public Instantiate(
      final String blueprintName, final String componentName, final Identity authority) {
  ...
  public Instantiate(
    ...
    this.blueprintAddress =
        StateAddress.from(
            Deploy.class.getAnnotation(StateContext.class).value(),
            Hash.valueOf(this.blueprintName));
    this.componentAddress =
        StateAddress.from(
            Component.class.getAnnotation(StateContext.class).value(),
            Hash.valueOf(this.componentName));
    this.packageAddress =
        StateAddress.from(
  ...
  void lock(final StateMachine stateMachine) {
    ...
    stateMachine.lock(
        StateAddress.from(
            Vault.class.getAnnotation(StateContext.class).value(),
            ComputeKey.from(getComponentAddress().getHash()).getIdentity()),
        StateLockMode.WRITE);
  }

  void execute(final StateMachine stateMachine)
    ...
    stateMachine.set(
        getComponentAddress(),
        NativeField.CODE,
        stateMachine.get(getBlueprintAddress(), NativeField.CODE),
        authority);
    stateMachine.set(
        getComponentAddress(),
    ...
    stateMachine.set(
        ...
        Strings.toLowerCase(this.componentName),
        this.authority);
    stateMachine.set(
        getComponentAddress(),
        NativeField.LANGUAGE,
        stateMachine.get(getBlueprintAddress(), NativeField.LANGUAGE),
        authority);
