...
public class LedgerStore extends DatabaseStore implements LedgerProvider
{
	...
	private static final ThreadLocal<DatabaseEntry> keyEntry = ThreadLocal.withInitial(() -> new DatabaseEntry());
	private static final ThreadLocal<DatabaseEntry> valueEntry = ThreadLocal.withInitial(() -> new DatabaseEntry());
	private static final ThreadLocal<ByteBuffer> 	byteBuffer = ThreadLocal.withInitial(() -> ByteBuffer.allocate(BYTE_BUFFER_SIZE));
	
	private static final Logger databaseLog = Logging.getLogger("database");
	static {databaseLog.setLevel(Logging.INFO);}
	...
	protected class InventoryRecord
	{
		...
		{
			// Liveness check
			if (Block.toHeight(this.head) == Block.toHeight(LedgerStore.this.inventoryHead))
				return false;
			
			// TODO 30 seconds is enough?  Very latent replicas should have transitioned to a "syncing" state by then
			return System.currentTimeMillis() - this.witnessedAt > TimeUnit.MILLISECONDS.convert(Constants.PRIMITIVE_STALE, TimeUnit.SECONDS);
		}
		...
		boolean isExpired()
		{
			...
			// TODO 60 seconds is enough?  Very latent replicas should have transitioned to a "syncing" state by then
			return System.currentTimeMillis() - this.witnessedAt > TimeUnit.MILLISECONDS.convert(Constants.PRIMITIVE_EXPIRE, TimeUnit.SECONDS);
		}
		...
		{
			return this.clazz.getName()+" "+this.hash+" "+this.witnessedAt+" "+this.clockStart+":"+this.clockEnd;
		}
	}
	...
	public LedgerStore(final Context context)
	{
		...
		this.substateInventory = Maps.mutable.<StateAddress, SubstateCommit>ofInitialCapacity(1<<16).asSynchronized();
		
		final ScheduledExecutable inventoryTask = new ScheduledExecutable(Constants.PRIMITIVE_GC_INTERVAL, TimeUnit.SECONDS) 
		{
			@Override
			public void execute() 
			{
				// NOTE shouldn't have any effect if there is a liveness break as proposals will still have been voted upon
				//		may be some considerations when networks converge, but the strongest branch should then start to receive
				//		2f+1 of the votes and become the commit branch.
				//
				// TODO currently nothing should be executed in a liveness break, if this was to change, then issues may arise here as the 
				//		primitive cache will grow excessively but will not be pruned as there wont be any commits
				
				int evictedPrimitiveInventory = 0;
				final Iterator<InventoryRecord> inventoryIterator = LedgerStore.this.primitivesInventory.values().iterator();
				while(inventoryIterator.hasNext())
				{
					...
					if (inventory.isExpired())
					{
						...
						if (databaseLog.hasLevel(Logging.DEBUG))
							databaseLog.debug(LedgerStore.this.context.getName()+": "+inventory+" is expired");
					}
				}

				databaseLog.info(LedgerStore.this.context.getName()+": Primitive inventory contains "+LedgerStore.this.primitivesInventory.size()+" items. Garbage collection evacuated "+evictedPrimitiveInventory+" items");
			}
		};
	...
	public void start() throws StartupException
	{
		try
		{
			...
			this.associations = this.context.getVamosEnvironment().open("associations", associationsConfig);
	...
	public void clean() throws ServiceException
	{
		try
		{
			...
			new File(this.getEnvironment().getEnvironment().getHome().toString()+File.separator+"ploom.dat").delete();
		}
		catch (Exception ex)
		{
	...
	public OperationStatus delete(final Hash hash, final Class<? extends EphemeralPrimitive> primitive) throws IOException
	{
		...
		try
        {
			...
			final DatabaseEntry key = keyEntry.get(); key.setData(stateAddress.getHash().toByteArray());
	...
	final OperationStatus delete(final Collection<Hash> hashes, final Class<? extends EphemeralPrimitive> primitive) throws IOException
	{
		...
		final Transaction transaction = new Transaction(this.context.getVamosEnvironment(), hashes.size());
		try 
		{
			for (final Hash hash : hashes)
			{
			    ...
			    if (status.equals(OperationStatus.SUCCESS) == false) 
		    		throw new DatabaseException("Failed to delete "+primitive+" "+hash+" due to "+status.name());
			}
	...
	public <T extends Primitive> Collection<T> get(final Collection<Hash> hashes, final Class<T> primitive) throws IOException
	{
		return get(hashes, primitive, null);
	}
	
	public <T extends Primitive> Collection<T> get(final Collection<Hash> hashes, final Class<T> primitive, final BiConsumer<Hash, T> consumer) throws IOException
	{
		if (hashes.isEmpty())
		...
		if (CompoundPrimitive.class.isAssignableFrom(primitive) == false)
		{
			for (final Hash hash : hashes)
			{
				...
				if (inventoryRecord != null)
					results.putIfAbsent(hash, inventoryRecord.get());
				else
					remaining.add(hash);
			}
		}
		...
		if (EphemeralPrimitive.class.isAssignableFrom(primitive) == false)
		{
			for (final Hash hash : remaining)
			{
				try
				{
					...
					if (result != null)
						results.putIfAbsent(hash, result);
				}
				catch (IOException ioex)
				{
	...
	@SuppressWarnings("unchecked")
	private <T extends Primitive> T getPrimitive(final Transaction transaction, final Hash hash, final Class<T> type) throws IOException
	{
		try
        {
			if (CompoundPrimitive.class.isAssignableFrom(type) == false)
			{
				final InventoryRecord inventoryRecord = this.primitivesInventory.get(hash);
				if (inventoryRecord != null)
					return (T) inventoryRecord.get();
			}
			
			if (EphemeralPrimitive.class.isAssignableFrom(type))
				return null;
			...
			else if (AtomCertificate.class.isAssignableFrom(type) || AtomTimeout.class.isAssignableFrom(type))
			{
				final DatabaseEntry key = keyEntry.get(); key.setData(hash.toByteArray());
				...
				final SubstateCommit atomSubstateCommit = Serialization.getInstance().fromDson(value.getData(), SubstateCommit.class);
			...
			else if (type.equals(Block.class))
			{
				...
				final List<Atom> accepted = new ArrayList<Atom>(header.getInventorySize(InventoryType.ACCEPTED));
				...
				final List<Atom> unaccepted = new ArrayList<Atom>(header.getInventorySize(InventoryType.UNACCEPTED));
				...
				final List<AtomCertificate> certificates = new ArrayList<AtomCertificate>(header.getInventorySize(InventoryType.COMMITTED));
				for (final Hash certificateHash : header.getInventory(InventoryType.COMMITTED))
				{
					final AtomCertificate certificate = getPrimitive(transaction, certificateHash, AtomCertificate.class);
					if (certificate == null)
				...
				final List<ExecutionTimeout> unexecuted = new ArrayList<ExecutionTimeout>(header.getInventorySize(InventoryType.UNEXECUTED));
				for (final Hash timeoutHash : header.getInventory(InventoryType.UNEXECUTED))
				{
					final ExecutionTimeout timeout = getPrimitive(transaction, timeoutHash, ExecutionTimeout.class);
					if (timeout == null)
						throw new IllegalStateException("Found block header "+hash+" but contained execution timeout "+timeoutHash+" is missing");
					
					unexecuted.add(timeout);
				}

				final List<CommitTimeout> uncommitted = new ArrayList<CommitTimeout>(header.getInventorySize(InventoryType.UNCOMMITTED));
				for (final Hash timeoutHash : header.getInventory(InventoryType.UNCOMMITTED))
				{
					...
					if (timeout == null)
						throw new IllegalStateException("Found block header "+hash+" but contained commit timeout "+timeoutHash+" is missing");
					
					uncommitted.add(timeout);
				}

				final List<PolyglotPackage> packages = new ArrayList<PolyglotPackage>(header.getInventorySize(InventoryType.PACKAGES));
				for (final Hash packageHash : header.getInventory(InventoryType.PACKAGES))
				{
					final PolyglotPackage pakage = getPrimitive(transaction, packageHash, PolyglotPackage.class);
					if (pakage == null)
						throw new IllegalStateException("Found block header "+hash+" but contained package "+packageHash+" is missing");
				...
				return (T) new Block(header, accepted, unaccepted, unexecuted, certificates, uncommitted, packages);
			}
			else 
				throw new IllegalArgumentException("Primitive type "+type+" not supported in LedgerStore");
	...
	Set<Hash> has(final Collection<Hash> hashes, final Class<? extends Primitive> primitive) throws DatabaseException
	{
		Objects.requireNonNull(hashes, "Hashes is null");
		...
		try
        {
			...
			for (final Hash hash : hashes)
			{
				if (this.primitivesInventory.containsKey(hash))
					known.add(hash);
				else
					remaining.add(hash);
			}
	...
	boolean has(final Hash hash, final Class<? extends Primitive> primitive) throws DatabaseException
	{
	...
	private OperationStatus store(final Transaction transaction, final Primitive primitive) throws IOException
	{
	...
	private OperationStatus store(final Transaction transaction, Hash referrer, Hash reference) throws IOException
	{
		Objects.requireNonNull(referrer, "Referring hash is null");
 		Objects.requireNonNull(reference, "Reference hash is null");

 		final DatabaseEntry key = keyEntry.get(); key.setData(referrer.toByteArray());
 		final DatabaseEntry value = valueEntry.get(); value.setData(reference.toByteArray());
 		final OperationStatus status = this.substates.putNoOverwrite(transaction, key, value, LockMode.DEFAULT);
		if (status.equals(OperationStatus.SUCCESS) == false)
			throw new DatabaseException("Failed to store reference to "+reference+" via referrer "+referrer+" due to "+status.name());
		...
		if (CompoundPrimitive.class.isAssignableFrom(primitive.getClass()) == false)
		{
			...
			if (Serialization.DEBUG_STATISTICS)
			{
				if (primitive instanceof StateVoteBlock svb)
					...
					Serialization.getInstance().toDson(primitive, Output.WIRE);
			}
			
			if (this.primitivesInventory.putIfAbsent(primitive.getHash(), new InventoryRecord(primitive, this.inventoryHead)) != null)
			{
	    		databaseLog.warn(this.context.getName()+": "+primitive.getClass()+" "+primitive.getHash()+" is already present in primitives");
				return OperationStatus.KEYEXIST;
			}
		}
		
		// TODO Special case for some primitives (primarily Atoms and BlockHeaders) as we want them to be known by the LedgerStore for 
		//		use with gossip and syncing other nodes etc, but we don't want to actually write them to storage at this point.  
		//
		//		These primitives are persisted later under some condition being met such as a proposal containing a "completion" 
		//		object (a certificate or timeout) being committed.
		//
		//		Storing them at this stage has a significant performance penalty later on, so we want to defer the write.
		//
		//		This is a bit hacky though, perhaps it and can be improved.
		if (primitive.isDeferredPersist())
			return OperationStatus.SUCCESS;
		...
		if (EphemeralPrimitive.class.isAssignableFrom(primitive.getClass()) == false)
		{
			...
			try 
			{
			    ...
			    if (status.equals(OperationStatus.SUCCESS) == false) 
			    {
			    	...
			    	{
			    		databaseLog.warn(this.context.getName()+": "+primitive.getClass()+" "+primitive.getHash()+" is already present");
			    		transaction.abort();
			    		return status;
			    	}
			    	else 
			    		throw new DatabaseException("Failed to store "+primitive.getClass()+" "+primitive.getHash()+" due to "+status.name());
			    } 
			    
			    transaction.commit();

			    if (databaseLog.hasLevel(Logging.DEBUG))
		    		databaseLog.debug(this.context.getName()+": Stored primitive "+primitive.getClass()+" "+primitive.getHash());
...
//				this.primitivesLookup.put(primitive.getClass(), primitive.getHash());
		...
		final Transaction transaction = new Transaction(this.context.getVamosEnvironment(), primitives.size());
		try 
		{
	...
	final Map<StateAddress, SubstateCommit> search(final Collection<StateAddress> stateAddresses) throws IOException
	{
		Objects.requireNonNull(stateAddresses, "State addresses is null");
		if (stateAddresses.isEmpty())
			return Collections.emptyMap();

	    final Transaction transaction = new Transaction(this.context.getVamosEnvironment(), stateAddresses.size()*2);
		try
        {
			final MutableMap<StateAddress, SubstateCommit> substateCommits = Maps.mutable.ofInitialCapacity(stateAddresses.size());
			for (final StateAddress stateAddress : stateAddresses)
			{
				final SubstateCommit substateCommit = getSubstateCommit(transaction, stateAddress);
				if (substateCommit != null)
					substateCommits.put(stateAddress, substateCommit);
			}
	...
	private OperationStatus has(final Transaction transaction, final StateAddress stateAddress) throws IOException
	{
		Objects.requireNonNull(stateAddress, "State address is null");

		try
        {
			if (this.substateInventory.containsKey(stateAddress))
				return OperationStatus.SUCCESS;

			final DatabaseEntry stateKey = keyEntry.get(); stateKey.setData(stateAddress.getHash().toByteArray());
			final OperationStatus status = this.substates.get(transaction, stateKey, null, LockMode.DEFAULT);
			return status;
        }
		catch (Exception ex)
		{
	...
	public Substate get(final StateAddress stateAddress) throws IOException
	{
		...
		try
        {
			...
			if (substateCommit == null)
				return new Substate(stateAddress);
	...
	final Map<StateAddress, Substate> get(final Collection<StateAddress> stateAddresses) throws IOException
	{
		Objects.requireNonNull(stateAddresses, "State addresses is null");
		if (stateAddresses.isEmpty())
			return Collections.emptyMap();
		...
		try
        {
			final Map<StateAddress, Substate> substates = new HashMap<StateAddress, Substate>(stateAddresses.size(), 1.0f);
			for (final StateAddress stateAddress : stateAddresses)
			{
				final Substate substate;
				final SubstateCommit substateCommit = getSubstateCommit(null, stateAddress);
				if (substateCommit == null)
					substate = new Substate(stateAddress);
				else 
					substate = substateCommit.getSubstate();
	...
	final OperationStatus commit(final Block block, final Collection<CommitOperation> commitOperations) throws IOException
	{
	    Objects.requireNonNull(block, "Block to commit is null");
	    
	    OperationStatus   status;
	    final Transaction transaction = new Transaction(this.context.getVamosEnvironment(), commitOperations.size()*2);
	    try 
	    {	
	    	final StateAddress blockHeaderStateAddress = StateAddress.from(BlockHeader.class, block.getHeader().getHash());
		    ...
		    final StateAddress blockStateAddress = StateAddress.from(Block.class, block.getHeader().getHash());
		    status = store(transaction, blockStateAddress.getHash(), blockHeaderStateAddress.getHash());
			if (status.equals(OperationStatus.SUCCESS) == false)
				throw new IllegalStateException("Failed to store reference to block header "+block.getHash()+" substate for block");
			...
			final Collection<AtomTimeout> timeouts = new ArrayList<AtomTimeout>(unaccepted.stream().map(a -> new AcceptTimeout(a.getHash())).collect(Collectors.toList()));
		    ...
		    for (final Atom atom : accepted)
		    {
		    	...
		    	final Substate 		atomSubstate = new Substate(atomStateAddress, ComputeKey.NULL.getIdentity()).set("block", block.getHeader().getHash()).set("atom", atom.getHash()).set("primitive", atom);
		    	final SubstateCommit  atomSubstateCommit = new SubstateCommit(atomSubstate, block.getHeader().getIndexOf(atom.getHash()), block.getHeader().getTimestamp());

	    		pushSubstateCommit(atomSubstateCommit);
				status = store(transaction, atomSubstateCommit, false);
				if (status.equals(OperationStatus.SUCCESS) == false)
					throw new IllegalStateException("Failed to store accepted atom substate "+atom.getHash()+" in proposal "+block.getHash());
		    }
		    
		    for (final Atom atom : unaccepted)
		    {
		    	final StateAddress	atomStateAddress = StateAddress.from(Atom.class, atom.getHash());
		    	final Substate 		atomSubstate = new Substate(atomStateAddress, ComputeKey.NULL.getIdentity()).set("block", block.getHeader().getHash()).set("atom", atom.getHash()).set("unaccepted", 1).set("primitive", atom);
		    	final SubstateCommit  atomSubstateCommit = new SubstateCommit(atomSubstate, block.getHeader().getIndexOf(atom.getHash()), block.getHeader().getTimestamp());

	    		pushSubstateCommit(atomSubstateCommit);
				status = store(transaction, atomSubstateCommit, false);
				if (status.equals(OperationStatus.SUCCESS) == false)
					throw new IllegalStateException("Failed to store unaccepted atom substate "+atom.getHash()+" in proposal "+block.getHash());
		    }
    		...
    		{	
				for (final SubstateTransitions substateTransitions : commitOperation.getSubstateTransitions())
				{
			    	SubstateCommit substateCommit = null;
			    	SubstateCommit committedSubstate = getSubstateCommit(transaction, substateTransitions.getAddress());
					if (committedSubstate == null)
					{
						substateCommit = new SubstateCommit(substateTransitions.toSubstate(SubstateOutput.TRANSITIONS), commitOperation.getAtom().getHash(), commitOperation.getHead().getIndexOf(commitOperation.getAtom().getHash()), commitOperation.getTimestamp());
					}
					else
					{
						committedSubstate.getSubstate().merge(substateTransitions);
						substateCommit = new SubstateCommit(committedSubstate.getSubstate(), commitOperation.getAtom().getHash(), commitOperation.getHead().getIndexOf(commitOperation.getAtom().getHash()), commitOperation.getTimestamp(), committedSubstate);
					}
					
					status = store(transaction, substateCommit, true);
					if (status.equals(OperationStatus.SUCCESS) == false)
						throw new IllegalStateException("Failed to store substate "+substateTransitions.getAddress()+" in proposal "+block.getHash());
				}
				
				for (Entry<StateAddress, Hash> association : commitOperation.getAssociations())
				{
					AssociationCommit associationCommit = new AssociationCommit(association.getValue(), association.getKey(), commitOperation.getHead().getIndexOf(commitOperation.getAtom().getHash()), commitOperation.getTimestamp());
					status = store(transaction, associationCommit);
					if (status.equals(OperationStatus.SUCCESS) == false)
						throw new IllegalStateException("Failed to store association "+association.getValue()+":"+association.getKey()+" in proposal "+block.getHash());
				}
				
				if (commitOperation.getSubstateOpLog() != null)
				{
					StateAddress atomStateAddress = StateAddress.from(Atom.class, commitOperation.getAtom().getHash());
			    	SubstateCommit atomSubstateCommit = peekSubstateCommit(transaction, atomStateAddress);
			    	atomSubstateCommit.getSubstate().set("substate.oplog", commitOperation.getSubstateOpLog());
				}
    		}
			...
			for (final AtomTimeout timeout : timeouts)
		    {
	    		...
	    		if (status.equals(OperationStatus.SUCCESS) == false)
					throw new IllegalStateException("Failed to store timedout atom substate "+timeout.getAtom()+" in proposal "+block.getHash());

	    		status = store(transaction, timeout.getHash(), atomStateAddress.getHash());
	    		if (status.equals(OperationStatus.SUCCESS) == false)
	    			throw new IllegalStateException("Failed to store timeout association to atom substate "+timeout.getAtom()+" in proposal "+block.getHash());
		    }
		    ...
		    for (final AtomCertificate certificate : certificates)
		    {
	    		...
	    		status = store(transaction, atomSubstateCommit, true);
	    		if (status.equals(OperationStatus.SUCCESS) == false)
					throw new IllegalStateException("Failed to store committed atom substate "+certificate.getAtom()+" in proposal "+block.getHash());

			    status = store(transaction, certificate.getHash(), atomStateAddress.getHash());
			    if (status.equals(OperationStatus.SUCCESS) == false)
					throw new IllegalStateException("Failed to store certificate association to atom substate "+certificate.getAtom()+" in proposal "+block.getHash());
			    
			    // Update the sync inventory record for the relevant Atom and its state information
				final InventoryRecord atomInventoryRecord = this.primitivesInventory.get(certificate.getAtom());
				if (atomInventoryRecord == null)
				{
					if (databaseLog.hasLevel(Logging.DEBUG))
						databaseLog.warn(this.context.getName()+": Inventory record not found for commit atom "+certificate.getAtom());
				}
				else
					atomInventoryRecord.setClockEnd(block.getHeader().getHeight());
				...
				for (final StateOutput stateOutput : certificate.getInventory(StateOutput.class))
				{
					...
					{
						final InventoryRecord stateOutputInventoryRecord = this.primitivesInventory.get(stateCertificate.getHash());
						if (stateOutputInventoryRecord == null)
						{
							if (databaseLog.hasLevel(Logging.DEBUG))
								databaseLog.warn(this.context.getName()+": Inventory record not found for state certificate "+stateCertificate.getHash()+" in commit atom "+certificate.getAtom());
						}
						else
							stateOutputInventoryRecord.setClockEnd(block.getHeader().getHeight());
					}
					
					final Hash stateInputHash = Hash.hash(certificate.getAtom(), stateOutput.getAddress().getHash());
					final InventoryRecord stateInputInventoryRecord = this.primitivesInventory.get(stateInputHash);
					if (stateInputInventoryRecord == null)
					{
						if (databaseLog.hasLevel(Logging.DEBUG))
							databaseLog.warn(this.context.getName()+": Inventory record not found for state input "+stateInputHash+" in commit atom "+certificate.getAtom());
					}
					else
						stateInputInventoryRecord.setClockEnd(block.getHeader().getHeight());
				}
				...
				if (atomRecord == null)
				{
					...
					atomRecord.<Atom>get().flushCachedDsonOutput();

				for (final StateCertificate stateCertificate : certificate.getInventory(StateCertificate.class))
				{
		    ...
		    {
		    	final StateAddress packageStateAddress = StateAddress.from(PolyglotPackage.class, pakage.getHash());
		    	final SubstateCommit packageSubstateCommit = getSubstateCommit(transaction, packageStateAddress);
		    	if (packageSubstateCommit == null)
		    		throw new IllegalStateException("Failed to load substate commit for package "+packageStateAddress+" at "+pakage.getAddress());
		    	
		    	packageSubstateCommit.getSubstate().set(NativeField.BLOCK, block.getHash());
		    	final SubstateCommit updatedPackageSubstateCommit = new SubstateCommit(packageSubstateCommit.getSubstate(), block.getHeader().getHash(), block.getHeader().getIndexOf(pakage.getHash()), block.getHeader().getTimestamp());
		    	
				status = store(transaction, updatedPackageSubstateCommit, true);
				if (status.equals(OperationStatus.SUCCESS) == false)
					throw new IllegalStateException("Failed to store substate for package "+packageStateAddress+" at "+pakage.getAddress()+" in proposal "+block.getHash());

			    status = store(transaction, pakage.getAddress().getHash(), packageStateAddress.getHash());
	    		if (status.equals(OperationStatus.SUCCESS) == false)
	    			throw new IllegalStateException("Failed to store package reference "+pakage.getHash()+" for "+pakage.getAddress()+" in proposal "+block.getHash());
		    }
			
		    final DatabaseEntry syncKey = keyEntry.get(); syncKey.setData(Longs.toByteArray(block.getHeader().getHeight()));
		    final DatabaseEntry syncValue = valueEntry.get(); syncValue.setData(block.getHeader().getHash().toByteArray());
	    ...
	    finally
	    {
	    	if (has(StateAddress.from(BlockHeader.class, block.getHeader().getHash())) == false)
	    		throw new IllegalStateException("Block "+block.getHeader().getHash()+" not found in state for "+block.getHash());
	    }
	}
	
	private SubstateCommit getSubstateCommit(final Transaction transaction, final StateAddress stateAddress) throws IOException
	{
	...
	private void pushSubstateCommit(final SubstateCommit substateCommit) throws IOException
	{
		synchronized(this.substateInventory)
		{
			if (this.substateInventory.containsKey(substateCommit.getSubstate().getAddress()))
				throw new DatabaseException("Failed to push substate commit "+substateCommit+" due to "+OperationStatus.KEYEXIST);
			
			this.substateInventory.put(substateCommit.getSubstate().getAddress(), substateCommit);
		}
	}

	private SubstateCommit peekSubstateCommit(final Transaction transaction, final StateAddress stateAddress) throws IOException
	{
		synchronized(this.substateInventory)
		{
			...
			if (substateCommit == null)
			{
				...
				if (substateCommit == null)
					throw new DatabaseException("Failed to peek substate commit "+stateAddress+" due to "+OperationStatus.NOTFOUND);
	...
	private SubstateCommit pullSubstateCommit(final StateAddress stateAddress) throws IOException
	{
		synchronized(this.substateInventory)
		{
			...
			if (substateCommit == null)
				throw new DatabaseException("Failed to pull substate commit "+stateAddress+" due to "+OperationStatus.NOTFOUND);
			
			return substateCommit;
		}
	}

	private OperationStatus store(final Transaction transaction, final SubstateCommit substateCommit, final boolean update) throws IOException
	{
		final ByteBuffer byteBuffer = LedgerStore.byteBuffer.get().clear();
		final ByteBufferOutputStream byteBufferOutputStream = new ByteBufferOutputStream(byteBuffer, true);
		Serialization.getInstance().toDson(byteBufferOutputStream, substateCommit, Output.PERSIST);
		
		final DatabaseEntry key = keyEntry.get(); key.setData(substateCommit.getSubstate().getAddress().getHash().toByteArray());
		final DatabaseEntry value = valueEntry.get(); value.setData(byteBufferOutputStream.getByteBuffer().array(), 0, byteBufferOutputStream.getByteBuffer().position());
		final OperationStatus status;
		
		if (update)
			status = this.substates.put(transaction, key, value, LockMode.DEFAULT);
		else
			status = this.substates.putNoOverwrite(transaction, key, value, LockMode.DEFAULT);
				
		if (status.equals(OperationStatus.SUCCESS) == false)
			throw new DatabaseException("Failed to "+(update ? "update":"store")+" substate "+substateCommit.getSubstate().getAddress()+" due to "+status.name());
			
		return status;
	}
	
	private OperationStatus store(final Transaction transaction, final AssociationCommit association) throws IOException
	{
		final DatabaseEntry key = keyEntry.get(); key.setData(association.getReferrer().toByteArray());
		final DatabaseEntry value = valueEntry.get(); value.setData(Serialization.getInstance().toDson(association, Output.PERSIST));
		final OperationStatus status = this.associations.put(transaction, key, value, LockMode.DEFAULT);
		if (status.equals(OperationStatus.SUCCESS) == false)
			throw new DatabaseException("Failed to commit assiociation "+association+" due to "+status.name());
	...
	boolean has(final long height) throws DatabaseException
	{
		...
		try
        {
			final DatabaseEntry key = keyEntry.get(); key.setData(Longs.toByteArray(height));
		...
		try
		{
			...
			while(iteration >= 0) 
			{
			    ...
			    if (status.equals(OperationStatus.SUCCESS)) 
			    {
			        ...
			        searchSyncHeight = searchSyncHeight + (1L << (iteration-1));
			    } 
			    else
			        searchSyncHeight = searchSyncHeight - (1L << (iteration-1));
	...
	OperationStatus setSyncBlock(BlockHeader header) throws DatabaseException 
	{
		try
        {
			final DatabaseEntry key = keyEntry.get(); key.setData(Longs.toByteArray(header.getHeight()));
			final DatabaseEntry value = valueEntry.get(); key.setData(header.getHash().toByteArray());
			final OperationStatus status = this.syncChain.putNoOverwrite(null, key, value, LockMode.DEFAULT);
			return status;
        }
		catch (Throwable t)
		{
	...
	List<InventoryRecord> getSyncInventory(final long clock, final Class<? extends Primitive> type) throws IOException
	{
		...
		{
			if (inventoryRecord.clockStart != clock)
				continue;
				
			if (type.isAssignableFrom(inventoryRecord.clazz) == false)
				continue;
				
			if (inventoryRecord.isStale())
			{
				if (databaseLog.hasLevel(Logging.DEBUG))
					databaseLog.debug("Sync inventory at "+clock+" is skipping stale primitive "+inventoryRecord);
	...
	final Collection<SubstateCommit> search(final AssociationSearchQuery query) throws IOException
	{
		...
		try
		{
			...
			{
				long nextOffset = -1;
				final DatabaseEntry associationKey = keyEntry.get(); associationKey.setData(association.toByteArray());
				...
				try
				{
					...
					if (query.getOrder().equals(Order.DESCENDING))
						...
						status = searchCursor.getFirst(associationValue);
					
					if (status.equals(OperationStatus.SUCCESS))
					{
						do
						{
							final AssociationCommit associationCommit = Serialization.getInstance().fromDson(associationValue.getData(), AssociationCommit.class); 
							nextOffset = associationCommit.getIndex();

							if (query.getOrder().equals(Order.DESCENDING) == false && nextOffset > query.getOffset())
								break;
							
							if (query.getOrder().equals(Order.DESCENDING) == true && nextOffset < query.getOffset())
								break;
	
							if (query.getOrder().equals(Order.DESCENDING))
								status = searchCursor.getPrev(associationValue);
							else
								status = searchCursor.getNext(associationValue);
						}
						...
						while(status.equals(OperationStatus.SUCCESS) && substateCommits.size() < query.getLimit())
						{
							if (query.getOrder().equals(Order.DESCENDING) == false && nextOffset > query.getOffset() ||
							    query.getOrder().equals(Order.DESCENDING) == true && nextOffset < query.getOffset())
							{
								// FIXME feels messy, probably just better to store a commit in the associations and use that, would save a read too at the expense of a few more bytes per record
								final AssociationCommit associationCommit = Serialization.getInstance().fromDson(associationValue.getData(), AssociationCommit.class);
								if (query.getFilterContext().isEmpty() || query.getFilterContext().equalsIgnoreCase(associationCommit.getAddress().context()))
								{
									DatabaseEntry stateKey = keyEntry.get(); stateKey.setData(associationCommit.getAddress().getHash().toByteArray());
									DatabaseEntry substateValue = valueEntry.get();
									status = this.substates.get(null, stateKey, substateValue, LockMode.DEFAULT);
									if (status.equals(OperationStatus.SUCCESS))
									{
										SubstateCommit substateCommit = Serialization.getInstance().fromDson(substateValue.getData(), SubstateCommit.class);
						...
						substateCommits.sort((ss1, ss2) -> {
							...
							else
							{
								...
								if (idx1 < idx2)
									return query.getOrder().equals(Order.ASCENDING) ? -1 : 1;
						...
						if (substateCommits.isEmpty() == false)
						{
							...
							while (substateCommits.size() > query.getLimit())
								substateCommits.removeLast();
							
							indexCutoff = query.getOrder().equals(Order.NATURAL) ? substateCommits.getLast().getID() : substateCommits.getLast().getIndex();
						}
					}
				}
				finally
