...
{
	private final static int MANIFEST_MAX_INSTRUCTION_ARGUMENTS = 16;
	private final static int MANIFEST_ARGUMENT_CACHE_SIZE = 2<<16;
	private final static int MANIFEST_ARGUMENT_MAX_LENGTH = 64;
	private final static LRUCacheMap<String, Argument<?>> argCache = new LRUCacheMap<String, Argument<?>>(MANIFEST_ARGUMENT_CACHE_SIZE);
	...
	private Blob getBlob(final HashArgument hash)
	{
		for (final Object object : this.objects)
		{
			...
			{
				if (persist.getBlob().getHash().equals(hash.get()))
					return persist.getBlob();
			}
		}
	...
	private Deploy deploy(final String entry) throws ManifestException
	{
		...
		if (codeBlob == null)
			throw new ManifestException("Code blob "+arguments[2]+" not found for blueprint::deploy "+arguments);
	...
	private Epoch epoch(final String entry) throws ManifestException
	{
		...
		if (powersBlob == null)
			throw new ManifestException("Powers blob "+arguments[2]+" not found for native::epoch "+arguments);
		
		final VotePowers powers;
		try
		{
			powers = Serialization.getInstance().fromJson(new String(powersBlob.getBytes(), StandardCharsets.UTF_8), VotePowers.class);
		} 
		catch (SerializationException sex)
		{
			throw new ManifestException("Failed to deserialize vote powers for native::epoch "+arguments, sex);
		}
	...
	private Method method(final String entry) throws ManifestException
	{
		...
		if (namespaceIndex > 0)
		{
			...
			final Class<? extends NativeComponent> nativeComponent = Component.getNative(componentAddress);
	...
	private BadgeArgument badge(final String entry) throws ManifestException
	{
		if (entry.length() <= MANIFEST_ARGUMENT_MAX_LENGTH)
		{
			...
			if (cachedArgument instanceof BadgeArgument badgeArgument)
		...
		if (entry.length() <= MANIFEST_ARGUMENT_MAX_LENGTH)
			argCache.putIfAbsent(entry, badgeArgument);
	...
	private IdentityArgument identity(final String entry) throws ManifestException
	{
		...
		{
			final Argument<?> cachedArgument = argCache.get(entry);
			if (cachedArgument instanceof IdentityArgument identityArgument)
	...
	private UInt256Argument uint256(final String entry) throws ManifestException
	{
		...
		{
			final Argument<?> cachedArgument = argCache.get(entry);
			if (cachedArgument instanceof UInt256Argument uint256Argument)
		...
		if (arguments[0] instanceof Long value)
			uint256 = UInt256.from(value);
		else if (arguments[0] instanceof String string)
			uint256 = UInt256.from(string);
		else if (arguments[0] instanceof UInt256 value)
			uint256 = value;
		else
			throw new ManifestException("Unsupported argument type for unint256: "+arguments[0]);
	...
	private AccountArgument account(final String entry) throws ManifestException
	{
		...
		{
			final Argument<?> cachedArgument = argCache.get(entry);
			if (cachedArgument instanceof AccountArgument accountArgument)
		...
		if (arguments.length > 1)
			lock = (StateLockMode)arguments[1];
	...
	private TokenArgument token(final String entry) throws ManifestException
	{
		...
		{
			final Argument<?> cachedArgument = argCache.get(entry);
			if (cachedArgument instanceof TokenArgument tokenArgument)
		...
		if (entry.length() <= MANIFEST_ARGUMENT_MAX_LENGTH)
			argCache.putIfAbsent(entry, tokenArgument);
	...
	private VaultArgument vault(final String entry) throws ManifestException
	{
		...
		{
			final Argument<?> cachedArgument = argCache.get(entry);
			if (cachedArgument instanceof VaultArgument vaultArgument)
		...
		if (entry.length() <= MANIFEST_ARGUMENT_MAX_LENGTH)
			argCache.putIfAbsent(entry, vaultArgument);
	...
	private Object[] parseArguments(final String entry) throws ManifestException
	{
		...
		for (int i = 0 ; i < numArguments ; i++)
		{
			...
			if ((argument.charAt(0) >= '0' && argument.charAt(0) <= '9') ||
				(argument.charAt(0) == '-' && argument.charAt(1) >= '0' && argument.charAt(1) <= '9'))
			{
				try
				{
				...
				catch (NumberFormatException nfe) { /* Not a long:  Ignored to continue to next type test */ }
				...
				catch (NumberFormatException nfe) { /* Not a uint256:  Ignored to continue to next type test */ }
			...
			catch (IllegalArgumentException iaex) { /* Not a state lock mode:  Ignored to continue to next type test */}
	...
	private String[] splitArguments(final String input, final int start, final int end) throws ManifestException 
	{
        ...
        for (int i = start; i < end; i++) 
        {
            ...
            switch (c) 
            {
	            case '(':
	                ...
	                break;
	            case ')':
	                if (inQuotes == false && parenthesisDepth > 0) 
	                    parenthesisDepth--;
                ...
                case '[':
                    ...
                    break;
                case ']':
                    if (inQuotes == false && bracketDepth > 0) 
                        bracketDepth--;
	...
	{
        if (array == null || length <= 0 || offset < 0 || offset + length > array.length)
            return "";
