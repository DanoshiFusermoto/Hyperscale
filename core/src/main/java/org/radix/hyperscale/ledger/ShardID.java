...
public final class ShardID implements Comparable<ShardID>
{
	...
	private ShardID(final byte[] bytes, final int offset)
	{
		...
		if (bytes.length < BYTES)
			throw new IllegalArgumentException("Shard bytes length must be " + BYTES + " bytes for Hash, was " + data.length);

		if (offset + BYTES > bytes.length)
			throw new IllegalArgumentException(String.format("Shard bytes length must be at least %s for offset %s, but was %s", offset + BYTES, offset, bytes.length));
