...
/**
 * Represents an Atom that is currently being processed. <br>
 * <br>
...
public final class PendingAtom implements Hashable, StateAddressable {
  ...
  /**
   * The wall clock timestamp when the pending atom was agreed by all shard group validators to be
   * included in a block *
   */
  ...
  private PendingAtom(
      final Context context, final Hash hash, final Atom atom, final long witnessedAt)
      throws ManifestException {
    ...
    this.prepareTimeoutAt =
        (this.witnessedAt == -1 ? Time.getSystemTime() : this.witnessedAt)
            + TimeUnit.SECONDS.toMillis(Constants.ATOM_PREPARE_TIMEOUT_SECONDS);
    ...
    this.pendingStates =
        Maps.mutable.<StateAddress, PendingState>ofInitialCapacity(4).asSynchronized();
    ...
    if (this.context.getConfiguration().get("ledger.faults.force.atom.timeout.commit.interval", 0l)
            > 0
        && hash.asLong()
                % this.context
                    .getConfiguration()
                    .get("ledger.faults.force.atom.timeout.commit.interval", 0l)
            == 0) {
      this.forceCommitTimeout = true;
      atomsLog.warn(this.context.getName() + ": Atom " + getHash() + " is forcing commit timeout");
    } else if (this.context
                .getConfiguration()
                .get("ledger.faults.force.atom.timeout.execution.interval", 0l)
            > 0
        && hash.asLong()
                % this.context
                    .getConfiguration()
      ...
      atomsLog.warn(
          this.context.getName() + ": Atom " + getHash() + " is forcing execution timeout");
    } else if (this.context
                .getConfiguration()
                .get("ledger.faults.force.atom.latency.execution.interval", 0l)
            > 0
        && hash.asLong()
                % this.context
                    .getConfiguration()
      ...
      atomsLog.warn(
          this.context.getName() + ": Atom " + getHash() + " is forcing latent execution");
    } else if (this.context
                .getConfiguration()
                .get("ledger.faults.force.atom.timeout.accept.interval", 0l)
            > 0
        && hash.asLong()
                % this.context
                    .getConfiguration()
      ...
      atomsLog.warn(this.context.getName() + ": Atom " + getHash() + " is forcing accept timeout");
                ...
                .getConfiguration()
                .get("ledger.faults.force.atom.timeout.prepare.interval", 0l)
            > 0
        && hash.asLong()
                % this.context
                    .getConfiguration()
  ...
  void setAtom(final Atom atom) throws ManifestException {
    ...
    if (this.hash.equals(atom.getHash()) == false)
      throw new IllegalArgumentException(
          "Atom hash " + atom.getHash() + " does not match expected hash " + this.hash);
    ...
    synchronized (this) {
      ...
      double additionalTimeout =
          Math.log(atom.getManifest().size()) * Constants.ATOM_ACCEPT_TIMEOUT_SECONDS;
      this.acceptTimeoutAt =
          this.witnessedAt
              + TimeUnit.SECONDS.toMillis(
                  (long) (Constants.ATOM_ACCEPT_TIMEOUT_SECONDS + additionalTimeout));
  ...
  public boolean isGlobal() {
    if (this.status.before(AtomStatus.State.PREPARED))
      throw new IllegalStateException(
          "Pending atom " + getHash() + " is not PREPARED but " + this.status);

    return this.stateMachine.isGlobal();
  }

  public void forShards(
      final StateLockMode lockMode, final BiConsumer<StateAddress, ShardID> consumer) {
    if (this.status.before(AtomStatus.State.PREPARED))
      throw new IllegalStateException(
          "Pending atom " + getHash() + " is not PREPARED but " + this.status);
    ...
    synchronized (this.pendingStates) {
      ...
      this.pendingStates.forEachWith(
          (ps, p) -> {
            if (p != null && ps.getStateLockMode().equals(p) == false) return;
  ...
  public Set<ShardID> getShards(final StateLockMode lockMode) {
    ...
    synchronized (this.pendingStates) {
      final Set<ShardID> shardIDs =
          Sets.mutable.<ShardID>ofInitialCapacity(this.pendingStates.size());
      this.pendingStates.collectIf(
          ps -> {
            if (lockMode != null && ps.getStateLockMode().equals(lockMode) == false) return false;

            return true;
          },
          ps -> ps.getAddress().getShardID(),
          shardIDs);
  ...
  public Set<PendingState> getStates(final StateLockMode lockMode) {
    ...
    synchronized (this.pendingStates) {
      ...
      final Set<PendingState> pendingStates =
          Sets.mutable.<PendingState>ofInitialCapacity(this.pendingStates.size());
      this.pendingStates.selectWith(
          (ps, p) -> {
            if (p != null && ps.getStateLockMode().equals(p) == false) return false;
  ...
  public void forStateAddresses(
      final StateLockMode lockMode, final Consumer<StateAddress> consumer) {
    if (this.status.before(AtomStatus.State.PREPARED))
      throw new IllegalStateException(
          "Pending atom " + getHash() + " is not PREPARED but " + this.status);
    ...
    synchronized (this.pendingStates) {
      ...
      this.pendingStates.forEach(
          ps -> {
            if (lockMode != null && ps.getStateLockMode().equals(lockMode) == false) return;
  ...
  public List<StateAddress> getStateAddresses(final StateLockMode lockMode) {
    ...
    synchronized (this.pendingStates) {
      ...
      final List<StateAddress> stateAddresses =
          new ArrayList<StateAddress>(this.pendingStates.size());
      this.pendingStates.collectIf(
          ps -> {
            if (lockMode != null && ps.getStateLockMode().equals(lockMode) == false) return false;
  ...
  public int numStateAddresses(final StateLockMode lockMode) {
    ...
    synchronized (this.pendingStates) {
      ...
      if (lockMode == null) return this.pendingStates.size();
  ...
  void prepare() throws IOException, StateMachinePreparationException, ManifestException {
    if (this.forcePrepareTimeout) return;

    synchronized (this) {
      if (this.atom == null) throw new IllegalStateException("Atom is null");
      ...
      if (atomsLog.hasLevel(Logging.INFO)) {
        ...
        if (this.stateMachine.isGlobal())
          shardGroupIDs =
              ShardMapper.toAllShardGroups(
                  this.context.getLedger().numShardGroups(this.context.getLedger().getEpoch()));
        else
          shardGroupIDs =
              ShardMapper.toShardGroups(
                  getShards(StateLockMode.WRITE),
                  this.context.getLedger().numShardGroups(this.context.getLedger().getEpoch()));
        ...
        atomsLog.info(
            this.context.getName()
                + ": Pending atom "
                + getHash()
                + " is prepared for shard group IDs "
                + shardGroupIDs.toString());
        if (atomsLog.hasLevel(Logging.DEBUG)) {
          for (String manifestItem : this.atom.getManifest()) {
            if (manifestItem.length() > 256)
              atomsLog.debug(
                  this.context.getName() + ":    " + manifestItem.substring(0, 256) + " ...");
            else atomsLog.debug(this.context.getName() + ":    " + manifestItem);
          }
        }
      }
    }
  ...
  void accepted(final BlockHeader header) throws IOException {
    ...
    synchronized (this) {
      ...
      this.executeLatentAt =
          this.acceptedAt + TimeUnit.SECONDS.toMillis(Constants.ATOM_EXECUTE_LATENT_SECONDS);
      this.executeTimeoutAt =
          this.acceptedAt + TimeUnit.SECONDS.toMillis(Constants.ATOM_EXECUTE_TIMEOUT_SECONDS);
  ...
  void provisioned() {
    synchronized (this) {
      ...
      if (atomsLog.hasLevel(Logging.INFO))
        atomsLog.info(
            this.context.getName() + ": Pending atom " + this.getHash() + " is provisioned");
    }
  }
  ...
  void completed() {
    synchronized (this) {
      boolean canComplete =
          this.status.thrown() != null || this.certificate != null || this.timeout != null;

      if (canComplete == false)
        throw new IllegalStateException(
            "Pending atom "
                + this.hash
                + " state can not be set as COMPLETED without an AtomCertificate, AtomTimeout or thrown exception");
      ...
      if (atomsLog.hasLevel(Logging.INFO)) {
        ...
        if (this.certificate != null)
          ...
          completedVia =
              "via timeout "
                  + this.timeout.getClass().getSimpleName()
                  + ":"
                  + this.timeout.getHash();
        else if (this.status.thrown() != null)
          completedVia =
              "via exception "
                  + this.status.thrown().getClass().getName()
                  + ":"
                  + this.status.thrown().getMessage();
        else completedVia = ": UNKNOWN COMPLETION CONDITIONAL";
  ...
  void load(final Package pakage) {
    ...
    synchronized (this) {
      if (isProvisioned())
        throw new IllegalStateException(
            "Atom " + getHash() + " is already PROVISIONED when loading package " + pakage);

      this.stateMachine.load(pakage);
      if (this.stateMachine.isProvisioned() && isProvisioned() == false) provisioned();
    }
  }

  Collection<PolyglotPackage> load(
    ...
    synchronized (this) {
      if (isProvisioned())
        throw new IllegalStateException(
            "Atom "
                + getHash()
                + " is already PROVISIONED when loading packages "
                + pakages.stream().map(p -> p.getAddress()).collect(Collectors.toList()));

      final Collection<PolyglotPackage> loadedPackages =
          this.stateMachine.load(pakages, throwIfNotRequired);
      if (loadedPackages.isEmpty() == false
          && this.stateMachine.isProvisioned()
          && isProvisioned() == false) provisioned();
  ...
  void provision(final StateInput stateInput) throws StateMachinePreparationException {
    ...
    if (stateInput.getAtom().equals(this.hash) == false)
      throw new IllegalArgumentException(
          "State input " + stateInput + " does not reference atom " + this.hash);

    synchronized (this) {
      if (this.status.after(AtomStatus.State.PROVISIONING))
        throw new IllegalStateException(
            "Pending atom " + getHash() + " is already PROVISIONED " + this.status);

      final PendingState pendingState =
          this.pendingStates.computeIfAbsent(
              stateInput.getAddress(), sa -> new PendingState(this.context, sa, this));
    ...
    synchronized (this) {
      try {
        if (this.status.thrown() != null)
          throw new IllegalStateException(
              "Detected thrown exception for pending atom " + getHash() + " when executing");

        if (this.status.current(AtomStatus.State.PROVISIONED) == false)
          throw new IllegalStateException(
              "Pending atom " + this.getHash() + " is not PROVISIONED but " + this.status);
        ...
        synchronized (this.pendingStates) {
          for (final PendingState pendingState : this.pendingStates.values()) {
            if (pendingState.getStateLockMode().equals(StateLockMode.READ)) {
              if (pendingState.isFinalized() == false) pendingState.tryFinalize();
            }
          }
        }

        // If global then some participants will be observers and wont be creating state
        // certificates
        // which would trigger finalization when processing them in the regular pipeline.
        //
        // Therefore we must call it here too as its possible that the local instance received all
        // the
        // state certificates from remote validators prior to execution.
        if (isGlobal()) tryFinalize();
      } catch (StateMachineExecutionException smex) {
        this.status.thrown(smex);

        if (atomsLog.hasLevel(Logging.DEBUG))
          atomsLog.debug(
              this.context.getName() + ": Atom " + getHash() + " executed with exception");
      } catch (Exception ex) {
        ...
        atomsLog.warn(
            this.context.getName()
                + ": Atom "
                + getHash()
                + " executed with exception which should be wrapped in a StateMachineException",
            ex);
      }
    }
  }
  ...
  CommitOperation getCommitOperation() {
    synchronized (this) {
      ...
      if (commitOperation == null)
        throw new IllegalStateException(
            "Commit operation not yet created for pending atom " + getHash());

      if (this.certificate == null
          && Universe.getDefault().getGenesis().contains(getHash()) == false)
        throw new IllegalStateException(
            "Commit operation not accessible without atom certificate for pending atom "
                + getHash());

      if (this.certificate != null
          && this.certificate.getDecision().equals(commitOperation.getDecision()) == false)
        throw new IllegalStateException(
            "Commit operation decision "
                + commitOperation.getDecision()
                + " for pending atom "
                + getHash()
                + " does not match atom certificate decision "
                + this.certificate.getDecision());
  ...
  public boolean equals(Object object) {
    ...
    if (object instanceof PendingAtom pendingAtom) {
      if (pendingAtom.getHash().equals(this.hash)) return true;
    }
    ...
    synchronized (this) {
      ...
      if (getStatus().after(AtomStatus.State.PREPARED))
        throw new IllegalStateException(
            "Pending atom "
                + getHash()
                + " is already ACCEPTED with state "
                + getStatus().current());
      ...
      this.acceptTimeoutAt =
          this.acceptableAt + TimeUnit.SECONDS.toMillis(Constants.ATOM_ACCEPT_TIMEOUT_SECONDS);
    }
  }
  ...
  PendingState vote(final StateVote stateVote, long weight) throws ValidationException {
    synchronized (this) {
      if (this.status.before(AtomStatus.State.FINALIZING))
        throw new IllegalStateException(
            "Pending atom " + getHash() + " is not FINALIZING but " + this.status);

      final PendingState pendingState = getState(stateVote.getAddress());
      if (pendingState == null)
        throw new ValidationException(
            this, "Pending state not found for state vote " + stateVote.toString());
  ...
  void setExecuteSignalledAtBlock(final BlockHeader header) {
    ...
    synchronized (this) {
      ...
      this.commitTimeoutAt =
          header.getTimestamp() + TimeUnit.SECONDS.toMillis(Constants.ATOM_COMMIT_TIMEOUT_SECONDS);
        ...
        if (isProvisioned())
          atomsLog.info(
              this.context.getName()
                  + ": Atom "
                  + getHash()
                  + " is signalled executable at "
                  + this.executeSignalledBlock);
        else
          atomsLog.info(
              this.context.getName()
                  + ": Atom "
  ...
  public boolean isExecuteLatent(final BlockHeader header) {
    synchronized (this) {
      if (this.executeLatentSignalledBlock != Hash.ZERO) return true;

      if (this.status.after(AtomStatus.State.PREPARED)
          && this.status.before(AtomStatus.State.EXECUTING)
          && header.getTimestamp() >= this.executeLatentAt) return true;
  ...
  void setExecuteLatentSignalledBlock(final BlockHeader header) {
    ...
    synchronized (this) {
      if (this.executeLatentSignalledBlock != Hash.ZERO)
        throw new IllegalStateException(
            "Execution latency signal has already been made for " + this);
      ...
      if (atomsLog.hasLevel(Logging.INFO))
        atomsLog.info(
            this.context.getName()
                + ": Atom "
                + getHash()
                + " is signalled execute latent at "
                + this.executeLatentSignalledBlock);
  ...
  boolean addStateVoteBlock(final StateVoteBlock stateVoteBlock) {
    synchronized (this) {
      if (this.stateVoteBlocks.putIfAbsent(stateVoteBlock.getHash(), stateVoteBlock) == null)
        ...
        stateLog.warn(
            PendingAtom.this.context.getName()
                + ": State vote block "
                + stateVoteBlock.getHash()
                + " is already known for pending atom "
                + this);
  ...
  void setCertificate(final AtomCertificate certificate) {
    synchronized (this) {
      ...
      if (Objects.requireNonNull(certificate).getAtom().equals(getHash()) == false)
        throw new IllegalArgumentException(
            "Atom certificate " + certificate.getHash() + " does not reference " + getHash());
  ...
  void complete(final AtomCertificate certificate) {
    ...
    if (this.status.current(AtomStatus.State.PROVISIONED) == false)
      throw new IllegalStateException(
          "Pending atom " + getHash() + " is not PROVISIONED but " + this.status);
    ...
    if (atomsLog.hasLevel(Logging.INFO))
      atomsLog.info(
          this.context.getName()
              + ": Pending atom "
              + getHash()
              + " has bypassed execution and been completed with certificate "
              + certificate.getHash());
  }
  ...
  public <T extends StateOutput> List<T> getOutputs(final Class<T> type) {
    ...
    synchronized (this.pendingStates) {
      ...
      for (final PendingState pendingState : this.pendingStates.values()) {
        ...
        if (type.isInstance(stateOutput) == false) continue;
  ...
  void provision(final StateOutput output) {
    synchronized (this) {
      if (stateLog.hasLevel(Logging.INFO))
        stateLog.info(
            this.context.getName() + ": Adding state output " + output + " to " + getHash());

      // Not in a block yet but has at least one state certificate so effectively cancel the
      // inclusion timeout
      if (this.block == null) {
        // TODO this actually should never happen if the local replica is healthy as
        // StateCertificates
        ...
        if (atomsLog.hasLevel(Logging.DEBUG))
          atomsLog.debug(
              this.context.getName()
                  + ": Atom "
                  + this.getHash()
                  + " accept timeout cancelled due to state certificate prior to ACCEPTED");
      }

      final PendingState pendingState =
          this.pendingStates.computeIfAbsent(
              output.getAddress(), sa -> new PendingState(this.context, sa, this));
  ...
  long getNextTimeoutAt() {
    // Commit timeout
    if (this.status.after(AtomStatus.State.EXECUTING)
        && this.status.before(AtomStatus.State.COMPLETED)) return getCommitTimeoutAt();
    // Execution timeout
    else if (this.status.after(AtomStatus.State.PREPARED)
        && this.status.before(AtomStatus.State.EXECUTING)
        && isExecuteLatentSignalled() == true) return getExecuteTimeoutAt();
    // Execution latent
    else if (this.status.after(AtomStatus.State.PREPARED)
        && this.status.before(AtomStatus.State.EXECUTING)
        && isExecuteLatentSignalled() == false) return getExecuteLatentAt();
    // Accept timeout
    else if (this.status.current(AtomStatus.State.PREPARED)) return getAcceptTimeoutAt();
    // Prepare timeout
    else if (this.status.before(AtomStatus.State.PREPARED)) return getPrepareTimeoutAt();
  ...
  AtomTimeout tryTimeout(long timestamp) {
    synchronized (this) {
      ...
      // Commit timeout
      if (this.status.after(AtomStatus.State.EXECUTING)
          && this.status.before(AtomStatus.State.COMPLETED)
          && timestamp >= getCommitTimeoutAt()) timeout = new CommitTimeout(getHash(), getInputs());
      // Execution timeout
      else if (this.status.after(AtomStatus.State.PREPARED)
          && this.status.before(AtomStatus.State.EXECUTING)
          && isExecuteLatentSignalled() == true
          && timestamp >= getExecuteTimeoutAt()) timeout = new ExecutionTimeout(getHash());
      // Execution latent
      else if (this.status.after(AtomStatus.State.PREPARED)
          && this.status.before(AtomStatus.State.EXECUTING)
          && isExecuteLatentSignalled() == false
          && timestamp >= getExecuteLatentAt()) timeout = new ExecutionLatentTimeout(getHash());
      // Accept timeout
        ...
        if (timeout instanceof CommitTimeout) {
          ...
          atomStatusLog.error(
              this.context.getName()
                  ...
                  + " for "
                  + getHash()
                  + " but has all state outputs");

          if (this.certificate == null) falsePositive = false;
          else
            atomStatusLog.error(
                this.context.getName()
                    + ": "
  ...
  void setTimeout(final AtomTimeout timeout) {
    synchronized (this) {
      ...
      if (Objects.requireNonNull(timeout).getAtom().equals(getHash()) == false)
        throw new IllegalArgumentException(
            "Atom timeout " + timeout.getHash() + " does not reference " + getHash());

      if (atomStatusLog.hasLevel(Logging.INFO))
        atomStatusLog.info(
            this.context.getName() + ": Atom " + getHash() + " is signalling timeout " + timeout);
  ...
  AtomCertificate tryFinalize() throws CryptoException, IOException, ValidationException {
    synchronized (this) {
      if (getCertificate() != null) {
        stateLog.warn(
            this.context.getName() + ": Atom certificate for " + getHash() + " already created");
        return this.certificate;
      }
      ...
      if (this.status.before(AtomStatus.State.EXECUTING)) {
        stateLog.warn(
            this.context.getName()
                + ": Attempted to create atom certificate for "
                + getHash()
                + " when status "
                + this.status.current());
        return null;
      ...
      if (this.pendingStates.allSatisfy(PendingState::isFinalized) == false) return null;

      // Investigate possibility of creating an atom certificate.  All state references /
      // certificates must have been created.
      ...
      synchronized (this.pendingStates) {
        ...
        for (final PendingState pendingState : this.pendingStates.values()) {
          ...
          if (pendingState.getStateLockMode().equals(StateLockMode.WRITE)) {
            if (pendingState.isFinalized() == false) return null;
            ...
            if (pendingState.getStateOutput() instanceof StateCertificate stateCertificate) {
              ...
              throw new IllegalArgumentException(
                  "Write locked pending state output "
          ...
          // Check read locked pending states are already finalized post execute
            ...
            if (pendingState.isFinalized() == false)
              throw new IllegalArgumentException(
                  "Read locked pending state output "
                      + pendingState.getAddress()
                      + " is not finalized as expected");
          }
      ...
      final AtomCertificate atomCertificate =
          new AtomCertificate(getHash(), decision, stateOutputs);
      ...
      if (stateLog.hasLevel(Logging.INFO))
        stateLog.info(
            this.context.getName()
                + ": Created atom certificate "
                + atomCertificate.getHash()
                + " for atom "
                + atomCertificate.getAtom()
                + " with decision "
                + atomCertificate.getDecision());
