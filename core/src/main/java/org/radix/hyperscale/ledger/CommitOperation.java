...
public class CommitOperation {
  ...
  public CommitOperation(
      ...
      final Atom atom,
      final Collection<SubstateTransitions> transitions,
      final Collection<Entry<StateAddress, Hash>> associations,
      final SubstateOpLog substateOpLog) {
    ...
    if (this.decision.equals(CommitDecision.REJECT)) {
      if (Objects.requireNonNull(transitions, "Substate transitions is null").isEmpty() == false)
        throw new IllegalArgumentException(
            "Rejections must have not have substates for atom " + atom.getHash());
      if (Objects.requireNonNull(associations, "Associations is null").isEmpty() == false)
        throw new IllegalArgumentException(
            "Rejections must have not have associations for atom " + atom.getHash());
      if (Objects.requireNonNull(substateOpLog, "Substate snapshot is null").isEmpty() == false)
        throw new IllegalArgumentException(
            "Rejections must have not have populated substate snapshot for atom " + atom.getHash());
      ...
      this.transitions =
          transitions.isEmpty() ? Collections.emptyList() : ImmutableList.copyOf(transitions);
      this.associations =
          associations.isEmpty() ? Collections.emptyList() : ImmutableList.copyOf(associations);
    }
  }
  ...
  public boolean equals(Object object) {
    ...
    if (object instanceof CommitOperation commitOperation) {
      if (this.head.equals(commitOperation.getHead()) == false) return false;

      if (this.atom.equals(commitOperation.getAtom()) == false) return false;
  ...
  public String toString() {
    return this.head.getHash()
        ...
        + this.atom.getHash()
        + " ["
        + getSubstateTransitions().toString()
        + "] ("
        + (getAssociations().toString())
        + ")";
  }
}
