...
public final class Blob extends BasicObject implements Primitive
{
	...
	private volatile transient Hash checksum;
	...
	public Blob(final String dataURL)
	{
		...
		if (encodingOffset > dataOffset)
			encodingOffset = -1;

		this.mime = dataURL.substring(mimeOffset+1, encodingOffset == -1 ? dataOffset : encodingOffset);
	...
	public Hash getChecksum()
	{
		if (this.checksum == null)
			this.checksum = Hash.hash(this.bytes);
	...
	public String asDataURL()
	{
		...
		if (this.mime != null && this.mime.isEmpty() == false)
