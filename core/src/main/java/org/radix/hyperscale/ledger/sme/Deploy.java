...
public final class Deploy implements Instruction
{
	...
	public Deploy(final String blueprintName, final String language, final Blob code, final Identity authority)
	{ 
		super();
	...
	PolyglotPackage execute(final StateMachine stateMachine)
	{
		...
		stateMachine.set(getBlueprintAddress(), NativeField.LANGUAGE, Strings.toLowerCase(this.language), this.authority);
		stateMachine.set(getBlueprintAddress(), NativeField.BLUEPRINT, Strings.toLowerCase(this.blueprintName), this.authority);
		stateMachine.associate(getBlueprintAddress(), this.authority);

		final PolyglotPackage pakage = new PolyglotPackage(this.packageAddress, this.language, this.code);
	...
	{
		return this.blueprintName+" "+this.language+" "+this.code.getHash()+" "+this.authority;
	}
}
