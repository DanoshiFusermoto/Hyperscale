...
public class SyncHandler implements Service {
  ...
  private final class SyncRequestTask extends ConnectionTask {
    ...
    public void execute() {
      ...
      if (isFailedRequest) {
        try {
          syncLog.error(
              SyncHandler.this.context.getName()
                  + ": "
                  + getConnection()
                  + " did not respond fully to block request of "
                  + this.requestedBlock);
          if (getConnection().getState().equals(ConnectionState.CONNECTED)
              || getConnection().getState().equals(ConnectionState.CONNECTING))
            getConnection()
                .disconnect("Did not respond to block request of " + this.requestedBlock);
        } catch (Throwable t) {
    ...
    public boolean completed() {
      SyncHandler.this.lock.lock();
      try {
        if (SyncHandler.this.blocksRequested.containsKey(this.requestedBlock) == false) return true;

        return !Long.valueOf(this.requestSeq)
            .equals(SyncHandler.this.blocksRequested.get(this.requestedBlock));
      } finally {
        SyncHandler.this.lock.unlock();
      }
    }
    ...
    public void onCancelled() {
      ...
      try {
        ...
        if (wasPending) rerequest(this.requestedBlock);
      } finally {
        SyncHandler.this.lock.unlock();
      }
    }
    ...
    private void rerequest(final Hash block) {
      ...
      ShardGroupID rerequestShardGroupID =
          ShardMapper.toShardGroup(
              getConnection().getNode().getIdentity(),
              SyncHandler.this.context.getLedger().numShardGroups());
      StandardConnectionFilter standardPeerFilter =
          StandardConnectionFilter.build(SyncHandler.this.context)
              .setStates(ConnectionState.CONNECTED)
              .setShardGroupID(rerequestShardGroupID);
      List<AbstractConnection> rerequestConnectedPeers =
          SyncHandler.this.context.getNetwork().get(standardPeerFilter);
  ...
  private final class SyncInventoryTask extends ConnectionTask {
    ...
    SyncInventoryTask(
        final AbstractConnection connection, final Hash block, final long requestSeq) {
        ...
        if (currentTask.requestSeq == this.requestSeq) {
          ...
          syncLog.error(
              SyncHandler.this.context.getName()
                  + ": "
                  + getConnection()
                  + " did not respond to inventory request "
                  + this.requestSeq
                  + ":"
                  + this.block);
          if (getConnection().getState().equals(ConnectionState.CONNECTED)
            ...
            getConnection()
                .disconnect(
                    "Did not respond to inventory request " + this.requestSeq + ":" + this.block);
        }
      } catch (Throwable t) {
        syncLog.error(SyncHandler.this.context.getName() + ": " + getConnection(), t);
      } finally {
  ...
  private Executable syncProcessor =
      new Executable() {
        @Override
        public void execute() {
          try {
            while (this.isTerminated() == false) {
              try {
                ...
                if (SyncHandler.this.checkSynced().equals(SyncStatus.UNSYNCED) == false) continue;
                ...
                if (SyncHandler.this.isPrepared == false) continue;
                ...
                final ShardGroupID localShardGroupID =
                    ShardMapper.toShardGroup(
                        SyncHandler.this.context.getNode().getIdentity(), numShardGroups);
                final StandardConnectionFilter standardPeerFilter =
                    StandardConnectionFilter.build(SyncHandler.this.context)
                        .setShardGroupID(localShardGroupID)
                        .setStates(ConnectionState.CONNECTED);
                ...
                try {
                  if (syncPeers.isEmpty() == false) {
                    final int numSyncBlockRequestsPerPeer =
                        MULTIPLE_SYNC_BLOCK_REQUESTS
                            ? Math.max(
                                1, Constants.DEFAULT_TCP_CONNECTIONS_OUT_SYNC / syncPeers.size())
                            : 1;
                    for (AbstractConnection syncPeer : syncPeers) {
                      ...
                      try {
                        synchronized (SyncHandler.this.requestTasks) {
                          final Collection<SyncRequestTask> pendingRequestTasks =
                              SyncHandler.this.requestTasks.get(syncPeer);
                          numRequestTasksAvailable -= pendingRequestTasks.size();
                          for (SyncRequestTask pendingRequestTask : pendingRequestTasks) {
                            if (pendingRequestTask.completed()) numRequestTasksAvailable++;
                          }

                          if (numRequestTasksAvailable <= 0) continue;
                        }

                        synchronized (SyncHandler.this.blockInventories) {
                          final Iterator<Hash> inventoryIterator =
                              SyncHandler.this.blockInventories.get(syncPeer).iterator();
                          while (inventoryIterator.hasNext()) {
                            ...
                            if (SyncHandler.this.blocks.containsKey(block)
                                || inventoryBlockHeight <= SyncHandler.this.syncHead.getHeight()) {
                            ...
                            if (SyncHandler.this
                                ...
                                .getLedger()
                                .getLedgerStore()
                                .has(StateAddress.from(Block.class, block))) {
                              if (inventoryBlockHeight > SyncHandler.this.syncHead.getHeight()) {
                                syncLog.warn(
                                    SyncHandler.this.context.getName()
                                        + ": Inventory block "
                                        + inventoryBlockHeight
                                        + "@"
                                        + block
                                        + " is committed after sync head "
                                        + SyncHandler.this.syncHead
                                        + " in block inventory for "
                                        + syncPeer);
                      ...
                      try {
                        synchronized (SyncHandler.this.blockInventories) {
                          if (SyncHandler.this.blockInventories.get(syncPeer).isEmpty() == false) {
                            ...
                            while (inventoryIterator.hasNext()) {
                              Hash block = inventoryIterator.next();
                              if (SyncHandler.this.blocksRequested.containsKey(block)) continue;

                              if (SyncHandler.this.blocks.containsKey(block)) continue;

                              if (SyncHandler.this.request(syncPeer, block)) {
                                numRequestTasksAvailable--;

                                if (numRequestTasksAvailable == 0) break;
                              }
                            }
                          }
                        }
                  ...
                  final List<Block> sortedBlocks =
                      new ArrayList<Block>(SyncHandler.this.blocks.values());
                  Collections.sort(sortedBlocks, Collections.reverseOrder());
                  for (int i = 0; i < sortedBlocks.size(); i++) {
                    final Block block = sortedBlocks.get(i);

                    if (block.getHeader().getHeight() <= SyncHandler.this.syncHead.getHeight()) {
                      SyncHandler.this.blocks.remove(block.getHash());
                      continue;
                    }

                    boolean waitingForRequested = false;
                    Block branchBlock = block;
                    final SyncBranch syncBranch =
                        new SyncBranch(SyncHandler.this.context, SyncHandler.this.syncHead);
                    syncBranch.push(block.getHeader());
                    while (branchBlock
                            .getHeader()
                            .getPrevious()
                            .equals(syncBranch.getRoot().getHash())
                        == false) {
                      if (SyncHandler.this.blocksRequested.containsKey(
                          branchBlock.getHeader().getPrevious())) {
                        waitingForRequested = true;
                        break;
                      }

                      final Block prevBranchBlock =
                          SyncHandler.this.blocks.get(branchBlock.getHeader().getPrevious());
                        ...
                        if (syncLog.hasLevel(Logging.DEBUG))
                          syncLog.warn(
                              SyncHandler.this.context.getName()
                                  ...
                                  + branchBlock.getHeader().getPrevious()
                                  + " of "
                                  + branchBlock.getHeader().toString()
                                  + " is not known");
                    ...
                    if (waitingForRequested) continue;

                    if (syncBranch.getHigh().getCertificate() == null) continue;

                    final long blockVotePower =
                        syncBranch.getVotePower(
                            syncBranch.getHigh().getHeight(),
                            syncBranch.getHigh().getCertificate().getSigners());
                    final long blockVotePowerThreshold =
                        syncBranch.getVotePowerThreshold(syncBranch.getHigh().getHeight());
                    final double blockVotePowerRatio =
                        blockVotePower / (double) blockVotePowerThreshold;
                    if (blockVotePower < blockVotePowerThreshold) continue;

                    if (blockVotePowerRatio < bestBranchVoteRatio) continue;
                  ...
                  if (bestBranch != null) {
                    ...
                    if (bestBranch.isCanonical() == false) {
                      syncLog.fatal(
                          SyncHandler.this.context.getName() + ": Selected branch in range:");
                      syncLog.fatal(
                          SyncHandler.this.context.getName() + ":   R: " + bestBranch.getRoot());
                      syncLog.fatal(
                          SyncHandler.this.context.getName() + ":   L: " + bestBranch.getLow());
                      syncLog.fatal(
                          SyncHandler.this.context.getName() + ":   H: " + bestBranch.getHigh());
                      syncLog.fatal(
                          SyncHandler.this.context.getName()
                              + ":   does not link to "
                              + SyncHandler.this.syncHead);
                  ...
                  if (syncPeers.isEmpty() == false) {
                    for (final AbstractConnection connection : syncPeers) {
                      if (SyncHandler.this.syncHead.isAheadOf(connection.getNode().getHead(), 1))
                        continue;

                      if (SyncHandler.this.inventoryTasks.containsKey(connection) == false
                          && (SyncHandler.this.blockInventories.containsKey(connection) == false
                              || SyncHandler.this.blockInventories.get(connection).size()
                        ...
                        if (SyncHandler.this.blockInventories.containsKey(connection) == true
                            && SyncHandler.this.blockInventories.get(connection).isEmpty() == false)
                          highestInventory =
                              SyncHandler.this.blockInventories.get(connection).last();

                        try {
                          final GetSyncBlockInventoryMessage getSyncBlockInventoryMessage =
                              new GetSyncBlockInventoryMessage(highestInventory);

                          syncInventoryTask =
                              new SyncInventoryTask(
                                  connection,
                                  highestInventory,
                                  getSyncBlockInventoryMessage.getSeq());
                          ...
                          SyncHandler.this
                              ...
                              .getNetwork()
                              .getMessaging()
                              .send(getSyncBlockInventoryMessage, connection);
                          syncLog.info(
                              SyncHandler.this.context.getName()
                                  + ": Requested block inventory "
                                  + getSyncBlockInventoryMessage.getSeq()
                                  + ":"
                                  + Block.toHeight(highestInventory)
                                  + "@"
                                  + highestInventory
                                  + " from "
                                  + connection);
                        } catch (Exception ex) {
                          if (syncInventoryTask != null) syncInventoryTask.cancel();

                          syncLog.error(
                              SyncHandler.this.context.getName()
                                  + ": Unable to send GetSyncBlockInventoryMessage from "
                                  + highestInventory
                                  + " to "
                                  + connection,
                              ex);
            ...
            syncLog.fatal(
                SyncHandler.this.context.getName() + ": Error processing sync", throwable);
          }
        }
      };
  ...
  SyncHandler(Context context) {
    ...
    this.inventoryTasks =
        Collections.synchronizedMap(new HashMap<AbstractConnection, SyncInventoryTask>());
    this.blocksRequested = Collections.synchronizedMap(new HashMap<Hash, Long>());
    this.blockInventories =
        Multimaps.synchronizedSortedSetMultimap(
            TreeMultimap.<AbstractConnection, Hash>create(Ordering.natural(), Ordering.natural()));
  ...
  public void start() throws StartupException {
    try {
      this.context
          ...
          .getMessaging()
          .register(
              SyncBlockMessage.class,
              this.getClass(),
              new MessageProcessor<SyncBlockMessage>() {
                @Override
                public void process(
                    final SyncBlockMessage syncBlockMessage, final AbstractConnection connection) {
                  try {
                    if (SyncHandler.this.isPrepared == false) {
                      syncLog.warn(
                          SyncHandler.this.context.getName()
                              + ": Received SyncBlockMessage, but sync is not prepared");
                      return;
                    }

                    if (syncBlockMessage.getBlock().getHeader().getHeight()
                        <= SyncHandler.this.syncHead.getHeight()) {
                      syncLog.warn(
                          SyncHandler.this.context.getName()
                              + ": Block is old "
                              + syncBlockMessage.getBlock().getHeader()
                              + " from "
                              + connection);
                      return;
                    }
                    ...
                    if (SyncHandler.this
                        ...
                        .has(
                            StateAddress.from(
                                Block.class, syncBlockMessage.getBlock().getHeader().getHash()))) {
                      syncLog.warn(
                          SyncHandler.this.context.getName()
                              + ": Block is committed "
                              + syncBlockMessage.getBlock().getHeader()
                    ...
                    if (syncLog.hasLevel(Logging.DEBUG))
                      syncLog.info(
                          SyncHandler.this.context.getName()
                              + ": Block "
                              + syncBlockMessage.getBlock().getHeader().getHash()
                              + " from "
                              + connection);

                    if (syncBlockMessage.getBlock().getHeader().getCertificate() != null) {
                      if (syncBlockMessage.getBlock().getHeader().getCertificate().getSigners()
                              == null
                          || syncBlockMessage
                                  .getBlock()
                                  .getHeader()
                    ...
                    SyncHandler.this.lock.lock();
                    try {
                      SyncHandler.this.blocks.put(
                          syncBlockMessage.getBlock().getHash(), syncBlockMessage.getBlock());
                      if (SyncHandler.this.blocksRequested.remove(
                              syncBlockMessage.getBlock().getHash())
                          == null) {
                        syncLog.error(
                            SyncHandler.this.context.getName()
      ...
      this.context
          ...
          .getMessaging()
          .register(
              GetSyncBlockMessage.class,
              this.getClass(),
              new MessageProcessor<GetSyncBlockMessage>() {
                @Override
                public void process(
                    final GetSyncBlockMessage getSyncBlockMessage,
                  ...
                  try {
                    ...
                    final Block block =
                        SyncHandler.this
                            ...
                            .getLedger()
                            .getLedgerStore()
                            .get(getSyncBlockMessage.getBlock(), Block.class);
                    if (block == null) {
                      syncLog.error(
                          SyncHandler.this.context.getName()
                              + ": Requested block "
                    ...
                    try {
                      SyncHandler.this
                          ...
                          .getNetwork()
                          .getMessaging()
                          .send(new SyncBlockMessage(block), connection);
                    } catch (IOException ex) {
                      syncLog.error(
                          SyncHandler.this.context.getName()
                              + ": Unable to send SyncBlockMessage for "
                              + getSyncBlockMessage.getBlock()
                              + " to "
                              + connection,
                          ex);
                    ...
                    syncLog.error(
                        SyncHandler.this.context.getName()
                            + ": ledger.messages.block.sync.get "
                            + connection,
                        ex);
                  }
                }
      ...
      this.context
          ...
          .getMessaging()
          .register(
              GetSyncBlockInventoryMessage.class,
              this.getClass(),
              new MessageProcessor<GetSyncBlockInventoryMessage>() {
                @Override
                public void process(
                    final GetSyncBlockInventoryMessage getSyncBlockInventoryMessage,
                    final AbstractConnection connection) {
                  try {
                    if (syncLog.hasLevel(Logging.DEBUG))
                      syncLog.info(
                          SyncHandler.this.context.getName()
                              + ": Block inventory request from "
                              + getSyncBlockInventoryMessage.getHead()
                              + " from "
                              + connection);
                    ...
                    do {
                      Hash next =
                          SyncHandler.this
                              ...
                              .getLedger()
                              .getLedgerStore()
                              .getSyncBlock(Block.toHeight(current) + 1);
                      if (next != null) inventory.add(next);

                      current = next;
                    } while (current != null
                        && inventory.size() < Constants.MAX_BROADCAST_INVENTORY_ITEMS);
                    ...
                    if (inventory.isEmpty())
                      syncBlockInventoryMessage =
                          new SyncBlockInventoryMessage(getSyncBlockInventoryMessage.getSeq());
                    else
                      syncBlockInventoryMessage =
                          new SyncBlockInventoryMessage(
                              getSyncBlockInventoryMessage.getSeq(), inventory);
                    ...
                    try {
                      SyncHandler.this
                          ...
                          .getNetwork()
                          .getMessaging()
                          .send(syncBlockInventoryMessage, connection);
                    } catch (IOException ex) {
                      syncLog.error(
                          SyncHandler.this.context.getName()
                              + ": Unable to send SyncBlockInventoryMessage "
                              + getSyncBlockInventoryMessage.getSeq()
                              + ":"
                              + getSyncBlockInventoryMessage.getHead()
                              + " for "
                    ...
                    syncLog.error(
                        SyncHandler.this.context.getName()
                            + ": ledger.messages.block.sync.inv.get "
                            + connection,
                        ex);
                  }
                }
      ...
      this.context
          ...
          .getMessaging()
          .register(
              SyncBlockInventoryMessage.class,
              this.getClass(),
              new MessageProcessor<SyncBlockInventoryMessage>() {
                @Override
                public void process(
                    final SyncBlockInventoryMessage syncBlockInventoryMessage,
                    final AbstractConnection connection) {
                  try {
                    if (SyncHandler.this.isPrepared == false) {
                      syncLog.warn(
                          SyncHandler.this.context.getName()
                              + ": Received SyncBlockInventoryMessage, but sync is not prepared");
                      return;
                    }
                    ...
                    if (syncBlockInventoryMessage.getInventory().isEmpty())
                      syncLog.warn(
                          SyncHandler.this.context.getName()
                              + ": Received empty block inventory from "
                              + connection);
                    else if (syncLog.hasLevel(Logging.DEBUG))
                      syncLog.debug(
                          SyncHandler.this.context.getName()
                              + ": Received block header inventory "
                              + syncBlockInventoryMessage.getInventory()
                              + " from "
                              + connection);

                    SyncHandler.this.lock.lock();
                    try {
                      final SyncInventoryTask syncInventoryTask =
                          SyncHandler.this.inventoryTasks.get(connection);
                      if (syncInventoryTask == null
                          || syncInventoryTask.requestSeq
                              != syncBlockInventoryMessage.getResponseSeq()) {
                        syncLog.error(
                            SyncHandler.this.context.getName()
                                + ": Received unrequested inventory "
                                + syncBlockInventoryMessage.getResponseSeq()
                                + " with "
                                + syncBlockInventoryMessage.getInventory().size()
                                + " items from "
                                + connection);
                        connection.disconnect(
                            "Received unrequested inventory "
                      ...
                      if (syncBlockInventoryMessage.getInventory().isEmpty() == false) {
                        SyncHandler.this.blockInventories.putAll(
                            connection, syncBlockInventoryMessage.getInventory());
                        SyncHandler.this.syncProcessorLatch.release();
                      }
                    ...
                    syncLog.error(
                        SyncHandler.this.context.getName()
                            + ": ledger.messages.block.header.inv "
                            + connection,
                        ex);
                  }
                }
      ...
      this.isSingleton =
          this.isPrepared && this.context.getConfiguration().get("ledger.synced.always", false);
  ...
  private void reset() {
    ...
    try {
      ...
      final Collection<SyncRequestTask> requestTasks =
          new ArrayList<SyncRequestTask>(this.requestTasks.values());
      for (SyncRequestTask requestTask : requestTasks) requestTask.cancel();
      this.requestTasks.clear();

      final Collection<SyncInventoryTask> inventoryTasks =
          new ArrayList<SyncInventoryTask>(this.inventoryTasks.values());
      for (SyncInventoryTask inventoryTask : inventoryTasks) inventoryTask.cancel();
  ...
  private boolean request(final AbstractConnection connection, final Hash block)
      throws IOException {
    ...
    try {
      if (this.blocksRequested.containsKey(block)) {
        syncLog.warn(
            SyncHandler.this.context.getName()
                + ": Block "
                + block
                + " is already requested when requesting from "
                + connection);
        return false;
      }
      ...
      if (syncRequestTask != null) {
        if (syncRequestTask.cancel() == true) this.requestTasks.remove(connection, syncRequestTask);
      }
  ...
  private void prepare()
    ...
    final Entry<BlockHeader, StateAccumulator> ledgerState =
        this.context.getLedger().current("sync");
    ...
    BlockHeader current =
        this.context.getLedger().getLedgerStore().get(this.desyncHead.getHash(), BlockHeader.class);
    do {
      recentBlockHeaders.add(current);
      if (current.getPrevious().equals(Universe.getDefault().getGenesis().getHash())) break;

      current =
          this.context.getLedger().getLedgerStore().get(current.getPrevious(), BlockHeader.class);
    ...
    for (final BlockHeader header : recentBlockHeaders) {
      final Block block =
          this.context.getLedger().getLedgerStore().get(header.getHash(), Block.class);
      ...
      for (final AtomCertificate certificate : block.getCertificates()) {
        ...
        if (this.atoms.containsKey(certificate.getAtom()) == false) {
          final Atom atom =
              this.context.getLedger().getLedgerStore().get(certificate.getAtom(), Atom.class);
          if (atom == null)
            throw new ValidationException(
                certificate,
                "Pending atom "
                    + certificate.getAtom()
                    + " not found for certificate "
                    + certificate.getHash()
                    + " in block "
                    + block.getHash());
          ...
          final SubstateCommit substateCommit =
              this.context
                  .getLedger()
                  .getLedgerStore()
                  .search(StateAddress.from(Atom.class, certificate.getAtom()));
          if (substateCommit == null)
            throw new ValidationException(
                certificate,
                "Substate for pending atom "
                    + certificate.getAtom()
                    + " not found for certificate "
                    + certificate.getHash()
                    + " in block "
          ...
          final BlockHeader commitHeader =
              this.context
                  .getLedger()
                  .getLedgerStore()
                  .get(
                      substateCommit.getSubstate().<Hash>get(NativeField.BLOCK), BlockHeader.class);
        ...
        } else pendingAtom = this.atoms.get(certificate.getAtom());
        ...
        for (StateOutput stateOutput : certificate.getInventory(StateOutput.class))
          stateInputs.add(
              new StateInput(
                  pendingAtom.getHash(), stateOutput.getStates().toSubstate(SubstateOutput.READS)));
      ...
      for (final Atom atom : block.getAccepted()) prepare(new Atom(atom), block.getHeader());
        ...
        if (pendingAtom == null) {
          ...
          if (atom == null)
            throw new ValidationException(
                "Pending atom "
                    + executable
                    + " for executable signal not found in block "
                    + block.getHash());
          ...
          final SubstateCommit substateCommit =
              this.context
                  .getLedger()
                  .getLedgerStore()
                  .search(StateAddress.from(Atom.class, executable));
          if (substateCommit == null)
            throw new ValidationException(
                "Substate for pending atom "
                    + executable
                    + " with executable signal not found in block "
                    + block.getHash());
        ...
        if (pendingAtom == null) {
          ...
          if (atom == null)
            throw new ValidationException(
                "Pending atom "
                    + latent
                    + " for latent execution signal not found in block "
                    + block.getHash());
          ...
          final SubstateCommit substateCommit =
              this.context
                  .getLedger()
                  .getLedgerStore()
                  .search(StateAddress.from(Atom.class, latent));
          if (substateCommit == null)
            throw new ValidationException(
                "Substate for pending atom "
                    + latent
                    + " with latent execution signal not found in block "
                    + block.getHash());
      ...
      for (final AtomTimeout timeout : timeouts) {
        ...
        if (pendingAtom == null) {
          final Atom atom =
              this.context.getLedger().getLedgerStore().get(timeout.getAtom(), Atom.class);
          if (atom == null)
            throw new ValidationException(
                timeout,
                "Pending atom "
                    + timeout.getAtom()
                    + " not found for timeout "
                    + timeout.getHash()
                    + " in block "
                    + block.getHash());
          ...
          final SubstateCommit substateCommit =
              this.context
                  .getLedger()
                  .getLedgerStore()
                  .search(StateAddress.from(Atom.class, timeout.getAtom()));
          if (substateCommit == null)
            throw new ValidationException(
                timeout,
                "Commit for pending atom "
                    + timeout.getAtom()
                    + " with timeout "
                    + timeout.getHash()
                    + " found in block "
      ...
      // IMPORTANT: Needed so that remaining SVCs post sync can be completed and validated
      // correctly.
      for (final AtomTimeout timeout : timeouts) {
        ...
        if (pendingAtom == null)
          throw new ValidationException(
              ...
              "Pending atom "
                  + timeout.getAtom()
                  + " not found when processing timeout in block "
                  + block.getHash());
    ...
    syncLog.info(
        this.context.getName()
            + ": State accumulator prepared "
            + this.syncAccumulator.locked().size()
            + " locked "
            + this.syncAccumulator.checksum());
  }

  private void commit(final SyncBranch branch)
      throws IOException,
          ValidationException,
          LockException,
          CryptoException,
    ...
    while (blockHeaderIterator.hasNext()) {
      ...
      for (final PolyglotPackage pakage : block.getPackages()) {
        if (this.context.getLedger().getLedgerStore().has(pakage.getHash(), PolyglotPackage.class)
            == false) this.context.getLedger().getLedgerStore().store(pakage);
        else
          syncLog.warn(
              this.context.getName()
                  + ": Package "
                  ...
                  + "@"
                  + header.getHash()
                  + " already seen and persisted before de-sync");
      }
      ...
      for (final AtomCertificate certificate : block.getCertificates()) {
        ...
        if (substateCommit == null)
          throw new ValidationException(
              ...
              "Atom "
                  + certificate.getAtom()
                  + " required for provisioning by certificate "
                  + certificate.getHash()
                  + " not committed");
        ...
        final Atom atom =
            this.context
                .getLedger()
                .getLedgerStore()
                .get(substateCommit.getSubstate().<Hash>get(NativeField.ATOM), Atom.class);
        if (atom == null)
          throw new ValidationException(
              certificate,
              "Atom "
        ...
        final BlockHeader commitHeader =
            this.context
                .getLedger()
                .getLedgerStore()
                .get(substateCommit.getSubstate().<Hash>get(NativeField.BLOCK), BlockHeader.class);
        if (commitHeader == null)
          throw new ValidationException(
              certificate,
              "Atom "
      ...
      for (final Hash executable : block.getExecutables()) {
        ...
        if (pendingAtom == null)
          throw new ValidationException(
              "Pending atom "
                  + executable
                  + " not found when processing execution signal in block "
                  + block.getHash());

        if (pendingAtom.getStatus().before(AtomStatus.State.ACCEPTED)
            || pendingAtom.getStatus().after(AtomStatus.State.FINALIZING))
          syncLog.warn(
              this.context.getName()
                  + ": Pending atom "
                  + executable
                  + " has invalid execution status "
      ...
      for (final Hash latent : block.getLatent()) {
        ...
        if (pendingAtom == null)
          throw new ValidationException(
              "Pending atom "
                  + latent
                  + " not found when processing execution latency signal in block "
                  + block.getHash());
        ...
        if (pendingAtom.getStatus().before(AtomStatus.State.ACCEPTED)
          ...
          syncLog.warn(
              this.context.getName()
                  + ": Pending atom "
                  + latent
                  + " has invalid execution latency status "
                  + pendingAtom.getStatus());
      ...
      for (final Atom atom : block.getUnaccepted()) {
        ...
        if (pendingAtom != null)
          throw new ValidationException(
              ...
              "Found pending atom "
                  + atom.getHash()
                  + " when processing unaccepted in block "
                  + block.getHash());
      }

      // Store the block header in preparation for a commit
      if (this.context.getLedger().getLedgerStore().has(header.getHash(), BlockHeader.class)
          == false) this.context.getLedger().getLedgerStore().store(header);
      else
        syncLog.warn(
            this.context.getName()
                + ": Block header "
  ...
  private PendingAtom prepare(final Atom atom, final BlockHeader header)
      throws ManifestException, IOException, StateMachineException, LockException {
    ...
    if (this.atoms.putIfAbsent(pendingAtom.getHash(), pendingAtom) != null)
      throw new IllegalStateException(
          "Pending atom "
              + pendingAtom.getHash()
              + " is already known and potentially loaded & prepared");
  ...
  private void provision(
      final PendingAtom pendingAtom,
      final Collection<StateInput> stateInputs,
      final Collection<PolyglotPackage> packages)
      throws StateMachinePreparationException {
    pendingAtom.provision();

    // Provision all the state inputs (will load any known or embedded packages too)
    for (final StateInput stateInput : stateInputs) pendingAtom.provision(stateInput);
    ...
    if (pendingAtom.isProvisioned() == false) {
      ...
      if (loadedPackages.isEmpty() == false && syncLog.hasLevel(Logging.DEBUG))
        syncLog.debug(
            this.context.getName()
                + ": Loaded packages for atom "
                + pendingAtom.getHash()
                + " "
                + loadedPackages.stream().map(p -> p.getAddress()).collect(Collectors.toList()));
    }
  }
  ...
  private SyncStatus checkSynced() throws IOException {
    ...
    if (this.isSingleton == false) {
      final StandardConnectionFilter standardPeerFilter =
          StandardConnectionFilter.build(this.context)
              .setShardGroupID(localShardGroupID)
              .setStates(ConnectionState.CONNECTED);
      ...
      if (syncPeers.isEmpty()) {
        if (isSynced()) {
          syncLog.info(
              this.context.getName()
                  + ": Out of sync state triggered due to no sync peers available");
          syncLost();
        }
    ...
    } else syncPeers = Collections.emptyList();
    ...
    if (syncStatus.equals(SyncStatus.SYNCED)) {
      ...
      if (isUnsynced == false && this.isSingleton == false) {
        ...
        for (final AbstractConnection syncPeer : syncPeers) {
          if (syncPeer
                  .getNode()
                  .getHead()
                  .isInRange(this.context.getLedger().getHead(), Constants.OOS_TRIGGER_LIMIT_BLOCKS)
              == true) continue;

          if (syncPeer
                  .getNode()
                  .getHead()
                  .isAheadOf(this.context.getLedger().getHead(), Constants.OOS_TRIGGER_LIMIT_BLOCKS)
              == true) {
            isUnsynced = true;
            break;
          }
          ...
          if (this.context
                      ...
                      .getHead()
                      .getHash()
                      .equals(Universe.getDefault().getGenesis().getHash())
                  == true
              && syncPeer
                      .getNode()
                      .getHead()
                      .isAheadOf(
                          this.context.getLedger().getHead(), Constants.OOS_RESOLVED_LIMIT_BLOCKS)
                  == true) {
            isUnsynced = true;
            break;
          }
        ...
        if (syncPeers.isEmpty()) isUnsynced = true;
      } else isUnsynced = this.isSingleton == false;

      if (isUnsynced) {
        syncLog.info(
            this.context.getName()
                + ": Out of sync state detected with OOS_TRIGGER limit of "
                + Constants.OOS_TRIGGER_LIMIT_BLOCKS);
        System.out.println(
            this.context.getName()
                + ": Out of sync state detected with OOS_TRIGGER limit of "
                + Constants.OOS_TRIGGER_LIMIT_BLOCKS);
        ...
        synchronized (this.blocks) {
          if (this.blocks.isEmpty() == false) {
            for (final Block block : this.blocks.values()) {
              if (block.getHeader().getHeight() > syncTip.getHeight()) syncTip = block.getHeader();
            }
            ...
            if (syncLog.hasLevel(Logging.INFO))
              syncLog.info(
                  this.context.getName()
                      ...
                      + syncTip
                      + " sync head is "
                      + this.syncHead.toString());

            // Not finished syncing blocks
            if (syncTip.equals(this.syncHead) == false) return syncStatus;
          } else
            syncLog.info(
                this.context.getName() + ": Sync tip is sync head is " + this.syncHead.toString());
        }

        AbstractConnection strongestPeer = null;
        for (final AbstractConnection syncPeer : syncPeers) {
          if (strongestPeer == null
              || syncPeer.getNode().getHead().getHeight()
                  > strongestPeer.getNode().getHead().getHeight()) strongestPeer = syncPeer;
        }

        if (strongestPeer != null
            && syncTip.isAheadOf(strongestPeer.getNode().getHead(), 0) == true)
      ...
      if (syncStatus.equals(SyncStatus.SYNCED)) {
        ...
        final List<AbstractConnection> allPeers =
            this.context
                .getNetwork()
                .get(
                    StandardConnectionFilter.build(this.context)
        ...
        for (final AbstractConnection connectedPeer : allPeers) {
          ShardGroupID shardGroupID =
              ShardMapper.toShardGroup(connectedPeer.getNode().getIdentity(), numShardGroups);
        ...
        for (int i = 0; i < shardsConnected.length; i++) {
          ...
          if (this.isSingleton && i == localShardGroupID.intValue()) continue;
        ...
        if (syncStatus.equals(SyncStatus.SYNCED)) {
          ...
          for (final AbstractConnection syncPeer : syncPeers) {
            try {
              ...
              syncLog.error(
                  this.context.getName()
                      + ": Unable to send SyncAcquiredMessage "
                      + syncAcquiredMessage
                      + " to "
                      + syncPeer,
                  ex);
          ...
          syncLog.info(
              this.context.getName() + ": Synced state reaquired " + this.syncHead.toString());
        }
      }
    }
  ...
  void createStateVoteCollector(final Block block) throws IOException {
    ...
    final long votePower =
        this.context
            .getLedger()
            .getValidatorHandler()
            .getVotePower(epoch, this.context.getNode().getIdentity());
    final long voteThreshold =
        this.context
            .getLedger()
            .getValidatorHandler()
            .getVotePowerThreshold(epoch, localShardGroupID);

    // Create state vote collectors //
    final Multimap<Integer, PendingAtom> atomsByStateCount = LinkedHashMultimap.create();
    for (final Hash atom : block.getHeader().getInventory(InventoryType.ACCEPTED)) {
      final PendingAtom pendingAtom = this.atoms.get(atom);
      if (pendingAtom == null)
        throw new IllegalStateException(
            "Pending atom " + atom + " not found for block " + block.getHeader());

      if (StateVoteCollector.DEBUG_SIMPLE_UNGROUPED)
        atomsByStateCount.put(Integer.MAX_VALUE, pendingAtom);
      else atomsByStateCount.put(pendingAtom.numStateAddresses(null), pendingAtom);
    }

    for (final Integer stateCount : atomsByStateCount.keySet()) {
      final List<PendingState> stateVoteCollectorStates = new ArrayList<PendingState>();
      for (final PendingAtom pendingAtom : atomsByStateCount.get(stateCount)) {
        if (pendingAtom.getStatus().before(AtomStatus.State.ACCEPTED))
          throw new IllegalStateException("Pending atom " + pendingAtom + " is not ACCEPTED");

        pendingAtom.forStates(
            StateLockMode.WRITE,
            pendingState -> {
              final ShardGroupID provisionShardGroupID =
                  ShardMapper.toShardGroup(pendingState.getAddress(), numShardGroups);
              if (provisionShardGroupID.equals(localShardGroupID) == false) return;
      ...
      if (stateVoteCollectorStates.isEmpty() == false) {
        if (syncLog.hasLevel(Logging.DEBUG))
          syncLog.debug(
              this.context.getName()
                  + ": Creating StateVoteCollector for proposal "
                  + block.toString()
                  + " with state keys "
                  + stateVoteCollectorStates.stream()
                      .map(sk -> sk.getAddress())
                      .collect(Collectors.toList()));

        final StateVoteCollector stateVoteCollector =
            new StateVoteCollector(
                this.context, block.getHash(), stateVoteCollectorStates, votePower, voteThreshold);
  ...
  private Collection<StateVoteCollector> removeStaleStateVoteCollectors() {
    ...
    synchronized (this.stateVoteCollectors) {
      final Iterator<StateVoteCollector> stateVoteCollectorIterator =
          this.stateVoteCollectors.values().iterator();
  ...
  private void vote(final PendingAtom pendingAtom) throws IOException {
    ...
    for (final PendingState pendingState : pendingAtom.getStates(StateLockMode.WRITE)) {
      final ShardGroupID stateShardGroupID =
          ShardMapper.toShardGroup(pendingState.getAddress(), numShardGroups);
      if (stateShardGroupID.equals(localShardGroupID) == false) continue;

      final StateVoteCollector stateVoteCollector = pendingState.getStateVoteCollector();
      if (stateVoteCollector == null) {
        if (pendingAtom.getBlockHeader().getHeight()
            > (this.desyncHead.getHeight() - Constants.OOS_PREPARE_BLOCKS))
          syncLog.error(
              this.context.getName()
                  + ": State vote collector for "
                  + pendingState.getAddress()
                  + " in atom "
                  + pendingAtom.getAtom()
                  + " in block "
                  + pendingState.getBlockHeader()
                  + " not found");
  ...
  private void processExecuteLatent(final BlockHeader header, final PendingAtom pendingAtom)
      throws IOException {
    if (pendingAtom.getStatus().after(AtomStatus.State.PREPARED) == false)
      throw new IllegalStateException("Pending atom " + pendingAtom.getHash() + " is not ACCEPTED");

    if (pendingAtom.isExecuteSignalled()
        || pendingAtom.getStatus().after(AtomStatus.State.PROVISIONED))
      throw new IllegalStateException(
          "Pending atom " + pendingAtom.getHash() + " is signalled executable");
    ...
    for (final PendingState pendingState : pendingAtom.getStates(StateLockMode.WRITE)) {
      ...
      if (stateVoteCollector != null) {
        ...
        if (stateVoteCollector.isCompletable())
          throw new IllegalStateException(
              "SVC "
                  + stateVoteCollector
                  + " is completable containing latent state "
                  + pendingState);

        if (stateVoteCollector.remove(pendingState) == false)
          throw new IllegalStateException(
              "Latent state " + pendingState + " not removed from SVC " + stateVoteCollector);
      ...
      final long voteThreshold =
          this.context
              ...
              .getValidatorHandler()
              .getVotePowerThreshold(epoch, localShardGroupID);
      final StateVoteCollector latentStateVoteCollector =
          new StateVoteCollector(
              this.context,
              pendingAtom.getBlockHeader().getHash(),
              pendingState,
              votePower,
              voteThreshold);
      this.stateVoteCollectors.put(
          pendingAtom.getBlockHeader().getHash(), latentStateVoteCollector);
  ...
  private void processTimeout(final PendingAtom pendingAtom, final AtomTimeout timeout) {
    ...
    for (final PendingState pendingState : pendingAtom.getStates(StateLockMode.WRITE)) {
      ...
      if (stateVoteCollector == null) {
        if (pendingAtom.getBlockHeader().getHeight()
            > this.context.getLedger().getHead().getHeight())
          syncLog.error(
              this.context.getName()
                  + ": SVC not found for timeout "
                  + timeout
                  + " of atom "
                  + pendingAtom.getHash()
                  + " with state "
  ...
  public boolean isSyncedInRange(final int range) {
    ...
    // Forced desync?
    if (this.forcedDesyncDuration > 0) return false;

    if (this.context.getConfiguration().get("ledger.synced.always", false) == true) return true;
    ...
    final StandardConnectionFilter standardPeerFilter =
        StandardConnectionFilter.build(this.context)
            ...
            .setStates(ConnectionState.CONNECTED);
    syncPeers = this.context.getNetwork().get(standardPeerFilter);
    if (syncPeers.isEmpty()) return false;
  ...
  SyncAcquiredMessage syncAcquired() {
    ...
    try {
      ...
      // Send a SyncAcquiredEvent to all listeners.  SyncAcquiredEvent listeners MUST be synchronous
      // listeners!
      this.context
          .getEvents()
          .post(
              new SyncAcquiredEvent(
                  this.syncHead,
                  this.syncAccumulator,
                  this.atoms.values(),
                  this.stateVoteCollectors.values()));

      BlockHeader lowSyncHead = this.syncHead;
      for (final PendingAtom pendingAtom : this.atoms.values()) {
        if (pendingAtom.getBlockHeader() == null) continue;

        if (pendingAtom.getBlockHeader().getHeight() >= lowSyncHead.getHeight()) continue;
  ...
  // PEER LISTENER //
  private EventListener peerListener =
      new EventListener() {
        @Subscribe
        public void on(final DisconnectedEvent event) {
          ...
          try {
            ...
            synchronized (SyncHandler.this.requestTasks) {
              final Collection<SyncRequestTask> requestTasks =
                  SyncHandler.this.requestTasks.removeAll(event.getConnection());
              for (SyncRequestTask requestTask : requestTasks) {
                try {
                  if (requestTask.isCancelled() == false) requestTask.cancel();

                  syncLog.info(
                      SyncHandler.this.context.getName()
                          + ": Cancelled block sync task requesting block "
                          + requestTask.requestedBlock
                          + " from "
                          + event.getConnection());
                } catch (Throwable t) {
                  syncLog.error(
                      SyncHandler.this.context.getName()
                          + ": Failed to cancel block sync task requesting block "
                          + requestTask.requestedBlock
                          + " from "
                          + event.getConnection());
                }
            ...
            final SyncInventoryTask inventoryTask =
                SyncHandler.this.inventoryTasks.remove(event.getConnection());
            if (inventoryTask != null) {
              try {
                if (inventoryTask.isCancelled() == false) inventoryTask.cancel();

                syncLog.info(
                    SyncHandler.this.context.getName()
                        + ": Cancelled inventory sync task of "
                        + inventoryTask.requestSeq
                        + ":"
                        + inventoryTask.block
                        + " of blocks from "
                        + event.getConnection());
                ...
                syncLog.error(
                    SyncHandler.this.context.getName()
                        + ": Failed to cancel inventory sync task of "
                        + inventoryTask.requestSeq
                        + ":"
                        + inventoryTask.block
                        + " of blocks from "
