...
public class SubstateLog implements StateAddressable {
  ...
  protected SubstateLog(final StateMachine stateMachine, final Substate input, boolean immutable) {
    ...
    } else this.inputs = Collections.emptyMap();

    if (immutable == true) {
      this.outputs = Collections.emptyMap();
      this.sealed = true;
    } else this.outputs = Maps.mutable.ofInitialCapacity(8);
  ...
  public Identity setAuthority(final Identity authority, final Signature signature)
  ...
  public boolean exists(String field) {
    ...
    synchronized (this) {
      ...
      if (value == null) value = this.inputs.get(field);
    ...
    synchronized (this) {
      ...
      if (value == null) value = def;
        ...
        if (value instanceof Collection) {
          ...
          // It is a private class, so just convert it to ArrayList.  Do the same for all other
          // private collections.
          // TODO better methods
          if (Modifier.isPrivate(clazz.getModifiers()))
            ...
            try {
              ...
              stateMachineLog.error(
                  this.stateMachine.getContext().getName()
                      + ": Failed to clone substate Collection of type "
                      + clazz,
                  ex);
              value = (T) new ArrayList<Object>((Collection<Object>) value);
            }
          ...
          Class<? extends Map<Object, Object>> clazz =
              (Class<? extends Map<Object, Object>>) value.getClass();

          // Quite a few uses of Collections.singletonMap() for code readability.
          // It is a private class, so just convert it to HashMap.  Do the same for all other
          // private collections.
          // TODO better methods
          if (Modifier.isPrivate(clazz.getModifiers()))
            ...
            try {
              ...
              stateMachineLog.error(
                  this.stateMachine.getContext().getName()
                      + ": Failed to clone substate Map of type "
                      + clazz,
                  ex);
              value = (T) new HashMap<Object, Object>((Map<Object, Object>) value);
            }
  ...
  private void read(final String field, final Object value) {
    synchronized (this) {
      ...
      if (instruction == -1) throw new IllegalStateException("Instruction is invalid");
  ...
  public SubstateTransitions toSubstateTransitions() {
    if (this.sealed == false)
      throw new IllegalStateException(
          "Substate log must be sealed to create substate transitions " + this.address);

    if (isVoid() == true && isTouched() == false) return this.untouched;

    final SubstateTransitions transitions = this.transitions.get(Integer.MAX_VALUE);
    if (transitions == null) return this.untouched;
  ...
  public SubstateTransitions toSubstateTransitions(final int instruction) {
    ...
    if (this.sealed == false)
      throw new IllegalStateException(
          "Substate log must be sealed to create substate transitions for instruction "
              + instruction
              + " at "
              + this.address);
