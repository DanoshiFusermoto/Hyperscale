...
public final class StateMachineStatus
{
	public enum State
	{
		NONE(0), PREPARED(1), PROVISIONED(2), EXECUTING(3), COMPLETED(4);
	...
	{
		if (this.thrown != null)
			throw new IllegalStateException("State machine exception already set for "+this.stateMachine.getPendingAtom().getHash(), this.thrown);
	...
	synchronized State set(final State state)
	{
		...
		if (this.state.equals(state))
			throw new IllegalStateException("State machine for atom "+this.stateMachine.getPendingAtom().getHash()+" is already set to "+state);
		
		// TODO review FINALIZED skipping needed by sync
		if (state.equals(StateMachineStatus.State.COMPLETED) == false && this.state.index() < state.index()-1 && this.thrown == null)
			throw new IllegalStateException("State machine for atom "+this.stateMachine.getPendingAtom().getHash()+" can not set to state "+state+" from "+this.state);
