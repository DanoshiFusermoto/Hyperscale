...
public final class SubstateRequestHandler implements Service {
  ...
  private static final int SUBSTATE_RECORD_LIFETIME_MS =
      Constants.BLOCK_INTERVAL_TARGET_MILLISECONDS * (Constants.MIN_COMMIT_ROUNDS / 2);
  ...
  final class SubstateRequest extends CompletableFuture<Substate> {
    ...
    boolean isTimeout() {
      ...
      else return (System.currentTimeMillis() - this.reattemptedAt) > this.timeout;
    }
    ...
    public boolean complete(final Substate substate) {
      ...
      if (this.reattempts == 0) this.connection.updateLatency(this.witnessedAt - this.requestedAt);
      else this.connection.updateLatency(this.witnessedAt - this.reattemptedAt);
    ...
    private void reattempt(final AbstractConnection connection, long timeout, TimeUnit timeunit)
        throws IOException {
      send(connection);
    ...
    private void request(final AbstractConnection connection, long timeout, TimeUnit timeunit)
        throws IOException {
      send(connection);
      this.timeout = TimeUnit.MILLISECONDS.convert(timeout, timeunit);
    }
    ...
    private void send(final AbstractConnection connection) throws IOException {
      ...
      final GetSubstateMessage getSubstateMessage =
          new GetSubstateMessage(this.address, this.version);
      SubstateRequestHandler.this
          .context
          .getNetwork()
          .getMessaging()
          .send(getSubstateMessage, connection);
  ...
  private final ScheduledExecutable timeoutExecutor =
      new ScheduledExecutable(
          0, Constants.BLOCK_INTERVAL_TARGET_MILLISECONDS, TimeUnit.MILLISECONDS) {
        ...
        public void execute() {
          ...
          try {
            for (final SubstateRequest substateRequest :
                SubstateRequestHandler.this.pendingRequests.values()) {
              if (substateRequest.isTimeout() == false) continue;
            ...
            final Iterator<SubstateRequest> completedRequestsIterator =
                SubstateRequestHandler.this.completedRequests.values().iterator();
            while (completedRequestsIterator.hasNext()) {
              final SubstateRequest completedSubstateRequest = completedRequestsIterator.next();

              // Expired?
              if (System.currentTimeMillis() - completedSubstateRequest.getWitnessedAt()
                  < SubstateRequestHandler.SUBSTATE_RECORD_LIFETIME_MS) continue;
  ...
  SubstateRequestHandler(final Context context) {
    ...
    this.lock =
        new MonitoredReentrantLock(this.context.getName() + " Substate Request Handler Lock", true);
  ...
  public void start() throws StartupException {
    ...
    this.context
        ...
        .getMessaging()
        .register(
            GetSubstateMessage.class,
            this.getClass(),
            new MessageProcessor<GetSubstateMessage>() {
              @Override
              public void process(
                  final GetSubstateMessage getSubstateMessage,
                  final AbstractConnection connection) {
                final int numShardGroups =
                    SubstateRequestHandler.this.context.getLedger().numShardGroups();
                final ShardGroupID localShardGroupID =
                    ShardMapper.toShardGroup(
                        SubstateRequestHandler.this.context.getNode().getIdentity(),
                        numShardGroups);
                final ShardGroupID substateShardGroupID =
                    ShardMapper.toShardGroup(getSubstateMessage.getAddress(), numShardGroups);
                if (ShardMapper.equal(
                        SubstateRequestHandler.this.context.getLedger().numShardGroups(),
                        getSubstateMessage.getAddress(),
                        SubstateRequestHandler.this.context.getNode().getIdentity())
                    == false) {
                  gossipLog.error(
                      SubstateRequestHandler.this.context.getName()
                          + ": Request from "
                          + connection
                          + " for substate in shard group "
                          + substateShardGroupID
                          + " but local is "
                          + localShardGroupID);
                  // Disconnect and ban?
                  return;
                ...
                if (SubstateRequestHandler.this
                            .context
                            .getConfiguration()
                  ...
                  gossipLog.warn(
                      SubstateRequestHandler.this.context.getName()
                          + ": Not delivering substate "
                          + getSubstateMessage.getAddress()
                          + " version "
                          + getSubstateMessage.getVersion()
                ...
                try {
                    ...
                    final StateInput stateInput =
                        SubstateRequestHandler.this
                            ...
                            .get(
                                Hash.hash(
                                    getSubstateMessage.getVersion(),
                                    getSubstateMessage.getAddress().getHash()),
                                StateInput.class);
                    if (stateInput != null) substate = stateInput.getSubstate();
                  }
                  ...
                  if (substate == null) {
                    ...
                    substate =
                        SubstateRequestHandler.this
                            ...
                            .getLedger()
                            .getLedgerStore()
                            .get(getSubstateMessage.getAddress());
                  }
                  ...
                  SubstateMessage substateMessage =
                      new SubstateMessage(substate, getSubstateMessage.getVersion());
                  SubstateRequestHandler.this
                      .context
                      .getNetwork()
                      .getMessaging()
                      .send(substateMessage, connection);
                } catch (Exception ex) {
                  gossipLog.error(
                      SubstateRequestHandler.this.context.getName()
                          + ": "
                          + getSubstateMessage.MSN()
                          + "  "
                          + connection,
                      ex);
                }
    ...
    this.context
        ...
        .getMessaging()
        .register(
            SubstateMessage.class,
            this.getClass(),
            new MessageProcessor<SubstateMessage>() {
              @Override
              public void process(
                  final SubstateMessage substateMessage, final AbstractConnection connection) {
                try {
                  if (SubstateRequestHandler.this.context.getNode().isSynced() == false) return;

                  if (gossipLog.hasLevel(Logging.INFO))
                    gossipLog.info(
                        SubstateRequestHandler.this.context.getName()
                            + ": Received substate "
                            + substateMessage.getSubstate().getAddress()
                            + " version "
                            + substateMessage.getVersion()
                            + " from "
                            + connection);

                  final Hash lookupHash =
                      requestLookUpHash(
                          substateMessage.getSubstate().getAddress(), substateMessage.getVersion());
                  SubstateRequestHandler.this.lock.lock();
                  try {
                    if (SubstateRequestHandler.this.requestAssignment.remove(connection, lookupHash)
                        == false) {
                      gossipLog.warn(
                          SubstateRequestHandler.this.context.getName()
                              + ": Received unrequested substate "
                              + substateMessage.getSubstate().getAddress()
                              + " version "
                              + substateMessage.getVersion()
                              + " from "
                    ...
                    final SubstateRequest substateRequest =
                        SubstateRequestHandler.this.pendingRequests.remove(lookupHash);
                    if (substateRequest == null) {
                      gossipLog.warn(
                          SubstateRequestHandler.this.context.getName()
                              + ": Substate request not found for "
                              + substateMessage.getSubstate().getAddress()
                              + " version "
                              + substateMessage.getVersion()
                              + " from "
                    ...
                    if (substateRequest.reattempts() > 0)
                      gossipLog.log(
                          SubstateRequestHandler.this.context.getName()
                              + ": Substate request fulfilled on attempt "
                              + substateRequest.reattempts()
                              + " for "
                              + substateMessage.getSubstate().getAddress()
                              + " version "
                              + substateMessage.getVersion()
  ...
  private Hash requestLookUpHash(final StateAddress address, final Hash version) {
    if (version.equals(Hash.ZERO)) return address.getHash();
    else return PendingState.getHash(version, address);
  }
  ...
  SubstateRequest request(final StateAddress address, final Hash version) throws IOException {
    ...
    try {
      ...
      if (completedRequestFuture != null) {
        // Expired?
        if (System.currentTimeMillis() - completedRequestFuture.getWitnessedAt()
            < SubstateRequestHandler.SUBSTATE_RECORD_LIFETIME_MS) return completedRequestFuture;

        if (this.completedRequests.remove(lookupHash, completedRequestFuture) == false)
          gossipLog.warn(
              this.context.getName()
                  + ": Expired substate request not removed for "
                  + completedRequestFuture.getAddress()
                  + " with version hash "
                  + completedRequestFuture.getVersion());
      }

      // Is there a current pending request?
      final SubstateRequest existingPendingRequestFuture = this.pendingRequests.get(lookupHash);
      if (existingPendingRequestFuture != null) return existingPendingRequestFuture;
      ...
      final StateInput stateInput =
          this.context.getLedger().getLedgerStore().get(lookupHash, StateInput.class);
      if (stateInput != null) {
      ...
      try {
        long timeout =
            Math.min(
                requestConnection.getNextTimeout(1, TimeUnit.MILLISECONDS),
                Constants.MAX_DIRECT_REQUEST_TIMEOUT_MILLISECONDS);
    ...
    try {
      for (final SubstateRequest request : requests) {
        ...
        try {
          ...
          try {
            if (requestConnection.getState().equals(ConnectionState.CONNECTED)
                || requestConnection.getState().equals(ConnectionState.CONNECTING))
              requestConnection.disconnect(
                  "Did not respond fully to substate request "
                      + request.getAddress()
                      + " version "
                      + request.getVersion());
          } catch (Throwable t) {
            gossipLog.error(
                SubstateRequestHandler.this.context.getName() + ": " + requestConnection.toString(),
                t);
          }
          ...
          if (request.reattempts() == Constants.MAX_DIRECT_ITEM_RETRIES) {
            request.completeExceptionally(
                new IllegalArgumentException(
                    "Re-attempts exceeded for substate request "
                        + request.getAddress()
                        + " version "
                        + request.getVersion()));
            continue;
          ...
          final ShardGroupID substateShardGroupID =
              ShardMapper.toShardGroup(request.getAddress(), numShardGroups);
          if (substateShardGroupID.equals(localShardGroupID)) {
            request.completeExceptionally(
                new IllegalArgumentException(
                    "Shard group for substate request " + request.getAddress() + " is local"));
            continue;
          }
          ...
          final List<AbstractConnection> connections =
              this.context.getNetwork().get(connectionFilter);
          if (connections.isEmpty()) {
            request.completeExceptionally(
                new IOException(
                    "No remote connections available to re-request substate "
                        + request.getAddress()
                        + " version "
                        + request.getVersion()));
            continue;
          ...
          try {
            long timeout =
                Math.min(
                    reattemptConnection.getNextTimeout(1, TimeUnit.MILLISECONDS),
                    Constants.MAX_DIRECT_REQUEST_TIMEOUT_MILLISECONDS);
          ...
          if (request.isDone()) this.pendingRequests.remove(lookupHash, request);
        }
      }
    } finally {
      this.lock.unlock();
  ...
  private SynchronousEventListener peerListener =
      new SynchronousEventListener() {
        ...
        public void on(final DisconnectedEvent event) {
          SubstateRequestHandler.this.lock.lock();
          try {
            final Collection<Hash> requestAssignments =
                SubstateRequestHandler.this.requestAssignment.removeAll(event.getConnection());
            ...
            for (Hash requestAssignmentLookupHash : requestAssignments) {
              final SubstateRequest substateRequest =
                  SubstateRequestHandler.this.pendingRequests.get(requestAssignmentLookupHash);
              if (substateRequest == null) {
                gossipLog.warn(
                    SubstateRequestHandler.this.context.getName()
                        + ": Expected substate request for "
                        + requestAssignmentLookupHash
                        + " not found on disconnect "
                        + event.getConnection());
                continue;
              }
              ...
              if (substateRequest.isDone() == true) {
                gossipLog.warn(
                    SubstateRequestHandler.this.context.getName()
                        + ": Substate request is completed on disconnect for "
                        + requestAssignmentLookupHash
                        + " to "
                        + event.getConnection());
                continue;
