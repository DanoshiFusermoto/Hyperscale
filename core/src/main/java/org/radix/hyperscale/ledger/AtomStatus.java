    ...
    synchronized (this) {
      if (this.state.equals(state))
        throw new IllegalStateException(
            "State of pending atom " + this.atom + " is already set to " + state);

      // TODO review FINALIZED skipping needed by sync
      if (state.equals(AtomStatus.State.FINALIZED) == false
          && state.equals(AtomStatus.State.COMPLETED) == false
          && this.state.index() < state.index() - 1
          && this.exception == null)
        throw new IllegalStateException(
            "Pending atom " + this.atom + " can not set to state " + state + " from " + this.state);
