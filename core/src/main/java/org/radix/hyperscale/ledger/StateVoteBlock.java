...
public final class StateVoteBlock extends ExtendedObject implements EphemeralPrimitive
{
	...
	public StateVoteBlock(final Hash ID, final Collection<StateVote> votes)
	{
		...
		for (StateVote vote : votes)
		{
			if (expectedBlock == null)
				expectedBlock = vote.getBlock();
			else if (vote.getBlock().equals(expectedBlock) == false)
				throw new IllegalStateException("Expected vote block "+expectedBlock+" but found "+vote.getBlock());

			if (expectedMerkle == null)
				expectedMerkle = vote.getVoteMerkle();
			else if (vote.getVoteMerkle().equals(expectedMerkle) == false)
				throw new IllegalStateException("Expected vote merkle "+expectedMerkle+" but found "+vote.getVoteMerkle());
				
			if (expectedKey == null)
				expectedKey = vote.getOwner();
			else if (vote.getOwner().equals(expectedKey) == false)
				throw new IllegalStateException("Expected owner "+expectedKey+" but found "+vote.getOwner());

			if (expectedSignature == null)
				expectedSignature = vote.getSignature();
			else if (vote.getSignature().equals(expectedSignature) == false)
				throw new IllegalStateException("Expected signature "+expectedSignature+" but found "+vote.getSignature());
	...
	{
		return getAge(TimeUnit.MILLISECONDS) > TimeUnit.MILLISECONDS.convert(Constants.PRIMITIVE_STALE, TimeUnit.MINUTES);
	}
	
	public List<StateAddress> getStateAddresses()
	{
		List<StateAddress> stateAddresses = new ArrayList<StateAddress>(this.votes.size());
		for (StateVote vote : this.votes)
			stateAddresses.add(vote.getAddress());
	...
	List<StateVote> deriveVotes(final StateVoteBlock referenceStateVoteBlock, final long votePower) throws ValidationException
	{
		if (referenceStateVoteBlock == this)
			return this.votes;
		...
		if (this.block.equals(referenceStateVoteBlock.block) == false)
			throw new ValidationException(referenceStateVoteBlock, "Target block "+this.block+" mismatch in "+referenceStateVoteBlock);

		if (this.merkle.equals(referenceStateVoteBlock.merkle) == false)
			throw new ValidationException(referenceStateVoteBlock, "Vote merkle "+this.merkle+" mismatch in "+referenceStateVoteBlock);
		
		if (this.states.equals(referenceStateVoteBlock.states) == false)
			throw new ValidationException(referenceStateVoteBlock, "State merkle "+this.states+" mismatch in "+referenceStateVoteBlock);
		
		if (this.size != referenceStateVoteBlock.size)
			throw new ValidationException(referenceStateVoteBlock, "State size "+this.size+" mismatch in "+referenceStateVoteBlock);
	...
	public void validate() throws ValidationException
	{
		...
		for (StateVote stateVote : this.votes)
			merkleTree.appendLeaf(stateVote.getVoteMerkle());
		
		Hash merkleHash = merkleTree.buildTree();
		if (merkleHash.equals(this.merkle) == false)
			throw new ValidationException(this, "Expected merkle hash "+this.merkle+" but computed "+merkleHash);
	}
	...
	{
		return getHash()+" "+size()+" "+getHeight()+"@"+getBlock()+" -> "+getID()+":"+getStateMerkle()+":"+getExecutionMerkle()+" "+getOwner();
	}
}
