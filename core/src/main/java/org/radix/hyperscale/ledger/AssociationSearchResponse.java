...
public final class AssociationSearchResponse extends BasicObject
{
	...
	public AssociationSearchResponse(final AssociationSearchQuery query, final long nextOffset, final Collection<SubstateCommit> results, final boolean EOR)
	{ 
		super();
		
		this.nextOffset = nextOffset;
		this.query = Objects.requireNonNull(query, "Association search query is null");
		this.results = new LinkedHashSet<SubstateCommit>(Objects.requireNonNull(results, "Association search results is null"));
		this.EOR = EOR;
	}
	...
	public boolean equals(Object object) 
	{
		...
		if (object instanceof AssociationSearchResponse associationSearchResponse)
		{
			...
			if (this.query.equals(associationSearchResponse.getQuery()) == false)
				return false;

			if (this.nextOffset != associationSearchResponse.getNextOffset())
				return false;

			if (this.EOR != associationSearchResponse.isEOR())
				return false;
			
			if (this.results.equals(associationSearchResponse.getResults()) == false)
				return false;
