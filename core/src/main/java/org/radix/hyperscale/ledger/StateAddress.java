...
public final class StateAddress implements Comparable<StateAddress>, Hashable {
  // Contexts are frequently the same, so cache the most popular to save an actual digest
  // computation when generating a hash
  private static final LRUCacheMap<String, Hash> contextHashCache =
      new LRUCacheMap<String, Hash>(1 << 10);
  ...
  private static final byte[] getCacheBytes(final String context) {
    ...
    if (contextLength > 12) return context.getBytes(StandardCharsets.UTF_8);

    synchronized (stringCache) {
      // Simple array-based cache lookup
      for (int i = 0; i < CONTEXT_CACHE_SIZE && stringCache[i] != null; i++) {
        if (contextLength == stringCache[i].length()) {
          if (context.equals(stringCache[i])) return bytesCache[i];
        }
      }
  ...
  public static final StateAddress from(final Class<?> context, final String scope) {
    return from(
        context, Hash.valueOf(Strings.toLowerCase(Objects.requireNonNull(scope, "Scope is null"))));
  }

  public static final StateAddress from(final Class<?> context, final Identity identity) {
    return from(
        Objects.requireNonNull(context, "Class for context is null")
  ...
  public static final StateAddress from(final String context, final Identity identity) {
    return new StateAddress(
        context, Hash.valueOf(Objects.requireNonNull(identity, "Identity is null")));
  }
  ...
  public static StateAddress from(final byte[] bytes, final int offset) {
    ...
    final String context = checkCache(bytes, p, contextLength);
    p += contextLength;
  ...
  public synchronized ShardID getShardID() {
    if (this.shardID == null) this.shardID = ShardID.from(scope);
  ...
  public boolean equals(Object obj) {
    ...
    if (obj instanceof StateAddress other) {
      if (this.scope.equals(other.scope) == false) return false;

      if (this.context.equals(other.context) == false) return false;
