...
public final class PendingBlock implements Hashable {
  ...
  PendingBlock(final Context context, final BlockHeader header) {
    ...
    this.unaccepted =
        Maps.mutable.ofInitialCapacity(header.getInventorySize(InventoryType.UNACCEPTED));
    this.committed =
        Maps.mutable.ofInitialCapacity(header.getInventorySize(InventoryType.COMMITTED));
    this.unexecuted =
        Maps.mutable.ofInitialCapacity(header.getInventorySize(InventoryType.UNEXECUTED));
    this.uncommitted =
        Maps.mutable.ofInitialCapacity(header.getInventorySize(InventoryType.UNCOMMITTED));
    this.executable =
        Maps.mutable.ofInitialCapacity(header.getInventorySize(InventoryType.EXECUTABLE));
    ...
    for (int i = 0; i < types.length; i++) this.absent.addAll(header.getInventory(types[i]));
            ...
            if (discoveredType != null) {
              blocksLog.fatal(
                  this.context.getName()
                      + ": Item "
                      + item
                      + " is discovered in inventory types "
                      + discoveredType
                      + " & "
                      + types[i]
                      + " in block "
              ...
              throw new IllegalStateException(
                  "Non-normalized header, must not contain duplicate references!");
            }
    ...
    int predictedStateCountsToLock =
        header.getInventorySize(InventoryType.ACCEPTED) * STATE_LOCKS_PER_ATOM_MULTIPLIER;
    int predictedStateCountsToUnlock =
        header.getInventorySize(InventoryType.COMMITTED) * STATE_LOCKS_PER_ATOM_MULTIPLIER;
    this.statesLocked =
        Maps.mutable.ofInitialCapacity(
            predictedStateCountsToLock == 0
                ? DEFAULT_STATE_LOCKS_PER_ATOM
                : predictedStateCountsToLock);
    this.statesUnlocked =
        Maps.mutable.ofInitialCapacity(
            predictedStateCountsToUnlock == 0
                ? DEFAULT_STATE_LOCKS_PER_ATOM
                : predictedStateCountsToUnlock);
  }

  PendingBlock(
      final Context context,
      final BlockHeader header,
      final Multimap<InventoryType, Hashable> inventory)
      throws ValidationException {
    this(context, header);

    Objects.requireNonNull(inventory, "Inventory is null");

    if (inventory.containsKey(InventoryType.ACCEPTED))
      inventory
          .get(InventoryType.ACCEPTED)
          .forEach(pa -> this.accepted.put(pa.getHash(), PendingAtom.class.cast(pa)));
    if (inventory.containsKey(InventoryType.UNACCEPTED))
      inventory
          .get(InventoryType.UNACCEPTED)
          .forEach(pa -> this.unaccepted.put(pa.getHash(), PendingAtom.class.cast(pa)));
    if (inventory.containsKey(InventoryType.COMMITTED))
      inventory
          .get(InventoryType.COMMITTED)
          .forEach(pa -> this.committed.put(pa.getHash(), PendingAtom.class.cast(pa)));
    if (inventory.containsKey(InventoryType.UNEXECUTED))
      inventory
          .get(InventoryType.UNEXECUTED)
          .forEach(pa -> this.unexecuted.put(pa.getHash(), PendingAtom.class.cast(pa)));
    if (inventory.containsKey(InventoryType.UNCOMMITTED))
      inventory
          .get(InventoryType.UNCOMMITTED)
          .forEach(pa -> this.uncommitted.put(pa.getHash(), PendingAtom.class.cast(pa)));
    if (inventory.containsKey(InventoryType.EXECUTABLE))
      inventory
          .get(InventoryType.EXECUTABLE)
          .forEach(pa -> this.executable.put(pa.getHash(), PendingAtom.class.cast(pa)));
    if (inventory.containsKey(InventoryType.LATENT))
      inventory
          .get(InventoryType.LATENT)
          .forEach(pa -> this.latent.put(pa.getHash(), PendingAtom.class.cast(pa)));
    if (inventory.containsKey(InventoryType.PACKAGES))
      inventory
          .get(InventoryType.PACKAGES)
          .forEach(pkg -> this.packages.put(pkg.getHash(), PolyglotPackage.class.cast(pkg)));
  ...
  PendingBlock(final PendingBlock other) {
    ...
    if (other.isConstructed() == false)
      throw new IllegalStateException(
          "Can only copy pending blocks via constructor which are constructed");
  ...
  void setInBranch(final PendingBranch branch) throws IOException {
    synchronized (this) {
      ...
      if (PendingBlock.caughtMuchVoteThresholdVeryMath == true) return;
    }
  }

  long getVoteThreshold() {
    synchronized (this) {
      if (this.unbranched) throw new IllegalStateException("Branch not set for " + this);
  ...
  public Block getBlock() {
    if (this.constructing.get()) return null;
  ...
  void constructBlock() throws ValidationException {
    if (this.constructing.compareAndSet(false, true) == false) return;
      ...
      synchronized (this) {
        ...
        if (this.accepted.size() != this.header.getInventorySize(InventoryType.ACCEPTED))
          throw new ValidationException(
              ...
              "Construction is missing "
                  + (this.header.getInventorySize(InventoryType.ACCEPTED) - this.accepted.size())
                  + " ACCEPTED atoms for "
                  + getHash());
        ...
        if (this.unaccepted.size() != this.header.getInventorySize(InventoryType.UNACCEPTED))
          throw new ValidationException(
              getHeader(),
              "Construction is missing "
                  + (this.header.getInventorySize(InventoryType.UNACCEPTED)
                      - this.unaccepted.size())
                  + " UNACCEPTED atoms for "
                  + getHash());
        ...
        if (this.committed.size() != this.header.getInventorySize(InventoryType.COMMITTED))
          throw new ValidationException(
              getHeader(),
              "Construction is missing "
                  + (this.header.getInventorySize(InventoryType.COMMITTED) - this.committed.size())
                  + " CERTIFICATES for "
                  + getHash());
        ...
        if (this.unexecuted.size() != this.header.getInventorySize(InventoryType.UNEXECUTED))
          throw new ValidationException(
              getHeader(),
              "Construction is missing "
                  + (this.header.getInventorySize(InventoryType.UNEXECUTED)
                      - this.unexecuted.size())
                  + " UNEXECUTED for "
                  + getHash());
        ...
        if (this.uncommitted.size() != this.header.getInventorySize(InventoryType.UNCOMMITTED))
          throw new ValidationException(
              getHeader(),
              "Construction is missing "
                  + (this.header.getInventorySize(InventoryType.UNCOMMITTED)
                      - this.uncommitted.size())
                  + " UNCOMMITTED for "
                  + getHash());
        ...
        if (this.executable.size() != this.header.getInventorySize(InventoryType.EXECUTABLE))
          throw new ValidationException(
              getHeader(),
              "Construction is missing "
                  + (this.header.getInventorySize(InventoryType.EXECUTABLE)
                      - this.executable.size())
                  + " EXECUTION signals for "
                  + getHash());
        ...
        if (this.latent.size() != this.header.getInventorySize(InventoryType.LATENT))
          throw new ValidationException(
              getHeader(),
              "Construction is missing "
                  + (this.header.getInventorySize(InventoryType.LATENT) - this.latent.size())
                  + " LATENT EXECTUTION signals for "
                  + getHash());
        ...
        if (this.packages.size() != this.header.getInventorySize(InventoryType.PACKAGES))
          throw new ValidationException(
              getHeader(),
              "Construction is missing "
                  + (this.header.getInventorySize(InventoryType.PACKAGES) - this.packages.size())
                  + " PACKAGE for "
                  + getHash());
        ...
        for (Hash hash : this.header.getInventory(InventoryType.ACCEPTED)) {
          ...
          if (pendingAtom == null)
            throw new ValidationException(
                ...
                "Atom "
                    + hash
                    + " for ACCEPTED is not in inventory during construction of "
                    + getHash());
        ...
        for (Hash hash : this.header.getInventory(InventoryType.UNACCEPTED)) {
          ...
          if (pendingAtom == null)
            throw new ValidationException(
                ...
                "Atom "
                    + hash
                    + " for UNACCEPTED is not in inventory during construction of "
                    + getHash());
        ...
        for (PendingAtom atom : this.committed.values()) {
          if (atom.getCertificate() == null)
            throw new ValidationException(
                ...
                "Atom "
                    + atom.getHash()
                    + " does not have certificate during construction of "
                    + getHash());
          ...
          if (inventory.contains(atom.getCertificate().getHash()) == false)
            throw new ValidationException(
                getHeader(),
                "Atom certificate "
                    + atom.getCertificate().getHash()
                    + " in atom "
                    + atom.getHash()
                    + " is not in inventory during construction of "
                    + getHash());
        ...
        for (PendingAtom atom : this.unexecuted.values()) {
          if (atom.getTimeout() == null
              || ExecutionTimeout.class.isAssignableFrom(atom.getTimeout().getClass()) == false)
            throw new ValidationException(
                getHeader(),
                "Atom "
                    + atom.getHash()
                    + " does not have execution timeout during construction of "
                    + getHash());
        ...
        for (PendingAtom atom : this.uncommitted.values()) {
          if (atom.getTimeout() == null
              || CommitTimeout.class.isAssignableFrom(atom.getTimeout().getClass()) == false)
            throw new ValidationException(
                getHeader(),
                "Atom "
                    + atom.getHash()
                    + " does not have commit timeout during construction of "
                    + getHash());
        ...
        for (Hash hash : this.header.getInventory(InventoryType.PACKAGES)) {
          ...
          if (pakage == null)
            throw new ValidationException(
                getHeader(),
                "Package " + hash + " is not in inventory during construction of " + getHash());

          packages.add(pakage);
        }

        this.accepted.forEach(
            (h, pa) ->
                pa.forStateAddresses(
                    StateLockMode.WRITE, sa -> PendingBlock.this.statesLocked.put(sa, pa)));
        this.committed.forEach(
            (h, pa) ->
                pa.forStateAddresses(
                    StateLockMode.WRITE, sa -> PendingBlock.this.statesUnlocked.put(sa, pa)));
        this.unexecuted.forEach(
            (h, pa) ->
                pa.forStateAddresses(
                    StateLockMode.WRITE, sa -> PendingBlock.this.statesUnlocked.put(sa, pa)));
        this.uncommitted.forEach(
            (h, pa) ->
                pa.forStateAddresses(
                    StateLockMode.WRITE, sa -> PendingBlock.this.statesUnlocked.put(sa, pa)));
        block =
            new Block(
                this.header, accepted, unaccepted, unexecuted, certificates, uncommitted, packages);
    ...
    if (extendedTestMask > 0) {
      synchronized (this) {
        if ((extendedTestMask & InventoryType.COMMITTED.value()) != 0) {
          if (this.committed.containsKey(item)) return InventoryType.COMMITTED;
        }

        if ((extendedTestMask & InventoryType.UNCOMMITTED.value()) != 0) {
          if (this.uncommitted.containsKey(item)) return InventoryType.UNCOMMITTED;
        }

        if ((extendedTestMask & InventoryType.UNEXECUTED.value()) != 0) {
          if (this.unexecuted.containsKey(item)) return InventoryType.UNEXECUTED;
        }
      }
    }
        ...
        if (item instanceof PendingAtom pendingAtom) {
          if (type.equals(InventoryType.COMMITTED)) {
            if (pendingAtom.getCertificate() == null
                || this.absent.contains(pendingAtom.getCertificate().getHash()) == false) continue;
          } else if (type.equals(InventoryType.ACCEPTED)) {
            if (pendingAtom.isPrepared() == false
                || this.absent.contains(pendingAtom.getHash()) == false) continue;
          } else if (type.equals(InventoryType.UNCOMMITTED)) {
            if (pendingAtom.getTimeout() == null
                || this.absent.contains(pendingAtom.getTimeout().getHash()) == false) continue;
            ...
            if (pendingAtom.getTimeout() == null
                || this.absent.contains(pendingAtom.getTimeout().getHash()) == false) continue;
          } else if (this.absent.contains(item.getHash()) == false) continue;
        } else if (item instanceof PolyglotPackage) {
          if (this.absent.contains(item.getHash()) == false) continue;
        } else
          throw new IllegalArgumentException(
              "Unsupported hashable type " + item.getClass() + ":" + item.getHash());
  ...
  <T> boolean put(final T item, final InventoryType type) {
    ...
    synchronized (this) {
      if (type == InventoryType.ACCEPTED) return putAccepted((PendingAtom) item);
      else if (type == InventoryType.UNACCEPTED) return putUnaccepted((PendingAtom) item);
      else if (type == InventoryType.COMMITTED) return putCommitted((PendingAtom) item);
      else if (type == InventoryType.EXECUTABLE) return putExecutable((PendingAtom) item);
      else if (type == InventoryType.LATENT) return putLatent((PendingAtom) item);
      else if (type == InventoryType.UNEXECUTED) return putUnexecuted((PendingAtom) item);
      else if (type == InventoryType.UNCOMMITTED) return putUncommitted((PendingAtom) item);
      else if (type == InventoryType.PACKAGES) return putPackage((PolyglotPackage) item);
    ...
    synchronized (this) {
      if (this.accepted.containsKey(pendingAtom.getHash()) == false) {
        if (this.absent.remove(pendingAtom.getHash()) == false)
          throw new IllegalStateException(
              "Expected to find ACCEPTED " + pendingAtom.getHash() + " as absent");
  ...
  private boolean putUnaccepted(final PendingAtom pendingAtom) {
    ...
    synchronized (this) {
      if (this.unaccepted.containsKey(pendingAtom.getHash()) == false) {
        if (this.absent.remove(pendingAtom.getHash()) == false)
          throw new IllegalStateException(
              "Expected to find UNACCEPTED " + pendingAtom.getHash() + " as absent");
  ...
  private boolean putExecutable(final PendingAtom pendingAtom) {
    ...
    synchronized (this) {
      if (this.executable.containsKey(pendingAtom.getHash()) == false) {
        if (this.absent.remove(pendingAtom.getHash()) == false)
          throw new IllegalStateException(
              "Expected to find EXECUTABLE " + pendingAtom.getHash() + " as absent");
  ...
  private boolean putLatent(final PendingAtom pendingAtom) {
    ...
    synchronized (this) {
      if (this.latent.containsKey(pendingAtom.getHash()) == false) {
        if (this.absent.remove(pendingAtom.getHash()) == false)
          throw new IllegalStateException(
              "Expected to find LATENT " + pendingAtom.getHash() + " as absent");
    ...
    synchronized (this) {
      ...
      if (this.unexecuted.containsKey(pendingAtom.getHash()) == false) {
        if (this.absent.remove(timeout.getHash()) == false)
          throw new IllegalStateException(
              "Expected to find UNEXECUTED " + timeout.getHash() + " as absent");
    ...
    synchronized (this) {
      if (this.committed.containsKey(pendingAtom.getHash()) == false) {
        if (this.absent.remove(pendingAtom.getCertificate().getHash()) == false)
          throw new IllegalStateException(
              "Expected to find " + pendingAtom.getCertificate().getHash() + " as absent");
  ...
  private boolean putUncommitted(final PendingAtom pendingAtom) {
    ...
    synchronized (this) {
      ...
      if (this.uncommitted.containsKey(pendingAtom.getHash()) == false) {
        if (this.absent.remove(timeout.getHash()) == false)
          throw new IllegalStateException(
              "Expected to find UNCOMMITTED " + timeout.getHash() + " as absent");
    ...
    synchronized (this) {
      if (this.packages.containsKey(pakage.getHash()) == false) {
        if (this.absent.remove(pakage.getHash()) == false)
          throw new IllegalStateException(
              "Expected to find PACKAGE " + pakage.getHash() + " as absent");
  ...
  boolean isConstructed() {
    if (this.constructing.get()) return false;
  ...
  List<Hash> getAbsent(final InventoryType type) {
    ...
    synchronized (this) {
      ...
      for (Hash item : this.header.getInventory(type)) {
        if (this.absent.contains(item) == true) absent.add(item);
      }
  ...
  boolean contains(final Hash item, final InventoryType... types) {
    ...
    synchronized (this) {
      ...
      for (int i = 0; i < types.length; i++) {
        ...
        switch (type) {
          case ACCEPTED:
            contains = this.accepted.containsKey(item);
            break;
          case COMMITTED:
            contains = this.committed.containsKey(item);
            break;
          case EXECUTABLE:
            contains = this.executable.containsKey(item);
            break;
          case LATENT:
            contains = this.latent.containsKey(item);
            break;
          case UNACCEPTED:
            contains = this.unaccepted.containsKey(item);
            break;
          case UNCOMMITTED:
            contains = this.uncommitted.containsKey(item);
            break;
          case UNEXECUTED:
            contains = this.unexecuted.containsKey(item);
            break;
          case PACKAGES:
            contains = this.packages.containsKey(item);
            break;
          default:
            throw new IllegalArgumentException("Inventory type " + type + " is not supported");
        }
  ...
  void forInventory(final InventoryType type, final Consumer<PendingAtom> consumer) {
    ...
    synchronized (this) {
      switch (type) {
        case ACCEPTED:
          this.accepted.forEach((h, pa) -> consumer.accept(pa));
          break;
        case COMMITTED:
          this.committed.forEach((h, pa) -> consumer.accept(pa));
          break;
        case EXECUTABLE:
          this.executable.forEach((h, pa) -> consumer.accept(pa));
          break;
        case UNACCEPTED:
          this.unaccepted.forEach((h, pa) -> consumer.accept(pa));
          break;
        case LATENT:
          this.latent.forEach((h, pa) -> consumer.accept(pa));
          break;
        case UNCOMMITTED:
          this.uncommitted.forEach((h, pa) -> consumer.accept(pa));
          break;
        case UNEXECUTED:
          this.unexecuted.forEach((h, pa) -> consumer.accept(pa));
          break;
        default:
          throw new IllegalArgumentException("Inventory type " + type + " is not supported");
      }
  ...
  <T> List<T> get(final InventoryType type) {
    ...
    synchronized (this) {
      switch (type) {
        case ACCEPTED:
          return (List<T>)
              (this.accepted.isEmpty()
                  ? Collections.emptyList()
                  : Collections.unmodifiableList(new ArrayList<>(this.accepted.values())));
        case UNACCEPTED:
          return (List<T>)
              (this.unaccepted.isEmpty()
                  ? Collections.emptyList()
                  : Collections.unmodifiableList(new ArrayList<>(this.unaccepted.values())));
        case COMMITTED:
          return (List<T>)
              (this.committed.isEmpty()
                  ? Collections.emptyList()
                  : Collections.unmodifiableList(new ArrayList<>(this.committed.values())));
        case EXECUTABLE:
          return (List<T>)
              (this.executable.isEmpty()
                  ? Collections.emptyList()
                  : Collections.unmodifiableList(new ArrayList<>(this.executable.values())));
        case LATENT:
          return (List<T>)
              (this.latent.isEmpty()
                  ? Collections.emptyList()
                  : Collections.unmodifiableList(new ArrayList<>(this.latent.values())));
        case UNCOMMITTED:
          return (List<T>)
              (this.uncommitted.isEmpty()
                  ? Collections.emptyList()
                  : Collections.unmodifiableList(new ArrayList<>(this.uncommitted.values())));
        case UNEXECUTED:
          return (List<T>)
              (this.unexecuted.isEmpty()
                  ? Collections.emptyList()
                  : Collections.unmodifiableList(new ArrayList<>(this.unexecuted.values())));
        case PACKAGES:
          return (List<T>)
              (this.packages.isEmpty()
                  ? Collections.emptyList()
                  : Collections.unmodifiableList(new ArrayList<>(this.packages.values())));
        default:
          throw new IllegalArgumentException("Inventory type " + type + " is not supported");
      }
    }
  ...
  public boolean equals(final Object object) {
    ...
    if (object instanceof PendingBlock pendingBlock) {
      if (pendingBlock.getHeader().getHash().equals(this.header.getHash())) return true;
    }
  ...
  BlockCertificate buildCertificate() throws CryptoException {
    synchronized (this) {
      if (this.header.getCertificate() != null) {
        blocksLog.warn(
            this.context.getName() + ": Block header already has a certificate " + this.header);
        return this.header.getCertificate();
      }

      if (this.voteWeight < this.voteThreshold)
        throw new IllegalStateException(
            "Can not build a certificate when vote weight of "
                + this.voteWeight
                + " is less than vote threshold "
                + this.voteThreshold);
      ...
      final BlockCertificate certificate =
          new BlockCertificate(this.header.getHash(), signers, key, signature);
  ...
  boolean vote(final BlockVote vote, final long voteWeight) throws ValidationException {
    ...
    try {
      if (vote.getObject().equals(getHash()) == false)
        throw new ValidationException(
            vote, "Vote from " + vote.getOwner() + " is not for " + getHash());
   ...
   *
   * <p>TODO needs to be more thorough to check cross-references such as unaccepted / accepted
   *
   * @param other
   * @return
   */
  boolean intersects(final PendingBlock other) {
    synchronized (this) {
      synchronized (other) {
        ...
        for (Hash hash : this.unaccepted.keySet())
          if (other.unaccepted.containsKey(hash)) return true;
        for (Hash hash : this.committed.keySet())
          if (other.committed.containsKey(hash)) return true;
        for (Hash hash : this.executable.keySet())
          if (other.executable.containsKey(hash)) return true;
        ...
        for (Hash hash : this.uncommitted.keySet())
          if (other.uncommitted.containsKey(hash)) return true;
        for (Hash hash : this.unexecuted.keySet())
          if (other.unexecuted.containsKey(hash)) return true;
