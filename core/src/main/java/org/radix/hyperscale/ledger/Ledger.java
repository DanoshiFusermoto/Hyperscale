...
public final class Ledger implements Service, LedgerInterface {
  ...
  private void integrity() throws Exception {
    ...
    try {
      ...
      if (headHash.equals(Universe.getDefault().getGenesis().getHeader().getHash()) == true
          && this.ledgerStore.has(headHash, BlockHeader.class) == false) {
        // Some simple manual actions here with regard to persistance & provisioning as don't want
        // to
        ...
        for (Atom atom : Universe.getDefault().getGenesis().getAccepted()) {
          // Need to take a "clean" copy of the atom in the case that multiple contexts are being
          // initialised
          // Otherwise left overs from execution in other contexts may throw exceptions for the
          // current context
          ...
          if (pendingAtom.thrown() != null) throw pendingAtom.thrown();
        ...
        if (header.equals(Universe.getDefault().getGenesis().getHeader()) == false
            && header.getCertificate() == null)
          ledgerLog.warn(
              Ledger.this.context.getName()
                  + ": Ledger head "
                  + header
        ...
        ledgerLog.info(
            Ledger.this.context.getName()
                + ": Setting ledger epoch as "
                + this.epoch.get().getClock());

        // Check continuity of sync blocks
        while (header.getHeight()
            > Math.max(1, this.head.get().getHeight() - Constants.BLOCKS_PER_EPOCH)) {
          ...
          if (header.getPrevious().equals(prevHeaderHash) == false) {
            ...
            if (OperationStatus.SUCCESS.equals(status) == false)
              throw new ValidationException(
                  "Failed to update missing sync block " + prevHeader + " due to " + status);
          ...
          if (1 == 1) {
            try {
              ...
              System.out.println(
                  "Checking integrity of sync block "
                      + header.getHeight()
                      + "@"
                      + header.getHash());
            } catch (Exception ex) {
              System.out.println(
                  "Failed to retrieve full sync block "
                      + header.getHeight()
                      + "@"
                      + header.getHash());
              ex.printStackTrace();
  ...
  Entry<BlockHeader, StateAccumulator> current() {
    this.headLock.readLock().lock();
    try {
      return new AbstractMap.SimpleEntry<BlockHeader, StateAccumulator>(
          this.head.get(), this.stateAccumulator);
    } finally {
      this.headLock.readLock().unlock();
    }
  }
  ...
  Entry<BlockHeader, StateAccumulator> current(final String label) {
    ...
    try {
      return new AbstractMap.SimpleEntry<BlockHeader, StateAccumulator>(
          this.head.get(), new StateAccumulator(label, this.stateAccumulator));
    } finally {
      this.headLock.readLock().unlock();
    }
  }
  ...
  private void commit(final PendingBlock pendingBlock) throws LockException, IOException {
    ...
    final List<CommitOperation> commitOperations =
        Lists.mutable.ofInitialCapacity(
            pendingBlock.getHeader().getInventorySize(InventoryType.COMMITTED));
    pendingBlock.forInventory(
        InventoryType.COMMITTED, pa -> commitOperations.add(pa.getCommitOperation()));
        ...
        if (pendingBlock.getHeight() - currentHead.getHeight() != 1)
          throw new IllegalStateException(
              "Head to commit "
                  ...
                  + " is not canonical with current head "
                  + currentHead.getHeight()
                  + "@"
                  + currentHead.getHash());
  ...
  public Block getBlock(long height) throws IOException {
    ...
    if (committedBlockHash == null) return null;
  ...
  private EventListener asyncAtomListener =
      new EventListener() {
        ...
        public void on(final AtomCommitEvent event) {
          processAtomCommitEvent(
              event.getProposalHeader(), event.getPendingAtom(), event.getDecision());
        }
        ...
        private void processAtomCommitEvent(
          ...
          if (ledgerLog.hasLevel(Logging.DEBUG) && decision.equals(CommitDecision.REJECT))
            ledgerLog.warn(
                Ledger.this.context.getName() + ": Atom " + pendingAtom.getHash() + " rejected");

          final Epoch epoch = Epoch.from(header);
          final int numShardGroups = numShardGroups(epoch);
          final double writeShardGroups =
              ShardMapper.toShardGroups(pendingAtom, StateLockMode.WRITE, numShardGroups).size();
          ...
          if (instructions.isEmpty() == false) {
            int blobCount = 0;
            int methodCount = 0;
            final MutableObjectIntMap<String> callsByMethod =
                ObjectIntMaps.mutable.ofInitialCapacity(instructions.size());
            for (int i = 0; i < instructions.size(); i++) {
              ...
              if (instruction instanceof Method method) {
                ...
                methodCount++;
              } else if (instruction instanceof Blob) blobCount++;
            }

            callsByMethod.forEachKeyValue(
                (method, count) -> {
                  Ledger.this
                      .context
                      .getTimeSeries("executions")
                      .increment(
                          "local:" + method, count, header.getTimestamp(), TimeUnit.MILLISECONDS);
                  Ledger.this
                      .context
                      .getTimeSeries("executions")
                      .increment(
                          "total:" + method,
                          count * shardGroupFactor,
                          header.getTimestamp(),
            ...
            Ledger.this
                ...
                .getTimeSeries("executions")
                .increment(
                    "local:executions", methodCount, header.getTimestamp(), TimeUnit.MILLISECONDS);
            Ledger.this
                .context
                .getTimeSeries("executions")
                .increment(
                    "total:executions",
                    methodCount * shardGroupFactor,
                    header.getTimestamp(),
                    TimeUnit.MILLISECONDS);
            ...
            Ledger.this
                .context
                .getMetaData()
                .increment("ledger.processed.executions.local", methodCount);
            Ledger.this
                .context
                .getMetaData()
                .increment("ledger.processed.executions.total", methodCount * shardGroupFactor);
            ...
            if (blobCount > 0) {
              Ledger.this
                  ...
                  .getTimeSeries("executions")
                  .increment(
                      "local:" + Blob.class.getAnnotation(StateContext.class).value(),
                      blobCount,
                      header.getTimestamp(),
                      TimeUnit.MILLISECONDS);
              Ledger.this
                  ...
                  .getTimeSeries("executions")
                  .increment(
                      "total:" + Blob.class.getAnnotation(StateContext.class).value(),
                      blobCount * shardGroupFactor,
                      header.getTimestamp(),
                      TimeUnit.MILLISECONDS);
            }
          }
        ...
        public void on(final ProgressPhaseEvent event) {
          if (event.getProgressPhase().equals(State.TRANSITION)
              || event.getProgressPhase().equals(State.VOTING)) {
            Ledger.this
                .context
                .getTimeSeries("proposals")
                .put(
                    "phase",
                    System.currentTimeMillis() - lastProgressPhase,
            ...
            Ledger.this
                ...
                .put(
                    "round",
                    event.getProgressRound().getDuration(),
                    System.currentTimeMillis(),
                    TimeUnit.MILLISECONDS);
          ...
          Ledger.this
              ...
              .getMetaData()
              .put(
                  "ledger.interval.phase",
                  Ledger.this
                      .context
                      .getTimeSeries("proposals")
                      .mean("phase", 30, System.currentTimeMillis(), TimeUnit.MILLISECONDS));
          Ledger.this
              .context
              .getMetaData()
              .put(
                  "ledger.interval.round",
                  Ledger.this
                      .context
                      .getTimeSeries("proposals")
                      .mean("round", 10, System.currentTimeMillis(), TimeUnit.MILLISECONDS));
          Ledger.this
              .context
              .getMetaData()
              .put(
                  "ledger.interval.commit",
                  Ledger.this
                      .context
                      .getTimeSeries("proposals")
                      .mean("commit", 10, System.currentTimeMillis(), TimeUnit.MILLISECONDS));
        }
      };
          ...
          if (ledgerLog.hasLevel(Logging.DEBUG)) {
            ledgerLog.debug(
                Ledger.this.context.getName()
                    ...
                    + " accepted into block "
                    + event.getPendingAtom().getBlockHeader().getHash());
            ledgerLog.debug(
                Ledger.this.context.getName()
                    + ":   "
  ...
  private SynchronousEventListener syncBlockListener =
      new SynchronousEventListener() {
        ...
        public void on(final BlockCommittedEvent event) throws LockException, IOException {
          if (Ledger.this.context.getNode().isSynced() == true
              && Ledger.this.context.getNode().isProgressing() == false) {
        ...
        private void commit(final PendingBlock pendingBlock) throws LockException, IOException {
          if (pendingBlock.getHeader().getPrevious().equals(Ledger.this.getHead().getHash())
              == false) {
            ledgerLog.error(
                Ledger.this.context.getName()
                    + ": Committed block "
                    + pendingBlock.getHeader()
                    + " does not attach to current head "
                    + Ledger.this.getHead());
            return;
          }
          ...
          if (ledgerLog.hasLevel(Logging.DEBUG))
            ledgerLog.debug(
                Ledger.this.context.getName() + ": Committing block " + pendingBlock.getHeader());
          ...
          if (pendingBlock.getHeader().getInventorySize(InventoryType.COMMITTED) > 0) {
            long finalityClientSum =
                pendingBlock.<PendingAtom>get(InventoryType.COMMITTED).stream()
                    .map(pa -> pa.getWitnessedAt())
                    .collect(Collectors.summingLong(t -> wallClockTimestamp - t));
            long finalityConsensusSum =
                pendingBlock.<PendingAtom>get(InventoryType.COMMITTED).stream()
                    .map(pa -> pa.getAcceptedAt())
                    .collect(Collectors.summingLong(t -> leddgerClockTimestamp - t));
            finalityClient =
                finalityClientSum
                    / pendingBlock.getHeader().getInventorySize(InventoryType.COMMITTED);
            finalityConsensus =
                finalityConsensusSum
                    / pendingBlock.getHeader().getInventorySize(InventoryType.COMMITTED);
          }
          Ledger.this
              .context
              .getTimeSeries("finality")
              .put("client", finalityClient, wallClockTimestamp, TimeUnit.MILLISECONDS);
          Ledger.this
              .context
              .getTimeSeries("finality")
              .put("consensus", finalityConsensus, leddgerClockTimestamp, TimeUnit.MILLISECONDS);

          double finalityClientMean =
              Ledger.this
                  .context
                  .getTimeSeries("finality")
                  .mean("client", 30, wallClockTimestamp, TimeUnit.MILLISECONDS);
          double finalityConsensusMean =
              Ledger.this
                  .context
                  .getTimeSeries("finality")
                  .mean("consensus", 30, leddgerClockTimestamp, TimeUnit.MILLISECONDS);
          Ledger.this
              .context
              .getMetaData()
              .put("ledger.throughput.latency.accepted", finalityConsensusMean);
          Ledger.this
              .context
              .getMetaData()
              .put("ledger.throughput.latency.witnessed", finalityClientMean);
        }
        ...
        private void sync(final Block block) throws IOException {
          ...
          ledgerLog.info(
              Ledger.this.context.getName()
                  + ": Synced "
                  + (block.getHeader().getCertificate() != null ? "super" : "")
                  + " block with "
                  + block.getHeader().getInventorySize(InventoryType.ACCEPTED)
                  + " atoms "
                  + block.getHeader().getInventorySize(InventoryType.UNACCEPTED)
                  + " unaccepted "
                  + block.getHeader().getInventorySize(InventoryType.COMMITTED)
                  + " certificates "
                  + (block.getHeader().getInventorySize(InventoryType.UNEXECUTED)
                      + block.getHeader().getInventory(InventoryType.UNCOMMITTED).size())
                  + " timeouts "
                  + block.getHeader().getInventorySize(InventoryType.EXECUTABLE)
                  + " executable signals "
                  + block.getHeader().getInventorySize(InventoryType.LATENT)
                  + " latent signals "
                  + block.getHeader().getInventorySize(InventoryType.PACKAGES)
                  + " packages "
                  + block.getHeader());
          stats(block);
        }
        ...
        private void stats(final Block block) throws IOException {
          ...
          if (ledgerLog.hasLevel(Logging.DEBUG))
            ledgerLog.info(
                Ledger.this.context.getName()
                    + ": "
                    + Ledger.this.context.getLedger().getStateAccumulator().locked().size()
                    + " locked in accumulator "
                    + Ledger.this.context.getLedger().getStateAccumulator().checksum());
          ...
          for (AtomCertificate atomCertificate : block.getCertificates()) {
            ...
            for (StateCertificate stateCertificate :
                atomCertificate.getInventory(StateCertificate.class))
              shardGroupsTouched.add(
          ...
          if (shardsTouchedIncrement > 0) {
            avgShardsTouched =
                ((double) shardsTouchedIncrement)
                    / (certificatesAcceptedIncrement + certificatesRejectedIncrement);
            shardsTouchedFactor = numShardGroups / avgShardsTouched;
          }
          Ledger.this
              .context
              .getTimeSeries("shardratio")
              .put(
                  "touched",
                  avgShardsTouched,
          ...
          Ledger.this
              ...
              .getMetaData()
              .put(
                  "ledger.throughput.shards.touched",
                  Ledger.this
                      .context
                      .getTimeSeries("shardratio")
                      .average(
          ...
          Ledger.this
              ...
              .getMetaData()
              .increment(
                  "ledger.processed.atoms.total",
                  (long) shardsTouchedFactor
                      * (certificatesAcceptedIncrement + certificatesRejectedIncrement));
          Ledger.this
              .context
              .getMetaData()
              .increment("ledger.commits.certificates.accept", certificatesAcceptedIncrement);
          Ledger.this
              .context
              .getMetaData()
              .increment("ledger.commits.certificates.reject", certificatesRejectedIncrement);
          Ledger.this
              .context
              .getMetaData()
              .increment(
                  "ledger.commits.certificates",
                  certificatesAcceptedIncrement + certificatesRejectedIncrement);
          ...
          Ledger.this
              ...
              .getMetaData()
              .increment(
                  "ledger.processed.atoms.latent.execution",
                  block.getHeader().getInventorySize(InventoryType.LATENT));
          for (Hash latent : block.getLatent()) {
            SubstateCommit substateCommit =
                Ledger.this.getLedgerStore().search(StateAddress.from(Atom.class, latent));
            if (substateCommit == null)
              ledgerLog.warn(
                  Ledger.this.context.getName()
                      + ": FIX THIS USING LEDGERSTORE PENDING ATOM SUBSTATES - Substate for latent execution atom "
                      + latent
                      + " not found");
          }
          ...
          for (ExecutionTimeout timeout : block.getUnexecuted()) {
            ...
            if (substateCommit.getSubstate().get(NativeField.TIMEOUT) != null)
              Ledger.this
                  .context
                  .getMetaData()
                  .increment("ledger.processed.atoms.timedout.execution");
          }
          ...
          Ledger.this
              .context
              .getMetaData()
              .increment("ledger.processed.atoms.timedout.accept", block.getUnaccepted().size());

          // Hack for single shard as "estimates" dont work too well
          long localProcessedAtoms =
              Ledger.this.context.getMetaData().get("ledger.processed.atoms.local", 0l);
          long totalProcessedAtoms =
              Ledger.this.context.getMetaData().get("ledger.processed.atoms.total", 0l);
          if (totalProcessedAtoms < localProcessedAtoms)
            Ledger.this
                .context
                .getMetaData()
                .put("ledger.processed.atoms.total", localProcessedAtoms);

          Ledger.this
              .context
              .getTimeSeries("throughput")
              .increment(
                  "local",
                  block.getHeader().getInventorySize(InventoryType.COMMITTED),
                  block.getHeader().getTimestamp(),
          ...
          Ledger.this
              ...
              .increment(
                  "total",
                  block.getHeader().getInventorySize(InventoryType.COMMITTED) * shardsTouchedFactor,
                  block.getHeader().getTimestamp(),
                  TimeUnit.MILLISECONDS);
          ...
          Ledger.this
              ...
              .getMetaData()
              .put(
                  "ledger.throughput.atoms.local",
                  Ledger.this
                      .context
                      .getTimeSeries("throughput")
                      .average("local", System.currentTimeMillis() - 30000, TimeUnit.MILLISECONDS));
          Ledger.this
              .context
              .getMetaData()
              .put(
                  "ledger.throughput.atoms.total",
                  Ledger.this
                      .context
                      .getTimeSeries("throughput")
                      .average("total", System.currentTimeMillis() - 30000, TimeUnit.MILLISECONDS));

          Ledger.this
              .context
              .getMetaData()
              .put(
                  "ledger.throughput.executions.local",
                  Ledger.this
                      .context
                      .getTimeSeries("executions")
                      .average(
          ...
          Ledger.this
              ...
              .getMetaData()
              .put(
                  "ledger.throughput.executions.total",
                  Ledger.this
                      .context
                      .getTimeSeries("executions")
                      .average(
          ...
          try {
            if (ledgerLog.hasLevel(Logging.INFO)) {
              ledgerLog.info(
                  Ledger.this.context.getName()
                      + ": Committed "
                      + (blockCommittedEvent.getPendingBlock().getHeader().getCertificate() != null
                          ? "super"
                          : "")
                      + " block with "
                      + blockCommittedEvent
                          .getPendingBlock()
                          .getHeader()
                          .getInventorySize(InventoryType.ACCEPTED)
                      + " atoms "
                      + blockCommittedEvent
                          .getPendingBlock()
                          .getHeader()
                          .getInventorySize(InventoryType.UNACCEPTED)
                      + " unaccepted "
                      + blockCommittedEvent
                          .getPendingBlock()
                          .getHeader()
                          .getInventorySize(InventoryType.COMMITTED)
                      + " certificates "
                      + (blockCommittedEvent
                              .getPendingBlock()
                              .getHeader()
                              .getInventorySize(InventoryType.UNEXECUTED)
                          + blockCommittedEvent
                              .getPendingBlock()
                              .getHeader()
                      ...
                      + blockCommittedEvent
                          .getPendingBlock()
                          .getHeader()
                          .getInventorySize(InventoryType.EXECUTABLE)
                      + " executable signals "
                      + blockCommittedEvent
                          .getPendingBlock()
                          .getHeader()
                          .getInventorySize(InventoryType.LATENT)
                      + " latent signals "
                      + blockCommittedEvent
                          .getPendingBlock()
                          .getHeader()
                          .getInventorySize(InventoryType.PACKAGES)
                      + " packages "
                      + blockCommittedEvent.getPendingBlock().getHeader()
                      + " "
                      + blockCommittedEvent.getPendingBlock().getBlock().getSize()
                      + " bytes");
            }
            ...
            ledgerLog.error(
                Ledger.this.context.getName()
                    + ": Could not serialize committed block "
                    + blockCommittedEvent.getPendingBlock().getHash()
                    + " for statistics",
                ioex);
          }
        }
  ...
  private SynchronousEventListener syncChangeListener =
      new SynchronousEventListener() {
        ...
        public void on(final SyncAcquiredEvent event) throws LockException {
          ledgerLog.log(
              Ledger.this.context.getName() + ": Sync status acquired, loading state accumulator");
          ledgerLog.log(
              Ledger.this.context.getName()
                  + ":    Sync accumulator checksum is "
                  + event.getAccumulator().checksum());
          ...
          try {
            ...
            if (event.getHead().getHash().equals(Universe.getDefault().getGenesis().getHash())
                == true) Ledger.this.context.getNode().setProgressing(true);

            if (ledgerLog.hasLevel(Logging.INFO)) {
              ledgerLog.info(
                  Ledger.this.context.getName()
                      + ":    Ledger accumulator checksum is "
                      + Ledger.this.stateAccumulator.checksum());
        ...
        public void on(final SyncLostEvent event) {
          ...
          try {
            ledgerLog.log(
                Ledger.this.context.getName() + ": Sync status lost, flushing state accumulator");
            Ledger.this.stateAccumulator.reset();
          } finally {
            Ledger.this.headLock.writeLock().unlock();
          }
          ...
          try {
            final ShardGroupID localShardGroupID =
                ShardMapper.toShardGroup(
                    Ledger.this.context.getNode().getIdentity(), Ledger.this.numShardGroups());
            final ShardGroupID remoteShardGroupID =
                ShardMapper.toShardGroup(
                    event.getConnection().getNode().getIdentity(), Ledger.this.numShardGroups());
            if (localShardGroupID.equals(remoteShardGroupID) == false) return;

            // Reasons NOT to ask for all the pool inventories
            if (Ledger.this.context.getNode().isSynced() == false) return;

            if (Ledger.this.getHead().getHash().equals(Universe.getDefault().getGenesis().getHash())
                    == false
                && Ledger.this.getHead().getHeight()
                    >= event.getConnection().getNode().getHead().getHeight()
                        - Constants.OOS_TRIGGER_LIMIT_BLOCKS) return;

            // Lagging, ask for inventory
            final List<Hash> knownAtoms =
                Ledger.this.getAtomHandler().getAll().stream()
                    .filter(pa -> pa.getAtom() != null)
                    .map(pa -> pa.getHash())
            ...
            Ledger.this
                ...
                .getMessaging()
                .send(
                    new SyncAcquiredMessage(Ledger.this.getHead(), knownAtoms),
                    event.getConnection());
            ledgerLog.info(
                Ledger.this.context.getName()
                    + ": Requesting full inventory from "
                    + event.getConnection());
          } catch (IOException ioex) {
            ledgerLog.error(
                Ledger.this.context.getName()
                    + ": Failed to request sync connected items from "
                    + event.getConnection(),
                ioex);
          }
        }
