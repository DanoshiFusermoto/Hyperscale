...
public abstract class Component extends BasicObject implements StateAddressable {
  ...
  static {
    ...
    ConfigurationBuilder reflectionsConfig =
    ...
    Set<Class<? extends NativeComponent>> nativeComponentClasses =
        reflections.getSubTypesOf(NativeComponent.class);
    for (Class<? extends NativeComponent> nativeComponentClass : nativeComponentClasses) {
      try {
        ...
        System.out.println(
            "Failed to load NativeComponent " + nativeComponentClass + " " + e.toString());
      }
    }
  }

  public static Component load(
      StateMachine stateMachine, Class<? extends NativeComponent> nativeComponentClass)
      throws ReflectiveOperationException {
    return nativeComponentClass.getConstructor(StateMachine.class).newInstance(stateMachine);
  }

  public static Component load(
      StateMachine stateMachine, PolyglotPackage pakage, SubstateLog substateLog) {
  ...
  protected static boolean registerNative(Class<? extends NativeComponent> componentClass) {
    ...
    if (nativeComponents.putIfAbsent(
            StateAddress.from(
                Component.class.getAnnotation(StateContext.class).value(), Hash.valueOf(context)),
            componentClass)
        != null)
      throw new IllegalStateException(
          "Native component " + componentClass + " is already registered");
  ...
  protected abstract void prepare(String method, Object[] arguments)
      throws ReflectiveOperationException;

  protected abstract <T> T call(String method, Object[] arguments)
      throws ReflectiveOperationException;
