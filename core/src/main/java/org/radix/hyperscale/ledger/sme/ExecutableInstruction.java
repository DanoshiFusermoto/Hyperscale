...
abstract class ExecutableInstruction extends ArgumentedInstruction {
  ...
  protected final void lock(StateMachine stateMachine, Object[] arguments)
      throws ReflectiveOperationException {
    Object[] extendedArguments = new Object[arguments.length + 1];
    Class<?>[] argumentClasses = new Class<?>[arguments.length + 1];
    extendedArguments[0] = stateMachine;
    argumentClasses[0] = StateMachine.class;
    ...
    for (Method methref : methods) {
      if (!methref.getName().equals("lock")) continue;

      Class<?>[] parameterTypes = methref.getParameterTypes();
      if (parameterTypes.length != extendedArguments.length) continue;
    ...
    throw new NoSuchMethodException(
        getClass().getName() + ".lock(" + Arrays.toString(argumentClasses) + ")");
  }

  @SuppressWarnings("unchecked")
  protected final <T> T execute(StateMachine stateMachine, Object[] arguments)
      throws ReflectiveOperationException {
    Object[] extendedArguments = new Object[arguments.length + 1];
    Class<?>[] argumentClasses = new Class<?>[arguments.length + 1];
    extendedArguments[0] = stateMachine;
    ...
    for (Method methref : methods) {
      if (!methref.getName().equals("execute")) continue;
      ...
      if (matches) return (T) methref.invoke(this, extendedArguments);
    }

    throw new NoSuchMethodException(
        getClass().getName() + ".execute(" + Arrays.toString(argumentClasses) + ")");
  }
}
