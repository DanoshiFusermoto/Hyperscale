...
public final class PrimitiveSearchQuery extends BasicObject {
  ...
  public PrimitiveSearchQuery(final Hash primitive, final Class<? extends Primitive> type) {
    this(
        ...
        Serialization.getInstance()
            .getIdForClass(Objects.requireNonNull(type, "Primitive type is null")),
        Isolation.COMMITTED);
  }

  public PrimitiveSearchQuery(
      final Hash primitive, final Class<? extends Primitive> type, final Isolation isolation) {
    this(
        primitive,
        Serialization.getInstance()
            .getIdForClass(Objects.requireNonNull(type, "Primitive type is null")),
  ...
  public Class<? extends Primitive> getType() {
    ...
    Class<? extends Primitive> type =
        (Class<? extends Primitive>) Serialization.getInstance().getClassForId(this.type);
    if (type == null)
  ...
  public boolean equals(Object object) {
    ...
    if (object instanceof PrimitiveSearchQuery primitiveSearchQuery) {
      ...
      if (this.primitive.equals(primitiveSearchQuery.getQuery()) == false) return false;

      if (this.isolation.equals(primitiveSearchQuery.getIsolation()) == false) return false;

      if (this.type.equals(primitiveSearchQuery.type) == false) return false;
