...
public final class AtomCommitEvent extends AtomEventWithProposalHeader {
  public AtomCommitEvent(final BlockHeader header, final PendingAtom pendingAtom) {
    super(header, pendingAtom);

    if (pendingAtom.getCertificate() == null)
      throw new IllegalArgumentException(
          "Atom certificate not present for " + pendingAtom.getHash());
    ...
    if (pendingAtom.getCertificate().getDecision().equals(CommitDecision.ACCEPT) == false
      ...
      throw new UnsupportedOperationException(
          "Commit decision of type "
              + pendingAtom.getCertificate().getDecision()
              + " is not supported");
  }
