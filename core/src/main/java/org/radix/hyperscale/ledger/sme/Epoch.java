...
public class Epoch implements GlobalInstruction
{
	...
	public boolean equals(Object object) 
	{
		...
		if (object instanceof Epoch epoch)
		{
			...
			if (this.shardGroupID.equals(epoch.shardGroupID) == false)
				return false;

			if (this.powers.equals(epoch.powers) == false)
				return false;
	...
	{
		final org.radix.hyperscale.ledger.Epoch localExpectedEpoch = org.radix.hyperscale.ledger.Epoch.from(stateMachine.getPendingAtom().getBlockHeader()).increment(Constants.VOTE_POWER_MATURITY_EPOCHS-1);
		...
		if (stateMachine.exists(epochAddress))
		{
			...
			if (stateMachine.get(epochAddress, NativeField.VOTE_POWER) == null)
				throw new StateMachineExecutionException("Expected field VOTE_POWER in epoch:"+getClock()+" substate not found");
		}
		else
		{
			stateMachine.assertCreate(epochAddress, ComputeKey.NULL.getIdentity());
			stateMachine.set(epochAddress, NativeField.VOTE_POWER, new TreeMap<ShardGroupID, VotePowers>(), ComputeKey.NULL.getIdentity());
		}
		...
		if (getClock() == 0 && Universe.getDefault().getGenesis().contains(stateMachine.getPendingAtom().getHash()))
		{
			...
			{
				final StateAddress epochShardGroupAddress = StateAddress.from("epoch", e+":"+getShardGroupID());
				stateMachine.assertCreate(epochShardGroupAddress, ComputeKey.NULL.getIdentity());
				stateMachine.set(epochShardGroupAddress, NativeField.ATOM, stateMachine.getPendingAtom().getHash(), ComputeKey.NULL.getIdentity());
			}
			...
			for (final Entry<Identity, Long> votePower : this.powers.getAll().entrySet())
			{
				...
				ShardGroupID validatorShardGroupID = ShardMapper.toShardGroup(votePower.getKey(), numShardGroups); 
				stateMachine.set(validatorAddress, NativeField.SHARD_GROUP, validatorShardGroupID, ComputeKey.NULL.getIdentity());
				stateMachine.set(validatorAddress, NativeField.VOTE_POWER, votePower.getValue(), ComputeKey.NULL.getIdentity());
			}			
		}
		else
		{
			// Epoch vote power map
			final Map<ShardGroupID, VotePowers> epochPowers = stateMachine.get(epochAddress, NativeField.VOTE_POWER);
			
			// The Epoch substate for the referenced shard group
			final StateAddress epochShardGroupAddress = StateAddress.from("epoch", getClock()+":"+getShardGroupID());
			...
			if (localShardGroupID.equals(getShardGroupID()))
			{
				if (getClock() != localExpectedEpoch.getClock())
					throw new StateMachineExecutionException("Epoch clock is invalid for shard group "+getShardGroupID()+" Expected "+localExpectedEpoch.getClock()+"  Presented "+getClock());
			}
			...
			for (final Entry<Identity, Long> votePower : this.powers.getAll().entrySet())
			{
				...
				// TODO look up shard group ID by epoch clock 
				final ShardGroupID validatorShardGroupID = ShardMapper.toShardGroup(votePower.getKey(), numShardGroups);
				...
				if (localShardGroupID.equals(getShardGroupID()))
				{
					...
					if (validatorShardGroupID.equals(getShardGroupID()) == false)
						throw new StateMachineExecutionException("Validator "+votePower.getKey()+" not present in shard group "+getShardGroupID()+" for epoch "+getClock());
					
					// Verify target validator vote power change matches to local expectation
					final long validatorPendingVotePower = stateMachine.getContext().getLedger().getValidatorHandler().verify(localExpectedEpoch, votePower.getKey());
					if (validatorPendingVotePower != votePower.getValue())
						throw new StateMachineExecutionException("Vote power mismatch for validator "+votePower.getKey()+" in shard group "+getShardGroupID()+" for epoch "+getClock()+":  Expected "+validatorPendingVotePower+"  Presented "+votePower.getValue());
				}

				stateMachine.set(validatorAddress, NativeField.VOTE_POWER, votePower.getValue(), ComputeKey.NULL.getIdentity());
				
				// TODO What additional checks can validators not currently in the specificied shard group perform. 
			...
			if (epochPowers.containsKey(getShardGroupID()) == true)
				throw new StateMachineExecutionException("Epoch "+getClock()+" vote powers for shard group "+getShardGroupID()+" already accepted");
			epochPowers.put(getShardGroupID(), getPowers());
			
			// Add the shuffle seed proposed by the validator presenting this epoch update
			final Map<ShardGroupID, Hash> epochShuffleSeeds = stateMachine.get(epochAddress, NativeField.SHUFFLE_SEEDS, new TreeMap<ShardGroupID, Hash>());
			epochShuffleSeeds.put(getShardGroupID(), stateMachine.getPendingAtom().getHash());
			stateMachine.set(epochAddress, NativeField.SHUFFLE_SEEDS, epochShuffleSeeds, ComputeKey.NULL.getIdentity());
		}
	}
}
