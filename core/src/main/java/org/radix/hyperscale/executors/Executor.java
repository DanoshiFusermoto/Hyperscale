...
public class Executor {
  ...
  private final ThreadFactory immediateThreadFactory =
      new ThreadFactory() {
  ...
  private final ThreadFactory scheduledThreadFactory =
      new ThreadFactory() {
        private final AtomicInteger counter = new AtomicInteger(0);
  ...
  private Executor() {
    ...
    int maxImmediateExecutors =
        Math.min(MAX_IMMEDIATE_EXECUTION_THREADS, Runtime.getRuntime().availableProcessors());
    int maxScheduledExecutors =
        Math.min(MAX_SCHEDULED_EXECUTION_THREADS, Runtime.getRuntime().availableProcessors() / 2);
    this.immediateExecutor =
        Executors.newFixedThreadPool(maxImmediateExecutors, this.immediateThreadFactory);
    this.scheduledExecutor =
        Executors.newScheduledThreadPool(maxScheduledExecutors, this.scheduledThreadFactory);
  }
  ...
  public Executor(
      final int numImmediateThreads,
      final ThreadFactory immediateThreadFactory,
      final int numScheduledThreads,
      final ThreadFactory scheduledThreadFactory) {
    this.immediateExecutor =
        Executors.newFixedThreadPool(numImmediateThreads, immediateThreadFactory);
    this.scheduledExecutor =
        Executors.newScheduledThreadPool(numScheduledThreads, scheduledThreadFactory);
  }
  ...
  public Future<?> schedule(final ScheduledExecutable executable) {
    ...
    executable.setFuture(
        this.scheduledExecutor.schedule(
            executable, executable.getInitialDelay(), executable.getTimeUnit()));
  ...
  public Future<?> scheduleWithFixedDelay(final ScheduledExecutable executable) {
    ...
    executable.setFuture(
        this.scheduledExecutor.scheduleWithFixedDelay(
            executable,
            executable.getInitialDelay(),
            executable.getRecurrentDelay(),
            executable.getTimeUnit()));
    return executable.getFuture();
  }
  ...
  public Future<?> schedule(
      final Executable executable, final int initialDelay, final TimeUnit unit) {
    Objects.requireNonNull(executable, "Executable to schedule is null");
