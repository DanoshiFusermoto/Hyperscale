...
public class AES {
  ...
  private static final byte[] InCo = {
    (byte) 0xB, (byte) 0xD, (byte) 0x9, (byte) 0xE
  }; /* Inverse Coefficients */

  public static final int BS = 16; /* Block Size */

  private static final byte[] ptab = {
    (byte) 1, (byte) 3, (byte) 5, (byte) 15, (byte) 17, (byte) 51, (byte) 85, (byte) 255, (byte) 26,
  ...
  private static final byte[] ltab = {
    (byte) 0, (byte) 255, (byte) 25, (byte) 1, (byte) 50, (byte) 2, (byte) 26, (byte) 198,
    ...
    (byte) 43, (byte) 121, (byte) 10, (byte) 21, (byte) 155, (byte) 159, (byte) 94, (byte) 202,
        (byte) 78, (byte) 212, (byte) 172, (byte) 229, (byte) 243, (byte) 115, (byte) 167,
        (byte) 87,
    (byte) 175, (byte) 88, (byte) 168, (byte) 80, (byte) 244, (byte) 234, (byte) 214, (byte) 116,
        (byte) 79, (byte) 174, (byte) 233, (byte) 213, (byte) 231, (byte) 230, (byte) 173,
        (byte) 232,
    ...
    (byte) 68, (byte) 17, (byte) 146, (byte) 217, (byte) 35, (byte) 32, (byte) 46, (byte) 137,
        (byte) 180, (byte) 124, (byte) 184, (byte) 38, (byte) 119, (byte) 153, (byte) 227,
        (byte) 165,
  ...
  public void reset(int m, byte[] iv) {
    ...
    if (mode != ECB && iv != null) for (int i = 0; i < 16; i++) f[i] = iv[i];
  }
  ...
  public boolean init(int m, int nk, byte[] key, byte[] iv) {
    ...
    for (j = nk, k = 0; j < N; j += nk, k++) {
      ...
      if (nk <= 6) {
        for (i = 1; i < nk && (i + j) < N; i++) fkey[i + j] = fkey[i + j - nk] ^ fkey[i + j - 1];
      } else {
        for (i = 1; i < 4 && (i + j) < N; i++) {
  ...
  public void end() {
    // clean up
    int i;
    for (i = 0; i < 4 * (Nr + 1); i++) fkey[i] = rkey[i] = 0;
  ...
  public static byte[] CBC_IV0_DECRYPT(byte[] K, byte[] C) {
    ...
    if (!bad) for (i = 0; i < 16 - padlen; i++) MM[opt++] = buff[i];
