...
public final class KYBER {
  ...
  static void poly_reduce(short[] r) {
    int i;
    for (i = 0; i < KY_DEGREE; i++) r[i] = barrett_reduce(r[i]);
  }
  ...
  static void poly_tomont(short[] r) {
    int i;
    short f = KY_ONE;
    for (i = 0; i < KY_DEGREE; i++) r[i] = montgomery_reduce((int) r[i] * f);
  }
  ...
  static void poly_copy(short[] p1, short[] p2) {
    int i;
    for (i = 0; i < KY_DEGREE; i++) p1[i] = p2[i];
  }

  // zero polynomial
  static void poly_zero(short[] p1) {
    int i;
    for (i = 0; i < KY_DEGREE; i++) p1[i] = 0;
  }

  // add polynomials
  static void poly_add(short[] p1, short[] p2, short[] p3) {
    int i;
    for (i = 0; i < KY_DEGREE; i++) p1[i] = (short) (p2[i] + p3[i]);
  }
  ...
  static void ExpandAij(byte[] rho, short[] Aij, int i, int j) {
    ...
    while (j < KY_DEGREE) {
      ...
      int d2 = (t2 / 16 + 16 * t3);
      if (d1 < KY_PRIME) Aij[j++] = (short) d1;
      if (d2 < KY_PRIME && j < KY_DEGREE) Aij[j++] = (short) d2;
      i += 3;
    }
  }
  ...
  static void decode(byte[] pack, int L, short[] t, int pos[]) {
    int i;
    for (i = 0; i < KY_DEGREE; i++) t[i] = nextword(L, pack, pos);
  }
  ...
  static void decompress(short[] t, int d) {
    int twod1 = (1 << (d - 1));
    for (int i = 0; i < KY_DEGREE; i++) t[i] = (short) ((KY_PRIME * t[i] + twod1) >> d);
  }
  ...
  static void CPA_keypair(int[] params, byte[] tau, byte[] sk, byte[] pk) {
    ...
    for (i = 0; i < ck; i++) {
      ...
      for (j = 0; j < 33; j++) sh.process(sigma[j]);
    ...
    int[] pos = new int[2];
    pos[0] = pos[1] = 0;
    for (i = 0; i < ck; i++) encode(s[i], pos, 12, sk, i);
    pos[0] = pos[1] = 0;
    for (i = 0; i < ck; i++) encode(p[i], pos, 12, pk, i);
    for (i = 0; i < 32; i++) pk[public_key_size - 32 + i] = rho[i];
  }
  ...
  static void CCA_keypair(int[] params, byte[] randbytes64, byte[] sk, byte[] pk) {
    ...
    for (i = 0; i < pks; i++) sk[sks + i] = pk[i];

    for (i = 0; i < pks; i++) sh.process(pk[i]);
    byte[] h = sh.hash();
    for (i = 0; i < 32; i++) sk[sks + pks + i] = h[i];
    for (i = 0; i < 32; i++) sk[sks + pks + 32 + i] = randbytes64[32 + i];
  }

  static void CPA_base_encrypt(
      int[] params, byte[] coins, byte[] pk, byte[] ss, short[][] u, short[] v) {
    int i, j;
    byte[] sigma = new byte[33];
    byte[] buff = new byte[256];
    byte[] rho = new byte[32];
    ...
    for (i = 0; i < 32; i++) sigma[i] = coins[i]; // i+6; //RAND_byte(RNG);
    sigma[32] = 0;

    for (i = 0; i < 32; i++) rho[i] = pk[public_key_size - 32 + i];
    ...
    int[] pos = new int[2];
    pos[0] = pos[1] = 0;
    for (i = 0; i < ck; i++) decode(pk, 12, p[i], pos);
    ...
    for (i = 0; i < ck; i++) compress(u[i], du);
    compress(v, dv);
  }
  ...
  static void CPA_encrypt(int[] params, byte[] coins, byte[] pk, byte[] ss, byte[] ct) {
    ...
    int[] pos = new int[2];
    pos[0] = pos[1] = 0;
    for (i = 0; i < ck; i++) encode(u[i], pos, du, ct, i);

    int len = (dv * KY_DEGREE / 8); // last part of CT
    byte[] last = new byte[len];

    encode(v, pos, dv, last, 0);
    for (i = 0; i < len; i++) ct[ciphertext_size - len + i] = last[i];
  }
  ...
  static byte CPA_check_encrypt(int[] params, byte[] coins, byte[] pk, byte[] ss, byte[] ct) {
    ...
    pos[0] = pos[1] = 0;
    d1 = 0;
    for (i = 0; i < ck; i++) d1 |= chk_encode(u[i], pos, du, ct, i);

    int len = (dv * KY_DEGREE / 8); // last part of CT
    byte[] last = new byte[len];
    for (i = 0; i < len; i++) last[i] = ct[ciphertext_size - len + i];
  ...
  static void CCA_encrypt(int[] params, byte[] randbytes32, byte[] pk, byte[] ss, byte[] ct) {
    ...
    byte[] hm = new byte[32];
    for (i = 0; i < 32; i++) hm[i] = randbytes32[i];
    ...
    SHA3 sh = new SHA3(SHA3.HASH256);
    for (i = 0; i < public_key_size; i++) sh.process(pk[i]);
    byte[] h = sh.hash();

    sh = new SHA3(SHA3.HASH512);
    for (i = 0; i < 32; i++) sh.process(hm[i]);
    for (i = 0; i < 32; i++) sh.process(h[i]);
    byte[] g = sh.hash();

    for (i = 0; i < 32; i++) coins[i] = g[i + 32];
  ...
  static void CPA_decrypt(int[] params, byte[] SK, byte[] CT, byte[] SS) {
    ...
    int[] pos = new int[2];
    pos[0] = pos[1] = 0;
    for (i = 0; i < ck; i++) decode(CT, du, u[i], pos);
    decode(CT, dv, v, pos);
    for (i = 0; i < ck; i++) decompress(u[i], du);
    decompress(v, dv);
    pos[0] = pos[1] = 0;
    for (i = 0; i < ck; i++) decode(SK, 12, s[i], pos);
  ...
  static void CCA_decrypt(int[] params, byte[] SK, byte[] CT, byte[] SS) {
    ...
    for (i = 0; i < public_key_size; i++) PK[i] = SK[secret_cpa_key_size + i];

    for (i = 0; i < 32; i++) h[i] = SK[secret_cpa_key_size + public_key_size + i];
    for (i = 0; i < 32; i++) z[i] = SK[secret_cpa_key_size + public_key_size + 32 + i];

    CPA_decrypt(params, SK, CT, m);

    SHA3 sh = new SHA3(SHA3.HASH512);
    for (i = 0; i < 32; i++) sh.process(m[i]);
    for (i = 0; i < 32; i++) sh.process(h[i]);
    byte[] g = sh.hash();
    ...
    sh = new SHA3(SHA3.SHAKE256);
    for (i = 0; i < 32; i++) sh.process(z[i]);
    for (i = 0; i < ciphertext_size; i++) sh.process(CT[i]);
    sh.shake(SS, shared_secret_size);

    byte mask = CPA_check_encrypt(params, coins, PK, m, CT);

    for (i = 0; i < 32; i++) SS[i] ^= (SS[i] ^ g[i]) & (~mask);
