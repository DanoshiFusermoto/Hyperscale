...
public final class HMAC {
  ...
  public static int HMAC1(int hash, int sha, byte[] tag, int olen, byte[] K, byte[] M) {
    ...
    } else for (int i = 0; i < K.length; i++) K0[i] = K[i];
  ...
  public static byte[] XOF_Expand(int hlen, int olen, byte[] DST, byte[] MSG) {
    ...
    for (int i = 0; i < MSG.length; i++) H.process(MSG[i]);
    H.process(olen / 256);
    H.process(olen % 256);

    for (int i = 0; i < DST.length; i++) H.process(DST[i]);
  ...
  static byte[] XMD_Expand_Short_DST(int hash, int hlen, int olen, byte[] DST, byte[] MSG) {
    ...
    TMP[1] = (byte) (olen % 256);
    TMP[2] = (byte) 0;
    for (int j = 0; j < DST.length; j++) TMP[3 + j] = DST[j];
    TMP[3 + DST.length] = (byte) DST.length;

    byte[] H0 = GPhashit(hash, hlen, 0, blk, MSG, -1, TMP);

    int k = 0;
    for (int j = 0; j < hlen; j++) H1[j] = 0;

    for (int i = 1; i <= ell; i++) {
      for (int j = 0; j < hlen; j++) H1[j] ^= H0[j];
      TMP2[0] = (byte) i;
      for (int j = 0; j < DST.length; j++) TMP2[1 + j] = DST[j];
      TMP2[1 + DST.length] = (byte) DST.length;
      H1 = GPhashit(hash, hlen, 0, 0, H1, -1, TMP2);
      for (int j = 0; j < hlen && k < olen; j++) OKM[k++] = H1[j];
    }
  ...
  */
  ...
  private static final byte[] SHA256ID = {
    0x30,
    0x31,
    0x30,
    0x0d,
  ...
  private static final byte[] SHA384ID = {
    0x30,
    0x41,
    0x30,
    0x0d,
  ...
  private static final byte[] SHA512ID = {
    0x30,
    0x51,
    0x30,
    0x0d,
  ...
  public static boolean PKCS15(int sha, byte[] m, byte[] w, int RFS) {
    ...
    w[i++] = 0;
    w[i++] = 1;
    for (int j = 0; j < olen - idlen - hlen - 3; j++) w[i++] = (byte) 0xff;
    w[i++] = 0;
    ...
    for (int j = 0; j < hlen; j++) w[i++] = H[j];

    return true;
  }

  /* SHAXXX identifier strings */
  private static final byte[] SHA256IDb = {
    0x30,
    0x2f,
    0x30,
    0x0b,
  ...
  private static final byte[] SHA384IDb = {
    0x30,
    0x3f,
    0x30,
    0x0b,
  ...
  private static final byte[] SHA512IDb = {
    0x30,
    0x4f,
    0x30,
    0x0b,
  ...
  public static byte[] PSS_ENCODE(int sha, byte[] m, RAND rng, int RFS) {
    ...
    for (int i = 0; i < 8; i++) MD[i] = 0;
    for (int i = 0; i < hlen; i++) MD[8 + i] = H[i];
    for (int i = 0; i < hlen; i++) MD[8 + hlen + i] = SALT[i];

    // System.out.print("MD= 0x"); printBinary(MD);

    H = SPhashit(MC_SHA2, sha, MD);
    byte[] f = new byte[RFS];
    for (int i = 0; i < emlen - hlen - hlen - 2; i++) f[i] = 0;
    f[emlen - hlen - hlen - 2] = 0x01;
    for (int i = 0; i < hlen; i++) f[emlen + i - hlen - hlen - 1] = SALT[i];
    // System.out.print("f= 0x"); printBinary(f);
    MGF1XOR(sha, H, emlen - hlen - 1, f);
    f[0] &= mask;
    for (int i = 0; i < hlen; i++) f[emlen + i - hlen - 1] = H[i];
    f[emlen - 1] = (byte) 0xbc;
    return f;
  }
  ...
  public static boolean PSS_VERIFY(int sha, byte[] m, byte[] f) {
    ...
    for (int i = 0; i < emlen - hlen - 1; i++) DB[i] = f[i];
    byte[] H = new byte[hlen];
    for (int i = 0; i < hlen; i++) H[i] = f[emlen + i - hlen - 1];

    MGF1XOR(sha, H, emlen - hlen - 1, DB);
    DB[0] &= mask;

    byte k = 0;
    for (int i = 0; i < emlen - hlen - hlen - 2; i++) k |= DB[i];
    if (k != 0) return false;
    if (DB[emlen - hlen - hlen - 2] != 0x01) return false;

    for (int i = 0; i < hlen; i++) SALT[i] = DB[emlen + i - hlen - hlen - 1];

    byte[] MD = new byte[8 + hlen + hlen];
    for (int i = 0; i < 8; i++) MD[i] = 0;
    for (int i = 0; i < hlen; i++) MD[8 + i] = HMASK[i];
    for (int i = 0; i < hlen; i++) MD[8 + hlen + i] = SALT[i];
    HMASK = SPhashit(MC_SHA2, sha, MD);

    k = 0;
    for (int i = 0; i < hlen; i++) k |= (H[i] - HMASK[i]);
    if (k != 0) return false;
    return true;
  }
  ...
  public static byte[] OAEP_ENCODE(int sha, byte[] m, RAND rng, byte[] p, int RFS) {
    ...
    /* pad to length RFS */
    int d = 1;
    for (i = RFS - 1; i >= d; i--) f[i] = f[i - d];
    for (i = d - 1; i >= 0; i--) f[i] = 0;
  ...
  public static byte[] OAEP_DECODE(int sha, byte[] p, byte[] f, int RFS) {
    ...
    for (i = seedlen; i < olen; i++) DBMASK[i - seedlen] = f[i + 1];
    ...
    for (i = 0; i < m; i++) DBMASK[i] = DBMASK[i + hlen];

    for (i = 0; i < hlen; i++) SEED[i] = CHASH[i] = 0;
    ...
    for (i = 0; i < m - k - 1; i++) r[i] = DBMASK[i + k + 1];
