...
public class DBIG {
  private static final ThreadLocal<DBIG> DBIGw0 = ThreadLocal.withInitial(() -> new DBIG(0));
  private static final ThreadLocal<DBIG> DBIGw1 = ThreadLocal.withInitial(() -> new DBIG(0));
  ...
  public boolean iszilch() {
    long d = 0;
    for (int i = 0; i < BIG.DNLEN; i++) d |= w[i];
  ...
  public DBIG(int x) {
    w[0] = x;
    for (int i = 1; i < BIG.DNLEN; i++) w[i] = 0;
  }

  public DBIG(DBIG x) {
    for (int i = 0; i < BIG.DNLEN; i++) w[i] = x.w[i];
  }

  public DBIG(BIG x) {
    for (int i = 0; i < BIG.NLEN - 1; i++) w[i] = x.w[i]; // get(i);
  ...
  public void zero() {
    for (int i = 0; i < BIG.DNLEN; i++) w[i] = 0;
  }
  ...
  public void ucopy(BIG x) {
    for (int i = 0; i < BIG.NLEN; i++) w[i] = 0;
    for (int i = BIG.NLEN; i < BIG.DNLEN; i++) w[i] = x.w[i - BIG.NLEN];
  }
  ...
  public void shl(int k) {
    ...
    w[BIG.DNLEN - 1] =
        ((w[BIG.DNLEN - 1 - m] << n)) | (w[BIG.DNLEN - m - 2] >> (CONFIG_BIG.BASEBITS - n));
  ...
  public void rsub(DBIG x) {
    for (int i = 0; i < BIG.DNLEN; i++) w[i] = x.w[i] - w[i];
  }
