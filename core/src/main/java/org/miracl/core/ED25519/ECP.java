...
public final class ECP {
  /* Working variables */
  private static final ThreadLocal<FP> rhsFPw0 = ThreadLocal.withInitial(() -> new FP());
  private static final ThreadLocal<FP[]> addFP7 =
      ThreadLocal.withInitial(
          () -> new FP[] {new FP(), new FP(), new FP(), new FP(), new FP(), new FP(), new FP()});
  private static final ThreadLocal<FP[]> dblFP4 =
      ThreadLocal.withInitial(() -> new FP[] {new FP(), new FP(), new FP(), new FP()});
  private static final ThreadLocal<ECP> selectECPw0 = ThreadLocal.withInitial(() -> new ECP());
  private static final ThreadLocal<ECP> subECPw0 = ThreadLocal.withInitial(() -> new ECP());
  private static final ThreadLocal<BIG> clmulBIGw0 = ThreadLocal.withInitial(() -> new BIG());
  private static final ThreadLocal<BIG> clmulBIGw1 = ThreadLocal.withInitial(() -> new BIG());
  private static final ThreadLocal<ECP> clmulECPw0 = ThreadLocal.withInitial(() -> new ECP());
  private static final ThreadLocal<ECP> clmulECPw1 = ThreadLocal.withInitial(() -> new ECP());
  private static final ThreadLocal<ECP[]> clmulECP8w0 =
      ThreadLocal.withInitial(
          () -> {
            ECP arr[] = {
              new ECP(), new ECP(), new ECP(), new ECP(), new ECP(), new ECP(), new ECP(), new ECP()
            };
            return arr;
          });
  private static final ThreadLocal<byte[]> clmulBYTES =
      ThreadLocal.withInitial(() -> new byte[1 + (BIG.NLEN * CONFIG_BIG.BASEBITS + 3) / 4]);

  private static final BIG BIG_ROM_CURVE_B = new BIG(ROM.CURVE_B);
  private static final FP FP_ROM_CURVE_B = new FP(BIG_ROM_CURVE_B);
  ...
  public static FP RHS(FP x) {
    ...
    if (CONFIG_CURVE.CURVETYPE == CONFIG_CURVE.WEIERSTRASS) { // x^3+Ax+B
    ...
    if (CONFIG_CURVE.CURVETYPE == CONFIG_CURVE.EDWARDS) { // (Ax^2-1)/(Bx^2-1)
      FP b = rhsFPw0.get();
      b.copy(FP_ROM_CURVE_B);
    ...
    if (CONFIG_CURVE.CURVETYPE == CONFIG_CURVE.MONTGOMERY) { // x^3+Ax^2+x
      FP x3 = new FP();
      x3.copy(r);
      x3.mul(x);
  ...
  public void dbl() {
    ...
    if (CONFIG_CURVE.CURVETYPE == CONFIG_CURVE.WEIERSTRASS) {
      if (CONFIG_CURVE.CURVE_A == 0) {
        ...
        if (ROM.CURVE_B_I == 0) b.copy(new FP(new BIG(ROM.CURVE_B)));
        ...
        if (ROM.CURVE_B_I == 0) y3.mul(b); // 8
        else y3.imul(ROM.CURVE_B_I);
        ...
        if (ROM.CURVE_B_I == 0) z3.mul(b); // 18
        else z3.imul(ROM.CURVE_B_I);
    ...
    if (CONFIG_CURVE.CURVETYPE == CONFIG_CURVE.MONTGOMERY) {
      ...
      A.copy(C);
      A.imul((CONFIG_CURVE.CURVE_A + 2) / 4);
  ...
  public void add(ECP Q) {
    ...
    if (CONFIG_CURVE.CURVETYPE == CONFIG_CURVE.WEIERSTRASS) {
      if (CONFIG_CURVE.CURVE_A == 0) {
        ...
        if (ROM.CURVE_B_I == 0) b.copy(FP_ROM_CURVE_B);
        ...
        if (ROM.CURVE_B_I == 0) y3.mul(b); // 18
        else y3.imul(ROM.CURVE_B_I);
    ...
    if (CONFIG_CURVE.CURVETYPE == CONFIG_CURVE.EDWARDS) {
      ...
      if (ROM.CURVE_B_I == 0) E.mul(FP_ROM_CURVE_B);
      else E.imul(ROM.CURVE_B_I);
  ...
  public ECP pinmul(int e, int bts) {
    if (CONFIG_CURVE.CURVETYPE == CONFIG_CURVE.MONTGOMERY) return this.mul(new BIG(e));
    else {
      int nb, i, b;
      ECP P = new ECP();
      ECP R0 = new ECP();
  ...
  // Typically P will be of order r (but not always), and typically e will be less than r (but not
  // always)
  // A problem can arise if a secret e is a few bits less than r, as the leading zeros in e will
  // leak via a timing attack
  // The secret e may however be greater than r (see RFC7748 which combines elimination of a small
  // cofactor h with the point multiplication, using an e>r)
  // Our solution is to use as a multiplier an e, whose length in bits is that of the logical OR of
  // e and r, hence allowing e>r while forcing inclusion of leading zeros if e<r.
  ...
  public ECP clmul(BIG e, BIG maxe) {
    ...
    if (CONFIG_CURVE.CURVETYPE == CONFIG_CURVE.MONTGOMERY) {
      ...
      // fixed size windows
      int i, b, nb, m, s, ns;
      BIG mt = clmulBIGw0.get(); // mt.zero();
      BIG t = clmulBIGw1.get(); // t.zero();
      ECP Q = clmulECPw0.get();
      ECP C = clmulECPw1.get();
      ECP[] W = clmulECP8w0.get();
      byte[] w = clmulBYTES.get();
  ...
  public ECP muln(int n, ECP X[], BIG e[]) {
    ...
    BIG mt = new BIG(e[0]);
    mt.norm();
    for (int i = 1; i < n; i++) { // find biggest
      t.copy(e[i]);
      t.norm();
      int k = BIG.comp(t, mt);
      mt.cmove(t, (k + 1) / 2);
  ...
  public static ECP map2point(FP h) {
    ECP P;
    if (CONFIG_CURVE.CURVETYPE == CONFIG_CURVE.MONTGOMERY) { // Elligator 2
      FP X1 = new FP();
      FP X2 = new FP();
      FP t = new FP(h);
      FP w = new FP();
    ...
    if (CONFIG_CURVE.CURVETYPE
        == CONFIG_CURVE.EDWARDS) { // Elligator 2 - map to Montgomery, place point, map back
      FP X1 = new FP();
      FP X2 = new FP();
      FP t = new FP(h);
      FP w = new FP();
      ...
      */
      ...
      if (CONFIG_FIELD.MODTYPE == CONFIG_FIELD.GENERALISED_MERSENNE) { // ED448 isogeny
        t.copy(X1);
        t.sqr();
        w.copy(t);
        w.add(one);
