...
public final class ECP 
{
	...
	private static final ThreadLocal<FP[]> addFP7 = ThreadLocal.withInitial(() -> new FP[] { new FP(), new FP(), new FP(), new FP(), new FP(), new FP(), new FP()});
	private static final ThreadLocal<FP[]> dblFP4 = ThreadLocal.withInitial(() -> new FP[] { new FP(), new FP(), new FP(), new FP()});
	...
	private static final ThreadLocal<ECP[]> clmulECP8w0 = ThreadLocal.withInitial(() -> { ECP arr[] = {new ECP(), new ECP(), new ECP(), new ECP(), new ECP(), new ECP(), new ECP(), new ECP()}; return arr;});
...
/* Calculate RHS of curve equation */
	public static FP RHS(FP x) {
		...
		if (CONFIG_CURVE.CURVETYPE==CONFIG_CURVE.EDWARDS)
		{ // (Ax^2-1)/(Bx^2-1)
			FP b=rhsFPw0.get(); b.copy(FP_ROM_CURVE_B);
