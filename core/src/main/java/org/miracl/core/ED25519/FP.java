...
public final class FP {
  ...
  private static final ThreadLocal<FP> fpowFPw0 = ThreadLocal.withInitial(() -> new FP());
  private static final ThreadLocal<FP> fpowFPw1 = ThreadLocal.withInitial(() -> new FP());
  private static final ThreadLocal<FP[]> fpowFP11 =
  ...
  private static final BIG BIG_ROM_MODULUS = new BIG(ROM.Modulus);
  ...
  public static BIG mod(DBIG d, BIG o) {
    ...
    if (CONFIG_FIELD.MODTYPE == CONFIG_FIELD.MONTGOMERY_FRIENDLY) {
      ...
      b = new BIG(0);
      for (int i = 0; i < BIG.NLEN; i++) b.w[i] = d.w[BIG.NLEN + i];
      b.norm();
      return b;
    }
  ...
  public void reduce() {
    BIG m = reduceBIGw0.get();
    m.copy(BIG_ROM_MODULUS);
    BIG r = reduceBIGw1.get();
    r.copy(BIG_ROM_MODULUS);
    int sr, sb, q;
    long carry;
    x.norm();
  ...
  private void progen() {
    ...
    BIG m = progenBIGw0.get();
    m.copy(FP.BIG_ROM_MODULUS);
    m.dec(1);
    m.shr(e);
    m.dec(1);
    m.fshr(1);
