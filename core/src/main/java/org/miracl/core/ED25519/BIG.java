...
public class BIG {
  private static final ThreadLocal<long[]> mulLONG2 = ThreadLocal.withInitial(() -> new long[2]);
  private static final ThreadLocal<BIG> BIGw0 = ThreadLocal.withInitial(() -> new BIG(0));
  ...
  /* Constructors */
  public BIG() {
    for (int i = 0; i < NLEN; i++) w[i] = 0;
  }

  public BIG(int x) {
    w[0] = x;
    for (int i = 1; i < NLEN; i++) w[i] = 0;
  }

  public BIG(BIG x) {
    for (int i = 0; i < NLEN; i++) w[i] = x.w[i];
  }
  ...
  static BIG monty(BIG md, long MC, DBIG d) {
    ...
    b = new BIG(0);
    for (int i = 0; i < NLEN; i++) b.w[i] = d.w[NLEN + i];
    b.norm();
    return b;
  }
  ...
  public boolean isunity() {
    long d = 0;
    for (int i = 1; i < NLEN; i++) d |= w[i];
    return ((1 & ((d - 1) >> CONFIG_BIG.BASEBITS) & (((w[0] ^ 1) - 1) >> CONFIG_BIG.BASEBITS))
        != 0);
  }
  ...
  public int bit(int n) {
    return (int)
        ((w[n / CONFIG_BIG.BASEBITS] & (cast_to_chunk(1) << (n % CONFIG_BIG.BASEBITS)))
            >> (n % CONFIG_BIG.BASEBITS));
    //        if ((w[n / CONFIG_BIG.BASEBITS] & (cast_to_chunk(1) << (n % CONFIG_BIG.BASEBITS))) >
    // 0) return 1;
    //        else return 0;
  }
  ...
  public int fshl(int k) {
    ...
    return (int)
        (w[NLEN - 1]
            >> ((8 * CONFIG_BIG.MODBYTES)
                % CONFIG_BIG.BASEBITS)); /* return excess - only used in FF.java */
  }
  ...
  public BIG plus(BIG x) {
    BIG s = new BIG(0);
    for (int i = 0; i < NLEN; i++) s.w[i] = w[i] + x.w[i];
    s.norm();
    return s;
  }
  ...
  public BIG minus(BIG x) {
    BIG d = new BIG(0);
    for (int i = 0; i < NLEN; i++) d.w[i] = w[i] - x.w[i];
    return d;
  }
  ...
  public void rsub(BIG x) {
    for (int i = 0; i < NLEN; i++) w[i] = x.w[i] - w[i];
  }
