...
public final class EDDSA {
  ...
  private static byte[] dom(String pp, boolean ph, byte cl) {
    ...
    for (int i = 0; i < len; i++) dom[i] = PP[i];
    if (ph) dom[len] = 1;
    else dom[len] = 0;
    dom[len + 1] = cl;
    return dom;
  ...
  private static byte[] H(byte[] S) {
    if (CONFIG_CURVE.AESKEY <= 16) { // for ed25519?
      ...
      for (int i = 0; i < S.length; i++) sh.process(S[i]);
      return sh.hash();
    } else { // for ed448?
  ...
  private static DBIG H2(boolean ph, byte[] ctx, byte[] R, byte[] Q, byte[] M) {
    ...
    if (CONFIG_CURVE.AESKEY <= 16) { // Ed25519??
      ...
      if (ph || cl > 0) { // if not prehash and no context, omit dom2()
        ...
        for (int i = 0; i < domain.length; i++) sh.process(domain[i]);
        for (int i = 0; i < cl; i++) sh.process(ctx[i]);
      }
      for (int i = 0; i < b; i++) sh.process(R[i]);
      for (int i = 0; i < b; i++) sh.process(Q[i]);
      for (int i = 0; i < M.length; i++) sh.process(M[i]);
      byte[] h = sh.hash();
      reverse(64, h);
  ...
  private static DBIG getR(boolean ph, int b, byte[] digest, byte[] ctx, byte[] M) {
    ...
    if (CONFIG_CURVE.AESKEY <= 16) { // Ed25519??
      ...
      for (int i = b; i < 2 * b; i++) sh.process(digest[i]);
      for (int i = 0; i < M.length; i++) sh.process(M[i]);
      byte[] h = sh.hash();
      reverse(64, h);
      return DBIG.fromBytes(h);
  ...
  private static int encode_int(BIG x, byte[] w) {
    int b, index = 0;
    if (8 * CONFIG_BIG.MODBYTES == CONFIG_FIELD.MODBITS)
  ...
  private static BIG decode_int(boolean strip_sign, byte[] ei) {
    ...
    for (int i = 0; i < b; i++) r[i] = ei[i];
    reverse(b, r);

    if (strip_sign) r[0] &= 0x7f;
  ...
  public static int KEY_PAIR_GENERATE(RAND RNG, byte[] D, byte[] Q) {
    ...
    if (RNG != null) {
      for (int i = 0; i < b; i++) D[i] = (byte) RNG.getByte();
    }
  ...
  // if ph=false, then context should be NULL for ed25519. However RFC8032 mode ed25519ctx is
  // supported by supplying a non-NULL or non-empty context
  public static int SIGNATURE(boolean ph, byte[] D, byte[] ctx, byte[] M, byte[] SIG) {
    ...
    return SIGNATURE(ph, D, Q, ctx, M, SIG);
  }
  ...
  public static int SIGNATURE(boolean ph, byte[] D, byte[] Q, byte[] ctx, byte[] M, byte[] SIG) {
    ...
    if (D.length != Q.length || D.length != b) res = INVALID_PUBLIC_KEY;
    if (res == 0) {
      DBIG dr = getR(ph, b, digest, ctx, M);
      BIG sr = dr.mod(q);
      ECP R = ECP.generator().mul(sr);
      encode(R, S);
      for (int i = 0; i < b; i++) SIG[i] = S[i];
      // reverse bytes for little endian
      reverse(b, digest);
      BIG s = BIG.frombytearray(digest, index);
      RFC7748(s);
  ...
  public static boolean VERIFY(boolean ph, byte[] Q, byte[] ctx, byte[] M, byte[] SIG) {
    ...
    QD.neg();
    for (int i = 0; i < lg; i++) { // use cofactor 2^c
      G.dbl();
      QD.dbl();
      R.dbl();
    }
