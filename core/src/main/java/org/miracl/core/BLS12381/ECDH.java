...
public final class ECDH {
  ...
  public static int KEY_PAIR_GENERATE(RAND RNG, byte[] S, byte[] W) {
    ...
    if (RNG == null) {
      ...
      else s = BIG.randomnum(r, RNG); // Removes biases
    }
  ...
  public static int VP_DSA(int sha, byte[] W, byte[] F, byte[] C, byte[] D) {
    ...
    if (c.iszilch() || BIG.comp(c, r) >= 0 || d.iszilch() || BIG.comp(d, r) >= 0) res = ERROR;
  ...
  public static byte[] ECIES_ENCRYPT(
      int sha, byte[] P1, byte[] P2, RAND RNG, byte[] W, byte[] M, byte[] V, byte[] T) {
    int i, len;
    ...
    for (i = 0; i < CONFIG_CURVE.AESKEY; i++) {
      K1[i] = K[i];
      K2[i] = K[CONFIG_CURVE.AESKEY + i];
    }
  ...
  public static byte[] ECIES_DECRYPT(
      int sha, byte[] P1, byte[] P2, byte[] V, byte[] C, byte[] T, byte[] U) {
