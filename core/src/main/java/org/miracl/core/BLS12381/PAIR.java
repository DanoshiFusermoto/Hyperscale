...
public final class PAIR {
  // Working Variables //
  private static final ThreadLocal<FP[]> ateFP_4 =
      ThreadLocal.withInitial(() -> new FP[] {new FP(), new FP(), new FP(), new FP()});
  private static final ThreadLocal<BIG[]> ateBIG_2 =
      ThreadLocal.withInitial(() -> new BIG[] {new BIG(), new BIG()});
  private static final ThreadLocal<ECP[]> ateECP_2 =
      ThreadLocal.withInitial(() -> new ECP[] {new ECP(), new ECP()});
  private static final ThreadLocal<ECP2[]> ateECP2_7 =
      ThreadLocal.withInitial(
          () ->
              new ECP2[] {
                new ECP2(), new ECP2(), new ECP2(), new ECP2(), new ECP2(), new ECP2(), new ECP2()
              });
  private static final ThreadLocal<FP12[]> ateFP12_3 =
      ThreadLocal.withInitial(() -> new FP12[] {new FP12(), new FP12(), new FP12()});
  private static final ThreadLocal<FP2[]> lineFP2_3 =
      ThreadLocal.withInitial(() -> new FP2[] {new FP2(), new FP2(), new FP2()});
  private static final ThreadLocal<FP4[]> lineFP4_3 =
      ThreadLocal.withInitial(() -> new FP4[] {new FP4(), new FP4(), new FP4()});
  ...
  private static void add(ECP2 A, ECP2 B, FP2 AA, FP2 BB, FP2 CC) {
    ...
    BB.sub(T1);
    BB.norm(); // BB=(CC-Z1.Y2).X2 - (AA-Z1.X2).Y2
    CC.neg();
    CC.norm(); // CC=-(CC-Z1.Y2).Xs
  ...
  public static FP12 line(ECP2 A, ECP2 B, FP Qx, FP Qy, FP12 o) {
    ...
    if (A == B) dbl(A, AA, BB, CC);
    else add(A, B, AA, BB, CC);
  ...
  public static FP12[] initmp() {
    ...
    for (int i = CONFIG_CURVE.ATE_BITS - 1; i >= 0; i--) r[i] = new FP12(1);
    return r;
  }
  ...
  public static FP12 miller(FP12[] r) {
    ...
    if (CONFIG_CURVE.SIGN_OF_X == CONFIG_CURVE.NEGATIVEX) res.conj();
    res.ssmul(r[0]);
    r[0].zero();
