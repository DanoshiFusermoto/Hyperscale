...
public final class FP4 {
  // Working Variables //
  private static final ThreadLocal<FP2[]> mulFP2_4 =
      ThreadLocal.withInitial(() -> new FP2[] {new FP2(), new FP2(), new FP2(), new FP2()});
  private static final ThreadLocal<FP2[]> negFP2_2 =
      ThreadLocal.withInitial(() -> new FP2[] {new FP2(), new FP2()});
  private static final ThreadLocal<FP2[]> sqrFP2_3 =
  ...
  public void toBytes(byte[] bf) {
    ...
    b.toBytes(t);
    for (int i = 0; i < 2 * CONFIG_BIG.MODBYTES; i++) bf[i] = t[i];
    a.toBytes(t);
    for (int i = 0; i < 2 * CONFIG_BIG.MODBYTES; i++) bf[i + 2 * CONFIG_BIG.MODBYTES] = t[i];
  }

  public static FP4 fromBytes(byte[] bf) {
    byte[] t = new byte[2 * CONFIG_BIG.MODBYTES];
    for (int i = 0; i < 2 * CONFIG_BIG.MODBYTES; i++) t[i] = bf[i];
    FP2 tb = FP2.fromBytes(t);
    for (int i = 0; i < 2 * CONFIG_BIG.MODBYTES; i++) t[i] = bf[i + 2 * CONFIG_BIG.MODBYTES];
