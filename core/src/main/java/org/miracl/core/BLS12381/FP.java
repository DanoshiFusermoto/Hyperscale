...
public final class FP {
  private static final ThreadLocal<FP[]> equalsFP_2 =
      ThreadLocal.withInitial(() -> new FP[] {new FP(0), new FP(0)});
  private static final ThreadLocal<BIG[]> reduceBIG_2 =
      ThreadLocal.withInitial(() -> new BIG[] {new BIG(0), new BIG(0)});
  ...
  private static final ThreadLocal<FP[]> fpowFP_2 =
      ThreadLocal.withInitial(() -> new FP[] {new FP(0), new FP(0)});
  private static final ThreadLocal<FP[]> fpowFP_11 =
      ThreadLocal.withInitial(
          () ->
              new FP[] {
                new FP(), new FP(), new FP(), new FP(), new FP(), new FP(), new FP(), new FP(),
                new FP(), new FP(), new FP()
              });
  ...
  private static final ThreadLocal<FP[]> powFP_16 =
      ThreadLocal.withInitial(
          () -> {
            FP arr[] = {
              new FP(), new FP(), new FP(), new FP(), new FP(), new FP(), new FP(), new FP(),
  ...
  private static final ThreadLocal<byte[]> powBYTE =
  ...
  public static final int TBITS =
      CONFIG_FIELD.MODBITS % CONFIG_BIG.BASEBITS; // Number of active bits in top word
  public static final long TMASK = ((long) 1 << TBITS) - 1;

  public static final FP ONE = new FP(1);
  public static final FP ROM_CURVE_HTPC2 = new FP(BIG.ROM_CURVE_HTPC2);
  ...
  static {
    ...
    for (int i = 0; i < ROM.PC.length; i++) ROM_PC[i] = new FP(new BIG(ROM.PC[i]));
  }
  ...
  public static BIG mod(DBIG d, BIG o) {
    ...
    if (CONFIG_FIELD.MODTYPE == CONFIG_FIELD.MONTGOMERY_FRIENDLY) {
      ...
      for (int i = 0; i < BIG.NLEN; i++) o.w[i] = d.w[BIG.NLEN + i];
      o.norm();
      return o;
    }
  ...
  public void reduce() {
    ...
    BIG m = BIG_2[0];
    m.copy(BIG.ROM_MODULUS);
    BIG r = BIG_2[1];
    r.copy(BIG.ROM_MODULUS);
    int sr, sb, q;
    long carry;
    x.norm();
    ...
    if (XES > 16) {
      ...
      carry = r.pmul(q);
      r.w[BIG.NLEN - 1] +=
          (carry << CONFIG_BIG.BASEBITS); // correction - put any carry out back in again
      x.sub(r);
      x.norm();
      sb = 2;
    } else sb = logb2(XES - 1);
  ...
  public void nres() {
    if (CONFIG_FIELD.MODTYPE != CONFIG_FIELD.PSEUDO_MERSENNE
        && CONFIG_FIELD.MODTYPE != CONFIG_FIELD.GENERALISED_MERSENNE) {
      BIG o = nresBIG.get();
      DBIG d = nresDBIG.get();
  ...
  private static final int[] FPOW_AC = {1, 2, 3, 6, 12, 15, 30, 60, 120, 240, 255};
