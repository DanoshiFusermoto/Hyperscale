...
public final class HPKE {
  ...
  */
  ...
  static byte[] LabeledExtract(byte[] SALT, byte[] SUITE_ID, String label, byte[] IKM) {
    ...
    else IKMlen = IKM.length;
    byte[] LIKM = new byte[RFC.length + SUITE_ID.length + LABEL.length + IKMlen];
    int k = 0;
    for (int i = 0; i < RFC.length; i++) LIKM[k++] = RFC[i];
    for (int i = 0; i < SUITE_ID.length; i++) LIKM[k++] = SUITE_ID[i];
    for (int i = 0; i < LABEL.length; i++) LIKM[k++] = LABEL[i];
    if (IKM != null) {
      for (int i = 0; i < IKM.length; i++) LIKM[k++] = IKM[i];
    }
  ...
  static byte[] LabeledExpand(byte[] PRK, byte[] SUITE_ID, String label, byte[] INFO, int L) {
    ...
    else INFOlen = INFO.length;
    byte[] LINFO = new byte[2 + RFC.length + SUITE_ID.length + LABEL.length + INFOlen];
    LINFO[0] = AR[0];
    LINFO[1] = AR[1];
    int k = 2;
    for (int i = 0; i < RFC.length; i++) LINFO[k++] = RFC[i];
    for (int i = 0; i < SUITE_ID.length; i++) LINFO[k++] = SUITE_ID[i];
    for (int i = 0; i < LABEL.length; i++) LINFO[k++] = LABEL[i];
    if (INFO != null) {
      for (int i = 0; i < INFO.length; i++) LINFO[k++] = INFO[i];
    }
  ...
  static byte[] ExtractAndExpand(int config_id, byte[] DH, byte[] CONTEXT) {
    ...
    int k = KEM.length;
    for (int i = 0; i < k; i++) SUITE_ID[i] = KEM[i];
    SUITE_ID[k] = KEM_ID[0];
  ...
  public static boolean DeriveKeyPair(int config_id, byte[] SK, byte[] PK, byte[] SEED) {
    ...
    for (int i = 0; i < GROUP; i++) SK[i] = S[i];
    ECDH.KEY_PAIR_GENERATE(null, SK, PK);
    if (kem == 32 || kem == 33) reverse(PK);
  ...
  public static byte[] encap(int config_id, byte[] skE, byte[] pkE, byte[] pkR) {
    ...
    int k = 0;
    for (int i = 0; i < pkE.length; i++) kemcontext[k++] = pkE[i];
    for (int i = 0; i < pkR.length; i++) kemcontext[k++] = pkR[i];
  ...
  public static byte[] authEncap(
      int config_id, byte[] skE, byte[] skS, byte[] pkE, byte[] pkR, byte[] pkS) {
  ...
  public static void keySchedule(
    ...
    for (int i = 0; i < ar.length; i++) context[k++] = ar[i];

    byte[] H = LabeledExtract(null, SUITE_ID, "psk_id_hash", pskID);
    for (int i = 0; i < CONFIG_CURVE.HASH_TYPE; i++) context[k++] = H[i];
