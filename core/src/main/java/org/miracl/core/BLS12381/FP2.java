...
public final class FP2 {
  // Working Variables //
  private static final ThreadLocal<BIG[]> mulBIG_3 =
      ThreadLocal.withInitial(() -> new BIG[] {new BIG(0), new BIG(0), new BIG(0)});
  private static final ThreadLocal<DBIG[]> mulDBIG_5 =
      ThreadLocal.withInitial(
          () -> new DBIG[] {new DBIG(0), new DBIG(0), new DBIG(0), new DBIG(0), new DBIG(0)});
  private static final ThreadLocal<FP[]> negFP_2 =
      ThreadLocal.withInitial(() -> new FP[] {new FP(0), new FP(0)});
  private static final ThreadLocal<FP[]> sqrFP_3 =
      ThreadLocal.withInitial(() -> new FP[] {new FP(0), new FP(0), new FP(0)});
  ...
  public void toBytes(byte[] bf) {
    ...
    b.toBytes(t);
    for (int i = 0; i < CONFIG_BIG.MODBYTES; i++) bf[i] = t[i];
    a.toBytes(t);
    for (int i = 0; i < CONFIG_BIG.MODBYTES; i++) bf[i + CONFIG_BIG.MODBYTES] = t[i];
  }

  public static FP2 fromBytes(byte[] bf) {
    byte[] t = new byte[CONFIG_BIG.MODBYTES];
    for (int i = 0; i < CONFIG_BIG.MODBYTES; i++) t[i] = bf[i];
    FP tb = FP.fromBytes(t);
    for (int i = 0; i < CONFIG_BIG.MODBYTES; i++) t[i] = bf[i + CONFIG_BIG.MODBYTES];
