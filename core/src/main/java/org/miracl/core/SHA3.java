...
public class SHA3 {
  ...
  public SHA3(SHA3 s) {
    ...
    rate = s.rate;
    len = s.len;
    for (int i = 0; i < 5; i++) for (int j = 0; j < 5; j++) S[i][j] = s.S[i][j];
  }
  ...
  private void transform() {
    ...
    for (k = 0; k < ROUNDS; k++) {
      ...
      for (i = 0; i < 5; i++) for (j = 0; j < 5; j++) S[i][j] ^= D[i];
      ...
      for (i = 0; i < 5; i++)
        for (j = 0; j < 5; j++) S[i][j] = B[i][j] ^ (~B[(i + 1) % 5][j] & B[(i + 2) % 5][j]);
  ...
  public void init(int olen) {
    ...
    int i, j;
    for (i = 0; i < 5; i++)
      for (j = 0; j < 5; j++) S[i][j] = 0; /* 5x5x8 bytes = 200 bytes of state */
    length = 0;
    len = olen;
