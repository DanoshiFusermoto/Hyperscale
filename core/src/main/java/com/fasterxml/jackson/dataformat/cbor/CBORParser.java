...
public class CBORParser extends ParserMinimalBase {
  ...
  private static final double MATH_POW_2_10 = Math.pow(2, 10);
  private static final double MATH_POW_2_NEG14 = Math.pow(2, -14);
   ...
   * object).
   *
   * <p>NOTE: before 2.13 was "_parsingContext"
   */
  ...
  /**
   * Input stream that can be used for reading more content, if one in use. May be null, if input
   * comes just as a full buffer, or if the stream has been closed.
   */
  ...
  /**
   * Marker flag to indicate that standard symbol handling is used (one with symbol table assisted
   * canonicalization. May be disabled in which case alternate stream-line, non-canonicalizing
   * handling is used: usually due to set of symbols (Object property names) is unbounded and will
   * not benefit from canonicalization attempts.
   *
   * @since 2.13
   */
  ...
  public CBORParser(
    ...
    DupDetector dups =
        JsonParser.Feature.STRICT_DUPLICATE_DETECTION.enabledIn(parserFeatures)
  ...
  /**
   * Method that can be used to access tag id associated with the most recently decoded value
   * (whether completely, for scalar values, or partially, for Objects/Arrays), if any. If no tag
   * was associated with it, -1 is returned.
   *
   * @since 2.5
   */
  ...
  /**
   * Overridden since we do not really have character-based locations, but we do have byte offset to
   * specify.
   */
  @Override
  public JsonLocation getTokenLocation() {
    ...
    return new JsonLocation(
        _ioContext.contentReference(),
        _tokenInputTotal, // bytes
        -1,
        -1,
        (int) _tokenInputTotal); // char offset, line, column
  }
  ...
  public JsonLocation getCurrentLocation() {
    ...
    return new JsonLocation(
        ...
        -1,
        -1,
        (int) offset); // char offset, line, column
  }
      ...
      switch (lowBits) {
        ...
        default:
          throw _constructReadException(
              "Invalid length indicator for ints (%d), token 0x%s",
  ...
  protected JsonToken _handleTaggedArray(int tag, int len) throws IOException {
    ...
    if (!_checkNextIsEndArray()) {
      _reportError(
          "Unexpected token (" + currentToken() + ") after 2 elements of 'bigfloat' value");
    }
  ...
  /**
   * Heavily simplified method that does a subset of what {@code nextToken()} does to basically only
   * (1) determine that we are getting {@code JsonToken.VALUE_NUMBER_INT} (if not, return with no
   * processing) and (2) if so, prepare state so that number accessor method will work).
   *
   * <p>Note that in particular this method DOES NOT reset state that {@code nextToken()} would do,
  ...
  /**
   * Method for forcing full read of current token, even if it might otherwise only be read if data
   * is accessed via {@link #getText} and similar methods.
   */
  @Override
  ...
  protected void _finishToken() throws IOException {
    ...
    if ((available >= needed)
        ...
        || ((_inputBuffer.length >= needed) && _tryToLoadToHaveAtLeast(needed))) {
  ...
  private final void _finishLongText(int len) throws IOException {
    ...
    while (--len >= 0) {
      ...
      if ((len -= code) < 0) { // may need to improve error here but...
        throw _constructReadException(
            "Malformed UTF-8 character at the end of a (non-chunked) text segment");
      }
    ...
    while (inPtr < end) {
      ...
      if (code != 0) {
        ...
        switch (code) {
          ...
          default: // invalid
            throw _constructReadException(
                "Invalid UTF-8 byte 0x%s in Object property name", Integer.toHexString(i));
        }
      }
      outBuf[outPtr++] = (char) i;
    }
  ...
  private final String _decodeLongerName(int len) throws IOException {
    ...
    // For the purpose of our "hack" we simply never intern any long field names to the symbol
    // tables.
    // 99% of the time long field names will be Map keys and not actually field names.
    // It makes no sense to place Map keys into field name tables as it simply just bloats the
    // symbol tables
    // and the Java memory usage if those same string are then also interned.
    if (1 == 0) {
      if (_symbolsCanonical) {
  ...
  protected final void _decodeNonStringName(int ch) throws IOException {
    ...
    if (type == CBORConstants.MAJOR_TYPE_INT_POS) {
      ...
      throw _constructReadException(
          "Unsupported major type (%d) for CBOR Objects, not (yet?) supported, only Strings", type);
    }
    _streamReadContext.setCurrentName(name);
  }

  /**
   * Helper method for trying to find specified encoded UTF-8 byte sequence from symbol table; if
   * successful avoids actual decoding to String.
   *
   * <p>NOTE: caller MUST ensure input buffer has enough content.
   */
  ...
  */
  ...
  /**
   * Method called to skip remainders of an incomplete token, when contents themselves will not be
   * needed any more. Only called or byte array and text.
   */
  protected void _skipIncomplete() throws IOException {
    ...
    // Either String or byte[]
    if (type != CBORConstants.MAJOR_TYPE_TEXT && type != CBORConstants.MAJOR_TYPE_BYTES) {
      _throwInternal();
    }
    ...
    while (true) {
      ...
      if (type != expectedType) {
        throw _constructError(
            "Mismatched chunk in chunked content: expected "
                + expectedType
                + " but encountered "
                + type);
      }
  ...
  private final int _decodeTag(int lowBits) throws IOException {
    ...
    throw _constructReadException(
        "Invalid low bits for Tag token: 0x%s", Integer.toHexString(lowBits));
  }

  /**
   * Method used to decode explicit length of a variable-length value (or, for indefinite/chunked,
   * indicate that one is not known). Note that long (64-bit) length is only allowed if it fits in
   * 32-bit signed int, for now; expectation being that longer values are always encoded as chunks.
   */
  private final int _decodeExplicitLength(int lowBits) throws IOException {
    ...
    throw _constructError(String.format("Invalid length for %s: 0x%02X,", currentToken(), lowBits));
  }
  ...
  private final int _decode32Bits() throws IOException {
    ...
    final byte[] b = _inputBuffer;
    int v =
        (b[ptr++] << 24) + ((b[ptr++] & 0xFF) << 16) + ((b[ptr++] & 0xFF) << 8) + (b[ptr++] & 0xFF);
    _inputPtr = ptr;
    return v;
  }
  ...
  /**
   * Helper method to encapsulate details of handling of mysterious `undefined` value that is
   * allowed to be used as something encoder could not handle (as per spec), whatever the heck that
   * should be. Current definition for 2.9 is that we will be return {@link JsonToken#VALUE_NULL},
   * but for later versions it is likely that we will alternatively allow decoding as {@link
  ...
  /**
   * Helper method that deals with details of decoding unallocated "simple values" and exposing them
   * as expected token.
   *
   * <p>As of Jackson 2.12, simple values are exposed as {@link JsonToken#VALUE_NUMBER_INT}s, but in
   * later versions this is planned to be changed to separate value type.
   *
   * @since 2.12
   */
  public JsonToken _decodeSimpleValue(int lowBits, int ch) throws IOException {
    ...
    if (lowBits < 24) {
      ...
      if (_numberInt < 32) {
        throw _constructError(
            "Invalid second byte for simple value: 0x"
                + Integer.toHexString(_numberInt)
                + " (only values 0x20 - 0xFF allowed)");
      }
    }
  ...
  */
  ...
  /**
   * Helper method that will try to load at least specified number bytes in input buffer, possible
   * moving existing data around if necessary
   */
  ...
  protected void _reportUnexpectedBreak() throws IOException {
    ...
    throw _constructError(
        "Unexpected Break (0xFF) token in definite length ("
  ...
  protected void _reportIncompleteBinaryRead(int expLen, int actLen) throws IOException {
    _reportInvalidEOF(
        String.format(" for Binary value: expected %d bytes, only found %d", expLen, actLen),
        _currToken);
  }
  ...
  // @since 2.13
  private String _reportTruncatedUTF8InName(
      int strLenBytes, int truncatedCharOffset, int firstUTFByteValue, int bytesExpected)
      throws IOException {
    throw _constructReadException(
        String.format(
