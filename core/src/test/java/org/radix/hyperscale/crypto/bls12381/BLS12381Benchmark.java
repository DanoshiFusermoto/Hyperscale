...
public class BLS12381Benchmark {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      for (BLSKeyPair keyPair : keyPairs) publicKeys.add(keyPair.getPublicKey());
      ...
      double verificationTimeMs =
          TimeUnit.NANOSECONDS.toMillis(endVerification - startVerification);
      double aggregationTimeMs =
          TimeUnit.NANOSECONDS.toMillis(endAggregationBenchmark - startAggregationBenchmark);

      System.out.println("Results:");
      System.out.println("---------");
      System.out.printf(
          "Key Generation: %.2f ms total, %.2f ms per key%n",
          keyGenTimeMs, keyGenTimeMs / NUM_KEYS);

      System.out.printf(
          "Signing: %.2f ms total, %.2f s per message%n",
          signingTimeMs, signingTimeMs * 1000 / NUM_MESSAGES);

      System.out.printf(
          "Verification: %.2f ms total, %.2f �s per message%n",
          verificationTimeMs, verificationTimeMs * 1000 / NUM_MESSAGES);

      System.out.printf(
          "Aggregation (signing, aggregating, verifying): %.2f ms total, %.2f ms per batch%n",
          aggregationTimeMs, aggregationTimeMs / (NUM_MESSAGES / AGGREGATE_KEYS));
  ...
  private static List<BLSKeyPair> generateKeyPairs(int count) {
    ...
    for (int i = 0; i < count; i++) keyPairs.add(new BLSKeyPair());
  ...
  private static List<byte[]> generateDigests(int count, int size) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      for (int j = 0; j < size; j++) message[j] = (byte) ((i * j) % 256);
  ...
  private static List<BLSSignature> signMessages(List<byte[]> digests, List<BLSKeyPair> keyPairs)
  ...
  private static boolean verifySignatures(
      List<byte[]> digests, List<BLSSignature> signatures, List<BLSKeyPair> keyPairs) {
  ...
  private static boolean runAggregationBenchmark(List<BLSKeyPair> keyPairs, int numBatches)
      throws CryptoException {
    boolean allValid = true;
    ...
    for (int batchIdx = 0; batchIdx < numBatches; batchIdx++) {
      ...
      for (int j = 0; j < MESSAGE_SIZE; j++) message[j] = (byte) ((batchIdx * j) % 256);
