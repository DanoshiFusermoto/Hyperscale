...
public class ED25519Test {
  ...
  public void testKeyPairGeneration() throws CryptoException {
    ...
    assertEquals(
        "Private key should be " + ED25519.PRIVATE_KEY_SIZE + " bytes",
        ED25519.PRIVATE_KEY_SIZE,
        keyPair.getPrivateKey().toByteArray().length);
    assertEquals(
        "Public key should be " + ED25519.PUBLIC_KEY_SIZE + " bytes",
        ED25519.PUBLIC_KEY_SIZE,
        keyPair.getPublicKey().toByteArray().length);
    ...
    assertArrayEquals(
        "Derived public key should match the one from the key pair",
  ...
  public void testVectors() throws CryptoException {
    for (int i = 0; i < TEST_VECTORS.length; i++) {
      ...
      assertArrayEquals(
          "Test vector " + (i + 1) + ": Derived public key should match expected public key",
          publicKey,
          derivedPublicKey);
      ...
      assertArrayEquals(
          "Test vector " + (i + 1) + ": Generated signature should match expected signature",
          signature,
          generatedSignature);
      ...
      assertTrue(
          "Test vector " + (i + 1) + ": Signature verification should succeed", verifyResult);
    }
  }
  ...
  public void testPrehashedSigning() throws CryptoException {
    ...
    boolean result =
        ED25519.verifyPrehashed(keyPair.getPublicKey().toByteArray(), messageHash, signature);
  ...
  private byte[] sha512(byte[] input) {
    ...
    for (byte b : input) hash.process(b);
    return hash.hash();
  }
}
